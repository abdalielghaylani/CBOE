...
if(!ig_WebControl.prototype.isPrototypeOf(ig_WebCalendarView.prototype))
{
		...
		if(me._info.getEnableSmartCallbacks())
		{
			...
			else
			{
				if(smartCallback != null)
				{
					...
					for(var i = 1; i < registeredControls.length; i++)
	...
	ig_WebCalendarView.prototype._registerSmartCB=function(smartCallback, operation,type, sc, cc)
	{
		...
		{
			if(sc == null)
				sc = {operation:operation, requestType:"html"};
			if(cc == null)
				cc = {operation:operation, requestType:"html"};
		}
		else if(type == "json")
		{
			if(sc == null)
				sc = {operation:operation, requestType:"json"};
			if(cc == null)
				cc = {operation:operation, requestType:"json"};
		}
