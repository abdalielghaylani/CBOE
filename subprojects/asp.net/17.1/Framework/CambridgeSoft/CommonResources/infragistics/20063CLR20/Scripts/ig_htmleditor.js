...
function iged_new(id,ta,tb,p1,p2,p3,p4)
{
	this.ID=id;
	this._ta=ta;
	this._tb=tb;
	...
	{
		x=x.split("|");
		var i=0,s=this._dlgS.split("|"),d=x[1]!="";
	...
	this._fire=function(id,act,p1,p2,p3,p4,p5)
	{
		...
		if(id&&id.length>0)
		{
			...
			if(p.indexOf('=')<0&&p.indexOf(';')<0&&p.indexOf(',')<0&&p.indexOf('(')<0&&p.indexOf('{')<0&&p.indexOf('[')<0&&p.indexOf('"')<0&&p.indexOf("'")<0&&p.indexOf(':')<0)
				p+="(this,act,evt,p1,p2,p3,p4,p5)";
			try{eval(p);}catch(i){window.status="Can't eval "+id;}
		}
		p=evt.needPostBack?true:(this._post&&!evt.cancelPostBack&&!evt.cancel);
		this._post=false;
	...
	{
		if(!e||!e.getAttribute||e._old)return;
	...
	this._wait=function(evt)
	{
		var pan=this._pan();
		if(!pan)return;
		var s=pan.style;s.height="15px";s.width="200px";s.border="";
		pan.innerHTML="<div style='padding:7px;font-family:verdana;font-size:10pt;font-weight:bold;color:#005000;background:#E0FFF0;border:1px solid #005080'>Please wait ...</div>";
		
		iged_all._popID=null;
		this._pos(evt,iged_all._pop=pan);
	}
	...
	this._pos=function(evt,pan,flag)
	{
		...
		if(!e)if((e=evt.srcElement)==null)return p;
		...
		if(z<y-f+panH){if(y-f-3>panH+elemH){y-=panH+elemH;ok=!this._popI;}else y=z+f-panH;}
		if(y<f)y=f;
		z=body.clientWidth;
		...
		if(pan&&this._popI)
		{
			iged_all._ip={panH:panH,h:1,pan:pan,overF:s.overflow,oldH:s.height,y:(ok?-1:y),fn:window.setInterval("iged_popTimer()",10)};
			s.overflow="hidden";s.height="1px";
		}
		return p;
	}
	...
	{
		if(this._elem.offsetWidth)this._ta.value=this._fixStr(this._decode(this.getText()),true);
	}
...
function iged_popTimer()
{
	var o=iged_all._ip;
	if(!o||!o.panH)return;
	var s=o.pan.style,h=o.h,d=(o.panH>>5)+(o.h>>3)+2;
	if((h+=d)>o.panH)h=o.panH;
	o.h=h;
	s.height=h+"px";
	if(o.y>=0)s.top=(o.y+o.panH-h)+"px";
	if(h<o.panH)return;
	o.panH=null;
...
function iged_mainEvt(e)
{
	...
	if(t==6){o._foc=false;iged_update(e);return;}
	if(o._ie)try
	{
