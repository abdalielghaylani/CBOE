...
{
    public class BatchHighlightingValue : BugFixBaseCommand
    {


        /// <summary>
        /// patcher for adding BatchHighlightingValue system setting for registration.
        /// </summary>
            ...
            )
        {
            ...
            try
            {
                foreach (XmlDocument config in configurations)
                {
                    if (config.SelectSingleNode("//applicationSettings[@name='Reg App Settings']") != null)
                    {
                        ...
                        string lockingEnabledPatch = "//add[@name='REGADMIN']/settings/add[@name='BatchHighlightingValue']";
                        ...
                        if (xpathRootNode == null)
                        {
                            ...
                            createNewAttribute("name", "BatchHighlightingValue", ref xpathRootNode);
                            ...
                            createNewAttribute("description", "In combination with BachHighlightingField configures a rule to highlight those batches that has a specific value for a given field", ref xpathRootNode);
                            ...
                            messages.Add("BatchHighlightingValue node was added succesfully.");
                        }
                        else
                            messages.Add("RegSettings already contains BatchHighlightingValue node.");
                        break;
                    }
                }
            }
            ...
            finally
            {
                if (!errorsInPatch)
                {
                    messages.Add("BatchHighlightingValue was successfully patched");
                }
                else
                    messages.Add("BatchHighlightingValue patch was aborted due to errors");
            }
            return messages;
        }
