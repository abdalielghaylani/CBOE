...
namespace CambridgeSoft.COE.Framework.Controls.COEFormGenerator {
    ...
    public class COETextBox : TextBox, ICOEGenerableControl, ICOELabelable, ICOERequireable, ICOEDesignable, ICOEHelpContainer, ICOEReadOnly
    {
        #region Constants
        ...
        protected override void Render(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            if (this.ShowHelp)
            {
                // Fix Coverity: CID-28976 Resource Leak
                using (Label lblHelp = new Label())
                {
                    lblHelp.Style.Add("float", "left"); // CSBR-120344 fix, new line added           
                    _helpImage = new Image();
                    _helpImage.ImageUrl = Page.ClientScript.GetWebResourceUrl(
                        typeof(ICOEGenerableControl),
                        "CambridgeSoft.COE.Framework.ServerControls.FormGenerator.Images.help.png");
                    ...
                    _helpImage.Attributes.Add(
                        "onclick",
                        "showHide(YAHOO.COEFormGenerator." + _helpImage.ClientID + "Tooltip, " + _helpImage.ClientID
                        + ");");
                    lblHelp.Controls.Add(_helpImage);
                    //_helpImage.RenderControl(writer);
                    lblHelp.RenderControl(writer);
                    string yahooScript =
                        "<script language='javascript' type='text/javascript'>YAHOO.namespace('COEFormGenerator');"
                        + "YAHOO.COEFormGenerator." + _helpImage.ClientID + "Tooltip = new YAHOO.widget.Tooltip(\""
                        + _helpImage.ClientID + "Tooltip\", " + "{ context:\"" + _helpImage.ClientID + "\", text:\""
                        + FormatText(this.HelpText) + "\"," + "showdelay: 20000," + "hidedelay: 20000,"
                        + "visible:false});" + "YAHOO.COEFormGenerator." + _helpImage.ClientID
                        + "Tooltip.isToBeShown = true;" + "</script>";
