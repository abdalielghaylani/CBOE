  ...
  */
  ...
  BEGIN
	...
	FROM
	 VW_COMPOUND C 
	 INNER JOIN VW_STRUCTURE S ON S.STRUCTUREID = C.STRUCTUREID 
	 INNER JOIN ( SELECT /*+ OPT_ESTIMATE(table, t, scale_rows=0.0001) */ MC2.MIXTUREID, MC2.COMPOUNDID FROM 
	  VW_MIXTURE_COMPONENT MC2 INNER JOIN TABLE 
	  (REGISTRYDUPLICATECHECK.EXTRACT_COMPOUNDS ( 
	  REGISTRYDUPLICATECHECK.MATCHED_COMPOUNDS_WRAPPER (p_type, p_params, p_value))) T ON 
	  T.COMPOUND_ID = MC2.COMPOUNDID)  MC ON MC.COMPOUNDID = C.COMPOUNDID 
	  INNER 
	  JOIN VW_MIXTURE_REGNUMBER MR ON MR.MIXTUREID = MC.MIXTUREID 
	  INNER JOIN 
	  VW_REGISTRYNUMBER R ON R.REGID = C.REGID 
	  GROUP BY MR.REGID, MR.REGNUMBER, C.COMPOUNDID, S.STRUCTUREID 
	  ORDER BY MR.REGID, MR.REGNUMBER, C.COMPOUNDID, S.STRUCTUREID;
