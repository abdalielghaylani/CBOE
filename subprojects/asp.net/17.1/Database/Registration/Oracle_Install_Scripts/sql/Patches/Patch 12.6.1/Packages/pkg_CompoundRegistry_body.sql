...
create or replace
PACKAGE BODY "COMPOUNDREGISTRY" IS
...
--  GetFragmentXML
  ...
  */
  ...
  BEGIN
    IF (VSameBatchesIdentity is NULL) THEN
      ...
      FROM DUAl;
      VSameBatchesIdentity := to_char(LValue);
    END IF;
  ...
  */
  ...
  BEGIN
    $if CompoundRegistry.TraceEnabled $then
      INSERT INTO LOG
        (LogProcedure,LogComment)
      VALUES
        (to_char(systimestamp, 'ss.FF') || ' - ' || $$plsql_unit||'.'||ALogProcedure,ALogComment);
      COMMIT;
    $end null;
  EXCEPTION
  ...
  */
  ...
  BEGIN
    ...
    LLevelRLS := GetActiveRLS; --RegistrationRLS.GetLevelRLS;
    ...
    IF upper(LLevelRLS)!='OFF' THEN
      ...
      IF LLevelRLS='Registry Level Projects' THEN
	      OPEN C_RegProjectAssigned FOR
			...
			FROM VW_RegistryNumber RN,VW_Batch B,VW_RegistryNumber_Project RNP
			WHERE RN.RegID=B.RegID AND RN.RegID = RNP.RegID AND B.BatchNumber=pBatchNumber AND RN.RegNumber=ARegNumber
			ORDER BY RNP.ProjectID;
      ELSE
	      OPEN C_RegProjectAssigned FOR
			SELECT Distinct BP.ProjectID
			FROM VW_RegistryNumber RN,VW_Batch B,VW_Batch_Project BP
			WHERE RN.RegID=B.RegID AND BP.BatchID=B.BatchID AND B.BatchNumber=pBatchNumber AND RN.RegNumber=ARegNumber
  ...
  */
  FUNCTION ValidateCompoundMulti(
    ...
    CURSOR C_RegNumbersOnlySingleRegistry(ACoumpoundID in VW_Compound.CompoundID%type) IS
      SELECT RegNumber
      FROM VW_RegistryNumber RN,VW_Mixture M,VW_Mixture_Component MC
      WHERE RN.RegID=M.RegID AND M.MixtureID=MC.MixtureID AND MC.CompoundID=ACoumpoundID AND
       MC.MixtureID IN (SELECT MixtureID FROM VW_Mixture_Component HAVING COUNT(1)=1 GROUP BY MixtureID)
      ORDER BY MC.CompoundID;
  BEGIN
    ...
    if LFormulaWeight != 0 then
      ...
      if (LDuplicateCount is not null and LDuplicateCount > 0 and LDuplicateCount < 300) then
        for rec in (
  ...
  */
  ...
  BEGIN
    ...
    v_sbi_setting := upper(GetSameBatchesIdentity);
...
-- We have to be careful to AVOID no data found error in case of SBI True
  ...
  */
  ...
  BEGIN
    ...
    SELECT StructureID INTO LInternalID
  ...
  */
  ...
  BEGIN
    if (LPrototypeXml is null) then
      LPrototypeXml := GetRegObjectTemplate;
    end if;
    ...
    LQContext := dbms_xmlgen.newContext(
      'select
        ...
        , c.regid, c.normalizedstructure, c.usenormalization
      from VW_Structure_Drawing s
        left outer join vw_compound c on c.structureid = s.structureid
  ...
  */
  ...
  LXmlCopyRestPos := 0;
		  ...
		  IF LPosField <> 0 THEN
			IF NVL(LpickListDomainID,0)<>0 THEN
			  ...
			  IF LFieldValue IS NOT NULL AND LExt_Display_Col IS NOT NULL AND LExt_Table IS NOT NULL AND LExt_ID_Col IS NOT NULL THEN
				 ...
				 TraceWrite('AddAttribPickListForBatch_Select', $$plsql_line, LSelect);
				 BEGIN
					EXECUTE IMMEDIATE LSelect INTO LPickListDisplayValue
					USING LFieldValue;
				 EXCEPTION
	...
	IF AFinalXml IS NOT NULL THEN
		AXml := AFinalXml || TRIM(SUBSTR(AXml,LXmlCopyRestPos,LENGTH(AXml)-LXmlCopyRestPos));
	END IF;
  ...
  */
  PROCEDURE RetrieveMultiCompoundRegistry(
    ...
    -- we will ALWAYS override the value for v_propertyFields, so this is just CLOB initialization
    v_propertyFields CLOB := '*';
    v_sqlFields1 varchar2(500);
    v_sqlFields2 varchar2(150);
    v_sqlFields3 varchar2(800);
    v_sqlBatchPropList varchar2(4000);
    v_sqlBatchIdentifierList varchar2(300);
    v_sqlBatchProjectList varchar2(300);
    v_sqlBatchComponentPropList varchar2(4000);
    v_sqlBatchComponentFragment varchar2(400);
  ...
  BEGIN
    ...
    IF (ARegNumber IS NOT NULL) THEN
      LCoeObjectConfigField := GetRegObjectTemplate;
      ...
      IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Mixture'), 0) > 1 ) THEN
        LResult :=
          ...
          ||'SameBatchesIdentity="'||GetSameBatchesIdentity
          ||'" ActiveRLS="'||GetActiveRLS
      ...
      IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Batch'), 0) > 0 ) THEN
        ...
        v_sqlFields1 :=
          'select b.batchid, b.fullregnumber, b.batchnumber, b.statusid
          , b.datecreated, b.personcreated, b.personapproved
          , b.personregistered, b.datelastmodified
          , pc.user_id personcreateddisplay
          , pr.user_id personregistereddisplay
          , pa.user_id personapproveddisplay
          , CompoundRegistry.GetIsBatchEditable(:bindvar, b.batchnumber) as IsBatchEditable';
          if (CompoundRegistry.FillPropertyTemplate = TRUE) then
            v_sqlBatchPropList := ', CompoundRegistry.GetFilledPropertyList(''BATCH'', :bindvar) BatchPropertyList';
          else
            v_sqlBatchPropList := '
          , cursor(
                select ' || LBatchFields || ' 
                  from vw_batch v where v.batchid = b.batchid
              ) BatchPropertyList';
          end if;
          v_sqlBatchIdentifierList := '
          , cursor(
                select bi.id, bi.type, bi.value, p.description, p.active, p.name
                  from vw_batchidentifier bi inner join vw_identifiertype p on p.id = bi.type
                  where bi.batchid = b.batchid
              ) BatchIdentifierList';
          v_sqlBatchProjectList := '
          , cursor(
                select bp.id, bp.projectid, p.description, p.active, p.name
                  from vw_batch_project bp inner join vw_project p on p.projectid = bp.projectid
                  where bp.batchid = b.batchid
              ) BatchProjectList';
          v_sqlFields2 := '
          , cursor(
                select p.id, p.batchid, m.compoundid, m.mixturecomponentid, -m.compoundid componentindex, r.regnumber';
                if (CompoundRegistry.FillPropertyTemplate = TRUE) then
                  v_sqlBatchComponentPropList := ', CompoundRegistry.GetFilledPropertyList(''BATCHCOMPONENT'', :bindvar) BatchComponentPropertyList';
                else
                  v_sqlBatchComponentPropList := '
                  , CAST( MULTISET(select ' || LBatchComponentFields || ' from VW_BatchComponent B WHERE B.ID = p.ID
                      ) AS BatchComponentPropertyList_T1) BatchComponentPropertyList';
                end if;
                v_sqlBatchComponentFragment := '
                  , CAST( MULTISET(
                        select cf.fragmentid, bcf.compoundfragmentid, bcf.equivalent, bcf.id  from vw_batchcomponentfragment bcf
                          inner join vw_compound_fragment cf on cf.id = bcf.compoundfragmentid
                        where bcf.batchcomponentid = p.ID
                      ) AS BatchComponentFragment_T1) BatchComponentFragment';
                v_sqlFields3 := '
                  from vw_batchcomponent p inner join vw_mixture_component m on m.mixturecomponentid = p.mixturecomponentid
                    inner join vw_compound c on c.compoundid = m.compoundid
                    inner join vw_registrynumber r on r.regid = c.regid
                  where p.batchid = b.batchid order by p.orderindex
              ) BatchComponent
          from vw_batch b left outer join cs_security.people pc on pc.person_id = b.personcreated
            left outer join cs_security.people pr on pr.person_id = b.personregistered
            left outer join cs_security.people pa on pa.person_id = b.personapproved
            inner join vw_registrynumber r on r.regid = b.regid
          where r.regnumber= :bindvar order by b.batchnumber';
        ...
        LQContext := dbms_xmlgen.newContext(v_sqlFields1 || v_sqlBatchPropList || v_sqlBatchIdentifierList || v_sqlBatchProjectList || v_sqlFields2 || v_sqlBatchComponentPropList || v_sqlBatchComponentFragment || v_sqlFields3);
        dbms_xmlgen.setBindValue(LQContext, 'bindvar', to_char(ARegNumber));
        dbms_xmlgen.setNullHandling(LQContext, 2);
        LXml := replace(dbms_xmlgen.getXML(LQContext), cXmlDecoration, null);
        dbms_xmlgen.closeContext(LQContext);
        ...
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENT_ROW>', '</BATCHCOMPONENT>'||CHR(10)||'<BATCHCOMPONENT>');
        LXml := REGEXP_REPLACE(LXml, '<PROPERTYLIST_OBJ>', '<ROW>');
        LXml := REGEXP_REPLACE(LXml, '</PROPERTYLIST_OBJ>', '</ROW>');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENT>'||CHR(10)||' </ROW>', '</BATCHCOMPONENT>'||CHR(10)||'</ROW>'||CHR(10)||'</Batch>'||CHR(10)||'<Batch>');
        LXml := REGEXP_REPLACE(LXml, '<ISBATCHEDITABLE>', '<IsBatchEditable>');
        LXml := REGEXP_REPLACE(LXml, '</ISBATCHEDITABLE>', '</IsBatchEditable>');
        ...
        LXml := REGEXP_REPLACE(LXml, '<BATCHCOMPONENTPROPERTYLIST>', '</ROW>'||CHR(10)||'<PropertyList>');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENTPROPERTYLIST>', '</PropertyList>');
        LXml := REGEXP_REPLACE(LXml, '<BATCHCOMPONENTPROPERTYLIST_T>', '<ROW>');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENTPROPERTYLIST_T>', '</ROW>');
        LXml := REGEXP_REPLACE(LXml, '<BATCHCOMPONENT>', '<BatchComponent>');
        LXml := REGEXP_REPLACE(LXml, '</ROW>'||CHR(10)||'  </BATCHCOMPONENT>', '</BatchComponent>');
        LXml := REGEXP_REPLACE(LXml, '<BATCHCOMPONENTFRAGMENT_T>', '<ROW>');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENTFRAGMENT_T>', '</ROW>');
        LXml := REGEXP_REPLACE(LXml, '<BATCHCOMPONENTFRAGMENT>', '<BatchComponentFragment>');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENTFRAGMENT>', '</BatchComponentFragment>');
        LXml := REGEXP_REPLACE(LXml, '</ROW>'||CHR(10)||'</Batch>', '</Batch>');
        LXml := REGEXP_REPLACE(LXml, '<BatchComponent>'||CHR(10)||'  </BATCHCOMPONENT>', '');
        LXml := REGEXP_REPLACE(LXml, '</BATCHCOMPONENT>', '</BatchComponent>');
      ...
      LResult := REGEXP_REPLACE(LResult, '<Batch>'||CHR(10)||'</MultiCompoundRegistryRecord>', '</MultiCompoundRegistryRecord>');
    ...
    ELSE
      ...
      AXml := GetRegObjectTemplate().GetClobVal();
      AXml:=
        '<MultiCompoundRegistryRecord ModuleName="''" SameBatchesIdentity="' || GetSameBatchesIdentity || '" ActiveRLS="'||GetActiveRLS||'" '
...
TraceWrite('UpdateMcrr_LFieldToUpdate', $$plsql_line, LFieldToUpdate);
  ...
  */
  PROCEDURE RetrieveTemporaryRegistration(
    ...
    v_regObjectDefinition xmltype := GetRegObjectTemplate;
    v_ctx dbms_xmlgen.ctxHandle;
    v_sql varchar2(4000);
    v_buf clob;
    ...
    v_sbiFlag varchar2(255) := GetSameBatchesIdentity;
    v_rlsFlag varchar2(255) := GetActiveRLS;
  ...
  ';
    ...
    */
    ...
    BEGIN
      ...
      -- Get XML
      LXml := GetRegObjectTemplate().GetClobVal();
      -- Transform
...
PROCEDURE UpdateLockedStatus(APermIDList VARCHAR2, AStatusID IN NUMBER) IS
  BEGIN
    execute immediate  'UPDATE VW_MIXTURE SET STATUSID=' || AStatusID ||' WHERE REGID IN (' || APermIDList ||')';
        INSERT INTO LOG  (LogProcedure,LogComment)
           VALUES
        ('UpdateLockedStatus',AStatusID ||'  ' || APermIDList);
  END;
