...
******************************************************************************/
    ...
    BEGIN
        ...
        LConfigParameterValue:=UPPER(COMPOUNDREGISTRY.GetActiveRLS);  
    ...
    BEGIN
        ...
        -- This check ensures than when materialized views are refreshed, they are not affected by RLS filter
        ...
        ELSE
            CASE p_object
                    ...
                    RETURN 'Project_Internal_ID IN
                             (
                              SELECT PP.ProjectID
                                ...
                                WHERE  PP.PersonID = P.Person_ID AND P.User_ID =''' || replace(UPPER(sys_context('userenv','client_identifier')),'''','''''') || '''
                             )
                ...
                WHEN 'REG_NUMBERS' THEN
                    RETURN '1=
                          case when sequence_internal_id in (select sequence_id from sequence where type=''C'') then
                             (
                              SELECT DISTINCT 1
                                FROM  VW_Compound C
                                WHERE Reg_ID = C.RegID)
                          else 
                              (SELECT DISTINCT 1
                                FROM  VW_Mixture M
                                WHERE Reg_ID = M.RegID
                             )
                          End';
