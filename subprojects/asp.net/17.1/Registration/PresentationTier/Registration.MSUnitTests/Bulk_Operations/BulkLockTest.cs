...
{
    /// <summary>
    /// BulkLockTest class contains unit test methods for BulkLock
    /// </summary>
    [TestClass]
    public class BulkLockTest
    {
        #region Variables
        ...
        #region BulkLock Test Methods

        /// <summary>
        /// Locking Temparary registry record in bulk order
        /// </summary>
        [TestMethod]
        public void Execute_LockingTempararyRegistryReocrdInBulk() 
        {
            ...
            if (dtHitListInfo != null && dtHitListInfo.Rows.Count > 0)
            {
                try
                {
                    bool blnResult = BulkLock.Execute(Convert.ToInt32(Convert.ToString(dtHitListInfo.Rows[0]["HITLISTID"])));

                    Assert.IsTrue(blnResult, "BulkLock.Execute(Convert.ToInt32(Convert.ToString(dtHitListInfo.Rows[0]['HITLISTID']))) did not return the expected value.");
                }
                catch (Exception ex)
                {
