...
{
    /// <summary>
    /// GetPermRegistriesCountTest class contains unit test methods for GetPermRegistriesCount
    /// </summary>
    [TestClass]
    public class GetPermRegistriesCountTest
    {
        #region Variables
        ...
        #region GetPermRegistriesCount Test Methods
        ...
        public void GetCustomItem_LoadingFormattedText() 
        {
            try
            {
                GetPermRegistriesCount theGetPermRegistriesCount = new GetPermRegistriesCount();
                ...
                foreach (Group myGroup in homeData.Groups)
                {
                    if (myGroup.PageSectionTarget.ToUpper().Equals("DASHBOARD"))
                    {
                        foreach (CustomItems customItems in myGroup.CustomItems)
                        {
                            ...
                            theGetPermRegistriesCount.SetConfiguration(customItems.Configuration);
                        }
                        break;
                    }
                }

                string strResult = theGetPermRegistriesCount.GetCustomItem();

                Assert.IsTrue(!string.IsNullOrEmpty(strResult), "GetPermRegistriesCount.GetCustomItem() did not return the expected value.");
            }
            catch (Exception ex)
            {
