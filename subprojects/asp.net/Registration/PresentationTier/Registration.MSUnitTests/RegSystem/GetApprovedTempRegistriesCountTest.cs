...
{
    /// <summary>
    /// GetApprovedTempRegistriesCountTest class contains unit test methods for GetApprovedTempRegistriesCount
    /// </summary>
    [TestClass]
    public class GetApprovedTempRegistriesCountTest
    {
        ...
        #region GetApprovedTempRegistriesCount Test Methods
        ...
        public void GetCustomItem_LoadingFormattedText() 
        {
            try
            {
                GetApprovedTempRegistriesCount theGetApprovedTempRegistriesCount = new GetApprovedTempRegistriesCount();
                ...
                foreach (Group myGroup in homeData.Groups)
                {
                    if (myGroup.PageSectionTarget.ToUpper().Equals("DASHBOARD"))
                    {
                        foreach (CustomItems customItems in myGroup.CustomItems)
                        {
                            ...
                            theGetApprovedTempRegistriesCount.SetConfiguration(customItems.Configuration);
                        }
                        break;
                    }
                }

                string strResult = theGetApprovedTempRegistriesCount.GetCustomItem();

                Assert.IsTrue(!string.IsNullOrEmpty(strResult), "GetApprovedTempRegistriesCount.GetCustomItem() did not return the expected value.");
            }
            catch (Exception ex)
            {
