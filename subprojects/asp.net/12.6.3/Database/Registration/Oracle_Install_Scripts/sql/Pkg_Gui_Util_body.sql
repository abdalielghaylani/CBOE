...
AS
 ...
 BEGIN
  OPEN O_RS FOR
    SELECT PRO.PROJECTID, PRO.NAME, PRO.ACTIVE, PRO.DESCRIPTION, PRO.TYPE
    FROM REGDB.VW_PROJECT PRO
        left JOIN REGDB.VW_PEOPLEPROJECT PEO ON  PEO.PROJECTID = PRO.PROJECTID
    WHERE PRO.ACTIVE = 'T' AND (PEO.PERSONID IS NULL OR PEO.PERSONID = APersonID)
    ORDER BY 2;
