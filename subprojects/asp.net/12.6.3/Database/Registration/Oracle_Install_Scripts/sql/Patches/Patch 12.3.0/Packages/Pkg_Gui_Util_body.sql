...
CREATE OR REPLACE PACKAGE BODY &schemaName.."GUI_UTIL"
AS
  ...
  BEGIN
    OPEN O_RS FOR
    SELECT ID,DESCRIPTION from &schemaName..VW_FRAGMENTTYPE;
  END;
  ...
  BEGIN
    ...
    FROM &schemaName..VW_PROJECT PRO
  ...
  BEGIN
  OPEN O_RS FOR
    SELECT F.FRAGMENTID, F.CODE, F.DESCRIPTION, F.FRAGMENTTYPEID,F.STRUCTURE, F.MOLWEIGHT, F.FORMULA FROM &schemaName..VW_FRAGMENT F
