		...
		ig_WebSpellCheckerDialog.prototype._fixScrollPosition = function()
		 {
			...
			for(var i =0; i < documentTextPanel.childNodes.length; i++)
			{
				...
				for(var j = 0; j < spans.length; j++)
				{
					...
					{
						documentTextPanel.scrollTop = spans[j].offsetTop;
						break;
					}
				}				
			}
		 ...
		 ig_WebSpellCheckerDialog.prototype.refresh = function ()
		 {
		 	...
		 	if (documentTextPanel != null)
		 	{
		 		if (this._currentWordIndex < this.getBadWords().length)
		 		{
		 			...
		 			var beginning = this._fixPlus(this.textToHtml(this.getText().substring(0, badWord.getStartPosition())));
		 			...
		 			var end = this._fixPlus(this.textToHtml(this.getText().substring(badWord.getEndPosition(), this.getText().length)));
