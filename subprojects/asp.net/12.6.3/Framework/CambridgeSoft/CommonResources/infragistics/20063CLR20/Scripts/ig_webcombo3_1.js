...
function igcmbo_initCombo(comboId
   ...
   var oCombo=null;
	...
	if(oCombo)
	{
		if(igtbl_isAtlas && oCombo.Element==comboElement)
		{
			this.Loaded=true;
...
function _igcmbo_setInputWidth(width)
{
	...
	var peWidth = this.Element.style.width;
	if(!peWidth)
...
function igcmbo_setDropDown(bDrop)
{
   // debugger;
	...
	else
	{
		...
		if (this.grid.event && typeof(this.grid.event.srcElement)!="undefined")
...
function igcmbo_highlightText(high)
{
	...
	else
	{
		...
		{
			oInput.style.backgroundColor = this.BackColor;
			oInput.style.color = this.ForeColor;
		}
	}
}
// private function
		...
		if (oCombo.keyCount<=0)
		{
				...
				)
			{
				if(oCombo.Editable && oCombo.ComboTypeAhead==3)
				{
					...
					oCombo.typeAheadTimeout = setTimeout("igtbl_processSuggestTypeAhead('"+oCombo.ClientUniqueId+"','"+newValue+"')",500);
				}
				else
				{
...
{
	var oCombo=igcmbo_getComboById(comboId);
	oCombo.selectWhere(oCombo.DataTextField+" LIKE '"+newValue+"%'");
	oCombo.setDropDown(true);
