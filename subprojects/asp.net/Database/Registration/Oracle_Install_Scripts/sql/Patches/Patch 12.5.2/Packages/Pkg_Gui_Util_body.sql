...
IS
 ...
 PROCEDURE GETIDENTIFIERLIST (
  ...
  BEGIN
    OPEN O_RS FOR
    SELECT ID, NAME, DESCRIPTION, ACTIVE FROM VW_IDENTIFIERTYPE WHERE (ACTIVE=AActive OR ACTIVE='T') AND (TYPE=AType OR TYPE='A');
  END;
 ...
 PROCEDURE GETFRAGMENTSLIST (O_RS OUT CURSOR_TYPE) IS
  ...
  PROCEDURE GETRECORDSCONTAININGCOMPOUND(
    pCompoundId IN VW_COMPOUND.COMPOUNDID%type
    , O_RS OUT CURSOR_TYPE
  ) IS
    LEnableRLS BOOLEAN;
  BEGIN
  ...
  PROCEDURE GETRECORDSCONTAININGSTRUCTURE(
    pStructureId IN VW_COMPOUND.STRUCTUREID%type
    , pExcludeNonStructureValues IN NUMBER
    , O_RS OUT CURSOR_TYPE
  ) IS
    LEnableRLS BOOLEAN;
  BEGIN
    ...
    IF LEnableRLS THEN
      RegistrationRLS.GEnableRLS := False;
    END IF;
    IF pExcludeNonStructureValues = 1 THEN
	    OPEN O_RS FOR
			SELECT RN.REGNUMBER
			...
			WHERE C.STRUCTUREID > 0 
				AND C.STRUCTUREID = pStructureId;
	ELSE
	    OPEN O_RS FOR
			SELECT RN.REGNUMBER
			FROM VW_REGISTRYNUMBER RN
				...
				INNER JOIN VW_MIXTURE_COMPONENT MC ON MC.MIXTUREID = M.MIXTUREID
				INNER JOIN VW_COMPOUND C ON C.COMPOUNDID = MC.COMPOUNDID 
			WHERE C.STRUCTUREID = pStructureId;
	END IF;	
    RegistrationRLS.GEnableRLS:=LEnableRLS;
  EXCEPTION
    WHEN OTHERS THEN
