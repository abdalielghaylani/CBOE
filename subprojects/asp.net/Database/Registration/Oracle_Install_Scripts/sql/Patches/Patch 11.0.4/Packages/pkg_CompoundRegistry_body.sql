...
--  GetFragmentXML
    ...
    IF (LFormulaWeight != 0) THEN
        ...
        IF NVL(ARegIDToValidate,0)<>0 THEN
            LQryCtx := DBMS_XMLGEN.newContext('
              SELECT RN.REGNUMBER
                  FROM VW_Compound C, VW_Structure_Drawing S,VW_RegistryNumber RN
  ...
  */
  ...
  FUNCTION GenerateFragmentSuffixForBatch (
    p_batchId IN number
    , p_fragment_delimiter IN varchar2 default to_char(UNISTR('\2022'))
  ...
  BEGIN
    ...
    where 1 = 1
      and b.batchid = p_batchId
    group by b.batchid, b.batchnumber, bcf.orderindex
  ...
  */
  ...
  BEGIN
    ...
    if ( v_sbi_setting != upper('true') OR v_mixture_setting = upper('true') ) then
  ...
  */
  ...
  BEGIN
    ...
    if ( v_component_count = 1 ) then
      ...
      begin
        v_suffix_value := GenerateFragmentSuffixForBatch(p_batchId, to_char(UNISTR('\2022')), v_frag_attribute);
      end;
  ...
  */
  ...
  BEGIN
    ...
    IF ARegNumber IS NOT NULL THEN
      ...
      begin
        select rn.regnumber into ARegNumber
        from vw_mixture m
  ...
  */
  ...
  BEGIN
    ...
    LQContext := dbms_xmlgen.newContext(
      'select
        m.mixtureid, m.created, m.modified
        , m.personcreated, m.approved, m.structureaggregation
  ...
  */
  ...
  BEGIN
    ...
    LQryCtx := DBMS_XMLGEN.newContext(
      'select
        p.id, p.batchid
      , m.compoundid, m.mixturecomponentid, -m.compoundid componentindex
    ...
    IF (ARegNumber IS NOT NULL) THEN
      ...
      IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Batch'), 0) > 0 ) THEN
        FOR R_Batch IN C_Batch(LRegID) LOOP
          LXml := GetBatchBlock(R_Batch.Batchid, LCoeObjectConfigField, ASectionsList);
          LResult := LResult || LXml;
        END LOOP;
      END IF;
      ...
      IF LSectionDelete THEN
        CASE UPPER(LTableName)
          ...
          WHEN 'VW_BATCH_PROJECT' THEN
            ...
            END;
/*
We don't care about BatchComponentFragment since DeleteFragment already takes care of it.
Otherwise, error occurs as the record has been deleted in VW_BatchComponentFragment.
          WHEN 'VW_BATCHCOMPONENTFRAGMENT' THEN
            BEGIN
...
TraceWrite('UpdateMcrr_LFieldToUpdate', $$plsql_line, LFieldToUpdate);
  ...
  */
  ...
  BEGIN
    ...
    v_isEditableFlag := GetIsEditable(ATempID);
    ...
    'select
      ...
      , cursor(
        select
          tc.tempcompoundid
          , tc.tempbatchid
          , cscartridge.molweight(tc.base64_cdx) formulaweight
          , cscartridge.formula(tc.base64_cdx, null) molecularformula
          , tc.personcreated
          , tc.DateLastModified
          , tc.datecreated
          , tc.sequenceid
          , tc.Base64_CDX as structure
          ...
          , case when tc.regid is null then 0 -- Related to CSBR-138689, the RegID for temp registry is null, this is a problem only in bulk registration
			else tc.regid
		  end
		  as regid
    ...
    v_buf := replace(v_buf, '<?xml version="1.0"?>', null);
    ...
    select XmlTransform(
      xmltype(v_buf)
      , v_xsl
      , 'sbiFlag="''' || v_sbiFlag || '''"  isEditableFlag="''' || v_isEditableFlag || '''"'
    )
  ...
  */
  ...
  BEGIN
    ...
    FOR itemRow in CRecords(p_mixid) LOOP
      v_idList(itemRow.batchnumber) := itemRow.batchid;
  ...
  */
  ...
  BEGIN
      ...
      dbms_xmlgen.setBindValue(LQryCtx, 'LBatchID', to_char(LBatchID));
      dbms_xmlgen.setBindValue(LQryCtx, 'LPersonRegistered', to_char(LPersonRegistered));
      dbms_xmlgen.setBindValue(LQryCtx, 'bindvar1', to_char(LBatchID));
      ...
      LQryCtx := DBMS_XMLGEN.newContext(LSelect);
      dbms_xmlgen.setBindValue(LQryCtx, 'LID', to_char(LID));
      dbms_xmlgen.setBindValue(LQryCtx, 'LBatchID', to_char(LBatchID));
      dbms_xmlgen.setBindValue(LQryCtx, 'LMixtureComponentID', to_char(LMixtureComponentID));
      dbms_xmlgen.setBindValue(LQryCtx, 'LCompoundID', to_char(LCompoundID));
      dbms_xmlgen.setBindValue(LQryCtx, 'bindvar1', to_char(LBatchID));
      dbms_xmlgen.setBindValue(LQryCtx, 'bindvar1', to_char(AID));
  ...
  BEGIN
    ...
    select extract(xmltype(AXml), '//BatchID').GetNumberVal() into v_batchId from dual;
