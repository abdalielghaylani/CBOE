...
--                  );
        -- for reg_numbers without any projects
                    select 
                    rn.cnt - rnp.cnt - bp.cnt into l_cnt
                    from
                                          (select ceil(count(reg_id)/2) as cnt, 1 as r   from reg_numbers rn
                    --                  join sequence s on s.sequence_id= rn.sequence_internal_id
                    --                  where  s.type !=  'C'
                                      ) rn, 
                                      (select count(distinct regid) as cnt, 1 as r from VW_RegistryNumber_Project) rnp,
                                      (select
                                          count(distinct b.reg_internal_id) as cnt, 1 as r
                                         from
                                            VW_BATCH_PROJECT bp
                                          join batches b on b.batch_internal_id=bp.batchid) bp
                     where rn.r = rnp.r and rnp.r = bp.r;
                     l_cnt := case when l_cnt<0 then 0 else l_cnt end;
                  select count(distinct regid) + l_cnt into l_ExactRecordCount from (
                  -- for private and public batches projects
                     select 
                      b.reg_internal_id as regid
                     from
                       (select distinct PROJECTID,type from (
                    select pr.project_internal_id as PROJECTID, pr.type,
                    case when upper(pr.is_public) = 'T' 
                    then upper(l_user_id) 
                    else UserID end as USERID 
                    ...
                    left join VW_PeopleProject PP on pr.project_internal_id = pp.projectid 
                    left join vw_People P on PP.PersonID = P.PERSONID 
                and upper(UserID)=nvl(l_user_id,1)) t where t.USERID is not null and upper(type) in ('A','B')) pp
                      join VW_BATCH_PROJECT bp   on bp.projectid = pp.PROJECTID
                      join batches b on b.batch_internal_id=bp.batchid
                      union all
                    -- for private and public registry projects
                      select
                ...
                left join VW_PeopleProject PP on pr.project_internal_id = pp.projectid
                left join vw_People P on PP.PersonID = P.PERSONID
                and upper(UserID)=nvl(l_user_id,1)) t where t.USERID is not null and upper(type) in ('A','R')) pp
                      join VW_RegistryNumber_Project rnp   on rnp.projectid = pp.PROJECTID);
