...
namespace CambridgeSoft.COE.DataLoader.Windows
{
    ...
    struct MappingTypeGeneric
    {
        ...
        {
            bool bRet = true;
            InputOutputMapper _oInputOutputMapper = new InputOutputMapper();
            string selectedOutputFieldName = _oInputOutputMapper.selectedctlTargetFieldName;
            try
            {
                    ...
                    case GenericType.Decimal:
                        {
                            ...
                            if (selectedOutputFieldName != null && selectedOutputFieldName == "Purity")
                            {
                                d = double.Parse(vstrThis);
                                Regex regex = new Regex(@"^[0-9]{1,3}([\.\,][0-9]{1,2})?$");
