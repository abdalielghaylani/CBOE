...
var igtbl_filterComparisionOperator=new Object();
igtbl_filterComparisionOperator.All=0;
igtbl_filterComparisionOperator.Empty=1;
igtbl_filterComparisionOperator.NotEmpty=2;
igtbl_filterComparisionOperator.Equals=3;
...
//** END OBSOLETE ***
	...
	this.eFilterOperand = igtbl_filterComparisionOperator;	
	...
	{		
		...
		if(ig_csom.IsIE)
		{
			this.Xslt=ig_createActiveXFromProgIDs(["MSXML2.FreeThreadedDOMDocument","Microsoft.FreeThreadedXMLDOM"]);
...
function()
{
	...
	else
	{
	    if(this.StatHeader)
	    {
	        ...
	        {
	            this.getDivElement().firstChild.style.width = this.StatHeader.Element.scrollWidth + "px";
            }
		    this.StatHeader.ScrollTo(this.getDivElement().scrollLeft);
	    }
	    if(this.StatFooter)
...
function(type,waitResponse)
{
	...
	var serverContext = {QueryString:igtbl_escape(igtbl_escape(this.QueryString)), requestType:"xml"};
...
function(col,band,parentRowId)
{
	...
	var currentFilters;
	if( ( (col && col.RowFilterMode==1)||band.Index==0)&&!band.IsGrouped )
	{
...
function(type,object,data,waitResponse)
{
	...
	switch(type)
	{
		...
		case g.eReqType.Filter:
		{
			...
			g.QueryString="Filter\x01"+col.getLevel(true)+"\x01"+parentRowDataKey+"\x01"+parentRowId+"\x01"+currentFilterString;
			g.xmlHttpRequest(type);			
		...
		case g.eReqType.UpdateRow:
		{
			...
			g.QueryString="UpdateRow\x01"+row._dataChanged+"\x02"+row.Band.Index+"\x02"+row.getLevel(true)+"\x02"+row.DataKey+"\x02"+g.RowsRetrieved+"\x02"+g.CurrentPageIndex+"\x04"+(cellInfo.length>0?"CellValues\x06"+cellInfo+"\x04":"")+g.QueryString;
			g.RowToQuery=row;
			g.xmlHttpRequest(type);
			break;
		}
				...
				if (bandFilters)
				{
					for (var colId in bandFilters)
					{
						...
						if (filter.IsActive())
						{
							...
							currentFilters+=col.getLevel(true)+"\x03"+filter.getOperator()+"\x03"+filter.getEvaluationValue()+"\x04";
						}
					}
				}
...
function()
{
	...
	if(ig_csom.IsNetscape6 && this.MainGrid.style.height.indexOf("%") != -1) 
	{
		igtbl_adjustNNHeight(this); 
	}
}
,"onCBSubmit",
function()
...
)
{
	...
	var defaultProps=new Array("Key","AllowAddNew","AllowColSizing","AllowDelete","AllowSort","ItemClass","AltClass","AllowUpdate",
								...
								"Filter_AllString","Filter_EmptyString","Filter_NonEmptyString","ServerPassedFilters","ApplyOnAdd","FilterDropDownRowCount","RowFilterMode"
...
function(oRow,oFilterCollection,oBand)
{
		...
		for (var filter in oFilterCollection)
		{
			...
			if (filter.IsActive())
			{
				if(! this._evaluateExpression(filter.getOperator(), oRow.getCell(filter.Column.Index).getValue() , filter.getEvaluationValue() ))
				{
					showRow = false;
					break;
				}
...
function(sourceElement)
{
	...
	{		
		...
		if (band.Index == 0 && band.GroupCount == 0)
		{
			...
			filterPanel.RowIsland = this.Band.Grid.Rows;
		}
		else
		{
			...
			if(!colE)
				colE=this.Band.Grid.event.target;
			
			var parentTable = colE;			
			
			if(this.Band.Index==0&&this.Band.IsGrouped&&(this.Band.Grid.StationaryMargins==1||this.Band.Grid.StationaryMargins==3))
			{
				parentTable=this.Band.Grid.Rows.Element;
			}
			do
			{
				parentTable = parentTable.parentNode;
			...
			filterPanel = this.Band._filterPanels[parentTable.id];
			if(filterPanel)
				filterPanel = filterPanel[this.Id];
			else
			{
				this.Band._filterPanels[parentTable.id] = new Object();
			}
			if (!filterPanel)
			{
				filterPanel = this.Band._filterPanels[parentTable.id][this.Id] = new igtbl_FilterDropDown(this);
	...
	else if (this.RowFilterMode == 1)
	{
		...
		filterPanel = this.Band._filterPanels[this.Id];
		if (!filterPanel)
		{
			filterPanel = this.Band._filterPanels[this.Id] = new igtbl_FilterDropDown(this);
		}
...
function(expand)
{
	...
	if(this.Node)
	{
		var rsn=this.Node.selectSingleNode("Rs");
			...
			if(this.Rows && typeof(this.Rows.Band.SortedColumns)!="undefined" && this.Rows.Band.SortedColumns!=null && igtbl_getLength(this.Rows.Band.SortedColumns)>0)
				this.Rows.sort();
	}
	else if(!this.Rows)
	{
			...
			if(!band.HeaderHTML)
			{
				...
				for(var i=0;i<band.Columns.length;i++)
				{
					...
					if(column.hasCells())
					{
						...
						{
							case 1:
								sortIndImg="&nbsp;<img src='"+g.SortAscImg+"' alt='"+g.SortAscImgAltText+"' border='0' height='12' width='12' imgType='sort'>";
								break;
							case 2:
								sortIndImg="&nbsp;<img src='"+g.SortDscImg+"' alt='"+g.SortDescImgAltText+"' border='0' height='12' width='12' imgType='sort'>";
								break;
						}
						ht+=sortIndImg;																		
		...
		if(col.hasCells())
		{
			if(this.Band.Grid.UseFixedHeaders && !col.getFixed())
			{
				...
				while(i<=index)
				{
					...
					{
						if(cells[cells.length-1].firstChild != null && cells[cells.length-1].firstChild.firstChild != null)
						{
...
{    
    ...
    if(tPan)
    {
		tPan.setPosition(editTempl.offsetTop-1,editTempl.offsetLeft-1,editTempl.offsetWidth+2,editTempl.offsetHeight+2);
		tPan.show();
    }
...
function(selFlag,fireEvent)
{
	...
	if(this==gs.oActiveRow
		&& (this.GroupByRow || !this.Band._optSelectRow)
	)
		this.renderActive();
	if(fireEvent!=false)
	{
...
function igtbl_initStatHeader(gs)
{
	...
	{	
		...
		if(outlGB)
		{
			var j=gs.Bands[0].getIndentation();
...
function igtbl_scrollStatHeader(scrollLeft)
{
	...
	if(parentNodeStyle.width && comWidth>0)
		parentNodeStyle.width=comWidth+"px";
}
...
function igtbl_changeBorder(g,elemFrom,elemTo,obj,attr,render)
{
	...
	var sel="";
	var attrs=new Array();
	if(obj && obj.getSelected && obj.getSelected())
		sel="Sel";
	if(attr)
		attrStyle="border"+attr+"Style";
	else
		attrStyle="borderStyle";
	attrs[attrs.length]=attrStyleSel=attrStyle;
	if(sel)
	{
		attrStyleSel+=sel;
		attrs[attrs.length]=attrStyleSel;
	}
	var attrColor;
	var attrColorSel;
	if(attr)
	...
	attrs[attrs.length]=attrColorSel=attrColor;
	if(sel)
	{
		attrColorSel+=sel;
		attrs[attrs.length]=attrColorSel;
	}
	var attrWidth;
	var attrWidthSel;
	if(attr)
	...
	attrs[attrs.length]=attrWidthSel=attrWidth;
	if(sel)
	{
		attrWidthSel+=sel;
		attrs[attrs.length]=attrWidthSel;
	}
	if(render==false)
	{
		if(typeof(obj["old"+attrStyleSel])!="undefined"
		 && obj["old"+attrStyleSel]!=null
		)
		{
			elemTo[attrStyle]=obj["old"+attrStyleSel];
		}
		if(typeof(obj["old"+attrColorSel])!="undefined"
		 && obj["old"+attrColorSel]!=null
		)
		{
			elemTo[attrColor]=obj["old"+attrColorSel];
		}
		if(typeof(obj["old"+attrWidthSel])!="undefined"
		 && obj["old"+attrWidthSel]!=null
		)
		{
			elemTo[attrWidth]=obj["old"+attrWidthSel];
		}
	...
	else
	{
		if(typeof(obj["old"+attrStyleSel])=="undefined"
		 || obj["old"+attrStyleSel]==null
			...
			obj["old"+attrStyleSel]=elemFrom[attrStyle];
		elemTo[attrStyle]=g.Activation.getValue(attr,"Style");
		if(typeof(obj["old"+attrColorSel])=="undefined"
		 || obj["old"+attrColorSel]==null
		)
			obj["old"+attrColorSel]=elemFrom[attrColor];
		elemTo[attrColor]=g.Activation.getValue(attr,"Color");
		if(typeof(obj["old"+attrWidthSel])=="undefined"
		 || obj["old"+attrWidthSel]==null
		)
			obj["old"+attrWidthSel]=elemFrom[attrWidth];
...
function()
{
	...
	{
		g.invokeXmlHttpRequest(g.eReqType.Filter, col, (col.RowFilterMode==1 ? null : this.RowIsland ) );
		return;
	}
