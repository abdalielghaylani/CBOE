...
<xsl:template match="R">
	...
	<tr id="{$gridName}_r_{$parentRowLevel}{$rowIndex}" level="{$parentRowLevel}{$rowIndex}">
		...
		<xsl:if test="$fac>0 and $rs!=2">
			<td id="{$gridName}_l_{$parentRowLevel}{$rowIndex}" class="{$band/@rowLabelClass}">
				<xsl:attribute name="style">
					"text-align:center;vertical-align:middle;"
					<xsl:if test="@rowSelectStyle">
						<xsl:value-of select="@rowSelectStyle" disable-output-escaping="yes" />
					</xsl:if>
				</xsl:attribute>
				<xsl:choose>
	...
	<xsl:if test="$useFixedHeaders">
		<td colspan="{$band/@nfspan}">
			...
			<div id="{$gridName}_drs">
				<xsl:attribute name="style">
					<xsl:choose>
						<xsl:when test="$row/@dtdh">overflow:hidden;height:100%;<xsl:if test="$isXhtml">position:relative;</xsl:if></xsl:when>
...
<xsl:template name="cellTemplate">
	...
	<xsl:if test="not($cell/@merged)">
		...
		<td id="{$gridName}_rc_{$parentRowLevel}{$rowIndex}_{$cellIndex}">
			...
			<xsl:choose>
				<xsl:when test="$cell/@br"><xsl:value-of select="$cell" disable-output-escaping="yes" /></xsl:when>
				<xsl:otherwise><nobr><xsl:value-of select="$cell" disable-output-escaping="yes" /></nobr></xsl:otherwise>
			</xsl:choose>
		</td>
	</xsl:if>
</xsl:template>
