...
href+=("?"+prefix+"="+encodeURIComponent(GetTocPath(moveNode)));}
...
var newWidth=-1;
if(gSlide){
newWidth=Math.min(Math.max(currWidth+gNavigationChangeStep,0),gNavigationWidth);}
else{
newWidth=0;}
for(var i=0,length=gChangingNavigationStateListeners.length;i<length;i++){
gChangingNavigationStateListeners[i](newWidth);}
...
var newHeight=gSlide?currHeight+gNavigationChangeStep:0;
var resizeBarHeight=7;
...
newXY.X+=FMCGetScrollLeft(window);
newXY.Y+=FMCGetScrollTop(window);
...
var newTop=y;
var newLeft=x;
if(y+parseInt(el.style.height)+5>FMCGetClientHeight(window,false)){
newTop=FMCGetScrollTop(window)+FMCGetClientHeight(window,false)-parseInt(el.style.height)-5;}
newTop-=absoluteTop;
if(x+parseInt(el.style.width)+5>FMCGetClientWidth(window,false)){
newLeft=FMCGetScrollLeft(window)+FMCGetClientWidth(window,false)-parseInt(el.style.width)-5;}
...
var img=FMCPopupThumbnailShow(node,clientCenter[0],clientCenter[1],"popup");
...
var img=FMCPopupThumbnailShow(node,mouseX,mouseY,"hover");
img.onmouseout=Onmouseout;
function Onmouseout(){
FMCRemoveOpacitySheet(window);
img.parentNode.removeChild(img);}}
function FMCPopupThumbnailShow(node,x,y,type){
...
if(MCGlobals.RootFrame.gNavigationState>0){
...
if(MCGlobals.InPreviewMode){
return;}
if(typeof(MCGlobals.BodyFrame.gRuntimeFileType)=="undefined"){
return;}
