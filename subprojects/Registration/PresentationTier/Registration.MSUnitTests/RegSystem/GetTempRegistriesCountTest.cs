...
{
    /// <summary>
    /// GetTempRegistriesCountTest class contains unit test methods for GetTempRegistriesCount
    /// </summary>
    [TestClass]
    public class GetTempRegistriesCountTest
    {
        #region Variables
        ...
        #region GetTempRegistriesCount Test Methods
        ...
        public void GetCustomItem_LoadingFormattedText() 
        {
            try
            {
                GetTempRegistriesCount theGetTempRegistriesCount = new GetTempRegistriesCount();
                ...
                foreach (Group myGroup in homeData.Groups)
                {
                    if (myGroup.PageSectionTarget.ToUpper().Equals("DASHBOARD"))
                    {
                        foreach (CustomItems customItems in myGroup.CustomItems)
                        {
                            ...
                            theGetTempRegistriesCount.SetConfiguration(customItems.Configuration);
                        }
                        break;
                    }
                }

                string strResult = theGetTempRegistriesCount.GetCustomItem();

                Assert.IsTrue(!string.IsNullOrEmpty(strResult), "GetTempRegistriesCount.GetCustomItem() did not return the expected value.");
            }
            catch (Exception ex)
            {
