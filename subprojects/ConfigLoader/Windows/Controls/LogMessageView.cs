    ...
    public partial class LogMessageView : UserControl
    {
        ...
        private class SeverityInfo
        {
            ...
            public int Count
            {
                ...
                {
                    _Count = value;
                    _Label.Text = Count.ToString() + " " + Name + ((Count != 1) ? "s" : "");
                    return;
                }
            } // Count
            public string Name
            ...
            set
            {
                ...
                _SummaryFilename.Text = "Log filename " + Path.GetFileNameWithoutExtension(_LogMessagePath);
                {
                    ...
                    try
                    {
                        ...
                        strText += " with an elapsed time of " + dElapsed + " second" + ((dElapsed != 1.0) ? "s" : "");
                    }
                    catch
                    {
                        ;
                ...
                if (Job != null)
                {
                    try
                    {
                        Job.Load(dictSummary["Specification"]);
                    }
                    catch
                    {
                        ;
