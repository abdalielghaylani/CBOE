...
namespace CambridgeSoft.COE.ConfigLoader.Windows.Controls
{
    ...
    public partial class UIBase : UserControl
    {
        ...
        private static COEProgressHelper _oPh = new COEProgressHelper();
        /// <summary>
        /// !!
        /// </summary>
        protected string _strStatusText = "";
        #endregion
        ...
        /// </summary>
        public static COEProgressHelper Ph
        {
            get
            {
                return _oPh;
        ...
        public UIBase()
        {
            // "{Name=ControlText, ARGB=(255, 0, 0, 0)}"
            // "{Name=InfoText, ARGB=(255, 0, 0, 0)}"
            // "{Name=MenuBar, ARGB=(255, 0, 0, 0)}"
            // "{Name=MenuHighlight, ARGB=(255, 0, 0, 0)}"
            // "{Name=MenuText, ARGB=(255, 0, 0, 0)}"
            // "{Name=WindowFrame, ARGB=(255, 0, 0, 0)}"
            // "{Name=WindowText, ARGB=(255, 0, 0, 0)}"

            // "{Name=HotTrack, ARGB=(255, 0, 0, 255)}"

            // "{Name=ActiveCaption, ARGB=(255, 10, 36, 106)}"
            // "{Name=Highlight, ARGB=(255, 10, 36, 106)}"

            // "{Name=Desktop, ARGB=(255, 58, 110, 165)}"

            // "{Name=ControlDarkDark, ARGB=(255, 64, 64, 64)}"

            // "{Name=AppWorkspace, ARGB=(255, 128, 128, 128)}"
            // "{Name=ButtonShadow, ARGB=(255, 128, 128, 128)}"
            // "{Name=ControlDark, ARGB=(255, 128, 128, 128)}"
            // "{Name=GrayText, ARGB=(255, 128, 128, 128)}"
            // "{Name=InactiveCaption, ARGB=(255, 128, 128, 128)}"

            // "{Name=GradientActiveCaption, ARGB=(255, 166, 202, 240)}"

            // "{Name=GradientInactiveCaption, ARGB=(255, 192, 192, 192)}"

            // "{Name=ActiveBorder, ARGB=(255, 212, 208, 200)}"
            // "{Name=ButtonFace, ARGB=(255, 212, 208, 200)}"
            // "{Name=Control, ARGB=(255, 212, 208, 200)}"
            // "{Name=ControlLight, ARGB=(255, 212, 208, 200)}"
            // "{Name=InactiveBorder, ARGB=(255, 212, 208, 200)}"
            // "{Name=InactiveCaptionText, ARGB=(255, 212, 208, 200)}"
            // "{Name=Menu, ARGB=(255, 212, 208, 200)}"
            // "{Name=ScrollBar, ARGB=(255, 212, 208, 200)}"

            // "{Name=ActiveCaptionText, ARGB=(255, 255, 255, 255)}"
            // "{Name=ButtonHighlight, ARGB=(255, 255, 255, 255)}"
            // "{Name=ControlLightLight, ARGB=(255, 255, 255, 255)}"
            // "{Name=HighlightText, ARGB=(255, 255, 255, 255)}"
            // "{Name=Info, ARGB=(255, 255, 255, 225)}"
            // "{Name=Window, ARGB=(255, 255, 255, 255)}"

            // test Color foo = ((SolidBrush)SystemBrushes.WindowText).Color;
        ...
        public static Icon GetIcon(string vstrIcon)
        {
            ...
            // string[] strResourceNames = oAssembly.GetManifestResourceNames();
            string strName = oAssembly.GetName().Name;
            System.IO.Stream oStream = oAssembly.GetManifestResourceStream("CambridgeSoft.COE." + strName + "." + "Resources" + "." + vstrIcon + ".ico");
        ...
        public static PictureBox GetPictureBox(PictureBoxType veIcon)
        {
            ...
            switch (veIcon)
            {
                case PictureBoxType.Logo:
                    {
                        oControl = GetPictureBoxGif("Config_Loader");
                        break;
                    }
        ...
        /// <returns>A string representation of the formatted xml document</returns>
