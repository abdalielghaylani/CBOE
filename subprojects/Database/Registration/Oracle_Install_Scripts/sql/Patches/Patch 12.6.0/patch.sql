...
--TABLES
--######################################################### 
@"sql\Patches\Patch 12.6.0\updatePicklistdomain.sql"
...
Connect &&schemaName/&&schemaPass@&&serverName
set define off
@"sql\Patches\Patch 12.6.0\Packages\pkg_CompoundRegistry_body.sql"
@"sql\Patches\Patch 12.6.0\Packages\pkg_RegistryDuplicateCheck_body.sql"
set define on
--#########################################################
--TRIGGERS
--#########################################################
...
UPDATE  &&schemaName.."FRAGMENTS" set DESCRIPTION = 'Ethyl formate' WHERE  trim(DESCRIPTION) ='Ethyl format' AND 
trim(FORMULA) ='C3H6O2';
