...
--CBOE-6299. query to analyze log table and find slow procedures in package
--select id,next_id,
--time1,next_time,
--case when next_time<time1 then next_time+60 else next_time end - time1 as diff_time,
--diff_date_sec,
--proc_name,
--run_seq,
--logdate, next_date,
...
--    (lead(run_seq) over (order by id)) as next_run_seq,
--to_number(substr(l.logprocedure,1,9)) as time1,
-- (lead(to_number(substr(l.logprocedure,1,9))) over (order by id)) as next_time,
...
-- and run_seq=next_run_seq;

--select id,to_char(LOGPROCEDURE) as LOGPROCEDURE,to_char(substr(LOGcomment,1,4000)) as LOGcom,LOGcomment
--   from regdb.log;
...
--  GetFragmentXML
  ...
  LEnableRLS BOOLEAN;
  --CBOE-6299. Added global variable to avoid muptiple runing of GetStructureDupCheckSettings function 
  GParameters          Varchar2(1000);
  -----------
  ...
  */
  ...
  BEGIN
    ...
    if LFormulaWeight != 0 then
      ...
      --CBOE-6299. Added global variable instead of runing of GetStructureDupCheckSettings function
      LParameters := GParameters;
      ------------
  ...
  */
  ...
  BEGIN
    dbms_application_info.read_module(mod1, act); TraceWrite( act||'CreateMultiCompoundRegistry_started', $$plsql_line,'start' );
     --CBOE-6299. Just for logging 
     TraceWrite( 'CreateMultiCompoundRegistry_in_param', $$plsql_line,
     to_clob( 'ADuplicateCheck: ' || ADuplicateCheck ||
     ', ARegNumGeneration: ' || ARegNumGeneration ||
      ', AConfigurationID: ' || to_char(AConfigurationID) ||
     ', ASectionsList: '||ASectionsList || 
     ', ASetBatchNumber: ' ||to_char(ASetBatchNumber) ||
      ', AXml: ') || AXml
     );
    ...
    IF UPPER(GetDuplicateCheckEnable)='TRUE' THEN
      ...
      IF Upper(ADuplicateCheck)='C' OR Upper(ADuplicateCheck)='V' THEN
        ...
        IF LListDulicatesCompound IS NOT NULL THEN
          ...
          END IF;
          --CBOE-6299. Just for logging 
          TraceWrite( act||'CreateMultiCompoundRegistry_AMessage', $$plsql_line,'AMessage '||AMessage );
    ...
    END IF;
     --CBOE-6299. Just for logging 
     TraceWrite( 'CreateMultiCompoundRegistry_AMessage2', $$plsql_line,AMessage );
     ------------
    TraceWrite( act||'CreateMultiCompoundRegistry_ended', $$plsql_line,'end' );
  END;
...
--    insert into FOR_DELETE_TEMP_REC select ATempID FROM dual;
  ...
  */
  ...
  PROCEDURE LoadMultiCompoundRegRecord(
    ...
    , ASectionsList IN Varchar2:=NULL
    , ASetBatchNumber IN Number := 1
      --CBOE-6299. Added Bulk Registration flag 
    , ABulkReg in number := 0--1 - Bulk Registration, 0 - usual Registration
      ---------------
    ) IS
  ...
  BEGIN
    ...
    IF ARegistration = 'Y' THEN
       ...
       --CBOE-6299. added check, if it is Bulk Registration and Duplicates option, then we don't need check duplicates  
        if ABulkReg=1 and upper(ADuplicateAction) = 'D'  then
            LError:='ok';
        -------------------
        else
...
begin
 ...
 --CBOE-6299. Added global variable to avoid muptiple runing of GetStructureDupCheckSettings function
 GParameters := GetStructureDupCheckSettings;
 -------------------
END;
/
