create or replace
PACKAGE BODY ConfigurationCompoundRegistry AS
    ...
    VW_MIXTURE_REGNUMBER VARCHAR2(1000):='CREATE OR REPLACE VIEW VW_MIXTURE_REGNUMBER AS
        SELECT    M.*, R.REGNUMBER, R.SequenceNumber
        FROM     VW_MIXTURE M, VW_REGISTRYNUMBER R
        WHERE     M.REGID = R.REGID';

    VW_MIXTURE_BATCH VARCHAR2(1000):='CREATE OR REPLACE VIEW VW_MIXTURE_BATCH AS
        SELECT     M.MIXTUREID, M.STRUCTUREAGGREGATION, R.REGNUMBER, B.*
        FROM     VW_MIXTURE M, VW_REGISTRYNUMBER R, VW_BATCH B
        WHERE     M.REGID = R.REGID AND R.REGID = B.REGID';

   VW_MIXTURE_BATCHCOMPONENT VARCHAR2(1000):='CREATE OR REPLACE VIEW VW_MIXTURE_BATCHCOMPONENT AS
	...
	PROCEDURE CreateOrReplaceTypes IS
	  vRegObjectTemplate XMLTYPE;
	  vBatchComponentFields CLOB;
	  vBatchComponentFlds varchar2(2000);
	  vSQL varchar2(1000);
	  vExists NUMBER;
	BEGIN
	  vBatchComponentFields := NULL;
	  vExists := 0;
	  ...
	  if vExists = 1 then
		...
		SELECT XmlTransform(vRegObjectTemplate, XmlType.CreateXml('
			...
			INTO VBatchComponentFields
			FROM dual;
		vSQL := 'SELECT wm_concat(COLUMN_NAME || '' '' || DATA_TYPE || ''('' || DATA_LENGTH || '')'' ) FROM all_tab_columns
		  WHERE lower(table_name)=''vw_batchcomponent''';
		IF vBatchComponentFields IS NOT NULL THEN
		  vBatchComponentFields := SUBSTR(vBatchComponentFields, 1, LENGTH(vBatchComponentFields)-1);
		  vBatchComponentFields := '''' || REPLACE(vBatchComponentFields, ',', ''',''') || '''';
		  vSQL := vSQL || ' and column_name in ( '|| vBatchComponentFields || ')';
		END IF;
		execute immediate vSQL into vBatchComponentFlds;
		SELECT count(1) into vExists FROM USER_OBJECTS where object_name = upper('BatchComponentPropertyList_T1') and object_type = upper('TYPE');
		if vExists = 1 then
		  execute immediate 'DROP TYPE BatchComponentPropertyList_T1';
		end if;
		execute immediate 'CREATE OR REPLACE TYPE BatchComponentPropertyList_T
					AS OBJECT ( ' || vBatchComponentFlds ||
				  ' )';
		execute immediate 'CREATE OR REPLACE TYPE BatchComponentPropertyList_T1 AS TABLE OF BatchComponentPropertyList_T';
		vExists := 0;
		SELECT count(1) into vExists FROM USER_OBJECTS where object_name = upper('BatchComponentFragment_T1') and object_type = upper('TYPE');
		if vExists = 1 then
		  execute immediate 'DROP TYPE BatchComponentFragment_T1';
		end if;
		execute immediate 'create or replace TYPE BatchComponentFragment_T AS OBJECT(
		  fragmentid NUMBER(8,0),
		  compoundfragmentid NUMBER(8,0),
		  equivalent NUMBER,
		  id NUMBER(8,0))';
		execute immediate 'CREATE OR REPLACE TYPE BatchComponentFragment_T1 AS TABLE OF  BatchComponentFragment_T';
	  END IF;
	END;
                ...
                IF ASection='MIXTURE' OR ASection='COMPOUND' OR ASection='BATCH' OR ASection='BATCHCOMPONENT' OR ASection='STRUCTURE' OR ASection='ADDIN' THEN
                     ...
                     FOR LIndex IN 0..DBMS_XMLDom.GetLength(LNodeListSource)-1 LOOP
                         ...
                         FOR LIndexAttr IN 0..DBMS_XMLDom.GetLength(LAttrs)-1 LOOP
                              ...
                              IF (LName='name' OR LName='friendlyName') THEN
                                LValidateChangesInDB:=ValidateChangesInDB(LValue,ASection);
...
/*     EXCEPTION
     ...
     BEGIN
      ...
      IF LFieldExist = 0 THEN
        LViewSelect :=
          SUBSTR(LViewSelect,1,INSTR(UPPER(LViewSelect),'FROM ', -1)-2)||','||ATableFieldName||' '||SUBSTR(LViewSelect,INSTR(UPPER(LViewSelect),'FROM ', -1),LENGTH(LViewSelect));
...
PROCEDURE AddFieldLog(AOperation IN VARCHAR2,AField IN VARCHAR2, AFieldType IN VARCHAR2, AObject IN VARCHAR2,AErrorCode IN VARCHAR2) IS
     BEGIN
        ErrorsLog:=ErrorsLog||
        '     <Error>'||CHR(10)||
        '          <Operation>'||AOperation||'</Operation>'||CHR(10)||
        ...
        '          <Fieldtype>'||AFieldType||'</Fieldtype>'||CHR(10)||
        '          <Object>'||AObject||'</Object>'||CHR(10)||
        '          <DefaultValue></DefaultValue>'||CHR(10)||
        '          <ErrorCode>'||AErrorCode||'</ErrorCode>'||CHR(10)||
        '     </Error>'||CHR(10);
     END;

PROCEDURE VerifyNullField(AField IN VARCHAR2, ATable IN VARCHAR2,AValidationType VARCHAR2)  IS
         LCount      Number;
         LStatement  Varchar2(4000);
         LFieldType Varchar2(200);
     BEGIN
          if(LOWER(AValidationType) = 'requiredfield') then
          LStatement:='SELECT COUNT(1) FROM '||ATable||' WHERE ROWNUM<2';
          else
          LStatement:='SELECT COUNT(1) FROM '||ATable||' WHERE ROWNUM<2 AND '||AField||' IS  NULL';
          End If;
          SELECT DATA_TYPE INTO LFieldType FROM sys.all_Tab_Columns  WHERE  Table_Name = ATable AND COLUMN_NAME = AField;
         $if ConfigurationCompoundRegistry.Debuging $then InsertLog('VerifyNullField'||' Line:'||$$plsql_line,'LStatement='||LStatement); $end null;
         BEGIN
             EXECUTE IMMEDIATE LStatement INTO LCount;
             IF LCount > 0 THEN
             AddFieldLog('Validating',AField,LFieldType,ATable,eFieldNotEmpty);
             END IF;
         END;
     END;

PROCEDURE VerifyFields(AField VARCHAR2,ATableName VARCHAR2,AValidationType VARCHAR2) IS
LIsException  Varchar2(200);
  BEGIN
      LIsException := 'false';
      VerifyNullField(AField,ATableName,AValidationType);
       EXCEPTION
         WHEN OTHERS THEN
          BEGIN
            LIsException := 'true';
  ...
  PROCEDURE ValidateTables(AErrorsLog OUT CLOB,APropertyListType VARCHAR2,AField VARCHAR2,AValidationType VARCHAR2) IS
  LTableName  Varchar2(200);
  LTempTableName  Varchar2(200);
  LIsException Varchar2(200);
  BEGIN
     LIsException:='';
     ErrorsLog :='';
     AErrorsLog:='';
            CASE UPPER(APropertyListType)
            WHEN 'MIXTURE' THEN
                 BEGIN
                    LTableName:='MIXTURES';
                    LTempTableName:='TEMPORARY_BATCH';
                    VerifyFields(AField,LTableName,AValidationType);
                    VerifyFields(AField,LTempTableName,AValidationType);
                 END;
             WHEN 'BATCH' THEN
                 BEGIN
                    LTableName:='BATCHES';
             ...
             WHEN 'BATCHCOMPONENT' THEN
                 BEGIN
                    LTableName:='BATCHCOMPONENT';
                    LTempTableName:='TEMPORARY_COMPOUND';
                    VerifyFields(AField,LTableName,AValidationType);
                    VerifyFields(AField,LTempTableName,AValidationType);
                 END;
             WHEN 'STRUCTURE' THEN
                 BEGIN
                    LTableName:='STRUCTURES';
                    LTempTableName:='TEMPORARY_COMPOUND';
                    VerifyFields(AField,LTableName,AValidationType);
                    VerifyFields(AField,LTempTableName,AValidationType);
                 END;
                 ELSE
                 BEGIN
                    DBMS_OUTPUT.PUT_LINE('No matching case statemenet: ');
                 END;
         END CASE;
...
PROCEDURE UpdateTables(AUpdateLogXML in clob,AErrorsLog OUT varchar2) IS


  LOperation VARCHAR2(200);
  LField VARCHAR2(200);
  LFieldtype VARCHAR2(200);
  Lobject VARCHAR2(200);
  LValuetext VARCHAR2(10);
  LValueNum NUMBER(22);
  LerrorCode VARCHAR2(200);
  LStatement  Varchar2(4000);
  ...
  BEGIN
     ...
     AErrorsLog:= 'True';
              ...
              l_xmltype := XMLTYPE(AUpdateLogXML);
              dbms_output.put_line(l_xmltype.getclobval());


              SELECT XMLSequence(l_xmltype.Extract('/ErrorList/*'))
              INTO LProjectsSequenceType FROM DUAL
              WHERE ExistsNode(l_xmltype,'/ErrorList/*')=1;


              FOR LProjectsIndex IN  LProjectsSequenceType.FIRST..LProjectsSequenceType.LAST LOOP
                LerrorCode := LProjectsSequenceType(LProjectsIndex).Extract('Error/ErrorCode/text()').getStringVal();

                IF LerrorCode = eFieldNotEmpty  THEN

                   LFieldtype := LProjectsSequenceType(LProjectsIndex).Extract('Error/Fieldtype/text()').getStringVal();
                   LField := LProjectsSequenceType(LProjectsIndex).Extract('Error/Field/text()').getStringVal();
                   Lobject := LProjectsSequenceType(LProjectsIndex).Extract('Error/Object/text()').getStringVal();

                     CASE UPPER(LFieldtype)
                       WHEN 'VARCHAR2' THEN
                         BEGIN
                            LValuetext := '';
                            LStatement:='UPDATE '||Lobject||' SET '||LField||' = :v1  WHERE '|| LField ||' IS NULL';
                            EXECUTE IMMEDIATE LStatement USING LProjectsSequenceType(LProjectsIndex).Extract('Error/DefaultValue/text()').getStringVal();
                            --Auditing.UpdateTrigger('TRG_'||Lobject||'_AU');
                            dbms_output.put_line('LStatement: ' || LStatement);
                            dbms_output.put_line('LField: ' || LField ||   ' LFieldtype: ' || LFieldtype ||   ' Object: ' || Lobject ||   ' Error Code: '|| eFieldNotEmpty||   ' DefaultValue: '||LValuetext);
                         END;
                       WHEN 'NUMBER' THEN
                         BEGIN
                            LValueNum := 0;
                            LStatement:='UPDATE '||Lobject||' SET '||LField||' = :v1  WHERE '|| LField ||' IS NULL';
                            EXECUTE IMMEDIATE LStatement USING  LProjectsSequenceType(LProjectsIndex).Extract('Error/DefaultValue/text()').getNumberVal();
                            --Auditing.UpdateTrigger('TRG_'||Lobject||'_AU');
                            dbms_output.put_line('LStatement: ' || LStatement);
                            dbms_output.put_line('LField: ' || LField ||   ' LFieldtype: ' || LFieldtype ||   ' Object: ' || Lobject ||   ' Error Code: '|| eFieldNotEmpty||   ' DefaultValue: '||LValueNum);
                         END;
					   WHEN 'DATE' THEN
                         BEGIN
                            LValuetext := '';
                            LStatement:='UPDATE '||Lobject||' SET '||LField||' = To_Date(:v1,''MM/DD/YYYY'') WHERE '|| LField ||' IS NULL';
                            EXECUTE IMMEDIATE LStatement USING LProjectsSequenceType(LProjectsIndex).Extract('Error/DefaultValue/text()').getStringVal();
                            --Auditing.UpdateTrigger('TRG_'||Lobject||'_AU');
                            dbms_output.put_line('LStatement: ' || LStatement);
                            dbms_output.put_line('LField: ' || LField ||   ' LFieldtype: ' || LFieldtype ||   ' Object: ' || Lobject ||   ' Error Code: '|| eFieldNotEmpty||   ' DefaultValue: '||LValuetext);
