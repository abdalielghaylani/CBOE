...
IS
 ...
 PROCEDURE GETFRAGMENTSLIST (O_RS OUT CURSOR_TYPE) IS
  ...
  BEGIN
    sqlQuery :='SELECT F.FRAGMENTID, F.CODE, F.DESCRIPTION, F.FRAGMENTTYPEID,F.STRUCTURE, F.MOLWEIGHT, F.FORMULA FROM &schemaName..VW_FRAGMENT F ORDER BY F.' || FragmentSortField;   
