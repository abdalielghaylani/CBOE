...
--  GetFragmentXML
  ...
  */
  ...
  BEGIN
    IF (v_batch_prefix_setting is NULL) THEN
      LSettings := GetSystemSettings;
      SELECT Extract(
        LSettings,
      ...
      FROM DUAl;
      v_batch_prefix_setting := to_char(LValue);
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    v_output_value := SUBSTR((GetBatchPrefixValue(p_batchId)||v_output_value ),1 ,50);
...
');
 ...
 PROCEDURE UpdateApprovedStatus(ATempID NUMBER, AStatusID IN NUMBER) IS
  BEGIN
    IF(AStatusID = cSubmittedStatus OR cApprovedStatus = 2) THEN
