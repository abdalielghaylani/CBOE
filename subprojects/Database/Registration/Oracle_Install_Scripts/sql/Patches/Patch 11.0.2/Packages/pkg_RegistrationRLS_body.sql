...
******************************************************************************/
    ...
    BEGIN
        ...
        IF UPPER(AState)='Y' OR UPPER(AState)='TRUE' THEN
            ...
            AddPolicy('BATCHES');
            AddPolicy('PROJECTS');
            COEDB.ConfigurationManager.CreateOrUpdateParameter('Registration','REGADMIN','ActiveRLS','True' ,NULL,NULL ,NULL , NULL);
         ELSE
            ...
            DropPolicy('BATCHES');
            DropPolicy('PROJECTS');
            COEDB.ConfigurationManager.CreateOrUpdateParameter('Registration','REGADMIN','ActiveRLS','False' ,NULL,NULL ,NULL , NULL);
         END IF;
    EXCEPTION
        WHEN OTHERS THEN
        BEGIN
    ...
    BEGIN
        $if RegistrationRLS.Debuging $then
            IF gEnableRLS
                THEN InsertLog('PeopleProject_RLL_Function','client_identifier='||UPPER(sys_context('userenv','client_identifier'))||'session_user='||UPPER(sys_context('userenv','session_user'))||' gEnableRLS=True');
                ELSE InsertLog('PeopleProject_RLL_Function','client_identifier='||UPPER(sys_context('userenv','client_identifier'))||'session_user='||UPPER(sys_context('userenv','session_user'))||' gEnableRLS=False');
            END IF;
        $end null;

        IF ((NOT gEnableRLS) OR UPPER(sys_context('userenv','session_user')) = 'SYS' OR UPPER(sys_context('userenv','session_user')) = 'SYSTEM') THEN
            RETURN '';
        ELSE
