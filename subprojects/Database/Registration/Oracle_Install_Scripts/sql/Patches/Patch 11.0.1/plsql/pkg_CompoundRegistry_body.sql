                        ...
                        <Compound>
                            ...
                            <BaseFragment>
                                <Structure>
                                    ...
                                    <StrucureFormat><xsl:value-of select="STRUCTUREFORMAT"/></StrucureFormat>
            ...
            <BatchList>
                <xsl:for-each select="Batch/ROW">
                    <Batch>
                        ...
                        <xsl:for-each select="PROJECTXMLBATCH"><xsl:value-of select="."/></xsl:for-each>
...
EXCEPTION
    WHEN OTHERS THEN
    BEGIN
        RAISE_APPLICATION_ERROR(eGenericException, AppendError('SaveRegNumberDuplicatedHidden', DBMS_UTILITY.FORMAT_ERROR_STACK||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE));
    END;
END;
...
BEGIN
    ...
    IF LFormulaWeight!= 0 THEN
        ...
        /*IF LRLSState AND LResultXML IS NOT NULL THEN  --Save in VW_Duplciated only when the duplicated are hidden
            ...
            $if CompoundRegistry.Debuging $then InsertLog('ValidateCompoundMulti','LQryCtx->'||'
                SELECT extract(value(RegNumberDuplicatedHidden), ''//COMPOUNDID/text()'').getNumberVal() COMPOUNDID FROM Table(XMLSequence(Extract(XmlType.CreateXml('''||LResult||'''), ''/ROWSET/COMPOUNDID''))) RegNumberDuplicatedHidden
                MINUS
                SELECT C.CompoundID
                    FROM  VW_Compound C, VW_Structure S
            ...
            LQryCtx := DBMS_XMLGEN.newContext('
                SELECT extract(value(RegNumberDuplicatedHidden), ''//COMPOUNDID/text()'').getNumberVal() REGNUMBER FROM Table(XMLSequence(Extract(XmlType.CreateXml('''||LResult||'''), ''/ROWSET/COMPOUNDID''))) RegNumberDuplicatedHidden
                MINUS
                SELECT C.CompoundID
                    FROM  VW_Compound C, VW_Structure S
                    WHERE C.StructureId = S.StructureId AND C.CompoundID IN ('||LResultSerealized||')
...
-->return                     a registration number
*/
FUNCTION GetFullRegNumber(ASequenceID in VW_SEQUENCE.SequenceId%Type, AXmlTables IN XmlType,ARegNumber VW_REGISTRYNUMBER.RegNumber%Type,ABatchNumber   VW_BATCH.BatchNumber%Type) RETURN VW_BATCH.FullRegNumber%Type IS
    PRAGMA AUTONOMOUS_TRANSACTION;
...
BEGIN
    ...
    IF LSaltSuffixType IS NOT NULL  THEN
        EXECUTE IMMEDIATE 'BEGIN :LSaltName:=CompoundRegistry.'||LSaltSuffixType||'(:AXmlTables); END;' USING  OUT LSaltName, IN AXmlTables ;
    ...
    LFullRegNumber:=SUBSTR(LFullRegNumber||LBatchDelimiter||LPad(ABatchNumber,2,'0'),1,50);
...
BEGIN
    ...
    IF LCountMixture=0 THEN
        DELETE VW_Compound_Identifier WHERE RegID = (SELECT RegID FROM VW_Compound WHERE CompoundID=ACompoundIDToDelete);
        AMessage:=AMessage || chr(10) ||  cast (SQL%ROWCOUNT  as string) || ' Row/s Deleted on "VW_Compound_Identifier".';
        DELETE VW_RegistryNumber WHERE RegID = (SELECT RegID FROM VW_Compound WHERE CompoundID=ACompoundIDToDelete);
...
PROCEDURE CreateMultiCompoundRegistry(
  ...
  , ASectionsList IN Varchar2:=NULL
    ...
    PROCEDURE ValidateRegNumber(ARegNumber VW_RegistryNumber.RegNumber%type, ASequenceID VW_SEQUENCE.SequenceId%Type, AProcessingMixture IN VARCHAR2) IS
        LRegNumber        VW_RegistryNumber.RegNumber%type;
        LFullRegNumber    VW_BATCH.FullRegNumber%Type;
        LSequenceNumber   VW_REGISTRYNUMBER.SequenceNumber%Type;
...
BEGIN
        ...
        CASE UPPER(LTableName)
            WHEN 'VW_STRUCTURE' THEN
                BEGIN
                    ...
                    IF NVL(LStructureID,0)>=0 THEN -- (LStructureID <> -1 ,-2 and -3)
                        ...
                        IF LRegID=0 THEN
                            ...
                            IF ARegNumGeneration = 'Y' THEN
                                ...
                                IF LRegNumber='null' THEN
                                    ...
                                    END IF;
                                ELSE
                                    ValidateRegNumber(LRegNumber,LSequenceID,LProcessingMixture);
            ...
            WHEN 'VW_BATCH' THEN
                BEGIN
                    ...
                    IF LFullRegNumber='null' THEN
                        LFullRegNumber:=ARegNumber;
                        IF LSequenceID IS NOT NULL THEN
                            LFullRegNumber:=GetFullRegNumber(LSequenceID,LXmlTables,ARegNumber,LBatchNumber);
                        END IF;
                    ELSE
                        NULL;
                        --LSequenceID:= LXmlTypeRows.extract('VW_RegistryNumber/ROW/SEQUENCEID/text()').getNumberVal();
    ...
    IF ARegNumber IS NOT NULL THEN
        ...
        IF LXMLRegNumberDuplicatedHidden IS NOT NULL THEN
            $if CompoundRegistry.Debuging $then InsertLog('CreateMultiCompoundRegistry', 'LXMLRegNumberDuplicatedHidden->'||LXMLRegNumberDuplicatedHidden.getClobVal()); $end NULL;
  ...
  , ASectionsList in Varchar2 := NULL
    ...
    BEGIN
        ...
        IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Compound'), 0) > 0 ) THEN
            ...
            LNormalizedStructureList:=TakeOffAndGetClobslist(LXml,'<NORMALIZEDSTRUCTURE>',NULL,Null,FALSE);
...
BEGIN
    ...
    IF (ARegNumber IS NOT NULL) THEN
        ...
        IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Mixture'), 0) > 1 ) THEN
            LResult:='<MultiCompoundRegistryRecord '||'SameBatchesIdentity="'||GetSameBatchesIdentity||'"'||' TypeRegistryRecord="Mixture">';
        ...
        IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Batch'), 0) > 0 ) THEN
            ...
            FOR  R_Batch IN C_Batch(LRegID) LOOP
                ...
                LQryCtx:=DBMS_XMLGEN.newContext(
                'SELECT BI.ID, BI.Type , P.Description, P.Active, P.Name
    ...
    ELSE
        ...
        SELECT XmlType.CreateXml(XML).GetClobVal()
         ...
         AXml:=
          '<MultiCompoundRegistryRecord SameBatchesIdentity="'
          || GetSameBatchesIdentity
...
<VW_RegistryNumber_Project>delete="yes"<ROW>
<ID><xsl:value-of select="."/></ID>
</ROW>
</VW_RegistryNumber_Project>
</xsl:for-each>
<VW_RegistryNumber_Project><xsl:if test="$VInsertProject=''yes''">insert="yes"</xsl:if>
<ROW>
<xsl:for-each select="ID">
<ID>
<xsl:value-of select="."/>
...
<xsl:if test="$VInsertProject=''yes''"><REGID><xsl:value-of select="$VMixtureRegID"/></REGID></xsl:if>
</ROW>
</VW_RegistryNumber_Project>
</xsl:for-each>
<xsl:for-each select="IdentifierList/Identifier">
...
<VW_Compound_Identifier>delete="yes"<ROW>
<ID><xsl:value-of select="."/></ID>
</ROW>
</VW_Compound_Identifier>
</xsl:for-each>
<VW_Compound_Identifier><xsl:if test="$VInsertIdentifier=''yes''">insert="yes"</xsl:if>
<ROW>
<xsl:for-each select="ID">
<ID>
<xsl:value-of select="."/>
...
<xsl:if test="$VInsertIdentifier=''yes''"><REGID><xsl:value-of select="$VMixtureRegID"/></REGID></xsl:if>
</ROW>
</VW_Compound_Identifier>
</xsl:for-each>
<xsl:for-each select="ComponentList/Component">
...
<xsl:for-each select="Structure">                   <!-- .=-1 ,-2,-3)-->
<xsl:for-each select="StructureID[@update=''yes'' or . &lt; 0 or $VInsertComponent=''yes'']">
    <STRUCTUREID>
        <xsl:value-of select="."/>
    </STRUCTUREID>
</xsl:for-each>
...
<VW_BatchComponentFragment>
<xsl:choose>
 <xsl:when test="($VInsertComponent=''yes'') or ((/MultiCompoundRegistryRecord/ComponentList/Component[ComponentIndex=$VComponentIndex]/Compound/FragmentList/Fragment[@insert=''yes'']/FragmentID=$VFragmentID) and
((../../../../Batch/@update!=''yes'' or (string-length(@update)=0) ) and ((../../../../Batch/@insert!=''yes'' or (string-length(@insert)=0) ))) )">insert="yes"<ROW>
...
<VW_BatchComponentFragment>delete="yes"<ROW>
<ID><xsl:value-of select="ID"/></ID>
</ROW>
</VW_BatchComponentFragment>
</xsl:for-each>
...
<xsl:variable name="VInsertProject" select="@insert"/>
<xsl:for-each select="ID[$VDeleteProject=''yes'']">
<VW_Batch_Project>delete="yes"<ROW>
<ID><xsl:value-of select="."/></ID>
</ROW>
</VW_Batch_Project>
</xsl:for-each>
<VW_Batch_Project><xsl:if test="$VInsertProject=''yes''">insert="yes"</xsl:if>
<ROW>
<ID><xsl:value-of select="ID"/></ID>
<xsl:for-each select="ProjectID[@update=''yes'' or $VInsertProject=''yes'']">
<PROJECTID>
...
<xsl:variable name="VInsertIdentifier" select="@insert"/>
<xsl:for-each select="ID[$VDeleteIdentifier=''yes'']">
<VW_BatchIdentifier>delete="yes"<ROW>
<ID><xsl:value-of select="."/></ID>
</ROW>
</VW_BatchIdentifier>
</xsl:for-each>
<VW_BatchIdentifier><xsl:if test="$VInsertIdentifier=''yes''">insert="yes"</xsl:if>
<ROW>
<xsl:for-each select="ID">
<ID>
<xsl:value-of select="."/>
...
BEGIN
            ...
            IF LStructureValue IS NOT NULL AND INSTR(UPPER(LXMLCompound.getClobVal()),'DELETE="YES"')=0 AND LStructureUpdating IS NOT NULL THEN
                ...
                IF NVL(LDuplicatedAuxStructureID,0)>=0 THEN
                    IF ValidateWildcardStructure(LStructureValue) THEN
                        ...
                        $if CompoundRegistry.Debuging $then InsertLog('UpdateMultiCompoundRegistry','LRegIDToValidate ->'||LRegIDToValidate); $end null;
    ...
    EXIT WHEN LXmlRows IS NULL;
                ...
                WHEN 'VW_BATCH' THEN
                    BEGIN
                        ...
                        IF LFullRegNumber='null' THEN
                            ...
                            IF LSequenceID IS NOT NULL THEN
                                LFullRegNumber:=GetFullRegNumber(LSequenceID,LXmlTables,LFullRegNumber,LBatchNumber);
                            END IF;
                        ELSE
                            NULL;
                        --LSequenceID:= LXmlTypeRows.extract('VW_RegistryNumber/ROW/SEQUENCEID/text()').getNumberVal();
               ...
               WHEN 'VW_STRUCTURE' THEN
                BEGIN
                    SELECT extractValue(XmlType.CreateXml(LXmlRows),'VW_Structure/ROW/STRUCTUREID/text()')
                        INTO LStructureID
                        FROM dual;
                    IF NVL(LStructureID,0)>=0 THEN -- (LStructureID <> -1,-2,-3)
                        SELECT MOLID_SEQ.NEXTVAL INTO LStructureID FROM DUAL;
        ...
        -- Update
            ...
            IF UPPER(LTableName)='VW_COMPOUND' AND LNormalizedStructureValue IS NOT NULL THEN
...
BEGIN
   ...
   /* FOR  R_Batches IN C_Batches(ARegNumber) LOOP
    ...
    DELETE VW_Compound_Identifier WHERE RegID IN (SELECT RegID FROM VW_RegistryNumber WHERE RegNumber=ARegNumber);
    ...
    DELETE VW_BatchIdentifier WHERE BatchID IN (SELECT B.BatchID FROM VW_Batch B,VW_RegistryNumber RN WHERE B.RegID=RN.RegID AND RN.RegNumber=ARegNumber);
...
BEGIN
    ...
    IF (NVL(ATempID,0)>0) THEN
        ...
        LXmlComponent:='<MultiCompoundRegistryRecord '||'SameBatchesIdentity="'||GetSameBatchesIdentity||'"'||'> <Compound>'||LXmlComponent||'</Compound>';
        ...
        LSelect:='SELECT
                      ...
                      '''||LProjectXML||''' PROJECTXML,'''||LProjectXMLBatch||''' PROJECTXMLBATCH,'''||LIdentifierXML||''' IDENTIFIERXML,'''||LIdentifierXMLBatch||''' IDENTIFIERXMLBATCH,
                      STRUCTUREAGGREGATION
                  FROM VW_TEMPORARYBATCH
                  WHERE TempBatchID='||ATempID;
         $if CompoundRegistry.Debuging $then InsertLog('RetrieveMultiCompoundRegTmp',' LSelect='||LSelect); $end null;
    ...
    ELSE
        ...
        AXml:='<MultiCompoundRegistryRecord '||'SameBatchesIdentity="'||GetSameBatchesIdentity||'"'||Substr(AXml,29,Length(AXml));
        ...
        IF LCompoundCountRegistry = LCompoundCountBatch THEN
           SELECT Min(BatchID)
            ...
            ELSE
                ...
                INSERT INTO VW_BatchComponentFragment(BATCHCOMPONENTID,COMPOUNDFRAGMENTID,EQUIVALENT,ORDERINDEX)
                    SELECT BC_Target.ID,CF_Target.ID,1,1
                        ...
                        CF_Target.FragmentID IN(SELECT CF_Source.FragmentID
                                                    FROM  VW_BatchComponentFragment BCF_Source, VW_BatchComponent BC_Source, VW_Compound_Fragment CF_Source ,VW_Mixture_Component MC_Source
                                                    WHERE BC_Source.BatchID=ABatchID AND BCF_Source.BatchComponentID=BC_Source.ID AND BCF_Source.CompoundfragmentID= CF_Source.ID AND CF_Source.CompoundID=MC_Source.CompoundID AND MC_Source.MixtureID=LSourceMixtureID);
                $if CompoundRegistry.Debuging $then InsertLog ('MoveBatch', 'INSERT INTO VW_BatchComponentFragment: ' || SQL%ROWCOUNT); $end null;
            ...
            $if CompoundRegistry.Debuging $then InsertLog('CreateMultiCompoundRegistry','LSequenceID='||LSequenceID||' ARegNumber='||ARegNumber||' LBatchNumber='||LBatchNumber); $end null;
            RetrieveMultiCompoundRegistry(ARegNumber, LXMLRegistryRecord);
            LFullRegNumber:=GetFullRegNumber(LSequenceID,XmlType(LXMLRegistryRecord),ARegNumber,LBatchNumber);
...
BEGIN
    ...
    $if CompoundRegistry.Debuging $then InsertLog ('AddBatch','LXmlNewBatch:' || LXmlNewBatch.getClobVal()); $end null;
    ...
    SELECT updateXml(
            ...
            '/Batch/BatchComponentList/BatchComponent/CompoundID/text()', LCompoundID,
            '/Batch/BatchComponentList/BatchComponent/ComponentIndex/text()', -LCompoundID )
    ...
    SELECT insertChildXml(LXmlNewBatch, '/Batch', 'PersonRegistered', XMLType('<PersonRegistered>'||LPersonRegistered||'</PersonRegistered>'))
        INTO LXmlNewBatch FROM dual;
    SELECT insertChildXml(LXmlNewBatch, '/Batch/BatchComponentList/BatchComponent', 'MixtureComponentID', XMLType('<MixtureComponentID>'||LMixtureComponentID||'</MixtureComponentID>'))
        INTO LXmlNewBatch FROM dual;
    SELECT insertChildXml(LXmlNewBatch, '/Batch/BatchComponentList/BatchComponent', 'MixtureComponentID', XMLType('<MixtureComponentID>'||LMixtureComponentID||'</MixtureComponentID>'))
        INTO LXmlNewBatch FROM dual;
    ...
    IF not GetSameBatchesIdentity='True' THEN
        ...
        FOR R_NewFragments IN C_NewFragments(LRegNumber,LXmlNewBatch) LOOP
            ...
            SELECT insertChildXml(LXmlNewFragment, '/Fragment', '@insert', 'yes')
                INTO LXmlNewFragment FROM dual;
             $if CompoundRegistry.Debuging $then InsertLog ('AddBatch', 'LXmlNewFragment:' || LXmlRows); $end null;
