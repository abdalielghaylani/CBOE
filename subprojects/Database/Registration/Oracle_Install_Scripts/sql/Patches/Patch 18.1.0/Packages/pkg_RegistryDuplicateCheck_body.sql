  ...
  */
  ...
  BEGIN
    ...
    ELSE
        ...
        insert into duplicate_check(id, compound_id ,regid ,structureid )
        select l_id, compoundid, regid ,structureid  from (
...
--    v_refcur sys_refcursor;
  ...
  */
  FUNCTION FindComponentDuplicates(
    ...
    l_id number;
  BEGIN
    TraceWrite('FindComponentDuplicates_BEGIN', $$plsql_line, 'p_type: ' || p_type||'; p_params: ' || p_params||'; p_fragmentListToCompare: ' || p_fragmentListToCompare||'; p_value: ' || p_value);
    ...
    SELECT /*+1 OPT_ESTIMATE(table, t, scale_rows=0.0001) */
      xmlelement( "RegistryList",
        xmlattributes(
          ...
          , count(distinct MC.structureid) as "uniqueStructs"
          , case p_type
              when 'S' then 1
              when 'P' then 2
              when 'I' then 3
            ...
            xmlagg(
              xmlelement( "Component",
                ...
                xmlelement( "Structure",
                  xmlattributes( MC.structureid as "id" )
                )
              )
            )
          )
...
--	 VW_COMPOUND C
--	 INNER JOIN
( SELECT /*+ full(t) */ MC2.MIXTUREID, MC2.COMPOUNDID,t.regid ,t.structureid FROM
...
--   INNER JOIN VW_STRUCTURE S ON S.STRUCTUREID = MC.STRUCTUREID
	  ...
	  GROUP BY MR.REGID, MR.REGNUMBER, MC.COMPOUNDID, MC.STRUCTUREID
	  ORDER BY MR.REGID, MR.REGNUMBER, MC.COMPOUNDID, MC.STRUCTUREID;
  ...
  */
  ...
  BEGIN
    ...
    IF UPPER(vAllowUnregistCompInMix)='TRUE' THEN
        SELECT /*+ OPT_ESTIMATE(table, c,  scale_rows=0.0001) */
          xmlelement( "matches",
            xmlattributes(
        ...
        FROM
        --vw_compound c
        (select d.compound_id as compoundid, d.regid ,d.structureid from duplicate_check d where d.id = l_id) c
--          INNER JOIN vw_structure s on s.structureid = c.structureid
          INNER JOIN vw_mixture_component mc on mc.compoundid = c.compoundid
--          INNER JOIN vw_mixture m on m.mixtureid = mc.mixtureid
          INNER JOIN vw_mixture_regnumber mrn on mrn.mixtureid = mc.mixtureid
