...
CREATE OR REPLACE PACKAGE BODY REGDB.ConfigurationCompoundRegistry AS
/******************************************************************************
   NAME:       PropertyList
   PURPOSE:
...
******************************************************************************/
    ...
    VW_MIXTURE_REGNUMBER VARCHAR2(1000):='CREATE OR REPLACE VIEW REGDB.VW_MIXTURE_REGNUMBER AS
        SELECT	M.*, R.REGNUMBER
        FROM 	VW_MIXTURE M, VW_REGISTRYNUMBER R
        WHERE 	M.REGID = R.REGID';

     VW_MIXTURE_STRUCTURE VARCHAR2(1000):='CREATE OR REPLACE VIEW VW_MIXTURE_STRUCTURE AS
    ...
    VW_MIXTURE_BATCH VARCHAR2(1000):='CREATE OR REPLACE VIEW REGDB.VW_MIXTURE_BATCH AS
        SELECT 	M.MIXTUREID, R.REGNUMBER, B.*
        FROM 	REGDB.VW_MIXTURE M, REGDB.VW_REGISTRYNUMBER R, REGDB.VW_BATCH B
        WHERE 	M.REGID = R.REGID AND R.REGID = B.REGID';

   VW_MIXTURE_BATCHCOMPONENT VARCHAR2(1000):='CREATE OR REPLACE VIEW REGDB.VW_MIXTURE_BATCHCOMPONENT AS
                ...
                EXCEPTION
                    ...
                    IF INSTR(DBMS_UTILITY.FORMAT_ERROR_STACK,eFieldNotEmpty)<>0 THEN
                        ...
                        ELSIF RFieldsExitent.Data_Type='CLOB' AND LFieldType='CLOB' THEN
                             NULL; --Continue
                        ELSE
                            RAISE_APPLICATION_ERROR(eFieldNotEmptyNotCompatible,'The field can''t be added neither modified. The field exists, it isn''t empty and the type isn''t the same. (The type is '||RFieldsExitent.Data_Type||')');
                        END IF;
                    ELSE
                ...
                WHEN 'MIXTURE' THEN
                 ...
                 WHEN 'COMPOUND' THEN
                     BEGIN
                        ...
                        EXECUTE IMMEDIATE VW_MIXTURE_STRUCTURE;
        ...
        BEGIN
         ...
         PROCEDURE ProcessAndSaveXMLInDB(ADocumentNode DBMS_XMLDom.DOMNode) IS
             ...
             PROCEDURE XMLNodeDelete(AxPathTarget VARCHAR2,AxPathSource VARCHAR2,ASection IN VARCHAR2) IS
                 ...
                 END;
             BEGIN
                IF ASection='MIXTURE' OR ASection='COMPOUND' OR ASection='BATCH' OR ASection='BATCHCOMPONENT' OR ASection='ADDIN' THEN
                     LNodeListSource:=dbms_xslprocessor.selectNodes(ADocumentNode,AxPathSource);
