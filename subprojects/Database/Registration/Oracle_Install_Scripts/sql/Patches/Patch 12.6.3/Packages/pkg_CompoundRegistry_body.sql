    ...
    if LFormulaWeight != 0 then
      ...
      if (LDuplicateCount is not null and LDuplicateCount > 0) then
        for rec in (         
          ...
          from (
          select TT.column_value.extract('/Component/@id').GETNUMBERVAL() as "compoundid"
           ,tt.column_value.extract('/Component/Structure/@id').getnumberval() as "structureid"
          from components t,table(xmlsequence(t.comp.extract('/RegistryList/Registration/*'))) tt) 
    ...
    IF (ARegNumber IS NOT NULL) THEN
      ...
      IF ( ASectionsList IS NULL ) OR ( NVL(INSTR(ASectionsList,'Batch'), 0) > 0 ) THEN
        ...
        v_sqlFields1 :=
          ...
          , XMLELEMENT("IsBatchEditable", CompoundRegistry.GetIsBatchEditable(''' || ARegNumber || ''', b.batchnumber))';
