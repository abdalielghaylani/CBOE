...
PROCEDURE getCOERoles (
      ...
      mySql := 'SELECT DISTINCT ROLE_NAME
				 ...
				 MINUS
				  ...
				  WHERE  s.role_name IN (SELECT granted_role 
										 FROM   dba_role_privs
										 WHERE	Upper(grantee) = Upper(''' || replace(pusername, '''', '''''') || ''')
   ...
   BEGIN
    ...
    mySql := 'SELECT ROLE_NAME FROM (SELECT DISTINCT ROLE_NAME
				  FROM   security_roles s
				  WHERE  s.PRIVILEGE_TABLE_INT_ID IS NOT NULL  AND ' || excludeFGRolesSql || '
				 MINUS
				  SELECT ROLE_NAME
				  FROM   security_roles s
				  WHERE  s.PRIVILEGE_TABLE_INT_ID IS NOT NULL AND s.role_name IN (SELECT granted_role
										 FROM   dba_role_privs
										 WHERE	Upper(grantee) = Upper(''' || replace(pusername, '''', '''''') || ''')                     
										   AND  Upper(dba_role_privs.granted_role) IN ( SELECT DISTINCT ROLE_NAME
																						FROM   security_roles s
																						WHERE  s.coeidentifier LIKE NVL(''' || pCOEIdentifier || ''', ''%'') OR s.coeidentifier IS NULL))
          ) WHERE   ROLE_NAME IN (SELECT ROLE FROM DBA_ROLES) 
				  ORDER BY 1 ASC'; 
	ELSE
	mySql := 'SELECT ROLE_NAME FROM (SELECT DISTINCT ROLE_NAME 
				  FROM   security_roles s
