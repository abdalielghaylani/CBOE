...
namespace CambridgeSoft.DataLoaderGUI.Controls
{
    public partial class InputOutputMapper : UIBase
    {
        ...
        private Dictionary<string, string[]> _BindingExpressions;
        ...
        public int Invalid
        {
            ...
            set
            {
                _cInvalid = value;
                bool bEnabled = false;
                if (_HaveStructure == true || (_HaveStructure == false && _noStructureDupCheck == true))
                {
                    bEnabled = ((_dictMapsFieldUseage.Count > 0) && (_dictMapsFieldUseage.ContainsKey(string.Empty) == false) && (Invalid == 0));
                }
                else
                {
                    bEnabled = (Invalid == 0);
        ...
        public JobArgumentInfo ParsedArgs
        {
            set
            {
                this.jobArgs = value;
        ...
        public string[] Mapping
        {
            get
            {
                ...
                string[] xmlMappings = new string[2];

                if (_MainGridInputOutputSource != null)
                {
                    //Coverity fix - CID 18781
                    COEXmlTextWriter oCOEXmlTextWriter = new COEXmlTextWriter();
                    try
                    {
                        oCOEXmlTextWriter.WriteStartElement("mappings");
                        oCOEXmlTextWriter.WriteElementString("destinationRecordType", "RegistryRecord");

                        if (_HaveStructure == false)
                        {
                            using (COEXmlTextWriter xmlStructure = new COEXmlTextWriter())
                            {
                                xmlMappings[0] = "<mappings><destinationRecordType>RegistryRecord</destinationRecordType>";

                                xmlStructure.WriteStartElement("mapping");
                                xmlStructure.WriteElementString("enabled", "true");

                                xmlStructure.WriteElementString("objectBindingPath", "this");
                                xmlStructure.WriteStartElement("memberInformation");
                                xmlStructure.WriteAttributeString("memberType", "method");
                                xmlStructure.WriteElementString("type", "instance");
                                xmlStructure.WriteElementString("name", "AssignDefaultStructureById");
                                xmlStructure.WriteElementString("description", "Provide a default structure if none is been mapped");
                                xmlStructure.WriteStartElement("args");
                                xmlStructure.WriteStartElement("arg");
                                xmlStructure.WriteAttributeString("index", "0");
                                xmlStructure.WriteAttributeString("type", "int");
                                xmlStructure.WriteAttributeString("input", "constant");
                                xmlStructure.WriteElementString("value", "0");
                                xmlStructure.WriteEndElement();
                                xmlStructure.WriteStartElement("arg");
                                xmlStructure.WriteAttributeString("index", "1");
                                xmlStructure.WriteAttributeString("type", "int");
                                xmlStructure.WriteAttributeString("input", "constant");
                                if (_noStructureDupCheck == true)
                                {
                                    xmlStructure.WriteElementString("value", "-1");
                                }
                                else
                                {
                                    xmlStructure.WriteElementString("value", "-2");
                                }
                                xmlStructure.WriteEndElement();
                                xmlStructure.WriteEndElement();
                                xmlStructure.WriteEndElement();
                                xmlStructure.WriteEndElement();
                                xmlMappings[0] = xmlMappings[0] + xmlStructure.XmlString + "</mappings>";

                                oCOEXmlTextWriter.WriteStartElement("mapping");
                                oCOEXmlTextWriter.WriteElementString("enabled", "true");

                                oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this");
                                oCOEXmlTextWriter.WriteStartElement("memberInformation");
                                oCOEXmlTextWriter.WriteAttributeString("memberType", "method");
                                oCOEXmlTextWriter.WriteElementString("type", "instance");
                                oCOEXmlTextWriter.WriteElementString("name", "AssignDefaultStructureById");
                                oCOEXmlTextWriter.WriteElementString("description", "Provide a default structure if none is been mapped");
                                oCOEXmlTextWriter.WriteStartElement("args");
                                oCOEXmlTextWriter.WriteStartElement("arg");
                                oCOEXmlTextWriter.WriteAttributeString("index", "0");
                                oCOEXmlTextWriter.WriteAttributeString("type", "int");
                                oCOEXmlTextWriter.WriteAttributeString("input", "constant");
                                oCOEXmlTextWriter.WriteElementString("value", "0");
                                oCOEXmlTextWriter.WriteEndElement();
                                oCOEXmlTextWriter.WriteStartElement("arg");
                                oCOEXmlTextWriter.WriteAttributeString("index", "1");
                                oCOEXmlTextWriter.WriteAttributeString("type", "int");
                                oCOEXmlTextWriter.WriteAttributeString("input", "constant");

                                if (_noStructureDupCheck == true)
                                {
                                    oCOEXmlTextWriter.WriteElementString("value", "-1");
                                }
                                else
                                {
                                    oCOEXmlTextWriter.WriteElementString("value", "-2");
                                }
                            ...
                            string strEquivalentsValue = string.Empty;
                            for (int nOutputField = 0; nOutputField < _MainGridInputOutputSource.Length; nOutputField++)
                            {
                                string strFieldName = _MainGridInputOutputSource[nOutputField].OutputFieldName;
                                string strFieldListValue = _MainGridInputOutputSource[nOutputField].FieldList_Value;
                                string strFieldType = _MainGridInputOutputSource[nOutputField].OutputType;
                                string strObjectBindingPath = string.Empty;
                                string strObjectBindingType = string.Empty;

                                foreach (KeyValuePair<string, string[]> kvp in _BindingExpressions)
                                {
                                    string[] strValue = kvp.Value;
                                    if (strValue[1] == strFieldName)
                                    {
                                        strObjectBindingPath = kvp.Key;
                                        strObjectBindingType = strValue[0];
                                        break;
                                    }
                                }
                                if (string.IsNullOrEmpty(strObjectBindingPath))
                                {
                                    //coverity fix
                                    return xmlMappings; // null;
                                }


                                switch (_MainGridInputOutputSource[nOutputField].WhichShowing)
                                {
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Default:
                                        {
                                            break;
                                        }
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Map:
                                        {
                                            EditMapping(
                                                _MainGridInputOutputSource,
                                                nOutputField,
                                                strObjectBindingPath,
                                                strFieldName,
                                                strFieldType,
                                                strFieldListValue,
                                                xmlMappings,
                                                strObjectBindingType,
                                                "derived",
                                                ref strEquivalentsValue,
                                                ref oCOEXmlTextWriter);
                                            break;
                                        }
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Textbox:
                                        {
                                            EditMapping(
                                                _MainGridInputOutputSource,
                                                nOutputField,
                                                ...
                                                strFieldName,
                                                strFieldType,
                                                _MainGridInputOutputSource[nOutputField].ConstantText,
                                                xmlMappings,
                                                strObjectBindingType,
                                                "constant",
                                                ref strEquivalentsValue,
                                    ...
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Date:
                                        {
                                            EditMapping(
                                                _MainGridInputOutputSource,
                                                nOutputField,
                                                ...
                                                strFieldName,
                                                strFieldType,
                                                _MainGridInputOutputSource[nOutputField].DataValue,
                                                xmlMappings,
                                                strObjectBindingType,
                                                "constant",
                                                ref strEquivalentsValue,
                                    ...
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Checkbox:
                                        {
                                            EditMapping(
                                                _MainGridInputOutputSource,
                                                nOutputField,
                                                ...
                                                strFieldName,
                                                strFieldType,
                                                _MainGridInputOutputSource[nOutputField].CheckResult.ToString(),
                                                xmlMappings,
                                                strObjectBindingType,
                                                "constant",
                                                ref strEquivalentsValue,
                                    ...
                                    case CambridgeSoft.DataLoaderGUI.Controls.InputOutputSource.Showing.Picklist:
                                        {
                                            EditMapping(
                                                _MainGridInputOutputSource,
                                                nOutputField,
                                                ...
                                                strFieldName,
                                                strFieldType,
                                                _MainGridInputOutputSource[nOutputField].PickListValue,
                                                xmlMappings,
                                                strObjectBindingType,
                                                "constant",
                                                ref strEquivalentsValue,
                            ...
                            oCOEXmlTextWriter.WriteEndElement();
                            xmlMappings[1] = oCOEXmlTextWriter.XmlString;                       
                    }
                    catch
                    {
                    }
        ...
        private void EditMapping(InputOutputSource[] mainGridInputOutputSource, int nOutputField,
            string strObjectBindingPath,
            string strFieldName, string strFieldType, string strFieldListValue,
            string[] xmlMappings,
            string strObjectBindingType,
            string strInput,
            ref string strEquivalentsValue,
            ref COEXmlTextWriter oCOEXmlTextWriter)
        {

            if (strObjectBindingType.ToLower() == MIXTURE)
            {
                if (strFieldName.ToLower() == "projects")
                {
                    oCOEXmlTextWriter.WriteStartElement("mapping");
                    oCOEXmlTextWriter.WriteElementString("enabled", "true");
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this");
                    ...
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "method");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("name", "AddProject");
                    oCOEXmlTextWriter.WriteElementString("description", "Add a project using its name");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
                    oCOEXmlTextWriter.WriteAttributeString("type", "int");
                    oCOEXmlTextWriter.WriteElementString("value", strFieldListValue);
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                }
                else if (strFieldName.ToLower() == "prefix")
                {
                    oCOEXmlTextWriter.WriteStartElement("mapping");
                    oCOEXmlTextWriter.WriteElementString("enabled", "true");

                    if ("derived".Equals(strInput))
                    {
                        oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.RegNumber.Sequence.Prefix");
                    }
                    else
                    {
                        oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.RegNumber.Sequence.ID");
                    }
                    oCOEXmlTextWriter.WriteStartElement("memberInformation");
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "property");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("description", "the sequence ID that defines the reg-number prefix used");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
                ...
                else if (strFieldName.ToLower() == "regnum")
                {
                    oCOEXmlTextWriter.WriteStartElement("mapping");
                    oCOEXmlTextWriter.WriteElementString("enabled", "true");
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.RegNumber.RegNum");                   
                    oCOEXmlTextWriter.WriteStartElement("memberInformation");
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "property");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("description", "Known Reg number");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
                ...
                else if (strObjectBindingPath.ToLower().Contains("propertylist") &&
                    !strObjectBindingPath.ToLower().Contains("batchlist") &&
                    !strObjectBindingPath.ToLower().Contains("componentlist"))
                {

                    oCOEXmlTextWriter.WriteStartElement("mapping");
                    oCOEXmlTextWriter.WriteElementString("enabled", "true");
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this." + strObjectBindingPath);
                    oCOEXmlTextWriter.WriteStartElement("memberInformation");
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "property");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("description", "Registration custom property");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
                    oCOEXmlTextWriter.WriteAttributeString("type", strFieldType);
                    oCOEXmlTextWriter.WriteElementString("value", strFieldListValue);
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                ...
                else
                {
                    ...
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "method");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("name", "AddIdentifier");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", "constant");
                    oCOEXmlTextWriter.WriteAttributeString("type", strFieldType);
                    oCOEXmlTextWriter.WriteElementString("value", strFieldName);
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "1");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
            ...
            else if (strObjectBindingPath.ToLower().Contains("identifierlist"))
            {
                oCOEXmlTextWriter.WriteStartElement("mapping");
                oCOEXmlTextWriter.WriteElementString("enabled", "true");

                if (strObjectBindingPath.ToLower().Contains("batchlist") ||
                    strObjectBindingType.ToLower().Contains("batchlist"))
                {
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.BatchList[0]");
                }
                else if (strObjectBindingPath.ToLower().Contains("structure"))
                {
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.ComponentList[0].Compound.BaseFragment.Structure");
                }
                else
                {
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.ComponentList[0].Compound");
                }
                oCOEXmlTextWriter.WriteStartElement("memberInformation");
                oCOEXmlTextWriter.WriteAttributeString("memberType", "method");
                oCOEXmlTextWriter.WriteElementString("type", "instance");
                oCOEXmlTextWriter.WriteElementString("name", "AddIdentifier");
                oCOEXmlTextWriter.WriteElementString("description", "new identifier");
                oCOEXmlTextWriter.WriteStartElement("args");
                oCOEXmlTextWriter.WriteStartElement("arg");
                oCOEXmlTextWriter.WriteAttributeString("index", "0");
                oCOEXmlTextWriter.WriteAttributeString("input", "constant");
            ...
            else if (strObjectBindingPath.ToLower().Contains("basefragment.structure.value"))
            {
                COEXmlTextWriter xmlStructure = new COEXmlTextWriter();
                xmlMappings[0] = "<mappings><destinationRecordType>RegistryRecord</destinationRecordType>";
                ...
                {
                    xmlStructure.WriteElementString("objectBindingPath", "this.ComponentList[0].Compound.BaseFragment.Structure.Value");
                    xmlStructure.WriteStartElement("memberInformation");
                    xmlStructure.WriteAttributeString("memberType", "property");
                    xmlStructure.WriteElementString("type", "instance");
                    xmlStructure.WriteElementString("description", "structural representation");
                    xmlStructure.WriteStartElement("args");
                    xmlStructure.WriteStartElement("arg");
                    xmlStructure.WriteAttributeString("index", "0");
                    xmlStructure.WriteAttributeString("input", strInput);
                    xmlStructure.WriteAttributeString("type", strFieldType);
                    xmlStructure.WriteElementString("value", strFieldListValue);
                    xmlStructure.WriteEndElement();
                    xmlStructure.WriteEndElement();
                    xmlStructure.WriteEndElement();
                    xmlStructure.WriteEndElement();
                ...
                if (_HaveStructure == true)
                {
                    ...
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.ComponentList[0].Compound.BaseFragment.Structure.Value");
                    oCOEXmlTextWriter.WriteStartElement("memberInformation");
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "property");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("description", "structural representation");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", strInput);
            ...
            else if (strObjectBindingPath.ToLower().Contains("propertylist"))
            {
                ...
                if (strObjectBindingPath.ToLower().Contains("batchlist") ||
                    strObjectBindingType.ToLower().Contains("batchlist"))
                {
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.BatchList[0]." + strObjectBindingPath);
                }
                else
                {
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this.ComponentList[0]." + strObjectBindingPath);
                }
                oCOEXmlTextWriter.WriteStartElement("memberInformation");
                oCOEXmlTextWriter.WriteAttributeString("memberType", "property");
                oCOEXmlTextWriter.WriteElementString("type", "instance");
            ...
            else if (strObjectBindingPath.ToLower().Contains("fragmentslist"))
            {
                if (string.IsNullOrEmpty(strEquivalentsValue))
                {
                    oCOEXmlTextWriter.WriteStartElement("mapping");
                    oCOEXmlTextWriter.WriteElementString("enabled", "true");
                    oCOEXmlTextWriter.WriteElementString("objectBindingPath", "this");
                    ...
                    oCOEXmlTextWriter.WriteAttributeString("memberType", "method");
                    oCOEXmlTextWriter.WriteElementString("type", "instance");
                    oCOEXmlTextWriter.WriteElementString("name", "AddFragment");
                    oCOEXmlTextWriter.WriteStartElement("args");
                    oCOEXmlTextWriter.WriteStartElement("arg");
                    oCOEXmlTextWriter.WriteAttributeString("index", "0");
                    oCOEXmlTextWriter.WriteAttributeString("input", "constant");
                    ...
                    strEquivalentsValue = _MainGridInputOutputSource[nOutputField + 1].FieldList_Value;

                    if (!string.IsNullOrEmpty(strEquivalentsValue))
                    {
                        oCOEXmlTextWriter.WriteStartElement("arg");
                        oCOEXmlTextWriter.WriteAttributeString("index", "2");
                        oCOEXmlTextWriter.WriteAttributeString("input", strInput);
                        oCOEXmlTextWriter.WriteAttributeString("type", "float");
                        oCOEXmlTextWriter.WriteElementString("value", strEquivalentsValue);
                        oCOEXmlTextWriter.WriteEndElement();
                    }
                    else
                    {
                        ...
                        oCOEXmlTextWriter.WriteAttributeString("input", "constant");
                        oCOEXmlTextWriter.WriteAttributeString("type", "float");
                        oCOEXmlTextWriter.WriteElementString("value", "1.0"); //Set default Eqivalent value if not available in the input file
                        oCOEXmlTextWriter.WriteEndElement();
                    }
                    oCOEXmlTextWriter.WriteEndElement();
                    oCOEXmlTextWriter.WriteEndElement();
                ...
                {
                    strEquivalentsValue = string.Empty;
                }

            }
            if (strObjectBindingType.ToLower() == BATCH)
            {
                if (strFieldName.ToLower() == "projects")
                {
                    oCOEXmlTextWriter.WriteStartElement("mapping");
        ...
        public InputOutputMapper()
        {
            ...
            _LoadButton.Text = "Load Configuration";
        ...
        private void AcceptButton_Click(object sender, EventArgs e)
        {
            ...
            if (_MainGridInputOutputSource != null)
            {
                for (int nOutputField = 0; nOutputField < _MainGridInputOutputSource.Length; nOutputField++)
                {
                    ...
                    if (strObjectBindingPath.ToLower().Contains("fragmentslist"))
                    {
                        if (string.IsNullOrEmpty(fragment))
                        {
                            fragment = strFieldListValue;
                            index = 0;
                        }
                        else
                        {
                            if (index == 2)
                            {
                                if (fragment == strFieldListValue)
                                {
                                    MessageBox.Show("Fragments must be different.", Resources.Message_Title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                    return;
                                }
                            }
                        }
            ...
            statusLabel.ImeMode = System.Windows.Forms.ImeMode.NoControl;
            statusLabel.Location = new System.Drawing.Point(_SaveButton.Location.X, AcceptButton.Location.Y);
            statusLabel.Name = "_CommentLabel";
            statusLabel.Size = new System.Drawing.Size(213, 13);
            statusLabel.Text = "Duplicate checking! Please wait...";
            statusLabel.Font = new System.Drawing.Font("Microsoft Sans Serif", 11F, System.Drawing.FontStyle.Bold);
            ...
            if ((e.AffectedComponent == e.AffectedControl) && (e.AffectedProperty == "Bounds"))
            {
                    ...
                    if (yMax > 0)
                    {
                        ...
                        if (_InputFieldsGridLabel != null)
                        {
                            ...
                            if (_InputFieldsGrid.AutoScroll != bAutoScroll)
                            {
                                ...
                                if (_InputFieldsGrid.AutoScroll) newWidth -= 12;
                        ...
                        if (_InputFieldsGrid.HorizontalScroll.Visible)
                            nHeight += 10;
                ...
                if (_MainGrid.Width != x)
                {
                    ...
                    if (x > 0)
                    {
                        ...
                        if (_MainGridInputOutputSource != null && _MainGridInputOutputSource.Length > 0)     //  Coverity Fix: CBOE-1942
                        ...
                        if (_MainGridInputOutputSource != null)     //  Coverity Fix: CBOE-1942
                        {
                        ...
                        _MainGrid.ColumnStyles[3].Width = (newWidth > 50) ? (newWidth - 50) : 0;
                    }
                }
        ...
        private void MainGridInputOutputSource_Validated(object sender, EventArgs e)
        {
            ...
            {
                int nOutputField = Convert.ToInt32(_MainGridInputOutputSource.FieldList_ComboBox.Tag);
        ...
        public void SetForm()
        {
            OutputObjectForm fr = new OutputObjectForm();
            int usingForm = (int)this.SubmissionFormId;
            if (jobArgs.KnownRegNum)
                _xmlOutputFieldSpec = fr.ConfigurationSet(string.Empty, usingForm, FormGroup.DisplayMode.Add, "RegNum", "String;map;map|calculation", "RegNumber.RegNum", _HaveStructure);//Legacy Data support
            else
                _xmlOutputFieldSpec = fr.ConfigurationSet(string.Empty, usingForm, FormGroup.DisplayMode.Add, string.Empty, string.Empty, string.Empty, _HaveStructure);


            OutputFieldSpec = _xmlOutputFieldSpec;
            _noStructureDupCheck = fr.NoStructureDupCheck;

            Dictionary<string, Dictionary<string, string>> dictBindingExpressions;
            dictBindingExpressions = fr.BindingExpressions;
            _BindingExpressions = new Dictionary<string, string[]>();

            foreach (KeyValuePair<string, Dictionary<string, string>> kvps in dictBindingExpressions)
            {
                foreach (KeyValuePair<string, string> kvp in kvps.Value)
                {
                    //int index = kvp.Value.IndexOf(':') + 1;
                    string[] name = kvp.Value.Split(':');
                    _BindingExpressions.Add(kvp.Key, name);
                }
            }
        }
        #endregion
