...
namespace CambridgeSoft.DataLoaderGUI.Common
{
    public class OutputObjectForm
    {
        ...
        public static string activeRLSvalue;
        ...
        )
        {
            ...
            //KeyValuePair<PreloadDupCheckMechanism, string> _dupCheckMechanismKeyValuePair;
            //_dupCheckMechanismKeyValuePair = new KeyValuePair<PreloadDupCheckMechanism, string>();// RegSvcUtilities.GetNonStructuralDuplicateCheckSettings();
            //_dupCheckMechanism = new string[2];
            //_dupCheckMechanism[0] = _dupCheckMechanismKeyValuePair.Key.ToString();
            //_dupCheckMechanism[1] = Convert.ToString(_dupCheckMechanismKeyValuePair.Value);
        ...
        public OutputObjectForm()
        {
            _registryRecord = RegistryRecord.NewRegistryRecord();
            activeRLSvalue = _registryRecord.RLSStatus.ToString();
        }
        ...
        private void SetForm(FormGroup.Form oForm, FormGroup.DisplayMode vnMode, string fieldListKey)
        {
            ...
            if (oFormElementList != null)    // Coverity Fix- CBOE-1942  
            {
        ...
        private void SetFormElement(FormGroup.FormElement oFormElement, string fieldListKey)
        {
            ...
            if (bindingExpression.ToLower() == "BatchComponentFragmentList".ToLower())
            {
                ...
                _listFieldList.Add(FRAGMENT + ";Fragment 1;String;default;default|map;Fragment1");
                _listFieldList.Add(FRAGMENT + ";Fragment 1 Equivalents;Decimal;default;default|map;Fragment1Eq");
                _listFieldList.Add(FRAGMENT + ";Fragment 2;String;default;default|map;Fragment2");
                _listFieldList.Add(FRAGMENT + ";Fragment 2 Equivalents;Decimal;default;default|map;Fragment2Eq");
            ...
            else if ((bindingExpression == "IdentifierList") || (bindingExpression.EndsWith(".IdentifierList")))
            {
                ...
                if (oKeyValueListHolder != null)        // Coverity Fix: CBOE-1942
                {
                    foreach (System.Collections.DictionaryEntry de in oKeyValueListHolder.KeyValueList)
                    {
                        string strFieldInfo;
                        if (_strDisplayData.Contains("value=\"S\""))
                            strFieldInfo = fieldListKey + ":s"; // structure field group
                        else if (_strDisplayData.Contains("value=\"C\""))
                            strFieldInfo = fieldListKey + ":c"; // component field group
                        else
            ...
            else
            {
                // Emit
                string strFieldInfo;
                if (fieldLabel.ToLower().Contains("structure"))
                    strFieldInfo = fieldListKey + ":s";
                else if (fieldLabel.ToLower().Contains("component"))
                    strFieldInfo = fieldListKey + ":c";
                else
                ...
                // No structure duplicate check
                //else if ((_dupCheckMechanism.Length == 2 && _dupCheckMechanism[1] != null &&
                //    (bindingExpression.ToLower() == _dupCheckMechanism[1].ToLower() ||
                //    nameValue.ToLower() == _dupCheckMechanism[1].ToLower())) &&
                //    _haveStructure == false)
                else if (fieldLabel.ToLower() == "Structure".ToLower() && _haveStructure == true)
                {
                    strFieldInfo += ";" + "map";  // fieldDefault
                    strFieldInfo += ";" + "default|map|constant"; // strSources
                    strFieldInfo += ";" + "true"; // strSources
                ...
                //Prefix field mapping should not be allowed for known Reg Numbers (Legacy data)
                if (_vstrLabel != string.Empty && _vstrLabel == "RegNum" && strFieldInfo.Contains("mixturecsladatasource") && strFieldInfo.Contains("Prefix"))
                    validField = false;
                switch (activeRLSvalue)
                {
                    case "Off":
                    case "RegistryLevelProjects":
                        if (strFieldInfo.Contains(BATCH) && strFieldInfo.Contains("Projects"))
                            validField = false;
                        break;
                    case "BatchLevelProjects":
                        if (strFieldInfo.Contains(MIXTURE) && strFieldInfo.Contains("Projects"))
                            validField = false;
                        break;
        ...
        private void SetFieldConfigInfo(FormGroup.FormElement oFormElement, string fieldListKey)
        {
            ...
            {
                ProjectList.ProjectTypeEnum projectType = ProjectList.ProjectTypeEnum.A;
                switch (activeRLSvalue)
                {
                    case "Off":
                        projectType = ProjectList.ProjectTypeEnum.A;
                        break;
                    case "BatchLevelProjects":
                        projectType = ProjectList.ProjectTypeEnum.B;
                        break;
                    case "RegistryLevelProjects":
                        projectType = ProjectList.ProjectTypeEnum.R;
                        break;
                }
                ...
                if (string.IsNullOrEmpty(dataSourceID))
                {
                    ...
                    if (dataSourceID.EndsWith("ProjectsCslaDataSource"))
                    {                        
                        ...
                        _strDisplayData += "  <method name=\"" + "GetActiveProjectListByPersonIDAndType" + "\">" + "\r\n";
                        _strDisplayData += "    <paramList>" + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Int32", COEUser.ID) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.String", projectType) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += "    </paramList>" + "\r\n";
                ...
                if (bindingExpression == "Compound.IdentifierList" && string.Compare(GetRegSettingsCommand.Execute("Registration", "MISC", "Component_Identifiers"), "true", true) == 0)
                {
                    ...
                    else
                    {
                        ...
                        _strDisplayData += "  <method name=\"" + "GetIdentifierListByType" + "\">" + "\r\n";
                        _strDisplayData += "    <paramList>" + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.String", IdentifierTypeEnum.C) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += "    </paramList>" + "\r\n";
                        _strDisplayData += "  </method>" + "\r\n";
                ...
                if (bindingExpression == "Compound.BaseFragment.Structure.IdentifierList" && string.Compare(GetRegSettingsCommand.Execute("Registration", "MISC", "Structure_Identifiers"), "true", true) == 0)
                {
                    XmlNode dataSourceNode = oXmlDocument.SelectSingleNode("//COE:DataSource", oXmlNamespaceManager);
                    if (dataSourceNode != null)
                        _strDropDownItemsSelect = dataSourceNode.InnerText;
                    else
                    {
                        ...
                        _strDisplayData += "  <method name=\"" + "GetIdentifierListByType" + "\">" + "\r\n";
                        _strDisplayData += "    <paramList>" + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.String", IdentifierTypeEnum.S) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += "    </paramList>" + "\r\n";
                        _strDisplayData += "  </method>" + "\r\n";
                ...
                && fieldListKey == MIXTURE
                 && string.Compare(GetRegSettingsCommand.Execute("Registration", "MISC", "Registry_Identifiers"), "true", true) == 0)
                {
                    XmlNode dataSourceNode = oXmlDocument.SelectSingleNode("//COE:DataSource", oXmlNamespaceManager);
                    if (dataSourceNode != null)
                        _strDropDownItemsSelect = dataSourceNode.InnerText;
                    else
                    {
                        ...
                        _strDisplayData += "  <method name=\"" + "GetIdentifierListByType" + "\">" + "\r\n";
                        _strDisplayData += "    <paramList>" + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.String", IdentifierTypeEnum.R) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += "    </paramList>" + "\r\n";
                        _strDisplayData += "  </method>" + "\r\n";
                    ...
                    && fieldListKey == BATCH
                     && string.Compare(GetRegSettingsCommand.Execute("Registration", "MISC", "Batch_Identifiers"), "true", true) == 0)
                {
                    XmlNode dataSourceNode = oXmlDocument.SelectSingleNode("//COE:DataSource", oXmlNamespaceManager);
                    if (dataSourceNode != null)
                        _strDropDownItemsSelect = dataSourceNode.InnerText;
                    else
                    {
                        ...
                        _strDisplayData += "  <method name=\"" + "GetIdentifierListByType" + "\">" + "\r\n";
                        _strDisplayData += "    <paramList>" + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.String", IdentifierTypeEnum.B) + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += string.Format("      <param type=\"{0}\" value=\"{1}\"/>", "System.Boolean", "True") + "\r\n";
                        _strDisplayData += "    </paramList>" + "\r\n";
                        _strDisplayData += "  </method>" + "\r\n";
        ...
        private void SetXmlFieldSpec()
        {
            ...
            string strUiSelectionCat = string.Empty;
            string strCurrentGroupHeading = string.Empty;
            foreach (string strField in _listFieldList)
            {
                ...
                string[] UiSelection = strFieldInfo[0].Split(':');

                string strUiSection = UiSelection[0];                
                if (UiSelection.Length == 2)
                    strUiSelectionCat = UiSelection[1];
                ...
                switch (fieldGroupCaption)
                {
                    ...
                    case COMPONENT:
                        if (strUiSelectionCat == "s")
                            fieldGroupCaption = "Structure Information";
                        else if (strUiSelectionCat == "c")
                            fieldGroupCaption = "Component Information";
                        break;
                    case BATCH:
                ...
                if (strCurrentSection != strUiSection)
                {
                    ...
                    {                        
                        strCurrentGroupHeading = fieldGroupCaption;
                ...
                else if (!string.IsNullOrEmpty(strUiSelectionCat))
                {
                    if (strCurrentGroupHeading != fieldGroupCaption)
                    {
                        if (strCurrentSection.Length > 0)
                        {
                            // close (previous) fieldlist
                        ...
                        case "picklist":    // EXCEPTION
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        //start PickListNameValueList for 1252
                                        strPicklistInfo = PickListNameValueList.GetDropdownSql(pickListDomainId, PickListStatus.Active, null);

                                        PickListNameValueList oPickListNameValueList =
                                            PickListNameValueList.GetPickListNameValueList(strPicklistInfo);
                                        //end PickListNameValueList for 1252

                                        //start PickListNameValueList for 1251
                                        //PickListNameValueList oPickListNameValueList =
                                        //    PickListNameValueList.GetPickListNameValueList(Constants.REGDB, pickListDomainId);
                                        //end PickListNameValueList for 1251
