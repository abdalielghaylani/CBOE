...
{      
      var docWindowName = 'DOCManager';
      var windowname = gup('windowname');
      if(windowname == '') windowname = docWindowName;     
...
function CheckToOpenNewWindow( versionSupported )
{
      ...
      { 
          if(window.name.toUpperCase()!= windowname) //Different window context
          {
            ...
            if(newWin == null || typeof(newWin) == "undefined") //Check if window was really open. If not, display a PopUp Blocker message.
            {
                window.alert('Pop-up blocker detected!\n\nTo use CambridgeSoft DocManger Enterprise, your web browser must allow pop-up windows.\n For information about allowing pop-up windows, see the instructions for your pop-up blocking software.');
                history.back();
            }
	        else
            {
...
function SetSQL(txtBoxId, selection)
{
   ...
   YAHOO.COEDocMangerWeb.PickListDomainPanel.hide();  
}
function ShowPanel(anchorCtrlId, innerCtrls, title) 
{
    if(!document.getElementById(anchorCtrlId).disabled)
    {
    YAHOO.namespace('COEDocManagerWeb');
    YAHOO.COEDocMangerWeb.PickListDomainPanel = new YAHOO.widget.Panel('pickListPanel',{width:150,visible:true,draggable:true,close:true,modal:false,constraintoviewport:true,zIndex:100,context:[anchorCtrlId,'br','br']});
    var header = '<span class="PickListDivTitle">' + title + '</span>';
    YAHOO.COEDocMangerWeb.PickListDomainPanel.setHeader(header); 
    YAHOO.COEDocMangerWeb.PickListDomainPanel.setBody(innerCtrls);
    YAHOO.COEDocMangerWeb.PickListDomainPanel.render(document.body);
    }
}
...
function IsAValidCas(input)
{
    ...
    else
    {
        ...
        //else if (regExpACX.test(input))
        //    retVal = regExpACX.test(input);
    }
    return retVal;
}                                
