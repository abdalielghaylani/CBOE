...
<%exportTask = Request.QueryString("export")
	...
	Select case UCase(exportTask) 
		...
		%>
				<!--webbot bot="HTMLMarkup" TAG="XBOT" StartSpan --><INPUT type="hidden" name="DataAction2" value="export_hits"><!--webbot BOT="HTMLMarkup" endspan -->
				...
				//var enable_export_templates = "<%=Application("ENABLE_EXPORT_TO_EXCEL")%>"
				var enable_export_to_excel = "1"
				...
				if (enable_export_templates == '1'){
					export_template_select = '<select name="export_hits_templates" onChange=doManageTemplates(this.form)><option value=placeholder>--Select Template Action--</option><option value=edit>>Create New Template</option><option value=manage>Manage Templates</option><option value=default>Use Defaults</option><option value=placeholder>--Saved Templates--</option><%=savedTemplateNames%></select>'
				}
				var export_excel_select =""
				if (enable_export_to_excel=='1'){
					export_excel_select = '<td><select name="File_Format_Type"><option value=sdfile>SDFile (.sdf)</option><option value=excel>Excel (.xls)</option></select></td>'
				...
				if (enable_export_to_excel=='1'){
					if (flat_file_export=="1"){
						document.write('<table valign="top"><tr>'+ export_excel_select + '<td><input type="radio" checked name="File_Ouput_Type" value="flat" >Flat')
						document.write('&nbsp;<input type="radio" value="nested" name="File_Ouput_Type">Nested</td>')
						if (enable_export_templates == '1'){
							document.write('<td>&nbsp;&nbsp;&nbsp;Export Templates</td></tr><tr><td>&nbsp;</td><td></td><td>&nbsp;&nbsp;&nbsp;' + export_template_select + '</td></tr></table>')
						}else{
							document.write('</tr></table>')
						}
					}else{
						document.write ('<table><tr>'+ export_excel_select + '<td><input type = "hidden" name="File_Ouput_Type" value = "nested"></td>')
						if (enable_export_templates == '1'){
							document.write('<td>&nbsp;&nbsp;&nbsp;Export Templates</td></tr><tr><td>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;' + export_template_select + '</td></tr></table>')
						}else{
							document.write('</tr></table>')
						}
					}
				...
				if (flat_file_export=="1"){
					document.write('<table valign="top"><tr><td><input type="radio" checked name="File_Ouput_Type" value="Flat_SDFile" >Flat SD File')
					document.write('&nbsp;<input type="radio" value="SDFile" name="File_Ouput_Type">Nested SD File</td>')
					if (enable_export_templates == '1'){
						document.write('<td>&nbsp;&nbsp;&nbsp;Export Templates</td></tr><tr><td>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;' + export_template_select + '</td></tr></table>')
					}else{
						document.write('</tr></table>')
					...
					document.write ('<table><tr><td><input type = "hidden" name="File_Ouput_Type" value = "SDFile"></td>')
					if (enable_export_templates == '1'){
						document.write('<td>&nbsp;&nbsp;&nbsp;Export Templates</td></tr><tr><td>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;' + export_template_select + '</td></tr></table>')
					}else{
						document.write('</tr></table>')
		...
		<%case "PREPAREEXPORT"
				...
				appendvars = "&export_structure_data=" & export_structure_data & "&exporttype=" & ExportType & "&MaxExportNumber=" & MaxExportNumber & "&File_Ouput_Type=" & OutputType & "&File_Format_Type=" & FileFormatType
				if Trim(FileFormatType) <> "" then
					actionPath ="/" & Application("appkey") & "/" & dbkey  & "/" & dbkey & "_action.asp?dataaction2=export_hits&dbname=" & dbkey & "&formgroup=" & formgroup & "&formmode=" & formmode & appendvars
				else
