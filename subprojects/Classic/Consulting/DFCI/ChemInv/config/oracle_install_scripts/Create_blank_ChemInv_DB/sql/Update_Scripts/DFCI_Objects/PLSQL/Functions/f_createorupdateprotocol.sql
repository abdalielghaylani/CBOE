...
(
 ...
 pSponsorId in inv_protocol.sponsor_id%type,
...
if vCount=0 and pProtocolId is null then
	insert into inv_protocol (protocol_identifier, protocol_name, nci_protocol_num, sponsor_id, start_date, end_date) values (pProtocolIdentifier, pProtocolName, pNCIProtocolNum,pSponsorId, pStartDate, pEndDate) returning protocol_id into vProtocolId;
else
	...
	if pNCIProtocolNum is not null then
		...
		updateSQL := updateSql || ' sponsor_id = ' || chr(39) || pSponsorId || chr(39)	;
