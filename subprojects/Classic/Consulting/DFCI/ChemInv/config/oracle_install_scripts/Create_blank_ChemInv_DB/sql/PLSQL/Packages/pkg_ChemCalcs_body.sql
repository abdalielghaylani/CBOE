...
--GetMW
  ...
  vCount integer;
   	BEGIN
        ...
        --check to see if we have access to reg tables
        SELECT count(*) INTO vCount FROM all_all_tables WHERE owner = 'REGDB';
        IF vCount > 0 THEN
				...
				EXECUTE IMMEDIATE 'SELECT r.Formula_Weight FROM regdb.batches r WHERE r.reg_internal_id = ' || pRegID || 'AND r.Batch_Number = ' || pBatchNumber INTO vMW;
        ELSE
	        vMW := 0;
        END IF;
   			--vMW := 0;
