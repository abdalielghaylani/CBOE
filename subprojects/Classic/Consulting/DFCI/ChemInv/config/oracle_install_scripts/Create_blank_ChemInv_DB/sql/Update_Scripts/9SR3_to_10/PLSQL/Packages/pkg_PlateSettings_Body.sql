CREATE OR REPLACE PACKAGE BODY PLATESETTINGS
	...
	FUNCTION CREATEGRIDFORMAT
     ...
     p_Cell_Naming in inv_grid_format.cell_naming%TYPE,
     p_Name_Delimeter in inv_grid_format.name_delimeter%TYPE,
	...
	begin
		...
		insert into inv_grid_format
			(grid_format_id, name, description, row_count, col_count,
			 ...
			 number_direction, grid_format_type_fk, zero_padding_count, cell_naming,
       name_delimeter)
		values
			...
			p_row_prefix, p_col_prefix, p_row_use_letters,
			 ...
			 p_number_direction, p_grid_format_type, p_zero_padding_count, p_Cell_Naming, p_Name_Delimeter) returning grid_format_id into new_grid_format_id;
        ...
        seqCnt := seqCnt + 1;
				...
				ELSE
        	IF power(10,p_zero_padding_count) > col_counter THEN
          ...
          END IF;
				...
				-- determine cell name
				if p_Cell_Naming = 1 THEN
        	IF power(10,p_zero_padding_count) > seqCnt THEN
						new_name := p_row_prefix || lpad(seqCnt,p_zero_padding_count,'0');
          ELSE
						new_name := p_row_prefix || seqCnt;
          END IF;
				elsif p_number_direction = 17 then
...
END;
	...
	is
		...
		rc integer;
	begin
        -- First remove all Rack format settings
        update inv_locations set collapse_child_nodes = null, location_type_id_fk = 0
        where location_id in (select location_id_fk from inv_grid_storage where grid_format_id_fk = p_grid_format_id);
        -- Delete grid format
	...
	FUNCTION UPDATEPHYSPLATETYPE
		(pPhysPlateId in inv_physical_plate.phys_plate_id%Type,
		 ...
		 pWellCapacity in inv_physical_plate.well_capacity%type,
		 pCapacityUnitID in inv_physical_plate.capacity_unit_id_fk%type,
		  pCellNaming  number) return inv_physical_plate.phys_plate_id%Type
	is
		rc integer;
	begin
