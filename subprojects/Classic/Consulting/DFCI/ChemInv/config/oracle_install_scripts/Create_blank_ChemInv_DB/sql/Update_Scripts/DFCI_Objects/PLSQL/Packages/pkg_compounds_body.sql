...
IS
   FUNCTION CREATESYNONYM (pCompoundID IN inv_Synonyms.Compound_ID_FK%Type, pSubstanceName IN inv_Synonyms.Substance_Name%Type)
	...
	row_c number;
	 BEGIN      
	   select count(*) into row_c from inv_synonyms where compound_id_fk=pCompoundId and substance_name=pSubstanceName;
	   if row_c=0 then
   ...
   FUNCTION DELETESUBSTANCE (pCompoundID in Inv_compounds.compound_ID%Type)
	...
	Function UpdateInvCompound(
    	...
    	pHBondDonors in inv_compounds.hbond_donors%TYPE,
			...
			pCompoundID in inv_compounds.compound_id%type,
			pNDCInvestigational in inv_compounds.ndc_investigational%type,
			pPackageSize in inv_compounds.package_size%type,
	...
	BEGIN
      ...
      HBOND_DONORS = pHBondDonors,
			...
			NDC_INVESTIGATIONAL = pNDCInvestigational,
			PACKAGE_SIZE = pPackageSize
		...
		--	if vExistingCompoundID > 0 and not vRegisterIfConflicts then  -- Use existing compound
			if vExistingCompoundID > 0  then  -- Use existing compound
		   		if vExistingCompoundID = pCompoundID then --This is and edit request
		   			oCompoundID := UpdateInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, pNDCInvestigational, pPackageSize, vConflictingFields);
		   		else
	   		...
	   		if vRegisterIfConflicts then -- Create a duplicate compound
		  		if pCompoundID is not Null then  -- This is an edit request which turns the edited compound into a duplicate
		  		  oCompoundID := UpdateInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, pNDCInvestigational, pPackageSize, vConflictingFields);
		  			...
		  			oCompoundID := InsertInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pNDCInvestigational, pPackageSize, vConflictingFields);
					end if;
