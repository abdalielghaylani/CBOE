CREATE OR REPLACE PACKAGE BODY "GUIUTILS" AS
	...
	BEGIN
		...
		IF pUseReg = 1 THEN
			...
			inv_Containers.Reg_ID_FK,  
			inv_Containers.Batch_Number_FK,
			DECODE(inv_Compounds.Substance_Name,'',(Select REGNAME from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),Null,(Select REGNAME from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),inv_Compounds.Substance_Name) AS Substance_Name,
			DECODE(inv_Compounds.CAS ,'',(Select REGCAS from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),Null,(Select REGCAS from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),inv_Compounds.CAS ) AS CAS,
			DECODE(inv_containers.Reg_ID_FK, NULL, NULL, (Select regnumber from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK)) AS RegNumber, 
			DECODE(inv_containers.Reg_ID_FK, NULL, NULL, (Select regbatchid from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK)) AS Reg_Batch_ID, 
			DECODE(inv_containers.Reg_ID_FK, NULL, inv_Compounds.BASE64_CDX, (Select base64_cdx From cheminvdb2.inv_vw_reg_structures Where inv_vw_reg_structures.regid=inv_containers.reg_id_fk)) AS Base64_CDX,
			...
			AND	inv_containers.batch_number_fk = ivrb.BatchNumber(+) 		
		...
		ELSE
			...
			inv_Containers.Reg_ID_FK as RegNumber,			
			inv_Containers.Batch_Number_FK as Reg_Batch_ID,			
			inv_Containers.Location_ID_FK,  
			inv_Containers.Compound_ID_FK,  
			inv_Containers.Container_Type_ID_FK,  
			inv_Containers.Container_Status_ID_FK,  
  ...
  */
  ...
  END UseGetLocation;
	...
	BEGIN	
		OPEN O_RS FOR 
		SELECT server_name, queue_name, display_name, header_filename, label_filename, footer_filename
