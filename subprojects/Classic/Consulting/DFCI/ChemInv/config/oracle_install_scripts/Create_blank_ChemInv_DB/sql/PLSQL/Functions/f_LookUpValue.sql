...
BEGIN
	...
	IF lower(pTableName) = 'inv_units' THEN
			SELECT count(*) INTO vCount FROM inv_units WHERE lower(unit_abreviation) = lower(pTableValue);
			IF vCount > 0 THEN
				SELECT unit_id INTO vReturn FROM inv_units WHERE lower(unit_abreviation) = lower(pTableValue);			
			END IF;
	...
	IF lower(pTableName) = 'inv_locations' THEN
			SELECT count(*) INTO vCount FROM inv_locations WHERE lower(location_name) = lower(pTableValue);
			IF vCount > 0 THEN
				SELECT location_id INTO vReturn FROM inv_locations WHERE lower(location_name) = lower(pTableValue);			
			END IF;
			-- pInsertIfNotFound does not apply to this table
	END IF;
