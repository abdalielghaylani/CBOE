...
BEGIN
  ...
  vWellIDs_t := STRINGUTILS.split(pWellIDS, ',');
    ...
    IF vIsReg THEN
      ...
      FOR j IN vWellIDs_t.FIRST..vWellIDs_t.LAST
      LOOP
      	INSERT INTO inv_well_compounds (well_id_fk, reg_id_fk, batch_number_fk) VALUES (vWellIDs_t(j), vRegID, vBatchNumber);
      END LOOP;
    END IF;
    ...
    IF length(pPlateIDs) > 0 THEN
	    ...
	    EXECUTE IMMEDIATE 'UPDATE inv_Wells SET ' || v_ValuePairs || ' WHERE Plate_ID_FK in (' || pPlateIDs || ')';
    END IF;
    RETURN '0';
