...
<!--Hide JavaScript
	...
	function clickLocation(value) {
		//alert(serverName);
		var strURL = "http://" + serverName + "/cheminv/GUI/ManageRoleLocationsDict.asp?locationID=" + value;	
		//alert(strURL);	
		...
		Do While Not RS.EOF
			...
			Else
				...
				If RS("LocationTypeName") = "Plate Map" then
					...
					Set Nodex = TreeView.Nodes.Add(ParentID,4,id,"<INPUT TYPE=""CHECKBOX"" NAME=""LocationID"" VALUE=""" & id & """ onclick=""clickLocation(this.value);"">" &Left(RS("Location_Name"),200), Icon_clsd, Icon_clsd, , , RS("LocationTypeName") & vblf & RS("Location_barcode").value & vblf & RS("Location_Description").value)
...
for (i=0;i<numLocations;i++){
	...
	//alert(document.form1.LocationID(i).value);
	//alert(inList(document.form1.LocationID(i).value,arrLocation));
	//test = inList(document.form1.LocationID(i).value,arrLocation);
	//alert(i);
//alert(document.form1.LocationID(1).value);
//alert(document.form1.LocationID.length);

</script>
		</td>
	</tr>
	<tr>
		<td align="right" valign="top" colspan="2"><br><input type="checkbox" name="propagate" value="1">Propagate exclusions to child locations</td>
	</tr>

<%end if%>
	<tr><td colspan="2" align="right">
		<%if stepValue = "2" then%>
			<a HREF="#" onclick="history.go(-2); return false;"><img SRC="/cheminv/graphics/sq_btn/cancel_dialog_btn.gif" border="0"></a>&nbsp;
