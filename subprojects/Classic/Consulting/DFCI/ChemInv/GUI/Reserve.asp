...
<%If NOT isDelete then%>
	<tr>
		...
		<td>
			...
			Response.write ShowSelectBox2("ReservationUserID", Ucase(ReservationUserID), "SELECT Upper(User_ID) AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText FROM CS_SECURITY.People where active = 1 ORDER BY lower(Last_Name) ASC", 27, "", "")
			else
			Response.write ShowSelectBox2("ReservationUserID", Ucase(ReservationUserID), "SELECT Upper(User_ID) AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText FROM CS_SECURITY.People ORDER BY lower(Last_Name) ASC", 27, "", "")
			end if%>
