...
<%
		...
		While (Not RS1.EOF)
			...
			CAS=RS1("cas")
			SubstanceName=RS1("substance_name")
			...
			if bShowBatch then
                ...
                Cmd.Parameters.Append Cmd.CreateParameter("pBatchID", adNumeric, 1, 1, cint(BatchID))
                if bDebugPrint then
                 For each p in Cmd.Parameters
                  Response.Write p.name & " = " & p.value & "<BR>"
                 Next 
                ...
                BatchAmount = Replace(BatchAmount,":"," ")
			end if
%>
...
<%
			...
			if RequestStatusID = "2" then
				...
				link = "<a Class=""MenuLink"" Href=""#"" onclick=""OpenDialog('/Cheminv/GUI/RequestSample.asp?action=edit&amp;" & editdata & "BatchID=" & BatchID & "&RequestID=" & RequestID & "', 'Diag', 1); return false"">Review</a>"
				link = link & "&nbsp;"
			elseif RequestStatusID = "3" then
				link = ""
