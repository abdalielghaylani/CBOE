...
<META NAME="GENERATOR" Content="Microsoft Visual Studio 6.0">
	...
	<SCRIPT LANGUAGE=javascript>
		...
		function LookUpContainer(barcode){
			if (barcode =="") return;
			var barcodepart = barcode.substr(barcode.lastIndexOf("|")+1, barcode.length);
 
			var strURL = "http://" + serverName + "/cheminv/api/GetKeyContainerAttributes.asp?ContainerBarcode='" + barcodepart + "'";	
			var httpResponse = JsHTTPGet(strURL) 
			
			if (httpResponse.length == 0){
				alert("Could not a find a container for barcode= " + barcodepart);
			}
			else{
