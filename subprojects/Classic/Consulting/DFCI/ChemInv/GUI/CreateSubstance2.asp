...
NDCInvestigational = Request("inv_compounds.NDC_INVESTIGATIONAL")
PackageSize = Request("inv_compounds.PACKAGE_SIZE")
...
Select Case lcase(action)
	...
	Case "showconflicts"
		...
		ALT_ID_4 = dBALT_ID_4
		ALT_ID_5 = dBALT_ID_5
		NDCInvestigational = dBNDCInvestigational		
...
'stop
GetInvConnection()
' Clear out current settings, if any
...
if NOT bSubstanceCreated then
	Select Case lcase(action)		
		...
		Case "edit"
			...
			dBNDCInvestigational = NDCInvestigational
            dBPackageSize = PackageSize
			bShowSelect = false
			bShowCreateDuplicate = false
			bShowEdit = true
