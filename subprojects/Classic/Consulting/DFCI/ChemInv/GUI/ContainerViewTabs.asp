...
End if
	...
	if Application("ShowRequestSample") then
		GetInvConnection()
		SQL_batches = "SELECT count(*) as numBatchRequests FROM inv_requests r,inv_containers c WHERE r.batch_id_fk = c.batch_id_fk and r.request_type_id_fk = 2 AND c.container_id = ?"
		Set Cmd = GetCommand(Conn, SQL_batches, adCmdText)
		Cmd.Parameters.Append Cmd.CreateParameter("ContainerID", 5, 1, 0, ContainerID)
		Set rsBatch = Cmd.Execute
		if not rsBatch.BOF or rsBatch.EOF then
			numBatchRequests = cint(rsBatch("numBatchRequests"))
		end if
	end if

	if RequestID <> "" or numSampleRequests > 0 or numBatchRequests > 0 then
