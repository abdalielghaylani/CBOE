...
Select case Lcase(action)
	case "in"
		...
		sql = "SELECT inv_Containers.Container_ID, barcode, inv_containers.Def_Location_ID_FK, inv_Locations.Location_Name AS DefLocationName, inv_locations.Owner_ID_FK AS LocationOwner, inv_Containers.Owner_ID_FK AS OwnerID FROM inv_Containers, inv_Locations WHERE inv_containers.Def_Location_ID_FK = inv_Locations.Location_ID AND inv_containers.Container_ID in (" & ContainerID & ")"
		'Response.Write sql
		'Response.End
		...
		isFirstRec = false
		While NOT RS.EOF
			defLocations = defLocations & "<tr><td>" & RS("barcode") & "</td><td>" & RS("DefLocationName") & "</td></tr>"
...
<input Type="hidden" name="iField_6" value="<%=Session("CurrentLocationID")%>">
...
<table border="0">
	...
	if action = "in" and Application("ShowCheckInDetails") then
		For each key in custom_checkin_fields_dict
			...
			Else
				'DFCI Change begins
				if instr(ucase(key), "FIELD_7") then
				Response.Write "<td align=""right"" valign=""top"" nowrap width=""150"">"
				if req_custom_checkin_fields_dict.Exists(Key) then Response.Write "<span class=""required"">"
				Response.Write custom_checkin_fields_dict.Item(key)
				if req_custom_checkin_fields_dict.Exists(Key) then Response.Write "</span>"
				Response.Write ":</td>"	
				Response.Write "<td>"
				Response.write ShowSelectBox2("iFIELD_7", iField_7, "SELECT picklist_display as Value, picklist_display AS DisplayText FROM " & Application("CHEMINV_USERNAME") & ".inv_picklists where picklist_domain=101 order by picklist_display",30,RepeatString(43, "&nbsp;"), "")
				Response.Write "</td>"
				else ' DFCI CHange ends
