	...
	if (cint(ParentCount) > 1 or (cint(CompoundCount)>1 and cint(ParentCount)= 1)) then
		...
		compoundSQL =  "SELECT DECODE(wc.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT BASE64_CDX FROM inv_vw_reg_batches WHERE regid=wc.reg_id_fk AND batchnumber=wc.Batch_Number_FK)) AS Base64_CDX," & _
						...
						if Application("RegServerName") <> "NULL" then
							...
							compoundSQL = compoundSQL & ", DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT RegNumber FROM inv_vw_reg_batches WHERE inv_vw_reg_batches.regid = wc.reg_id_fk)) AS Reg_Number "
						End if
			...
			if session("DrawPref") = "ISIS" then
 			...
 			if not rsMixtureCompounds.EOF then
				...
				elseif not isNull(regBatchID) then
					IDHTML = IDHTML & "<td CLASS=""LeftAlign"" WIDTH="""" HEIGHT="""">Reg Batch ID:</td><td CLASS=""GrayedTextLeft"" WIDTH=""60""><a class=""MenuLink"" href=""ViewRegDetails.asp?reg_number=Sameer000001"" target=""_blank"">" & regBatchID & "</a></td>"
				else
