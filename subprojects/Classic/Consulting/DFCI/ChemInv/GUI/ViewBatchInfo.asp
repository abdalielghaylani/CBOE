...
<!--Hide JavaScript
   ...
   /*function postDataFunction(sTab) {
	...
	function showContainerDetails(ContainerBarcode){
		var detailsUrl = "";
		detailsUrl = "http://<%=ServerName%>/cheminv/gui/ViewBatchContainer.asp?getData=db&hideMenu=true&ContainerID="  + ContainerBarcode ;
...
<%
	...
	SQL = "SELECT (replace(" & schemaName & ".GUIUTILS.GETBATCHUOMAMOUNTSTRING(NULL," & BatchID & "),':',' ')) AS Batch_Amount_String, " & vblf & _
			...
			if Application("RegServerName") <> "NULL" then
				...
				SQL = SQL & " DECODE(inv_containers.Reg_ID_FK, NULL, (inv_compounds.substance_name) ,(rb.RegName)) as SubstanceName , " & vblf
			else
			...
			if Application("RegServerName") <> "NULL" then
				SQL = SQL & " AND rb.regid = inv_containers.reg_id_fk" & vblf & _
				" AND rb.batchnumber = inv_containers.batch_number_fk" & vblf
			end if
			SQL = SQL & " AND batch_id_fk = " & BatchID
	...
	If RS.BOF and RS.EOF then
		...
		'-- get batch details
        SQL =   " SELECT container_id, batch_id_fk, parent_container_id_fk, " & vblf & _
			    ...
			    if Application("RegServerName") <> "NULL" then
					...
					SQL = SQL & " DECODE(inv_containers.Reg_ID_FK, NULL, (inv_compounds.substance_name) ,(RegName)) as SubstanceName , " & vblf
				else
				    SQL = SQL & " inv_compounds.substance_name as SubstanceName," & vblf
			    end if
			   ...
			   " FROM inv_containers, inv_locations l, inv_compounds, inv_units uom, inv_units uoc, inv_vw_grid_location gl, inv_locations lp " & vblf
			    ...
			    " AND l.location_id not in (3) " & vblf & _
			    " AND compound_id_fk = compound_id(+) " & vblf
	    		if Application("RegServerName") <> "NULL" then
    	    		SQL = SQL & " AND reg_id_fk = regid(+) " & vblf
			    end if
		...
		<td valign="top">
			<table border="0" cellpadding="5" cellspacing="2">
				...
				for each key in batching_fields_dict
				    ...
				    response.Write ShowField(batching_fields_dict.item(key) & ":", "BatchField" & key, 15, "BatchField" & key)
				    if count mod 2 = 1 then response.Write "<td></td></tr>"
				    count = count + 1
                next				    
                if count mod 2 = 1 then response.Write "<td></td><td></td><td></td></tr>"
                %>
