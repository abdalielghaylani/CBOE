...
%>
		...
		<%if cint(Session("wCompoundCount")) <= 1 then %>
			...
			<td>
				<%if Application("RegServerName") <> "NULL" then%>
					<input type="text" Size="15" name="iCompoundID" value="<%=Compound_ID_FK%>" onchange="if(IsValidCompoundID(this.value, true)==1){NewCompoundID.value= this.value; RegBatchID.value=''; iRegID.value=''; iBatchNumber.value='';iCompound_ID_FK.value=this.value;NewRegID.value='';}else{this.value = '';}">
				<%else%>
	...
	<tr height="25">
		...
		<%if cint(Session("wCompoundCount")) <= 1 then %>
			...
			<td>
				<input type="text" Size="15" name="RegBatchID" value="<%=Session("wRegBatchID")%>" onchange="GetRegIDFromRegNum(this.value); iCompoundID.value='';">
