...
if Application("BYPASS_ORDERING_SCREENS") then
	...
	AddNameValuePair FormData, "PO", PO
	...
	Else
		...
		Do While NOT lRS.EOF
			...
			AddItemToList PriceList2, Price
		...
		AddNameValuePair FormData, "PriceList2", PriceList2
...
<script LANGUAGE="javascript">
	...
	function Validate(){
		...
		//pjd add container count for price calculation
		var ContC = "";
		...
		for (i=0; i< NumContainers; i++){
			...
			(NumContainers > 1) ? price = document.form1.PriceList2[i].value : price = document.form1.PriceList2.value;
			...
			//MCD: end changes
			//pjd added qty to validation list
			(NumContainers > 1) ? qty = document.form1.QtyList[i].value : qty = document.form1.QtyList.value;
			//pjd
			...
			//pjd added container count per item
			(NumContainers > 1) ? ContC = document.form1.ContainerCountList[i].value : ContC = document.form1.ContainerCountList.value;
			//pjd
			//pjd added qty
			if (qty.length == 0) {
				errmsg = errmsg + "   - Qty is required.\r";
				bWriteError = true;
				blocalError = true
			}
			else{
				 //Qty must be a number
				if (qty.length > 0 && !isNumber(qty)){
					errmsg = errmsg + "   - qty must be a number.\r";
					bWriteError = true;
					blocalError = true;
				} 
			}
			...
			if (!blocalError) {
				...
				(NumContainers > 1) ? document.form1.PriceList[i].value = price*ContC/size : document.form1.PriceList.value = price*ContC/size;				
}
...
<!-- PJD add PO -->
	<tr height="25">
		<td align="right" valign="top">
			<span class="required" title="Enter a PO number">PO#:</span>
		</td>
		<td>
		<input type=text size=15 maxlength=15 name="PO" value="<%=PO%>">
		</td>
	</tr>
<!-- PJD -->
<!-- MCD: added extra ordering fields -->
...
<table border="0">
	<tr>
		<th>Container</th>
		<th>Qty</th>
		<th><!--pjdCAS # pjd--> NDC</th>
		<th><!--pjd Chemical pjd-->Drug Name</th>
		<!--pjd remove <th>Supplier</th> -->
		<th>Size</th>
		<th>Catalog Num</th>
...
Else
	...
	PriceList2 = Request("PriceList2")
	if PriceList2 <> "" then
		PriceList2_arr = split(PriceList2,",")
	...
	'	Response.Write "<td>" & Qty 
		...
		'Response.Write "<input type=hidden size=5 maxlength=5 name=""PriceList2"" value=""" & lRS("Price") & """></td>"
		'MCD: instead of this one
		Response.Write "<td align=right><input type=""text"" size=7 name=""PriceList2"" value="""  & Price & """><input type=""hidden"" size=7 name=""PriceList"" value="""  & Price & """></td>"
		'MCD: and these two
