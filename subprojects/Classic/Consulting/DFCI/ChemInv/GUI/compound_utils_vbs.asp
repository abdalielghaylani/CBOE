...
Sub GetSubstanceAttributesFromDb(pCompoundID)
	...
	if NOT IsEmpty(pCompoundID) then 
		SQL = "SELECT inv_compounds.Compound_ID, inv_compounds.CAS, inv_compounds.ACX_ID, inv_compounds.density, inv_compounds.ALT_ID_1, inv_compounds.ALT_ID_2, inv_compounds.ALT_ID_3, inv_compounds.ALT_ID_4, inv_compounds.ALT_ID_5, inv_compounds.Substance_Name, inv_compounds.Conflicting_Fields, inv_compounds.base64_cdx, inv_compounds.NDC_INVESTIGATIONAL, inv_compounds.PACKAGE_SIZE FROM inv_compounds WHERE inv_compounds.Compound_ID= " & pCompoundID	
		'Response.Write sql
		'Response.end
		...
		dBNDCInvestigational = RS("NDC_INVESTIGATIONAL").value
        dBPackageSize = RS("PACKAGE_SIZE").value        
...
Sub DisplaySubstance(Caption, Header, bShowSelect, bShowCreateDuplicate, bShowEdit, bShowEditExisting, bShowConflicts, InLineCdx)
	...
	'DFCI: Response.Write						ShowField("Molecular Weight:", "MOLWEIGHT", 15, "MOLWEIGHT" & CDCounter)
	
	Response.Write						ShowField("CompoundID:", "dBCompoundID", 15, "")
	Response.Write "				<td></td></tr>" & vblf
	Response.Write "				<tr>" & vblf
	'Response.Write						ShowField("Molecular Formula:", "FORMULA", 15, "FORMULA" & CDCounter)
	
	'DFCI: Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.CAS", "CAS Number:"), "dBCAS", 15, "")
	Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.CAS", "NDC:"), "dBCAS", 15, "")
	'DFCI: Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.ACX_ID", "ACX ID:"), "dBACX_ID", 15, "")
	Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.ACX_ID", "ABC #:"), "dBACX_ID", 15, "")
	Response.Write "				</tr>" & vblf
	
	' DFCI custom fields:
	Response.Write "				<tr>" & vblf
	Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.NDC_INVESTIGATIONAL", "NDC for Investigational:"), "dBNDCInvestigational", 15, "")
	Response.Write						ShowField(HighlightConflictingField(dBCompoundID, "inv_compounds.PACKAGE_SIZE", "Package Size:"), "dBPackageSize", 15, "")
	Response.Write "				</tr>" & vblf
	
	'DFCI: Response.Write "				<tr>" & vblf	
	'DFCI: Response.Write "					<td></td><td></td>" & vblf
	'DFCI: Response.Write "				</tr>" & vblf
		...
		for each key in custom_fields_value_dict
			if CustomField_GetCategory(key) <> "" then	' WJC Note incorrect test for "boolean"
				if (custom_fields_value_dict(key) <> CustomField_GetDefault(key)) then
					...
					if strList <> "" then strList = strList & Chr(255)
					strList = strList & key
				end if
			end if
		next
...
Sub CustomFieldsInfo_Construct()
	...
	Sql = "SELECT custom_field_name,custom_field_id,picklist_type_id_fk,picklist_display,custom_field_group_name,readonly FROM ChemInvDb2.Inv_Custom_Fields JOIN ChemInvDb2.Inv_Picklists ON (Inv_Custom_Fields.NULL_PICKLIST_ID_FK = Inv_Picklists.PICKLIST_ID) JOIN ChemInvDb2.Inv_Custom_Field_Groups ON (Inv_Custom_Fields.CUSTOM_FIELD_GROUP_ID_FK = Inv_Custom_Field_Groups.CUSTOM_FIELD_GROUP_ID) WHERE (CUSTOM_FIELD_GROUP_ID_FK IS NOT NULL) ORDER BY custom_field_group_name,custom_field_name" 
	Set RS = Conn.Execute(Sql)
	
	custom_fields_info_dict.RemoveAll
	While NOT RS.EOF
	    custom_fields_info_dict.Add RS("custom_field_name").value, Array(RS("custom_field_id").value, RS("picklist_type_id_fk").value, RS("picklist_display").value, RS("custom_field_group_name").value, RS("readonly").value)
		RS.MoveNext
	Wend
	Set RS = Nothing
    Conn.Close
...
function CustomFieldsInfo_GetCategories()
	...
	Sql = "SELECT DISTINCT custom_field_group_name FROM ChemInvDb2.Inv_Custom_field_groups JOIN ChemInvDb2.Inv_Custom_fields ON (Custom_Field_Group_ID_FK = Custom_Field_Group_ID) ORDER BY custom_field_group_name" 
	Set RS = Conn.Execute(Sql)
	
	Dim return_dict
	set return_dict = Server.CreateObject("Scripting.Dictionary")
	While NOT RS.EOF
	    return_dict.Add RS("custom_field_group_name").value, RS("custom_field_group_name").value
		RS.MoveNext
	Wend
	Set RS = Nothing
    Conn.Close
...
sub AltFieldsValue_CompoundGet()
	...
	for each key in alt_ids_dict
	    if left(alt_ids_dict.item(key),1) = "*" then
	        ...
	        if Eval(strVar) <> "" then nPicklistID = CInt(Eval(strVar)) else nPicklistID = AltField_GetDefault(key)
...
function ShowCustomPicklist(Key,OnChangeFunc)
    ' Prepare
    strLabel = alt_ids_dict.Item(Key)
    strLabel = mid(strLabel, 2)
    strVar = replace(Key, "inv_compounds.","dB")
    ...
    strHTML = strHTML & "<SELECT name=""" & Key & """" & " onchange=""" & OnChangeFunc & """" & ">"
    ' Populate OPTIONs
	Dim RS
    GetInvConnection()
    Set cmd = Server.CreateObject("ADODB.Command")
    cmd.ActiveConnection = Conn
    cmd.CommandType = adCmdText
    Sql = "SELECT Picklist_ID AS Value, picklist_display AS DisplayText, picklist_code FROM ChemInvDb2.Inv_Picklists JOIN ChemInvDb2.Inv_Picklist_Types ON Picklist_Domain = Picklist_Type_ID JOIN ChemInvDb2.Inv_Custom_Fields ON Picklist_Type_ID_FK = Picklist_Type_ID WHERE Custom_Field_Name = ? AND PICKLIST_ID >= 1000 ORDER BY DisplayText"
    cmd.CommandText = sql
    cmd.Parameters.Append cmd.CreateParameter("Custom_Field_Name", adLongVarChar, adParamInput, len(strLabel) + 1, strLabel)
	...
	While NOT RS.EOF
        ...
        if rs("picklist_code") & " - " & strDisplayText = strVal then
