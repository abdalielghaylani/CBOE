...
SQL = "SELECT /*+ ORDERED */ " & vblf & _
    ...
    "inv_Containers.Batch_ID_FK AS Batch_ID_FK, " & vblf & _
     "DECODE(inv_Containers.Batch_ID_FK, NULL, ' ', replace(" & schemaName & ".GUIUTILS.GETBATCHUOMAMOUNTSTRING(container_ID,NULL),':',' ')) as totalBatchAmt,  " & _
    ...
    if Application("RegServerName") <> "NULL" then
        SQL = SQL & "DECODE(inv_Compounds.Substance_Name,'',(Select REGNAME from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),Null,(Select REGNAME from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),inv_Compounds.Substance_Name) AS Substance_Name,"
        SQL = SQL & "DECODE(inv_Compounds.CAS ,'',(Select REGCAS from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),Null,(Select REGCAS from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK),inv_Compounds.CAS ) AS CAS,"
                
		SQL = SQL & "DECODE(inv_containers.Reg_ID_FK, NULL, NULL, (Select regnumber from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK)) AS RegNumber, "
		SQL = SQL & "DECODE(inv_containers.Reg_ID_FK, NULL, NULL, (Select regbatchid from cheminvdb2.inv_vw_reg_batches Where regid = inv_containers.reg_id_fk AND batchnumber= inv_containers.Batch_Number_FK)) AS Reg_Batch_ID, "
...
if Application("RegServerName") <> "NULL" then
	SQL = SQL & ", inv_vw_reg_batches ivrb, inv_vw_reg_structures ivrs " & _
		"WHERE inv_containers.reg_id_fk = ivrb.RegID(+) " & _
		"		AND	inv_containers.batch_number_fk = ivrb.BatchNumber(+) "  & _
