...
With oPlatesElement
	...
	BatchProject = .getAttribute("BATCH_PROJECT")
...
if RegisterCompounds = "true" then
	...
	For Each oNode in nlCompounds
		...
		trace "CreatePlateXML.asp: RegSubstance Return Value=" & httpResponse & "<BR />", 15
		...
		arrRegData = split(httpResponse,":")
		action = arrRegData(0)
		bRegistrationFailed = false
		set oRCNode = oResultXML.createElement("REG_SUBSTANCE")
		Select Case lcase(action)
			Case "multiple duplicates"
			    'Multiple Duplicates: temp_id=241dup reg_id:Batch_Commit:432,Batch_Commit:401 record added to temporary table.
				regID = ""
				batchNumber = ""
				bRegistrationFailed = true				
				AddErrorToNode oRCNode, oResultXML, "More than one match in Registration was found.  Record was added to the temporary table."
				
				' Start with this
				tempID = "Unknown"
				if UBound(arrRegData) > 0 then
				    arrTempID = Split(arrRegData(1),"=")
				    if UBound(arrTempID) > 0 then
				        ' Keep incrementing the length until we reach non-numeric characters
				        i = 1
				        while IsNumeric(Mid(arrTempID(1),1,i))
				            i = i + 1
				        wend
				        tempID = Mid(arrTempID(1),1,i-1)
				    end if
				end if	    	    
			Case "new_compound"
			    bRegistrationFailed = false			    
			...
			Case else			    
			    ...
			    if InStr(lcase(httpResponse),"duplicate- ") > 0 then			        
				    AddErrorToNode oRCNode, oResultXML, "Record was added to the temporary table."
				    bHandled = true
			        
			        'From ACX code:  post_result ="Duplicate- " & "temp_id=" & temp_id  & "-" & "Reg Number" & ":" & RegNumber
			        tempID = "Unknown"
			        arrRegData = split(httpResponse,"-")
			        if UBound(arrRegData) > 0 then
			            arrTempID = Split(arrRegData(1),"=")
			    ...
			    if InStr(lcase(httpResponse),"compound is duplicate of ") > 0 then			        			    
			        AddErrorToNode oRCNode, oResultXML, "A duplicate compound was found in Registration.  Adding to temp table failed."
				    bHandled = true
				    
				    'From ACX code: post_result="Compound is duplicate of :" & RegNumber & ". Adding to temp table failed: " & temp_id
				    arrRegData = split(httpResponse,":")
			        if UBound(arrRegData) > 0 then
			            regID = arrRegData(1)
			        end if
			    ...
			    if InStr(lCase(httpResponse), "the duplicate found for this compound is not approved") > 0 then			        
			        AddErrorToNode oRCNode, oResultXML, "An unapproved duplicate compound was found in Registration."
			        bHandled = true
			        
			        'From ACX code: post_result = "The duplicate found for this compound is not approved. No batch could be added: "& Reg_Number & ": Batch -" & 0
			        arrRegData = split(httpResponse,":")
			        if UBound(arrRegData) > 0 then
			            regID = arrRegData(1)			            
			        end if
			    ...
			    if InStr(lCase(httpResponse), "reg failed. add to temp failed") > 0 then
			        AddErrorToNode oRCNode, oResultXML, "Registration of compound failed.  Adding to temp table failed."
                    bHandled = true                    
                    'post_result="Reg Failed. Add to Temp failed: " & temp_id
                end if
                
                if InStr(lCase(httpResponse), "reg failed. compound added to temp") > 0 then
			        AddErrorToNode oRCNode, oResultXML, "Registration of compound failed.  Record was added to the temporary table."
                    bHandled = true                    
			    ...
			    if InStr(lCase(httpResponse), "unrecognized duplicate action in post") > 0 then
			        AddErrorToNode oRCNode, oResultXML, "An unrecognized duplicate action was encountered.  Registration aborted."
			        bHandled = true
                end if
                
                if not bHandled then
                    AddErrorToNode oRCNode, oResultXML, "An unhandled error was encountered with Registration."
                end if				
		end Select
		
		if bRegistrationFailed then
		    ' Find some useful information to add
		    AddSubstanceAttribute oNode, "SUBSTANCE_NAME", oResultXML, oRCNode
		    AddSubstanceAttribute oNode, "CAS", oResultXML, oRCNode
		    AddSubstanceAttribute oNode, "ACX_ID", oResultXML, oRCNode
		    
		    if Len(tempID) > 0 then
		        Set oTempIDNode = oResultXML.createAttribute("TEMP_COMPOUND_ID")
		        oTempIDNode.Value = tempID
		        oRCNode.SetAttributeNode(oTempIDNode)
		    end if
		end if
		...
		if Len(cpdInternalID) > 0 then		    
		    Set ocpdIDNode = oResultXML.createAttribute("CPD_INTERNAL_ID")
			ocpdIDNode.Value = cpdInternalID
			oRCNode.SetAttributeNode(ocpdIDNode)
	    end if
...
'check for solvent volume unit id
