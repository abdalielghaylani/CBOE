...
bDebugPrint = false
bWriteError = False
strError = "Error:UpdatePhysPlateType<BR>"

pPhysPlateID = Request("pPhysPlateID")
pGridFormatID = Request("pGridFormatID")
...
If Len(Request.QueryString) = 0 AND Len(Request.Form)= 0 then
	Response.Redirect "/cheminv/help/admin/api/UpdatePhysPlateType.htm"
	Response.end
End if
...
If IsEmpty(pPhysPlateID) then
	...
	bWriteError = True
End if
If IsEmpty(pGridFormatID) then
	strError = strError & "GridFormatID is a required parameter<BR>"
	bWriteError = True
End if
...
Call GetInvCommand(Application("CHEMINV_USERNAME") & ".PLATESETTINGS.UpdatePhysPlateType", adCmdStoredProc)
...
Cmd.Parameters.Append Cmd.CreateParameter("pGridFormatID",131, 1, 0, pGridFormatID)
Cmd.Parameters("pGridFormatID").Precision = 6
...
if bDebugPrint then
	...
	Next	
Else
	Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".PLATESETTINGS.UpdatePhysPlateType")
End if
