...
' DFCI: avoid duplicate checking of structures since they will not be using them
Cmd.Parameters.Append Cmd.CreateParameter("pStructure", adLongVarChar, 1, len(Structure)+1, Structure)
'Cmd.Parameters.Append Cmd.CreateParameter("pStructure", adLongVarChar, 1, 1, Null)
Cmd.Parameters.Append Cmd.CreateParameter("pNDCInvestigational", adLongVarChar, 1, 11, NDCInvestigational)
Cmd.Parameters.Append Cmd.CreateParameter("pPackageSize", adLongVarChar, 1, 15, PackageSize)
...
End if
	...
	dupStrucID = -1
	' DFCI: don't check for duplicate structures
	'if structure <> ""  AND Request("isEmptyStruc") <> "1" then	
	if false then
		dupID = DupStr
		If dupID > 0 AND dupID <> oCompoundID then
			dupStrucID = dupID
			dupID = CStr(dupID)
			Call AddToDuplicatesDict(dupID, "STRUCTURE")
			Call AddToDuplicatesDict(Cstr(oCompoundID), "STRUCTURE")
	...
	If dupID > 0 AND dupID <> oCompoundID then
		...
		Call AddToDuplicatesDict(Cstr(oCompoundID), "inv_compounds.Substance_Name") 
	...
	If dupID > 0 AND dupID <> oCompoundID then
		...
		Call AddToDuplicatesDict(Cstr(oCompoundID), "inv_compounds.CAS")
	...
	If dupID > 0 AND dupID <> oCompoundID then
		...
		Call AddToDuplicatesDict(Cstr(oCompoundID), "inv_compounds.ACX_ID")
	...
	For each key in unique_alt_ids_dict
		...
		If dupID > 0 AND dupID <> oCompoundID then
			...
			Call AddToDuplicatesDict(Cstr(oCompoundID), key)
