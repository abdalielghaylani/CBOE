...
'DFCI Custom
'Check barcode in synonyms
'Add if necessary.
'Update Return Field
		Set oCmd = Server.CreateObject("ADODB.Command")
		oCmd.ActiveConnection = Conn
		oCmd.CommandType = adCmdText
		oCmd.CommandText = "SELECT inv_containers.compound_id_fk FROM inv_synonyms, inv_containers where inv_synonyms.compound_id_fk=inv_containers.compound_id_fk and container_id=? and substance_name=?"
		oCmd.Parameters.Append Cmd.CreateParameter("PCONTAINERID",131, 1, 0, ContainerID)
		oCmd.Parameters.Append Cmd.CreateParameter("PFIELD3",200, 1, 2000, Field3)
		Set oRS = oCmd.Execute
		if oRS.EOF then
		FIELD4="ADD TO DRUG DICTIONARY"
		addToSynonyms=true
		else
		FIELD4="BARCODE OK"
		end if
...
if addToSynonyms=true then

' Set up and ADO command
		Set oCmd = Server.CreateObject("ADODB.Command")
		oCmd.ActiveConnection = Conn
		oCmd.CommandType = adCmdText
		oCmd.CommandText = "SELECT inv_containers.compound_id_fk FROM inv_containers where container_id=? "
		oCmd.Parameters.Append Cmd.CreateParameter("PCONTAINERID",131, 1, 0, ContainerID)
		Set oRS = oCmd.Execute
		if not oRS.EOF then
		CompoundID=oRS("Compound_id_fk")
		end if
...
Cmd.Parameters.Append Cmd.CreateParameter("PSUBSTANCENAME",200, adParamInput, 255, Field3)	
if bDebugPrint then
	For each p in Cmd.Parameters
		Response.Write p.name & " = " & p.value & "<BR>"
	Next	
