...
CsUserID = URLDecode(CryptVBS(request("tempCsUserID"),"protocol"))

ProtocolID = Request("ProtocolID")
ProtocolIdentifier = Request("ProtocolIdentifier")
ProtocolName = Request("ProtocolName")
NCIPROTOCOLNUM =request("NCIProtocolNum")
...
If IsEmpty(ProtocolID) and IsEmpty(ProtocolIdentifier) then
	strError = strError & "Either Protocol ID or Protocol Identifier is a required parameter<BR>"
	bWriteError = True
End if
...
Call GetInvCommand(Application("CHEMINV_USERNAME") & ".CreateOrUpdateProtocol", adCmdStoredProc)
...
Cmd.Parameters.Append Cmd.CreateParameter("PPROTOCOLIDENTIFIER",200, adParamInput, 255, PROTOCOLIDENTIFIER)	
Cmd.Parameters.Append Cmd.CreateParameter("PPROTOCOLNAME",200, adParamInput, 255, PROTOCOLNAME)
Cmd.Parameters.Append Cmd.CreateParameter("PSponsorID",200, adParamInput, 100, SponsorID)
Cmd.Parameters.Append Cmd.CreateParameter("PNCIPROTOCOLNUM",200, adParamInput, 100, NCIPROTOCOLNUM)
...
if bDebugPrint then
	...
	Next	
Else
	Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".CREATEORUPDATEPROTOCOL")
End if
