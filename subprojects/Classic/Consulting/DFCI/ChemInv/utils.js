...
function UpdateLocationPickerFromIDs(pLocationIDs, formelm, elm1, elm2, elm3){
	...
	for (i=0;i<arrLocations.length;i++){
	    strURL = "http://" + serverName + "/cheminv/api/GetLocationFromID.asp?LocationID=" + arrLocations[i];	
...
{
	var strURL = "http://" + serverName + "/cheminv/api/IsChildGridLocation.asp?LocationID=" + locationId; 	
	var httpResponse = JsHTTPGet(strURL);
	if (httpResponse == 1){
		return 1;
	}
...
{
if (LocationID!=''){
	var strURL = "http://" + serverName + "/cheminv/api/IsPlateTypeAllowed.asp?plateID=" + PlateId + "&LocationID=" + LocationID + "&PlateTypeID=" + PlateTypeID + "&IsPlateMap=" + IsPlateMap ; 	
	var httpResponse = JsHTTPGet(strURL);
	if (httpResponse == 1){
		return 1;
	}
...
{
	var strURL = "http://" + serverName + "/cheminv/api/NumberOpenPositions.asp?locationIds=" + locationIds; 	
...
function GetRegIDFromRegNum(RegBatchID){
	if (RegBatchID.length > 0) {
		...
		if (!isNumber(BatchNumber)) {
			...
			var strURL = "http://" + serverName + "/cheminv/api/GetBatchInfo.asp?RegNumber=" + RegNumber + "&BatchNumber=" + BatchNumber + "&tempCsUserName=" + CsUserName + "&tempCsUserID=" + CsUserID ;
			var httpResponse = JsHTTPGet(strURL)
			if (httpResponse.length == 0){
				document.form1.RegBatchID.value = ""
				document.form1.iRegID.value = ""
...
function UpdateBatchInfo(RegID, BatchNum){
	...
	var strURL = "http://" + serverName + "/cheminv/api/GetBatchInfo.asp?RegID=" + RegID + "&BatchNumber=" + BatchNum + "&tempCsUserName=" + tempCSUserName + "&tempCsUserID=" + tempCSUserID ;;	
	var httpResponse = JsHTTPGet(strURL) 
...
function GetSumofValues(ValueList){
	var strURL = "http://" + serverName + "/cheminv/gui/GetSumofValues.asp?valuelist=" + ValueList;
	var httpResponse = JsHTTPGet(strURL)
	return httpResponse;
}
...
{
	var strURL = "http://" + serverName + "/cheminv/api/isUniqueBarcode.asp?barcode=" + Barcode; 	
	var httpResponse = JsHTTPGet(strURL);
	if (httpResponse==0) {
	    return true;
	}
