...
arrCategories = array("Update","Create","Manage","Other")
...
arrLinks(0,1) = "/cheminv/gui/ChangeQty.asp?LocationId="&LocationID&"&ContainerID=" & ContainerID & "&QtyRemaining=" & QtyRemaining & "&UOMAbv=" & UOMAbv & "&TotalQtyReserved=" & TotalQtyReserved
...
arrLinks(5,1) = "/Cheminv/GUI/RequestSample.asp?action=create&ContainerID=" & ContainerID & "&LocationID=" & LocationID & "&UOMAbv=" & UOMAbv & "&QtyRequired=" & QtyAvailable & "&BatchID=" & BatchID
...
arrLinks(18,1) = "/Cheminv/GUI/PrintLPRLabel.asp?ContainerID=" & ContainerID
...
arrLinks(20,1) = "/cheminv/gui/manageDocuments.asp?FK_value=" & ContainerID & "&FK_name=CONTAINER%20ID&Table_Name=INV_CONTAINERS&LINK_TYPE=CHEMINVCONTAINERID"
arrLinks(20,2) = "Manage Documents"
arrLinks(20,3) = "2"
arrLinks(20,5) = arrCategories(3)
arrLinks(20,6) = "Manage Documents"
...
If (Application("AllowRequests")) then
    ...
    if (Session("INV_CHECKOUT_CONTAINER" & dbkey) AND IsNull(RequestID) AND ContainerStatusID = "1") then
        allowContainerRequest="1"
        arrLinks(5,1) = arrLinks(5,1) & "&allowContainerRequest=1"
    end if
    if Application("ShowRequestSample") and Session("INV_CHECKOUT_CONTAINER" & dbkey) and cint(ContainerStatusID) = cint(Application("StatusApproved")) then
        allowSampleRequest="1"
        arrLinks(5,1) = arrLinks(5,1) & "&allowSampleRequest=1"
    end if        
    if allowContainerRequest = "1" or allowSampleRequest = "1" then
        arrLinks(5,0) = 1
    else
        arrLinks(5,4) = 0
    end if
...
'PJD turn off these links--not necessary.
arrLinks(2,0) = 0
arrLinks(13,0) = 0
arrLinks(5,0) = 0
arrLinks(15,0) = 0
