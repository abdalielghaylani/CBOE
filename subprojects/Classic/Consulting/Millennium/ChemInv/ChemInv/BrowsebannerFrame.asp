...
<html>
	<head>
        <link rel="<%=Application("appTitle")%>" href="favicon.ico" >	
	...
	<form name="LocationBar" METHOD="post" Action="ChangeLocation.asp" Target="HiddenFrame">
		<table border="0" cellspacing="0" cellpadding="2" width="700" align="left">
			...
			<tr>
				<td colspan="2" class="grayedTextLeft">
...
<%
    if ( FormPopup = "NewContainer" ) then            
%>
            var wndNewContainer = OpenDialog('../gui/CreateOrEditContainer.asp?GetData=new&amp;DefaultLocationOverride=true', 'ConDiag', 2);
            if( wndNewContainer )
            {                
                wndNewContainer.focus();
            }            
<%
    end if
%>
