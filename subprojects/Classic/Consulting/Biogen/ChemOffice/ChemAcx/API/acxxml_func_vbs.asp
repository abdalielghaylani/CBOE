...
Sub GetProductRS(byRef AdoConnection, byVal sFieldName, byVal sValueList)
	if sValueList <> "" then
		Select Case UCase(sFieldName)
			Case "CSNUM"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT CsNum, ProductID AS ACXprodID, CatalogNum AS catNum, NVL(Catalog2Num,product.CatalogNum) AS catNum2,supplierID, prodName, ProdDescrip AS prodDescription FROM Product WHERE CsNum IN(" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
					SQLQuery = "SELECT CsNum, ProductID AS ACXprodID, CatalogNum AS catNum,NVL(Catalog2Num,CatalogNum) AS catNum2,supplierID, prodName, ProdDescrip AS prodDescription FROM Product WHERE CsNum IN(" & sValueList & ")"
					SQLQuery_Parameters = ""
				End if
			Case "PRODUCTID"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT CsNum, ProductID AS ACXprodID, CatalogNum AS catNum,NVL(Catalog2Num,product.CatalogNum) AS catNum2,supplierID, prodName, ProdDescrip AS prodDescription FROM Product WHERE ProductID IN(" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
					SQLQuery = "SELECT CsNum, ProductID AS ACXprodID, CatalogNum AS catNum,NVL(Catalog2Num,CatalogNum) AS catNum2,supplierID, prodName, ProdDescrip AS prodDescription FROM Product WHERE ProductID IN(" & sValueList & ")"
					SQLQuery_Parameters = ""
				End if
			Case "PACKAGEID"
				'SQLQuery = "SELECT DISTINCT Product.CSNum, Product.ProductID AS ACXprodID, Product.CatalogNum AS catNum,NVL(Catalog2Num,product.CatalogNum) AS catNum2, Product.SupplierID, Product.ProdName, Product.ProdDescrip AS prodDescription FROM Product INNER JOIN Package ON Product.ProductID = Package.ProductID WHERE Package.PackageID In (" & sValueList & ")"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT DISTINCT Product.CSNum, Product.ProductID AS ACXprodID, Product.CatalogNum AS catNum, NVL(Catalog2Num,product.CatalogNum) AS catNum2, Product.SupplierID, Product.ProdName, Product.ProdDescrip AS prodDescription FROM Product, Package WHERE Product.ProductID = Package.ProductID AND Package.PackageID IN (" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
		...
		if NOT (productRS.EOF AND productRS.BOF) then
			While Not productRS.EOF
				...
				AddAttributeToElement myDOM, elemProduct, "catNum2", productRS("catNum2")
	...
	if sValueList <> "" then
		...
		if NOT (packageRS.EOF AND packageRS.BOF) then
			While Not packageRS.EOF
				...
				AddAttributeToElement myDOM, elemPackage, "UNSPSC", packageRS("UNSPSC")
...
Sub GetSupplierRS(byRef AdoConnection, byVal sFieldName, byVal sValueList)
	if sValueList <> "" then
		Select Case UCase(sFieldName)
			Case "CSNUM"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT DISTINCT Supplier.DUNS as DUNS, Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Product, Supplier WHERE Product.SupplierID = Supplier.SupplierID AND Product.CSNum IN (" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
					SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Product, Supplier WHERE Product.SupplierID = Supplier.SupplierID AND Product.CSNum IN (" & sValueList & ")"
					SQLQuery_Parameters = ""
				End if
			Case "PRODUCTID"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Product, Supplier WHERE Product.SupplierID = Supplier.SupplierID AND Product.ProductID IN (" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
					SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Product, Supplier WHERE Product.SupplierID = Supplier.SupplierID AND Product.ProductID IN (" & sValueList & ")"
					SQLQuery_Parameters = ""
				End if
			Case "PACKAGEID"
				If Application("BASE_DBMS") = "ORACLE" then
					SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Package, Supplier WHERE Package.SupplierID = Supplier.SupplierID AND Package.PackageID IN (" & BuildInClause(sValueList) & ")"
					SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
				Else
					SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Package, Supplier WHERE Package.SupplierID = Supplier.SupplierID AND Package.PackageID IN (" & sValueList & ")"
					SQLQuery_Parameters = ""
				End if
			' blank means no restriction.  Returns all suppliers
			Case ""
				SQLQuery = "SELECT DISTINCT  Supplier.DUNS as DUNS,Supplier.SupplierID AS supplierID, Supplier.Name AS supplierName, Supplier.ShortName AS supplierShortName, Supplier.SupplierCode AS supplierCode, Supplier.supplierType FROM Supplier"
				SQLQuery_Parameters = "InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & sValueList 'Name|Type|Direction|Size|Value
		End Select	 
		...
		if NOT (supplierRS.EOF AND supplierRS.BOF) then
			While Not supplierRS.EOF
				...
				AddAttributeToElement myDOM, elemSupplier, "DUNS", supplierRS("DUNS")
