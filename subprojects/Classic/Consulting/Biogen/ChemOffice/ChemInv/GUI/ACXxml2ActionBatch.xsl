			...
			<xsl:for-each select="//package">
				<xsl:element name="CREATECONTAINER">
					...
					<xsl:element name="OPTIONALPARAMS">
						...
						<xsl:element name="FIELD_6">
							<xsl:value-of select="@Field_6" />				
						</xsl:element>
						<xsl:element name="FIELD_7">
							<xsl:value-of select="@Field_7" />				
						</xsl:element>
						<xsl:element name="FIELD_8">
							<xsl:value-of select="@Field_8" />				
						</xsl:element>
						<xsl:element name="FIELD_9">
							<xsl:value-of select="@Field_9" />				
						</xsl:element>
						<xsl:element name="FIELD_10">
							<xsl:value-of select="@Field_10" />				
						</xsl:element>
						<xsl:element name="REQNUMBER">
							<xsl:value-of select="@reqnumber" />				
						</xsl:element>
						<xsl:element name="PONUMBER">
							<xsl:value-of select="@ponumber" />				
						</xsl:element>
						<xsl:element name="PKGNUM">
							<xsl:value-of select="@pkgnum" />				
						</xsl:element>
						<xsl:element name="PKGNUM">
							<xsl:value-of select="@pkgnum" />				
						</xsl:element>
						<xsl:element name="DUNS">
							<xsl:value-of select="@duns" />				
						</xsl:element>
						<xsl:element name="UNSPSC">
