...
case "DELETE"
	...
	User_ID = Session("USER_SETTINGS_ID" & dbkey)
	
	'remove item from user settings table and dictionary 
	SettingName  = "SDFEXPORTTEMPLATE" &  Session("USER_SETTINGS_ID" & dbkey) & UCase(dbkey) &  UCase(formgroup) & ":" & templateName
...
case "SAVE"
	...
	if not dupslist <> ""  then
		...
		if bDelete = true then
			...
			SettingName  = "SDFEXPORTTEMPLATE" &  Session("USER_SETTINGS_ID" & dbkey) & UCase(dbkey) &  UCase(formgroup) & ":" & originalTemplateName
			deleteCurrentSettings request("dbname"), request("formgroup"), SettingName, User_ID
			'remove from dictionary 
...
if templateName <> "" then
	...
	tables_string = session("export_hits_table_display_names")%>
			...
			document.write('<table ><tr><td>Edit field names and click OK to use in your current sdfile export. Field names must be unique.</td></tr><tr><td>To save the template for repeated use, enter a ahort template name (no spaces in name), description and specify checked state before clicking OK.</td></tr></table>')
...
%>
<table border=0><tr><td><select name='saved_templates'><option value=0>--Select a template---</option><%=savedTemplateNames%></select></td></tr></table>
<%End select%>
</form></body>
</html>
