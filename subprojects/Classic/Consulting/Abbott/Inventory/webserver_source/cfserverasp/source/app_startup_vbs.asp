...
Sub SetAdditionalAppVariables()
	...
	SetIniParamDefault "CDAX_ProgID", "ChemDrawControl9.ChemDrawCtl"
	...
	Application("AUTHENTICATION_MODE") = GetINIValue2("optional", "GLOBALS", "AUTHENTICATION_MODE", "chemoffice", "chemoffice")
	SetIniParamDefault "AUTHENTICATION_MODE", "ORACLE"
	
	' Check Active Directory Domain
	Application("ACTIVE_DIRECTORY_DOMAIN") = GetINIValue2("optional", "GLOBALS", "ACTIVE_DIRECTORY_DOMAIN", "chemoffice", "chemoffice")
	SetIniParamDefault "ACTIVE_DIRECTORY_DOMAIN", ""
	
	' Check LDAP Service Account Name
	Application("LDAP_SERVICE_ACCOUNT_NAME") = GetINIValue2("optional", "GLOBALS", "LDAP_SERVICE_ACCOUNT_NAME", "chemoffice", "chemoffice")
	SetIniParamDefault "LDAP_SERVICE_ACCOUNT_NAME", "ORACLE"
	
	' Check LDAP Service Account PWD
	Application("LDAP_SERVICE_ACCOUNT_PWD") = GetINIValue2("optional", "GLOBALS", "LDAP_SERVICE_ACCOUNT_PWD", "chemoffice", "chemoffice")
	...
	if Application("BaseConnDBMS") = "ORACLE" then
		...
		if Application("CSCartridgeVersion") <> "" then Application("CSCartridgeMajorVersion") = Left(Application("CSCartridgeVersion"),1)
	else
...
Function GetCSCartridgeVersion
	...
	if err then
		...
		if (RS.eof and Rs.eof) then
			...
			GetCSCartridgeVersion = RS(0).value		
		end if	
	end if
End function
