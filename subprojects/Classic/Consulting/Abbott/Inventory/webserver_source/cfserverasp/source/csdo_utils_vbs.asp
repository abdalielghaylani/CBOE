	...
	select Case ucDataType
		case "FORMULA"
			...
			if bUseCartridge = true then
				...
				if UCase(fieldname) = "FORMULA" or UCase(fieldname)="STRUCTURE"  then 
		...
		case "MW", "MOLWEIGHT", "MOLWT"
			...
			if bUseCartridge = true then
				orig_fieldname = fieldname
				orig_tablename = tablename
				if UCase(fieldname) = "MOLWEIGHT" or UCase(fieldname)="STRUCTURE" then 
...
Function CheckChemicalContent(myPath)
	if Not isObject(Application("Molserver_Molecule")) then
		Application.Lock
			Set Application("Molserver_Molecule") = Server.CreateObject("Molserver8.Molecule")
		Application.UnLock
	end if
