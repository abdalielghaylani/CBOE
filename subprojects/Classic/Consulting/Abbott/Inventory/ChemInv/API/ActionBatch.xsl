...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
      ...
      <BODY>
		...
		<table>
			...
			<xsl:choose>
			   ...
			   <xsl:otherwise>
				  <a class="MenuLink" href="#" onclick="history.back(); return false">Back</a> | <a class="MenuLink" href="/cheminv/api/ProcessActionBatch.asp?action=test">Test</a> | <a class="MenuLink" href="/cheminv/api/ProcessActionBatch.asp?action=commit">Commit</a>
			   </xsl:otherwise>
			</xsl:choose>
			</td>
		</tr>	
  ...
  <xsl:template match="CREATECONTAINER[1]">
	<TABLE BORDER="0">
          <TR>
			<Th colspan="8">Create Container</Th>
          </TR>
          <TR>
            <Th>LocationID</Th>
            <Th>Copies</Th>
          ...
          <xsl:for-each select="/CHEMINVACTIONBATCH/CREATECONTAINER">
            <TR> 
              ...
              <TD><xsl:value-of select="@UOMID"/></TD> 
              <TD><xsl:value-of select="@InitialQty"/></TD> 
              <TD><xsl:value-of select="@ContainerTypeID"/></TD>  
