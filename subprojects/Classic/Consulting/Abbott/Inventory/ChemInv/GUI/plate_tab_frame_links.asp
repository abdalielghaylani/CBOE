...
If (Session("INV_CREATE_PLATE" & dbkey)) and not bIsPlateMap then
	call CheckIfFirst()	
%>			
	<a class="MenuLink" HREF="#" onclick="OpenDialog('/Cheminv/GUI/ReformatPlate.asp?pageAction=daughter&PlateID=<%=PlateID%>', 'Diag', 1); return false">Create Daughter Plates</a>
<%End if%>
...
<%If Session("INV_DILUTE_PLATE" & dbkey) then
	call CheckIfFirst()	
%>	
	<a class="MenuLink" HREF="#" onclick="OpenDialog('/Cheminv/GUI/ReformatPlate_SourceSolvate.asp?PlateID=<%=PlateID%>&BarcodeList=<%=Session("plPlate_Barcode")%>&pageMode=dilute', 'Diag', 4); return false">Dilute Plate</a>
<%End if%>
