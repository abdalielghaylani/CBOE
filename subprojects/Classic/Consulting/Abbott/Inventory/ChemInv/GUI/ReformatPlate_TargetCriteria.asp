...
If reformatAction = "daughter" then
	...
	Cmd.Parameters.Append Cmd.CreateParameter("RETURN_VALUE", 5, adParamReturnValue, 0, NULL)
	Cmd.Parameters.Append Cmd.CreateParameter("PREFORMATMAPID", 200, 1, 200, XMLDoc_ID)
	Cmd.Parameters.Append Cmd.CreateParameter("PPLATETYPE", 200, 1, 2000, "target")
	if bDebugPrint then
		For each p in Cmd.Parameters
			Response.Write p.name & " = " & p.value & "<BR>"
		Next
...
<html>
<head>
<title><%=Application("appTitle")%> -- Daughter Plate Criteria</title>
...
<!--Hide JavaScript
	...
	function Validate(){
		...
		if (!document.form1.AutoGen_r[1].checked){
			...
			if (document.form1.AutoGen_r[2].checked){
				...
				else if (!isPositiveNumber(document.form1.Barcode_Start.value)){
					errmsg = errmsg + "- Barcode Start must be a positive integer.\r";
					bPlateBarcodeErr = true
					bWriteError = true
				}
				else {
			...
			if (!bPlateBarcodeErr){
				//alert(Barcodes);
				var strURL = "http://" + serverName + "/cheminv/api/CheckDuplicateBarcode.asp?Barcodes=" + Barcodes + "&BarcodeType=plate";	
		...
		//dataMode = document.all.dataMode.value;
		for (i=0; i<numTargetPlates; i++) {
			//location is required
			LocElement = eval("document.all.LocationID" + i);
			if(LocElement.value.length == 0) {
				errmsg = errmsg + "- Location is required.\r";
				bWriteError = true;
			}		
			//if (dataMode == "sourceAmount"){
				VolElement = eval("document.all.SourceVolTaken" + i);
				AmtElement = eval("document.all.SourceAmtTaken" + i);
				//Either Source Volume or Source Amount is required
				if (VolElement.value.length == 0 && AmtElement.value.length == 0) {
					errmsg = errmsg + "- You must enter either a Source Volume Taken\r";
					errmsg = errmsg + "     OR a Source Amount Taken.\r";
					bWriteError = true;
				}
				else if (VolElement.value.length > 0 && AmtElement.value.length == 0) {
					//Source Volume Taken must be a number
					if (!isNumber(VolElement.value)){
						errmsg = errmsg + "- Source Volume Taken must be a number.\r";
						bWriteError = true;
					}
					else {
						//Source Volume Taken must be a positive number
						if (VolElement.value <= 0){
							errmsg = errmsg + "- Source Volume Taken must be a positive number.\r";
							bWriteError = true;
						}
					}
				}
	...
	function CalculateConcentration(divNumber, element){
		//calculate molar concentration
		MolarAmount = eval("document.all.MolarAmount" + divNumber + ".value");
		MolarUnitFK = eval("document.all.MolarUnitFK" + divNumber + ".value");
		SolventVolume = eval("document.all.SolventVolume" + divNumber + ".value");
		SolventVolumeUnitID = eval("document.all.SolventVolumeUnitID" + divNumber + ".value");
		//ConcentrationUnitID = eval("document.all.ConcentrationUnitID" + divNumber + ".value");
		ConcentrationUnitID = 15;
		if (SolventVolume.length != 0){
			var strURL = "http://" + serverName + "/cheminv/api/CalcMolarConc.asp?MolarAmount=" + MolarAmount + "&MolarUnitFK=" + MolarUnitFK + "&SolventVolume=" + SolventVolume + "&SolventVolumeUnitID=" + SolventVolumeUnitID + "&ConcentrationUnitID=" + ConcentrationUnitID;	
			var httpResponse = JsHTTPGet(strURL) 
			/*
			alert(strURL);
			httpResonse = 1;
			*/
			if (httpResponse == -1) {
				eval("document.all.MolarConcentration" + divNumber + ".value = 'Invalid'");
			}
			else {
				eval("document.all.MolarConcentration" + divNumber + ".value = httpResponse");
			}	
		}
	}
	function CalculateVolume(divNumber, element){
		//calculate molar concentration
		MolarAmount = eval("document.all.MolarAmount" + divNumber + ".value");
		MolarUnitFK = eval("document.all.MolarUnitFK" + divNumber + ".value");
		Concentration = eval("document.all.Concentration" + divNumber + ".value");
		ConcentrationUnitID = eval("document.all.ConcentrationUnitID" + divNumber + ".value");
		//ConcentrationUnitID = eval("document.all.ConcentrationUnitID" + divNumber + ".value");
		VolumeUnitID = 4;
		if (Concentration.length != 0){
			var strURL = "http://" + serverName + "/cheminv/api/CalcVolume.asp?MolarAmount=" + MolarAmount + "&MolarUnitFK=" + MolarUnitFK + "&Concentration=" + Concentration + "&ConcentrationUnitID=" + ConcentrationUnitID + "&VolumeUnitID=" + VolumeUnitID;	
			var httpResponse = JsHTTPGet(strURL) 
			/*
			alert(strURL);
			httpResonse = 1;
			*/
			if (httpResponse == -1) {
				eval("document.all.SolventVolume" + divNumber + ".value = 'Invalid'");
			}
			else {
				eval("document.all.SolventVolume" + divNumber + ".value = httpResponse");
			}	
		}
	}
...
<table border="0" cellspacing="0" cellpadding="1">
	<tr>
		...
		%>
			<div id="div_<%=i%>" style="display:block;">
		...
		<%
			...
			if cLng(numCurrDaughterPlate) > clng(numDaughterPlates) then
				numCurrDaughterPlate = 1
		...
		<tr height="25">
			<td align="right" valign="top" nowrap width="150">
				<span id="sp0_<%=i%>" style="display:block;"><span class="required">Plate Barcode:</span></span>
				<span id="sp1_<%=i%>" style="display:none;"><span class="required">Barcode Description:</span></span>
			...
			<td>
				...
				<span id="sp3_<%=i%>" style="display:none">
				...
				<input type="radio" name="AutoGen_r" onclick="AutoGen_OnClick(this);" value="0" CHECKED>Assign barcodes manually<BR>
				<input type="radio" name="AutoGen_r" onclick="AutoGen_OnClick(this);" value="1">Autogenerate barcode from barcode description<BR>
				<input type="radio" name="AutoGen_r" onclick="AutoGen_OnClick(this);" value="2">Autogenerate barcode from custom sequence
...
<!--
	<tr>
		<td align="right" width="150" nowrap>Well Capacity<%=unit_abreviation%>:</td>
		<td>
			<input TYPE="text" SIZE="15" Maxlength="50" NAME="WellCapacity" CLASS="GrayedText" VALUE="<%=well_capacity%>" READONLY>
		</td>
	</tr>
-->
	<!--
	<tr>
		<td align="right" valign="top" nowrap width="150">
			<span class="required">Barcode Description:</span>
		</td>
		<td>
			<%=ShowSelectBox("BarcodeDescID" & num, Barcode_desc_ID,"SELECT barcode_desc_id AS Value, barcode_desc_name AS Displaytext FROM inv_barcode_desc ORDER BY lower(DisplayText) ASC")%>
		</td>
	</tr>
	-->
	<tr>
	...
	<%'elseif dataMode = "targetConcentration" then%>
	<!--
		<TR>
			<%=ShowInputBox2("<SPAN CLASS=""required"">Target Concentration:</SPAN>", "TargetConc" & i, 150, 15, ShowSelectBox("TargetConcUnitID" & i, Session("plConc_Unit_FK"),"SELECT unit_id AS Value, unit_name AS DisplayText FROM inv_units WHERE unit_type_id_fk in (3) ORDER BY lower(DisplayText) ASC"), False, False)%>
		</TR>
	-->
	<!--
	<TR>
		<TD ALIGN="right" WIDTH="150" NOWRAP><span class="required">Target Concentration:</span></TD>
		<TD>
			<INPUT TYPE="text" SIZE="10" MAXLENGTH="50" NAME="TargetConc<%=num%>" VALUE="">
			<!--<INPUT TYPE="text" SIZE="10" MAXLENGTH="50" NAME="TargetConc<%=i%>" VALUE="" ONCHANGE="CalculateVolume(<%=i%>, this);">-->
	<!--
			<%=ShowSelectBox3("TargetConcUnitID" & num, 15,"SELECT unit_id AS Value, unit_name AS DisplayText FROM inv_units WHERE unit_type_id_fk in (3) ORDER BY lower(DisplayText) ASC",null,null,null,"")%>
			<%'=ShowSelectBox3("TargetConcUnitID" & i, 15,"SELECT unit_id AS Value, unit_name AS DisplayText FROM inv_units WHERE unit_type_id_fk in (3) ORDER BY lower(DisplayText) ASC",null,null,null,"CalculateVolume(" & i &", this);")%>
		</TD>
	</TR>
	<TR>
		<TD ALIGN="right" WIDTH="150" NOWRAP><span class="required">Target Volume:</span></TD>
		<TD>
			<INPUT TYPE="text" SIZE="10" MAXLENGTH="50" NAME="TargetVolume<%=num%>" VALUE="">
			<%=ShowSelectBox3("TargetVolumeUnitID" & num, 15,"SELECT unit_id AS Value, unit_name AS DisplayText FROM inv_units WHERE unit_type_id_fk in (1) ORDER BY lower(DisplayText) ASC",null,null,null,"//CalculateConcentration(" & num &", this);")%>
		</TD>
	</TR>
	<tr>
		<td align=right  width="150" valign=top nowrap><span class="required">Select Solvent:</span></td>
		<td><%=ShowSelectBox2("SolventID" & num,null,"SELECT solvent_id AS Value, solvent_name AS DisplayText FROM " & Application("CHEMINV_USERNAME") & ".inv_solvents ORDER BY lower(DisplayText) ASC", null, "Select a Solvent", "-1")%>
	</tr>
	-->
	<%'end if%>
