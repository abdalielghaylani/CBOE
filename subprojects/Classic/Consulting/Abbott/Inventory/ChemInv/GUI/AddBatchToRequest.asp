...
<!--Hide JavaScript
	...
	function updateQtySelected(){
		...
		var dispText = Math.round(eval(qtySelected)*100)/100 + " <%=UOMAbbrv%>";
	...
	function validateAliquot(){
		...
		if (cntContainers == 0){
			...
			OpenDialog('/Cheminv/GUI/CreateSamplesFromBatch.asp?Action=new&amp;batchid=<%=BatchID%>&amp;DefLocationID=' + defLocationID + '&amp;RequestID=<%=RequestID%>&amp;SampleRegID=<%=SampleRegID%>&amp;SampleBatchNumber=<%=SampleBatchNumber%>&amp;ContainerIDList='+strContainerIDList, 'Diag1', 2); return false;
		}		
	}
...
<table border="0" cellpadding="0">
	...
	<tr><td align="right">Requested Amount:</td><td><strong><%=QtyRequired%>&nbsp;<%=UOMAbbrv%></td></tr>
	<tr><td align="right">Selected Amount:</td><td><strong><div id="amountSelected">0 <%=UOMAbbrv%></div></td></tr>
