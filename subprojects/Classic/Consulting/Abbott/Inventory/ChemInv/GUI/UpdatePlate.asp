		...
		if (strMode.toLowerCase() == "edit") {
			...
			for (i=0; i<arrFields.length; i++) {
				...
				switch (arrFields[i]) {
					...
					case "solvent_volume":
						// Concentration if present must be a positive number
						if (document.form1.isolvent_volulme.value.length >0 && !isPositiveNumber(document.form1.isolvent_volume.value)){
	...
	<tr>
		...
		<td>		
			<SELECT NAME="plateFields" SIZE="8" MULTIPLE>
				...
				<OPTION VALUE="Solvent Volume:solvent_volume_unit_id_fk">Solvent Volume Unit</OPTION>
