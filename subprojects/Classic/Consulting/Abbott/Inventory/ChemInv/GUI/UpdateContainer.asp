...
if ContainerID <> "" then
	...
	GetInvConnection()
	SQL = "SELECT min(qty_max) minQtyMax, max(qty_remaining) as maxQtyRemaining, min(CHEMINVDB2.RESERVATIONS.GETTOTALQTYRESERVED(container_id)) as TotalQtyReserved FROM inv_containers WHERE container_id in (" & ContainerID & ")"
	...
	if right(fieldName,2) = "fk" then
		Select case fieldName
			...
			Case "Compound:compound_id_fk"
				...
				theRow = theRow & "<input type=""text"" Size=""15"" name=""iCompoundID"" onchange=""icompound_id_fk.value= this.value; RegBatchID.value=''; iRegID.value=''; iBatchNumber.value='';""><input Type=""hidden"" name=""icompound_id_fk""><input Type=""hidden"" name=""iContainerName""><input type=""hidden"" name=""NewCompoundID""></TD></TR>"
...
<%sub CreateTextFieldRow(field)
	...
	Select Case lcase(fieldColumn)
		...
		case "purity"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("UOPID", Session("UOPIDOptionValue"),"SELECT UNIT_ID||'='||Unit_Abreviation AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 3 ORDER BY Unit_Name ASC"), False, False)
		case "concentration"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("UOCID", Session("UOCIDOptionValue"),"SELECT UNIT_ID||'='||Unit_Abreviation AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK in (3,6) ORDER BY Unit_Name ASC"), False, False)
		case "density"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("UODID", Session("UODIDOptionValue"),"SELECT UNIT_ID||'='||Unit_Abreviation AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 6 ORDER BY Unit_Name ASC"), False, False)
		case "container_cost"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("UOCostID", Session("UOCostIDOptionValue"),"SELECT UNIT_ID||'='||Unit_Abreviation AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 5 ORDER BY Unit_Name ASC"), False, False)
		case else
