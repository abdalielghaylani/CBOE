...
<%		
	...
	if cint(ParentCount) > 1 then
		...
		compoundSQL =  "SELECT DECODE(wc.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT BASE64_CDX FROM inv_vw_reg_batches WHERE regid=wc.reg_id_fk AND batchnumber=wc.Batch_Number_FK)) AS Base64_CDX," & _
						"DECODE(wc.Reg_ID_FK, NULL, cpd.Substance_Name, (SELECT RegName FROM inv_vw_reg_batches WHERE regid = wc.reg_id_fk)) as Substance_Name, " & _
						"wc.COMPOUND_ID_FK " 
						if Application("RegServerName") <> "NULL" then
							compoundSQL = compoundSQL & ", DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT RegBatchID FROM inv_vw_reg_batches WHERE inv_vw_reg_batches.regid = wc.reg_id_fk)) AS Reg_Batch_ID "
		...
		for i = 0 to ubound(arrParentWellIDs)
			if count > 5 then
 			...
 			if not rsMixtureCompounds.EOF then
				...
				structureHTML = structureHTML & "<script language=""javascript"">cd_insertObject(""chemical/x-cdx"",150,130,""mycdx"",""" & TempcdxPath & """, true, true, dataURL=document.form1.structure" & i & ".value);</script></td></tr></table></td>"
				end if
