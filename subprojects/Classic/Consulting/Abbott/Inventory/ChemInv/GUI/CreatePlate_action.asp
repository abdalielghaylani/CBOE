	...
	if len(Session(field)) > 0 then
		if InStr("iContainerIDs,multiscan,multiSelect,fieldList,AutoGen", field) = 0 then
			Select case lcase(field)
				'number fields
				case "pllocation_id_fk","plplate_type_id_fk","plqty_unit_fk","plqty_initial","plqty_remaining","plstatus_id_fk","plsupplier_shipment_number","pllibrary_id_fk","plft_cycles","plweight_unit_fk","plweight","plconc_unit_fk","plconcentration","plsolvent_id_fk","plsolvent_volume","plsolvent_volume_unit_id_fk"
		...
		<TD HEIGHT=50 VALIGN=MIDDLE>
			...
			If numPlates > 0 and not bWellUpdateError then
				If not bUpdateWells then
					...
					Response.Write "<SCRIPT LANGUAGE=javascript>SelectLocationNode(0, " & Session("CurrentLocationID") & ", 0, '" & Session("TreeViewOpenNodes1") & "'," & Request("iPlateIDs") & ",1); opener.focus(); window.close();</SCRIPT>" 
				else				
