...
if ContainerID <> "" then Session("ContainerID") = ContainerID
...
<!--Hide JavaScript
	...
	function ValidateContainer(strMode){
		...
		<% if sTab <> "Required" and sTab <> "" and Session("OrigLocationID") <> "" then %>
...
<% end if%>
	...
	function postDataFunction(sTab) {
		document.form1.action = "CreateOrEditContainer.asp?GetData=form&TB=" + sTab
	...
	function toggleRackListDisplay() {
		...
		if (document.form1.AssignToRack.checked) {
			...
			//alert(document.form1.ContainerID.value + ":" + "<%=Session("ContainerID")%>" + ":" + "<%=Request("ContainerID")%>");
			<% if isEdit then %>
			location.href = "/cheminv/gui/CreateOrEditContainer.asp?isEdit=true&getData=db&AssignToRack=on&ShowFullRackList=" + strRackShowFullList + "&LocationID=" + LocationID + "&ContainerID=" + document.form1.ContainerID.value + "&ContainerName=<%=ContainerName%>&QtyInitial=<%=QtyInitial%>&QtyMax=<%=QtyMax%>&RegBatchID=<%=RegBatchID%>&CompoundID=<%=CompoundID%>&Concentration=<%=Concentration%>&UOCIDOptionValue=<%=Session("UOCIDOptionValue")%>&toggleDefaults=" + toggleDefaults;
			<% else %>
			location.href = "/cheminv/gui/CreateOrEditContainer.asp?GetData=new&AssignToRack=on&ShowFullRackList=" + strRackShowFullList + "&LocationID=" + LocationID + "&ContainerID=" + document.form1.ContainerID.value + "&ContainerName=<%=ContainerName%>&QtyInitial=<%=QtyInitial%>&QtyMax=<%=QtyMax%>&RegBatchID=<%=RegBatchID%>&CompoundID=<%=CompoundID%>&Concentration=<%=Concentration%>&UOCIDOptionValue=<%=Session("UOCIDOptionValue")%>&toggleDefaults=true";
			<% end if %>
			document.form1.AssignToRack.checked = true;
		}
	}
	...
	function validateRackSelect() {
		...
		var numCopies = 1;
		<% else %>
		var numCopies = document.form1.iNumCopies.value;
		<% end if %>
		...
		if (document.form1.RackID.selectedIndex==-1) {
			...
			if (totPosCnt < numCopies) {
				var diffTot = numCopies-totPosCnt;
			...
			if (bWriteError){
				...
				OpenDialog('/cheminv/gui/ViewRackLayout.asp?ActionType=select&PosRequired='+document.form1.iNumCopies.value+'&IsMulti=true&LocationID='+document.form1.RackID.options[document.form1.RackID.selectedIndex].value+'&RackIDList='+TempIDs, 'RackGrid', 2);
				return false;
			}
		}
	}
	...
	function setDefaultStartingPosition(){
		if (eval(document.form1.iNumCopies.value) == 1){
			selectRackDefaults();
		}else{
			document.form1.iRackGridID.value='';
			document.form1.iRackGridName.value='';
		}
	}
...
<input TYPE="hidden" NAME="IsRack" value="false">
...
%>
	...
	<% if Application("RACKS_ENABLED") then
	RackInstructions = "The Location ID above will be ignored and the Container(s) will be placed in a Rack location. If the Location ID is changed, the list of available Racks to choose from will list those in the new Location.<br /><br />"
	%>
	<% if IsEdit then %>
	<input type="hidden" name="iNumCopies" value="1" />

	<tr height="25">
		<input type="hidden" name="RackGridList" value />
		<td align="right" valign="top">Assign Rack:</td><td colspan="3"><input class="noborder" type="checkbox" name="AssignToRack" onclick="toggleRackDisplay()">Assign container to Rack<br />
			<div class="singleRackElement"><span class="instructions"><%=RackInstructions%></span></div>
		</td>
	</tr>
	...
	<tr class="singleRackElement">
		<!-- ShowPickList("Select Rack:", "RackID", RackID,"select l.location_id as Value, l.location_name as DisplayText from inv_grid_storage s, inv_locations l where s.location_id_fk = l.location_id and l.collapse_child_nodes=1 and l.parent_id = " & LocationID & " order by DisplayText") -->
	...
	<tr class="singleRackElement" height="25">
		<td align="right" valign="top">Rack Position:</td><td>
			<table cellpadding="0" cellspacing="0"><tr><td>
				...
				<!--<a href="#" onclick="OpenDialog('/cheminv/gui/ViewRackLayout.asp?ActionType=select&amp;LocationID='+document.form1.RackID.options[document.form1.RackID.selectedIndex].value, 'RackGrid', 2); return false;"><img src="/cheminv/graphics/btn_search.gif" border="0"></a>-->
	...
	<% else %>
	<tr height="25">
		<td align="right">Assign Rack:</td><td colspan="3">
			<input type="hidden" name="RackGridList" value />
			<input class="noborder" type="checkbox" name="AssignToRack" onclick="toggleRackDisplay()">Assign container to Rack<br />
			<div class="singleRackElement"><span class="instructions"><%=RackInstructions%></span></div>
		</td>
	</tr>
	<tr class="singleRackElement" height="25">
	...
	<tr class="singleRackElement">
		<% 'ShowPickList3("Select Rack(s):", "RackID", RackID,"select l.location_id as Value,  l.location_name||' :: '||  ((f.row_count*f.col_count)-(     (select count(*) from inv_containers where location_id_fk in (select location_id from inv_vw_grid_location where parent_id=l.location_id))+     (select count(*) from inv_plates where location_id_fk in (select location_id from inv_vw_grid_location where parent_id=l.location_id))+     (select count(*) from inv_locations where parent_id in (select location_id from inv_vw_grid_location where parent_id=l.location_id)) ))||' open' as DisplayText, (select min(location_id||'::'||name) from inv_vw_grid_location where parent_id=l.location_id and location_id not in (select distinct location_id_fk from inv_containers) and location_id not in (select distinct location_id_fk from inv_plates) and location_id not in (select distinct parent_id from inv_locations where collapse_child_nodes=1)) as DefaultValue from inv_grid_storage s, inv_locations l , inv_grid_format f  where s.location_id_fk = l.location_id  and l.collapse_child_nodes=1  and s.grid_format_id_fk = f.grid_format_id  and l.location_id in (select location_id from inv_locations where collapse_child_nodes=1 connect by prior location_id = parent_id start with location_id=" & LocationID & ")  and collapse_child_nodes=1  and ((f.row_count*f.col_count)-(     (select count(*) from inv_containers where location_id_fk in (select location_id from inv_vw_grid_location where parent_id=l.location_id))+     (select count(*) from inv_plates where location_id_fk in (select location_id from inv_vw_grid_location where parent_id=l.location_id))+     (select count(*) from inv_locations where parent_id in (select location_id from inv_vw_grid_location where parent_id=l.location_id)) )) > 0 order by DisplayText",100,"","","selectRackDefaults();",5,true,"No Racks in this location") %>
	...
	<% end if %>
	<script language="javascript">
		<%RackGridInfo = replace(RackGridInfo,"\","\\")%>
		var rackGridInfo = "<%=RackGridInfo%>";
		//rackGridInfo = rackGridInfo.replace("\\",",");
		//alert(rackGridInfo);
		var copyContainer = "<%=Request("isCopy")%>";
		var containerID = <% if ContainerID <> "" then response.write ContainerID else response.write 0 end if %>;
		if (rackGridInfo.length > 0){
			var arrRackGridInfo = rackGridInfo.split("::");
			document.form1.AssignToRack.checked = true;
			toggleRackDisplay();
			//alert(document.form1.iRackGridName.value);
			// Display current position only when users are not copying container
			if (copyContainer != "true"){
				...
				for(l=0;l<document.form1.RackID.length;l++){
					...
					if (rackIDValue == arrRackGridInfo[3]){
						...
						document.form1.RackID.options[l].selected=false;
					}
				}
				//tmpRackLocationPath = arrRackGridInfo[1].replace("\",",");
				//alert(tmpRackLocationPath);
				//alert(arrRackGridInfo[6] + " in " + arrRackGridInfo[1]);
				document.form1.iRackGridID.value = arrRackGridInfo[2];
				document.form1.iRackGridName.value = arrRackGridInfo[6] + " in " + arrRackGridInfo[1];
				//alert(document.form1.iRackGridID.value + " : " + document.form1.iRackGridName.value);
			}

		} else if(document.form1.AssignToRack.checked) {
			selectRackDefaults();
		}
	</script>
	...
	<tr height="25">
		...
		<%if Application("RegServerName") <> "NULL" then%>
			<td width="200" align="right">Compound ID:</td><td><input type="text" Size="15" name="iCompoundID" value="<%=CompoundID%>" onchange="if(IsValidCompoundID(this.value, true)==1){NewCompoundID.value= this.value; RegBatchID.value=''; iRegID.value=''; iBatchNumber.value='';}else{this.value = '';}"></td>
		<%else%>
			<td width="200" align="right">Compound ID:</td><td><input type="text" Size="15" name="iCompoundID" value="<%=CompoundID%>" onchange="NewCompoundID.value= this.value;"></td>
		<%End if%>
...
%>
	...
	<tr height="25">
		...
		<td>
			<%=ShowSelectBox2("iCurrentUserID", UCASE(CurrentUserID), "SELECT Upper(User_ID) AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText FROM CS_SECURITY.People ORDER BY lower(Last_Name) ASC", 27, RepeatString(43, "&nbsp;"), "")%>
		</td>
	</tr>
	<tr height="25">
		<td colspan="2">
