...
GetInvConnection()
sql = "SELECT * FROM inv_vw_reg_batches WHERE inv_vw_reg_batches.regid = " & RegID &" AND inv_vw_reg_batches.batchnumber = " & BatchNumber
...
if Not(RS.BOF and RS.EOF) then
	...
	for each key in reg_fields_dict
		if key <> "BASE64_CDX" and key <> "REGNAME" then
			Response.write("<tr><td align=""right"">" & reg_fields_dict.item(key) & ":</td><td bgcolor=""#d3d3d3"">" & RS(key) & "</td></tr>")
		end if
	next
end if
RS.Close()
