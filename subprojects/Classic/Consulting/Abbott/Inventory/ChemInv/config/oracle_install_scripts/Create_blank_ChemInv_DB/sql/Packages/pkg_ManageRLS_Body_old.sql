    ...
    DELETE inv_role_locations WHERE role_id_fk = p_roleID;
      ...
      --' add a row to inv_user_locations for each location_id in the list
        ...
        FORALL i IN l_locationIDs_t.FIRST..l_locationIDs_t.LAST
  		  	INSERT INTO inv_role_locations SELECT p_roleID, location_id FROM inv_locations CONNECT BY PRIOR location_id = parent_id START WITH location_id = l_locationIDs_t(i);
      ELSE
       	--' don't propagate
        FORALL i IN l_locationIDs_t.FIRST..l_locationIDs_t.LAST
