   ...
   FUNCTION DELETESUBSTANCE (pCompoundID in Inv_compounds.compound_ID%Type)
	...
	Function UpdateInvCompound(
    	...
    	pHBondDonors in inv_compounds.hbond_donors%TYPE,
			...
			pCompoundID in inv_compounds.compound_id%type,
			pConflictingFields in inv_compounds.conflicting_fields%type) Return inv_compounds.compound_id%type AS
	...
	BEGIN
		...
		if InStr(lower(pUniqueAltIDList),'alt_id_1') > 0 then BuildConflictTypeSQL(vSQL, 'alt_id_2',  'alt_id_2', pAltID_2); end if;
		if InStr(lower(pUniqueAltIDList),'alt_id_1') > 0 then BuildConflictTypeSQL(vSQL, 'alt_id_3',  'alt_id_3', pAltID_3); end if;
		if InStr(lower(pUniqueAltIDList),'alt_id_1') > 0 then BuildConflictTypeSQL(vSQL, 'alt_id_4',  'alt_id_4', pAltID_4); end if;
		if InStr(lower(pUniqueAltIDList),'alt_id_1') > 0 then BuildConflictTypeSQL(vSQL, 'alt_id_5',  'alt_id_5', pAltID_5); end if;
		...
		if vConflictingCompoundCount > 0 then --Conflicts found
			if vExistingCompoundID > 0 and not vRegisterIfConflicts then  -- Use existing compound
		   		if vExistingCompoundID = pCompoundID then --This is and edit request
		   			oCompoundID := UpdateInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, vConflictingFields);
		   		else
	   		...
	   		if vRegisterIfConflicts then -- Create a duplicate compound
		  		if pCompoundID is not Null then  -- This is an edit request which turns the edited compound into a duplicate
		  		  oCompoundID := UpdateInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, vConflictingFields);
		  			...
		  			oCompoundID := InsertInvCompound(pSubstanceName, vStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, vConflictingFields);
					end if;
