...
AS
  ...
  AS
    ...
    OPEN O_RS FOR
        'SELECT delivery_location_id_fk, user_id_fk, inv_containers.container_id, inv_requests.batch_id_fk, qty_required, qty_available
        ...
        LOOP
		      FETCH O_RS INTO DestinationLocation, CurrentUserID, ContainerID, BatchID, QtyRequired, QtyAvailable;
		      EXIT WHEN O_RS%NOTFOUND;
		      rowCount := O_RS%ROWCOUNT;
  ...
  RETURN inv_requests.request_ID%Type AS
	...
	BEGIN
		OPEN O_RS FOR
          Select
            ...
            , cb.container_status_id_fk as BatchStatus
            , c.reg_id_fk
            , c.batch_number_fk
	...
	BEGIN
		OPEN O_RS FOR
          ...
          FROM inv_requests r
               , inv_locations l
               , cs_security.people p1
               , cs_security.people p2
...
PROCEDURE GETBATCHREQUEST2(
	...
	BEGIN
      ...
      SELECT	request_id,
        ...
        ivrb.*
      FROM inv_requests r, inv_locations l, cs_security.people p1,cs_security.people p2, inv_container_types ct, inv_request_status rs, inv_container_batches icb, inv_vw_reg_batches ivrb
      WHERE	r.user_id_fk = p1.user_id(+)
	...
	BEGIN
      ...
      l_batchFieldName3Display := Rtrim(Ltrim(constants.cContainerBatchField3Display));
	    ...
	    my_sql := 'SELECT request_id,
                  ...
                  to_char(trunc(r.date_delivered), ''' || pDateFormat || ''') as date_delivered,
                  l.location_name,
                  ivrb.regnumber as batchvalue1,
                  ivrb.batchnumber as batchvalue2,
                  ...
                  (select unit_abreviation FROM inv_containers c, inv_units u where batch_id_fk=r.batch_id_fk AND c.unit_of_meas_id_fk = u.unit_id(+) And rownum = 1) as uomabbrv,
																		...
																		r.date_1,
																		r.date_2
                  FROM	inv_requests r, inv_locations l, cs_security.people p1, cs_security.people p2, inv_container_batches cb, inv_request_status rs, inv_vw_reg_batches ivrb
                  WHERE	Upper(r.user_id_fk) = Upper(p1.user_id)
                  AND cb.batch_field_1 = ivrb.regid(+)
                  AND cb.batch_field_2 = ivrb.batchnumber(+)
                  AND r.batch_id_fk = cb.batch_id(+)
                  AND Upper(r.delivered_by_id_fk) = p2.user_id(+)
                  AND r.delivery_location_id_fk = l.location_id
                  AND r.request_status_id_fk = rs.request_status_id
                  AND r.timestamp BETWEEN NVL(' || pFromDate || ',to_date(''1980-01-01 0:0:0'',''YYYY-MM-DD HH24:MI:SS'')) AND NVL(' || pToDate || ',SYSDATE)
