...
Sub BuildResultsTree(ByVal pSearchType)
		...
		Select Case pSearchType
			...
			Case "ByContainerID"
				...
				'sSQL = "SELECT Max(Location_barcode) AS Location_barcode, Max(Location_description) AS Location_Description,  Max(inv_locations.Parent_ID) As parent_id, inv_locations.Location_ID As Id, max(collapse_child_nodes) as isRack, Max(inv_locations.Location_Name) AS Location_Name, Max((SELECT Location_Type_Name FROM inv_Location_Types WHERE Location_Type_ID = inv_Locations.Location_Type_ID_fk)) AS LocationTypeName, Min(Level) AS MaxLevel FROM inv_locations Where inv_locations.Location_ID not in (select location_id_fk from inv_grid_element) CONNECT BY Location_id = prior Parent_id START WITH Location_id IN (SELECT DISTINCT inv_locations.Location_ID FROM inv_locations, inv_containers WHERE inv_locations.Location_ID = inv_containers.Location_ID_FK AND inv_containers.Container_ID=" & ContainerID & ") GROUP BY inv_locations.Location_ID  ORDER BY Max(Level) DESC"
		...
		Do While Not RS.EOF
			if RS("IsRack") = "1" then
				bPrevIsRack = true
				prevRackID = RS("id").value
				Icon_clsd = "rack_closed.gif" 
				Icon_open = "rack_open.gif"
			else
				if bPrevIsRack = true then
				bCurrIsRack = false
				end if
				Icon_clsd = "icon_clsdfold.gif" 
				Icon_open = "icon_openfold.gif"
			...
			if (CLng(level) = 1) then
				...
				if bPrevIsRack = true then
					if IdName = "ContainerID" then
						SelectedType = "SelectContainer" 
					end if
					URL = BuildListURL & "?LocationID=" & prevRackID & "&" & SelectedType & "=" & BaseID & "&showInList=racks&" & Request.QueryString & "&LocationCount=1"
				else
					URL = BuildListURL & "?" & IdName & "=" & BaseID & "&" & Request.QueryString & "&LocationCount=1" & showInList
				end if
