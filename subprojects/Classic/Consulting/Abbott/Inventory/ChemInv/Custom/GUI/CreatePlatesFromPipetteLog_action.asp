...
elseif ActionType = "source" then
				...
				For j = 1 To Rows
					...
					For k = 1 To Cols
						...
						With oWellNode
				            ...
				            .setAttribute "PLATE_FORMAT_ID_FK", rsWell("PLATE_FORMAT_ID_FK").Value
							.setAttribute "GRID_POSITION_ID_FK", rsWell("GRID_POSITION_ID").Value
							.setAttribute "CONCENTRATION", rsWell("CONCENTRATION").Value
							.setAttribute "CONC_UNIT_FK", rsWell("CONC_UNIT_FK").Value
						End With
					    oColNode.appendChild oWellNode
					Next
				Next
	...
	'Response.End
		if instr(ProblemBarcodes,currSourcePlate) = 0 then
			if plateExists(SourcePlateBarcode) then
				'ProblemBarcodes = ProblemBarcodes & SourcePlateBarcode & ":1,"
				if not oProblemBarcodes1.Exists(SourcePlateBarcode) then oProblemBarcodes1.Add SourcePlateBarcode,1
				errCount = errCount + 1
			else
				'create plate xml
				rsWell.MoveFirst
				...
				if newPlateID <> "-1" then
					...
					'ProblemBarcodes = ProblemBarcodes & SourcePlateBarcode & ":2,"
...
<%
			...
			If clng(errCount) > 0 then 
				...
				problemBarcodes3 = ""
				problemBarcodes4 = ""
				if oProblemBarcodes1.Count > 0 or oProblemBarcodes2.Count > 0 or oProblemBarcodes3.Count > 0 or oProblemBarcodes4.Count > 0 then
					'arrProblemBarcodes = split(left(ProblemBarcodes,len(ProblemBarcodes)-1),",")
					for each key in oProblemBarcodes1
						problemBarcodes1 = problemBarcodes1 & key & ","
					next
					for each key in oProblemBarcodes2
						problemBarcodes2 = problemBarcodes2 & key & ","
					next
					for each key in oProblemBarcodes3
						problemBarcodes3 = problemBarcodes3 & key & ","
					next
					for each key in oProblemBarcodes4
						problemBarcodes4 = problemBarcodes4 & key & ","
					next
				end if
			...
			Select Case theAction
				...
				Case "WriteOtherError"
					...
					Response.Write "<P><center><a HREF=""3"" onclick=""history.back(); return false;""><img SRC=""../../graphics/ok_dialog_btn.gif"" border=""0""></a></center>"
				Case "MiscError"
					if problemBarcodes1 <> "" then sError = sError & "Error: Plate(s) - " & left(problemBarcodes1, len(problemBarcodes1)-1) & " - " & iif(oProblemBarcodes1.Count>1,"were","was") & " not created due to a barcode conflict.<BR>" 
					if problemBarcodes2 <> "" then sError = sError & "Error: Plate(s) - " & left(problemBarcodes2, len(problemBarcodes2)-1) & " - " & iif(oProblemBarcodes2.Count>1,"were","was") & " not created due to an Oracle error.<BR>" 
					if problemBarcodes3 <> "" then sError = sError & "Error: Source plate(s) - " & left(problemBarcodes3, len(problemBarcodes3)-1) & " - " & iif(oProblemBarcodes3.Count>1,"do","does") & " not exist in Inventory Manger.  Plate(s) - " & left(problemBarcodes4, len(problemBarcodes4)-1) & " - " & iif(oProblemBarcodes4.Count>1,"have","has") & " not been created.<BR>" 
...
Sub PreProcess(sPath, Template, ByRef oStream)
	...
	if Template = "5" or Template = "6" then
		...
		ConvertFileToCSFormat sPath, oStream, HeaderLength, Delimiter, SourceBarcodeCol, SourceWellCol, TargetBarcodeCol, TargetWellCol, QtyInitialCol, QtyUnitCol, ConcentrationCol, ConcUnitCol, SolventIDCol, bUseWellCoordinates, NumColumns, Template
	end if
	'oStream.Position = 0
...
Sub ConvertFileToCSFormat(sPath, ByRef oStream, HeaderLength, Delimiter, SourceBarcodeCol, SourceWellCol, TargetBarcodeCol, TargetWellCol, QtyInitialCol, QtyUnitCol, ConcentrationCol, ConcUnitCol, SolventIDCol, bUseWellCoordinates, NumColumns, Template)
