...
'if ( "INTERNAL" = sCalledBy ) then
	' This page was called from within the D3 app.  Show the number of records in the
...
%>					</td>
				...
				<tr>
					...
					<td valign="top">
						<% 
							...
							sSQL = "Select STATUS_TEXT from D3_STATUSES where STATUS_KEY =" & rsParent("STATUS")

							rsDisplayStatus.Open sSQL, connD3

						ShowResult dbkey, formgroup, rsDisplayStatus, "D3_STATUS.STATUS_TEXT", "RAW", 0, 40 %>
				...
				<tr>
					...
					<td valign="top">
						<% ShowResult dbkey, formgroup, rsParent, "D3_PARENTS.SUBMITTED_BY", "RAW", 0, 40 %>
					</td>
				</tr>
				<tr>
					<td valign="top">
...
<%
	...
	rsExperiments.Open "SELECT * FROM D3_Expts WHERE Parent_Cmpd_FK = " & rsParent.Fields( "PARENT_CMPD_KEY" ), connD3
...
<%
		...
		while not rsExperiments.EOF
			...
			rsExperimentConds.Open "Select * from D3_CONDS where DEG_COND_KEY = " & rsExperiments.Fields( "DEG_COND_FK" ), connD3
...
<%
	...
	'JHS commented out 3/25/2003
	'if ( "INTERNAL" = sCalledBy ) then
		' This page was called from within the D3 app, so we may want to allow
...
<%
	...
	'JHS commented out 3/25/2003	
	'end if  ' if we can to this page from within D3, ...
	'JHS commented out 3/25/2003 end
