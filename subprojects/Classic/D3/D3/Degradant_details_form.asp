...
set connChem = GetConnection( dbkey, formgroup, "D3_DEGS" )

' Make and fill a record set for the degradant.
set rsDegradant = Server.CreateObject( "ADODB.Recordset" )
rsDegradant.Open "Select * from D3_DEGS where DEG_CMPD_KEY = " & keyDeg, connChem
...
select case lcase(wizardStep)
	...
	case "wiz_edit_deg_2"
		...
		<P><a class="headinglink" href="/<%=Application( "appkey" )%>/D3/mainpage.asp?dbname=<%=dbkey%>" target="_top">Return to main page</a>
...
%>
<input type=hidden name="dataaction2" value="update_D3">
...
<table  border = 1 cellpadding="3" cellspacing="0">
	<tr>
		...
		<%
			...
			rsBase64.Open "Select * from D3_BASE64 where MOL_ID = " & rsDegradant.Fields( "MOL_ID" ), connChem
			'if rsBase64.EOF then
				'if LCase(formmode = "edit") then
					'Response.Write "No Structure Submitted"
				'else
					'ShowStrucInputField  dbkey, formgroup, "D3_DEGS.Structure", "5", widthStructArea, heightStructArea, "Exact", "SelectList"
				'end if
			'else
			...
			<table  border = 0  color = "red">
				...
				<tr>
					<td  valign = "top">
						<blockquote> 
						<%
						if "edit" = LCase( formmode ) and "" = rsDegradant.Fields( "DEG_EXPT_FK" ) then
							' Not editting and there is nothing in the ID field.
							Response.Write( "No Compound ID found" )
						else
							
							ShowResult dbkey, formgroup, rsDegradant, "D3_DEGS.mol_id", "RAW", "0", "12"
						end if %>
						</blockquote>
					</td>
				</tr>
				...
				<tr>
					<td  valign = "top">
						...
						if "edit" = LCase( formmode ) and "" = rsDegradant.Fields( "ALIAS" ) then
							...
							ShowResult dbkey, formgroup, rsDegradant, "D3_DEGS.ALIAS", "RAW", "0", "30"
						end if 
						%>
						
						</blockquote>
						<%' ShowInputField dbkey, formgroup, "D3_DEGS.ALIAS", 0, "30" %>
					</td>
				</tr>
				<!-- A little vertical spacing before the next item. -->
				<tr>
				...
				<tr>
					<td  valign = "top">
						...
						<%
							...
							'	ShowLookUpList dbkey, formgroup, rsDegradant, "D3_DEGS.MW_DIR_CHG", sMWDirectionList, rsDegradant.Fields( "MW_DIR_CHG" ), sDisplayDirection, 0, false, "value", "1"
									
							'end if
							ShowResult dbkey, formgroup, rsDegradant, "D3_DEGS.MW_AMT_CHG", "RAW", "0", "12"
				...
				<tr>
					<td  valign = "top">
						<strong>Change in Molecular Weight</strong>
						<br><font size="-1">Please modify these fields if the<br>
										  parent of degradant contain an<br>R-Group.</font>
					</td>
				</tr>
				<tr>
					<td  valign = "top">
				...
				<tr>
					<td  valign = "top">
						...
						<blockquote>
						<%
						current_deg_fgroup_list=GetCurrentFunctionalGroups(dbkey, formgroup,  connChem,  "Deg_FGroup_Key", "Deg_FGroup_text", "D3_FGroups.Deg_FGroup_text", Session( "PrimaryKey" & dbkey ))
						...
						all_fgroups_list = ListAllFGroupsOrdered(dbkey, formgroup, connChem, "Deg_FGroup_Key", "Deg_FGroup_text", "D3_FGroups.Deg_FGroup_text")
						%>
						<script language="javascript">
