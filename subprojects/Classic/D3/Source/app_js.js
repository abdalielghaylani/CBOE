...
function add_D3(){
		...
		if (formgroup.toLowerCase() == "condition_form_group") {	
			...
			if (MainWindow.document.forms["cows_input_form"].elements["D3_CONDS.DEG_COND_TEXT"].value.length > 50) {
		...
		if (formgroup.toLowerCase() == "fgroup_form_group") {	
			...
			if (MainWindow.document.forms["cows_input_form"].elements["D3_FGROUPS.DEG_FGROUP_TEXT"].value.length > 50) {
			...
			if (MainWindow.document.forms["cows_input_form"].elements["D3_STATUSES.STATUS_TEXT"].value.length > 50) {
			...
			if (MainWindow.document.forms["cows_input_form"].elements["D3_EXPTS.EXPT_CONDS"].value.length > 255) {
				
				alert("The length of the experimental conditions field is greater than 255 characters and cannot be submitted.")
				keepgoing = "stop"
				
			}	
			else if (MainWindow.document.forms["cows_input_form"].elements["D3_EXPTS.DEG_COND_FK"].value == "") {
...
function D3UpdateRecord(){
		...
		if (formgroup.toLowerCase() == "addexperiment_form_group"){		
			...
			var keepgoinga = checkFieldMax(baseid, "D3_EXPTS.EXPT_CONDS", 255, "Experimental Conditions")
			var keepgoingb = "continue"
			...
			//this looks weird but a bunch of code was removed
			//this has been left in for safety
			var keepgoinga = "continue"
			var keepgoingb = "continue"
...
function getD3Button( buttonname, formgroup_override, gif_override ){
...
function fillFGroupsList(elm){
		...
		<%=Application("mainwindow")%>.document.forms[0].elements[elm].length = 0
		
		for (i=0;i<temp.length;i++){
			temp2 = temp[i].split(":")
			<%=Application("mainwindow")%>.document.forms[0].elements[elm].options[i] = new Option(temp2[0])
			<%=Application("mainwindow")%>.document.forms[0].elements[elm].options[i].value = temp2[0]
			<%=Application("mainwindow")%>.document.forms[0].elements[elm].options[i].text = temp2[1]
			
			
			if (hasCurrent) {
				selectFGroupOption(<%=Application("mainwindow")%>.document.forms[0].elements[elm].options[i])
			}
		}
}

function fill_deg_fgroup_list(elm){
	fillFGroupsList(elm)
}
