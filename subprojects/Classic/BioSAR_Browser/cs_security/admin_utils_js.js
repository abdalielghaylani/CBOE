...
function doUpdateUserPwd(orig_password, user_name){
	...
	new_password = <%=Application("mainwindow")%>.document.forms[0].elements["password"].value
	confirm_password = <%=Application("mainwindow")%>.document.forms[0].elements["password_confirmed"].value
...
function doDeleteUser(user_name){
	...
	if(confirm("You are about to completely remove " + user_name + " from Oracle.  Continue?")){
...
function getOtherRoleWindow(myUrl,dbkey,thecaller){
	...
	if (mywindow.name == null){
		mywindow = window.open("/<%=Application("Appkey")%>/cs_security/manage_security/add_existing_role.asp?dbname=" + dbkey,"add_role", "toolbar=0,status=0,menubar=0,scrollbars=0,resizable=0,width=400,height=100")
		mywindow.focus()
	}else{
		mywindow.focus()
	}
