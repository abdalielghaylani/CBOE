...
<% Select Case UCase(action)
	Case "UPDATE_USER"
		...
		<%=Application("mainwindow")%>.location.replace("<%=Session("ListLocation"  & dbkey & formgroup)%>")
		</script>
		<%
		else
...
<%If CBool(Application("ENABLE_ADD_NON_CS_ROLE")) = true then%>	
		<br>
		<a href="javascript:getOtherRoleWindow('/<%=Application("appkey")%>/blank.html','<%=dbkey%>','EDIT_USER')">Add Existing Role To CS Security</a>
<%end if%>	
</td>
</tr>
...
<%end select%>
<script language="javascript">
	<%=Application("nav_bar_window")%>.location.replace("/<%=Application("appkey")%>/navbar.asp?nav_override=true&dbname=<%=dbkey%>&formmode=manage_users")
</script>
</body>
</html>
