				...
				<xsl:choose>
					...
					<xsl:when test="string-length($dataNode/@DISPLAY_NAME) = 0">
						<td>
							...
							<xsl:attribute name="WIDTH"><xsl:value-of select="$dataNode/@WIDTH"/></xsl:attribute>
							<xsl:attribute name="HEIGHT"><xsl:value-of select="$dataNode/@HEIGHT"/></xsl:attribute>
							<xsl:attribute name="CLASS"><xsl:value-of select="$dataNode/@VALUE_CLASS"/><xsl:text> </xsl:text>valueClass<xsl:call-template name="replace-string">
...
<!--									<xsl:value-of select="$dataNode/@DISPLAY_NAME"/> -->
						...
						<td>
							<xsl:attribute name="CLASS">
								<xsl:value-of select="$dataNode/@VALUE_CLASS"/><xsl:text> </xsl:text>valueClass<xsl:call-template name="replace-string">
									<xsl:with-param name="text">
										<xsl:value-of select="$dataNode/@DISPLAY_NAME"/>
									</xsl:with-param>
									<xsl:with-param name="from" select="' '"/>
				...
				<xsl:choose>
					...
					<br/>
							<div>
								...
								<xsl:attribute name="CLASS"><xsl:value-of select="$dataNode/@HEADER_NAME"/> columnNameClass<xsl:call-template name="replace-string">
										<xsl:with-param name="text">
											<xsl:value-of select="$dataNode/@HEADER_NAME"/>
										</xsl:with-param>
										<xsl:with-param name="from" select="' '"/>
