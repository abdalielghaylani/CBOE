CREATE OR REPLACE PROCEDURE "&&SchemaName"."UPDATEHAZMATDATA"
...
BEGIN
  UPDATE INV_EHS_CatNum_Substance 
  SET EHS_Group_1 = pEHSGroup1, 
   ...
   WHERE (supplier_id_fk, supplier_catnum) =
         (SELECT supplier_id_fk, supplier_catnum
          FROM   inv_containers
          WHERE  container_id = pContainerID);
END "UPDATEHAZMATDATA";
/
show errors;
