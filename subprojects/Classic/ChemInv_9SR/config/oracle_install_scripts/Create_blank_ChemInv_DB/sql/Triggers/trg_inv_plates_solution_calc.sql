...
BEGIN
  ...
  --* if solution volume changed don't calculate anything b/c the user is setting the value
	  IF (:NEW.solution_volume = :OLD.solution_volume AND :NEW.solvent_volume <> :OLD.solvent_volume) OR (:NEW.solution_volume IS NULL AND :OLD.solution_volume IS NULL) THEN
    ...
    IF :NEW.solution_volume IS NULL THEN :NEW.solution_volume := 0; END IF;
  	...
  	IF :NEW.qty_remaining > 0 AND :NEW.qty_remaining IS not NULL AND CHEMCALCS.GetUnitType(:NEW.qty_unit_fk) = 1 AND :NEW.solvent_volume > 0 THEN
