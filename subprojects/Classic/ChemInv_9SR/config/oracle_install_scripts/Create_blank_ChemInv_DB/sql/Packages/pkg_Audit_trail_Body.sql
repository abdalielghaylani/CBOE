CREATE OR REPLACE PACKAGE BODY audit_trail IS
  ...
  BEGIN
 		mySQL:=	 'SELECT c.raid, r.rid, old_value, new_value
			FROM audit_column c, audit_row r, (SELECT MAX(ar.raid) AS maxRAID FROM audit_row ar, audit_column ac WHERE ar.raid = ac.raid AND column_name = upper(''' || pColumnName || ''') GROUP BY rid) maxRAID
			WHERE c.raid = r.raid
      AND table_name = upper(''' || pTableName || ''')
			AND column_Name = upper(''' || pColumnName || ''')
			AND r.rid = (SELECT rid from inv_containers where container_ID = ' || pContainerID ||')
