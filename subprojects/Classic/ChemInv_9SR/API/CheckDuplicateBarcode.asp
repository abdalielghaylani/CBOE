...
Select Case lcase(BarcodeType)
	Case "location"
	Case "container"
		SQL = "SELECT barcode as theBarcode FROM inv_containers WHERE barcode in (" & left(barcodeList,len(barcodeList)-1) & ")"
	Case "plate"
		SQL = "SELECT plate_barcode as theBarcode FROM inv_plates WHERE plate_barcode IN (" & left(barcodeList,len(barcodeList)-1) & ")"
End Select
...
duplicates = " "
while not rs.EOF
	duplicates = duplicates & RS("theBarcode") & ","
	RS.MoveNext
wend
