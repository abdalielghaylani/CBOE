					...
					<%if Application("RegServerName") <> "NULL" then%>
		      		<tr>
		        		<td>
							...
							<%ShowInputField dbkey, formgroup, "reg_numbers_alias.reg_number", "0","30"%>
						</td>
		      		</tr>
		      		<%end if%>
		      		<tr>
					...
					For each key in custom_fields_dict
						...
						if inStr(Key, "DATE_") then
							...
							if custom_lists_dict.Exists("CUSTOM_FIELDS." & key) then
								Response.Write "<select name=""inv_containers." & lcase(key) & """>"
								'-- build the select from the custom list
								Response.Write GetCustomListOptions("CUSTOM_FIELDS", key, eval(key), RepeatString(43, "&nbsp;"))
							else
					...
					<tr>
		      		...
		      		<tr>
		        		<td>
							...
							<script language="javascript">
								//document.forms("cows_input_form").ExcludeSpecialLocations.checked = true;
								//alert(document.forms("cows_input_form").ExcludeSpecialLocations.checked);
							</script>
						</td>
		      		</tr>
		      		<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0">
			...
			<td>
				...
				<table border="0">
					...
					<tr>
			        	<td valign="top" align="left" colspan="2">
			        		<input type="hidden" name="inv_wells.well_id">
			...
			<td valign="top">
				...
				<table border="0">
					...
					<%if Application("RegServerName") <> "NULL" then%>
		      		...
		      		<tr>
		        		<td>
							<span title="(e.g. DMSO, Water)">Solvent:</span><br>
			...
			<td valign="top">
				...
				<table border="0">
					<tr>
			        	<td>
							<span title="(Plate Barcode)">Plate Barcode:<%=GetBarcodeIcon()%>&nbsp;</span><br>
							<%ShowInputField dbkey, formgroup, "inv_plates.plate_barcode", "0","30"%>        
						</td>
		      		</tr>
		      		<tr>
		        		<td>
							<span title="Plate Map">Plate Map:<br>
							<%= ShowSelectBox2("inv_plates.plate_map_id_fk", "", "SELECT plate_id AS Value, plate_barcode AS DisplayText FROM inv_plates WHERE is_plate_map = 1 ORDER BY lower(plate_name) ASC", 30, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
					<tr>
			        	<td>
							<span title="Internal Plate ID">Plate ID (internal):&nbsp;</span><br>
			...
			<td valign="top">
				...
				<table border="0">
					...
					<tr>
		      		...
		      		<tr>
		        		<td>
							<span title="Plate Format">Plate Format:<br>
		      		...
		      		<tr>
			        	<td>
							<span title="Plate Library">Library:</span><br>
		      		...
		      		<tr>
		        		<td>
							<span title="Plate Group Name">Group Name:<br>
					...
					For each key in custom_plate_fields_dict
						...
						if inStr(Ucase(Key), "DATE_") then
							...
							if custom_lists_dict.Exists("CUSTOM_PLATE_FIELDS." & key) then
								Response.Write "<select name=""inv_plates." & lcase(key) & """>"
								'-- build the select from the custom list
								Response.Write GetCustomListOptions("CUSTOM_PLATE_FIELDS", key, eval(key), RepeatString(43, "&nbsp;"))
							else
...
<%End Select%>
	...
	<script language="JavaScript">
		...
		// set default exclude special locations
		<%if (Session("ExcludeChecked") = "checked"  or (isEmpty(Session("ExcludeChecked")) and Application("DefaultExcludeSpecialLocations"))) then%>
		if (document.cows_input_form["ExcludeSpecialLocations"]) {
			document.cows_input_form["ExcludeSpecialLocations"].click();	
		}
		<%end if%>
		// set default search sublocations
		<%if ((Session("SearchSubLocations") = "on" or Session("SearchSubLocations") = "checked")  or (isEmpty(Session("SearchSubLocations")) and Application("DefaultSearchSublocations"))) then%>
		if (document.cows_input_form["searchSubLocations"]) {
			document.cows_input_form["searchSubLocations"].click();	
		}
		<%end if%>
