...
SQL = "SELECT w.WELL_ID AS WELL_ID, " & _
		...
		"w.qty_initial AS QTY_INITIAL, " & _
		"w.qty_remaining AS QTY_REMAINING, " & _
		...
		"w.concentration AS CONCENTRATION, " & _
		...
		"w.solvent_volume as SOLVENT_VOLUME , " & _
		"w.solvent_volume_unit_id_fk as SOLVENT_VOLUME_UNIT_ID_FK , " & _
		"w.solution_volume as SOLUTION_VOLUME, " & _
		...
		"DECODE(to_char(w.molar_conc, '0D00EEEE'), NULL, ' ', to_char(w.molar_conc, '0D00EEEE') ||' M') AS MolarConcString , " & _
...
FlatWellSQL = "SELECT w.WELL_ID AS WELL_ID, " & _
		...
		if Application("RegServerName") <> "NULL" then
			FlatWellSQL = FlatWellSQL & "DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number||'-'||batches.batch_Number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK)) AS Reg_Batch_ID, "
			FlatWellSQL = FlatWellSQL & "DECODE(wc.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT Structures.BASE64_CDX AS REG_BASE64_CDX FROM structures, batches, reg_numbers WHERE reg_numbers.reg_id = batches.reg_internal_id AND structures.CPD_Internal_ID = batches.CPD_Internal_ID AND reg_numbers.reg_id=wc.reg_id_fk AND batches.batch_number=wc.Batch_Number_FK)) AS Base64_CDX, "
		else
			FlatWellSQL = FlatWellSQL & "cpd.BASE64_CDX AS BASE64_CDX, " & vblf
	    End if
	    FlatWellSQL = FlatWellSQL & "wc.Reg_ID_FK,  " & vblf & _ 
	    "(SELECT reg_numbers.reg_number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK) as Reg_Number, " & vbcrlf & _
...
WellCompoundSQL = "SELECT " & _
		...
		if Application("RegServerName") <> "NULL" then
			WellCompoundSQL = WellCompoundSQL & "DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number||'-'||batches.batch_Number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK)) AS Reg_Batch_ID, "
			WellCompoundSQL = WellCompoundSQL & "DECODE(wc.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT Structures.BASE64_CDX AS REG_BASE64_CDX FROM structures, batches, reg_numbers WHERE reg_numbers.reg_id = batches.reg_internal_id AND structures.CPD_Internal_ID = batches.CPD_Internal_ID AND reg_numbers.reg_id=wc.reg_id_fk AND batches.batch_number=wc.Batch_Number_FK)) AS Base64_CDX, "
		    WellCompoundSQL = WellCompoundSQL & "(SELECT reg_numbers.reg_number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK) as Reg_Number, " & vblf
		else
...
WellCompoundSQL2 = "SELECT " & _
		...
		"cpd.ACX_ID AS ACX_ID, " & vblf & vblf
		if Application("RegServerName") <> "NULL" then
			WellCompoundSQL2 = WellCompoundSQL2 & "DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number||'-'||batches.batch_Number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK)) AS Reg_Batch_ID, "
	    End if
