...
<%		
	...
	'countSQL = "SELECT count(*) as ParentCount FROM inv_well_parent WHERE child_well_id_fk = ?"
	...
	'if cint(CompoundCount) > 1 then
	if cint(ParentCount) > 1 then
	...
	if cint(ParentCount) > 1 then
		...
		compoundSQL =  "SELECT DECODE(wc.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT Structures.BASE64_CDX AS REG_BASE64_CDX FROM structures, batches, reg_numbers WHERE reg_numbers.reg_id = batches.reg_internal_id AND structures.CPD_Internal_ID = batches.CPD_Internal_ID AND reg_numbers.reg_id=wc.reg_id_fk AND batches.batch_number=wc.Batch_Number_FK)) AS Base64_CDX," & _
						...
						if Application("RegServerName") <> "NULL" then
							compoundSQL = compoundSQL & ", DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number||'-'||batches.batch_Number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK)) AS Reg_Batch_ID "
							compoundSQL = compoundSQL & ", DECODE(wc.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= wc.reg_id_fk AND batches.batch_number= wc.Batch_Number_FK)) AS Reg_Number "
						End if
