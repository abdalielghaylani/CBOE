...
if app = "inv" then
	sql =	"SELECT v.raid RAID," & _
			...
			" TO_CHAR(timestamp, '" & dateFormatString & " HH24:MI:SS') timestamp" &_
	...
	if containerBarcode <> "" then
			sql = sql & " AND v.rid = (SELECT rid from inv_containers where barcode ='" & containerBarcode & "')"
	End if
	if locationBarcode <> "" then
			sql = sql & " AND v.rid = (SELECT rid from inv_locations where location_barcode ='" & locationBarcode & "')"
	End if
	if compoundID <> "" then
			sql = sql & " AND v.rid = (SELECT rid from inv_compounds where compound_id ='" & compoundID & "')"
	End if
	if act = "'U'" then	
