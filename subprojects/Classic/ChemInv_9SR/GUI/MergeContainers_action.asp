...
'update aggregated container's qty remaining
QueryString = "ContainerIDs=" & Request("ContainerID") & "&ValuePairs=" & "Qty_Remaining%3D" & Request("NewQtyRemaining") & "::Qty_Available%3D" & Request("NewQtyRemaining")
...
'updated merged containers qty remaining = 0
QueryString = "ContainerIDs=" & Request("MergedConatinerIDs") & "&ValuePairs=" & "Qty_Remaining%3D0"
QueryString = QueryString & Credentials
httpResponse = CShttpRequest2("POST", ServerName, "/cheminv/api/UpdateContainer2.asp", "ChemInv", QueryString)
out2 = httpResponse

'delete merged containers
FormData = "ContainerID=" & Request("MergeContainerIDs") & Credentials
...
<html>
<head>
<title><%=Application("appTitle")%> -- Merge Containers</title>
		...
		<TD HEIGHT=50 VALIGN=MIDDLE align=center>
			...
			If isNumeric(httpResponse) then
				If CLng(httpResponse) > 0 then
					...
					Response.Write "<center><SPAN class=""GuiFeedback"">Containers have been merged.</SPAN><center>"
					if Request("multiscan") = "1" then
						Response.Write "<SCRIPT language=JavaScript>opener.location.href='multiscan_list.asp?clear=1&message=" & Ccount & " Containers have been merged.'; opener.focus(); window.close();</SCRIPT>"				
					Else
					...
					Response.Write "<center><SPAN class=""GuiFeedback"">Containers could not be merged</SPAN></center>"
