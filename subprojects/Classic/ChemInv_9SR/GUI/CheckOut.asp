...
Select case Lcase(action)
	case "in"
		...
		sql = "SELECT inv_Containers.Container_ID, inv_containers.Def_Location_ID_FK, inv_Locations.Location_Name AS DefLocationName, inv_locations.Owner_ID_FK AS LocationOwner, inv_Containers.Owner_ID_FK AS OwnerID FROM inv_Containers, inv_Locations WHERE inv_containers.Def_Location_ID_FK = inv_Locations.Location_ID AND inv_containers.Container_ID in (" & ContainerID & ")"
		'Response.Write sql
		'Response.End
		...
		isFirstRec = false
		While NOT RS.EOF
			defLocations = defLocations & "<tr><td>" & RS("Container_ID") & "</td><td>" & RS("DefLocationName") & "</td></tr>"
	...
	case "out"
		...
		'-- if this is a multiselect flag the function to set the default location equal to the current location
		if Lcase(Request("multiSelect")) = "true" then 
			DefaultLocationID = "-1"
		else			
...
<table border="0">
	...
	<tr>
		...
		<td>
			<input TYPE="tetx" SIZE="44" Maxlength="50" VALUE="<%=ContainerBarcode%>" onfocus="blur()" disabled id="Text1" name="Text1">
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<input type="checkbox" name="UseDefaultLocation">&nbsp; Return containers to their default location.<br />
			<table border="1" cellpadding="2" cellspacing="1" style="border-collapse: collapse">
			<tr><td bgcolor="#cccccc">ID</td><td bgcolor="#cccccc">Default Location</td></tr>
			<%=defLocations%>
			</table>
		</td>
	</tr>
