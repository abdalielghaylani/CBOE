...
if isEmpty(WellList) then 
	WellList = Session("WellList")
else
	Session("WellList") = WellList
end if
...
'-- start GetSortField adapted for wells
SortByFieldName = Request("SortByFieldName")
SortDirection = Session("LastSortDirection")
	
if SortByFieldName = "" then
	SortByFieldName = Session("LastSortByFieldNameWell")
End if
if SortByFieldName = "" then 
	SortByFieldName = "WELL_ID"
	Session("LastSortByFieldNameWell") = SortByFieldName
	SortDirection = 1
	Session("LastSortDirection") = 1
End if
	
if Request("SortByFieldName") = Session("LastSortByFieldNameWell") then
...
If NOT IsArray(Session("WellReportFieldArray3")) then
	ColDefStr = GetUserProperty(Session("UserNameCheminv"),"WellChemInvFA3")
	If ColDefStr = "" OR IsNull(ColDefStr) then 
		' Default column definition
		ColDefstr= Application("WellReportColDef3")
		rc= WriteUserProperty(Session("UserNameCheminv"), "WellChemInvFA3", ColDefstr)
	End if
	fieldArray = GetFieldArray(colDefstr, Application("WellFieldMap"))
	Session("WellReportFieldArray3") = fieldArray
Else
	...
	<SCRIPT LANGUAGE=javascript src="/cheminv/gui/refreshGUI.js"></SCRIPT>
		...
		<SCRIPT LANGUAGE=JAVASCRIPT>
			...
			function ParentWellLink(LocationID, TreeViewOpenNodes1, PlateID, WellID) {
				//alert(event.ctrlKey);
				if (event.ctrlKey) {
					SelectLocationNode(0, LocationID, 0,TreeViewOpenNodes1, PlateID, 1, WellID);	
					window.close();
				}
				else {
					OpenDialog('/cheminv/gui/ViewWell.asp?wellID=' + WellID + '&refresh=true', 'ParentDiag', 5);
				}
			}
		</SCRIPT>
</HEAD>
	...
	<TABLE cellspacing=0 cellpadding=0>	
		<TR>
			...
			<td align="right" nowrap>
				<a class="MenuLink" href="#" title="Click to choose report columns" onclick="OpenDialog('../cheminv/columnPicker2.asp?ArrayID=3&showWells=true', 'CCDiag', 4); return false">Column Chooser</a>
					|
		...
		<TR>
			<td align="left" colspan="2" style="font-size:10px">*Sorts within column only</td>
		</tr>
	</table>	
<% 
...
<%		
	...
	'-- sort
	if lcase(SortByFieldName) <> "reg_batch_id" and lcase(SortByFieldName) <> "compound_id_fk" then
	...
	if lcase(SortByFieldName) = "reg_batch_id" or lcase(SortByFieldName) = "compound_id_fk" then
		WellCompoundSQL = WellCompoundSQL & " ORDER BY Upper(" & SortbyFieldName & ") " & SortDirectionTxt
	end if
	...
	if NOT (RS.BOF AND RS.EOF) then 
		RS.movefirst
		'Response.Write "<BR><BR><BR><BR><BR><BR><BR><BR>" & mypageSize
		...
		For i=0 to Ubound(fieldArray)
			...
			select case lcase(fn)
				...
				case "reg_batch_id"
					...
					if rsCompound.EOF and not rsCompound.BOF then rsCompound.MoveFirst
					While not rsCompound.EOF 
						regBatchIDText = regBatchIDText & rsCompound("reg_batch_id") & "<BR>"
						rsCompound.movenext
					Wend
...
function SortReport(fieldName){
	...
	url = "BuildWellList.asp?<%=QS%>&SortByFieldName=" + fieldName;
