...
<% end if%>
		...
		if (document.form1.RegBatchID.value.length == 0) {
			if (document.form1.iCompoundID.value.length >0 && (!isPositiveNumber(document.form1.iCompoundID.value) || document.form1.iCompoundID.value < 1)){
...
<INPUT TYPE="hidden" NAME="iBatchNumber">
...
%>
		...
		<%if cint(Session("wCompoundCount")) <= 1 then %>
			...
			<td>
				<%if Application("RegServerName") <> "NULL" then%>
					<input type="text" Size="15" name="iCompoundID" value="<%=Compound_ID_FK%>" onpropertychange="iCompound_ID_FK.value=this.value;NewCompoundID.value= this.value; RegBatchID.value=''; iReg_ID_FK.value=''; iBatch_Number_FK.value='';">
				<%else%>
					<input type="text" Size="15" name="iCompoundID" value="<%=Compound_ID_FK%>" onpropertychange="iCompound_ID_FK.value=this.value;NewCompoundID.value= this.value;">
				<%End if%>
			</td>
		<%else%>
			<td></td>
	...
	<tr height="25">
		...
		<%if cint(Session("wCompoundCount")) <= 1 then %>
			...
			<td>
				<input type="text" Size="15" name="RegBatchID" value="<%=Session("wReg_Batch_ID")%>" onchange="GetRegIDFromRegNum(this.value); iCompoundID.value=''; NewCompoundID.value='';">
	...
	For each key in custom_well_fields_dict
		...
		Else
			if custom_lists_dict.Exists("CUSTOM_WELL_FIELDS." & key) then
				Response.Write "<td align=right valign=top nowrap width=""150"">" 
				if req_custom_well_fields_dict.Exists(Key) then
					Response.Write "<span class=""required"">" & custom_well_fields_dict.Item(key) & ":</span></td>"
				else
					Response.Write custom_well_fields_dict.Item(key) & ":</td>"
				end if
				
				Response.Write "<td><select name=""i" & key & """>"

				'-- build the select from the custom list
				Response.Write GetCustomListOptions("CUSTOM_WELL_FIELDS", key, eval(key), null)
			...
			<table border="1">
				<tr>
					...
					<td>
						...
						Base64DecodeDirect "invreg", "base_form_group", Session("wRegBase64"), "Structures.BASE64_CDX", Session("wcpdID"), Session("wcpdID"), specifier, 130
						%>
					</td>
				</tr>
			</table>
