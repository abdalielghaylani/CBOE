...
SQL =	"SELECT DISTINCT v.WELL_ID, " & _
		"		REG_ID_FK, " & _
		"		BATCH_NUMBER_FK, " & _
		"		Decode(REG_ID_FK,NULL,NULL,reg_numbers.reg_number || '-' || batch_number_FK) as RegBatchID, " & _
		...
		"FROM	INV_VW_WELL v, regdb.reg_numbers , inv_compounds, regdb.compound_molecule " & _
		"		, regdb.structures " & _
		"		, INV_UNITS UOM " & _
		"		, INV_UNITS UOC " & _
		"		, INV_UNITS UOW " & _
		"WHERE reg_id_fk = reg_numbers.reg_id (+) " & _
		"		AND	reg_numbers.cpd_internal_id = structures.cpd_internal_ID (+) " & _	
		"		AND	reg_numbers.mol_id = compound_molecule.mol_id (+) " & _	
...
'		"		cscartridge.molweight(DECODE(v.COMPOUND_ID_FK,NULL,structures.base64_cdx,inv_compounds.base64_cdx)) as mw " & _
	...
	SQL =	"SELECT DISTINCT v.WELL_ID, " & _
		...
		"FROM	INV_VW_WELL v, inv_compounds " & _
		"		, INV_UNITS UOM " & _
		"		, INV_UNITS UOC " & _
		"		, INV_UNITS UOW " & _
...
Response.Write "<DIV ID=""waitGIF"" ALIGN=""center""><img src=""/cfserverasp/source/graphics/processing_Ybvl_Ysh_grey.gif"" WIDTH=""130"" HEIGHT=""100"" BORDER=""""></DIV>"
Response.Flush
'Response.Write SQL
'Response.End
...
Function WrapWellContents(FldName, wellID, strText, Length, isSelected)
	...
	if (len(strText) > Length) AND (strText <> "&nbsp;") then 
		...
		str = str & "<a href=""ViewWellFrame.asp?wellID=" & wellID &  "&filter=" & FldName & """ target=""wellJSFrame"">" & left(strText, Length)& "</a>"
	else
		if isSelected then
			str = str & "><a style=""color:red;"" href=""ViewWellFrame.asp?wellID=" & wellID &  "&filter=" & FldName & """ target=""wellJSFrame"" title=""click to view well details"">" & strText & "</a>"
		else
			str = str & "><a href=""ViewWellFrame.asp?wellID=" & wellID &  "&filter=" & FldName & """ target=""wellJSFrame"" title=""click to view well details"">" & strText & "</a>"
		end if
