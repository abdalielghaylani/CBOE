...
For each field in Request.Form
	if InStr("iPlateIDs,multiscan,multiSelect,fieldList", field) = 0 then
		Select case lcase(field)
			case "ift_cycles","istatus_id_fk","ilibrary_id_fk","iconcentration","iconc_unit_fk","iqty_initial","iqty_remaining","iqty_unit_fk","iweight","iweight_unit_fk","isolvent_id_fk","isolvent_volume_unit_id_fk","iplate_type_id_fk","isolution_volume","isolvent_volume"
				ValuePairs = ValuePairs & mid(field,2) & "%3D" & Request(field) & "::"
			case "ift_cycles_increment"
				ValuePairs = ValuePairs & "ft_cycles%3Ddecode(ft_cycles,NULL,0" & server.URLEncode(Request(field)) & ",ft_cycles" & server.URLEncode(Request(field)) & ")::"	
