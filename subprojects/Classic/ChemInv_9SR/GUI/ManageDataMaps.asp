...
if mapAction = "update" then sql_mapNames = sql_mapNames & " WHERE data_map_id <> " & dataMapID
...
<html>
<head>
<title><%=Application("appTitle")%> Manage Date Maps</title>
...
<!--
	...
	function Validate(){
		...
		var varMapName = "<%=replace(mapNames,"""","\""")%>";
		var currTemplateName = "<%=TemplateName%>";
		...
		if (document.form1.TemplateName.value.length == 0) {
			...
			if (varMapName.indexOf(document.form1.TemplateName.value) > 0 && document.form1.TemplateName.value != currTemplateName){
		...
		if (document.form1.stepValue.value == 2) {
			...
			next
			%>
			// source well is required
			if (lstColMapValues.indexOf(',[2]') == -1){
				errmsg = errmsg + "- Source Well Column Mapping is required.\rPlease add Source Well as a Column Mapping.\r\r";
				bWriteError = true;
			}
			// if target barcode is mapped target well is required
			if (lstColMapValues.indexOf(',[3]') > -1){
				if (lstColMapValues.indexOf(',[4]') == -1){
					errmsg = errmsg + "- Target Well Column Mapping is required if Target Plate Barcode is mapped.\rPlease add Target Well as a Column Mapping.\r\r";
					bWriteError = true;
				}
			}
			// if target well is mapped target barcode is required
			if (lstColMapValues.indexOf(',[4]') > -1){
				if (lstColMapValues.indexOf(',[3]') == -1){
					errmsg = errmsg + "- Target Plate Barcode Column Mapping is required if Target Well is mapped.\rPlease add Target Plate Barcode as a Column Mapping.\r\r";
					bWriteError = true;
				}
			}
			// if concentration is mapped concentration unit id is required
			if (lstColMapValues.indexOf(',[7]') > -1){
				if (lstColMapValues.indexOf(',[8]') == -1){
					errmsg = errmsg + "- Concentration Unit ID Column Mapping is required if Concentration is mapped.\rPlease add Concentration Unit ID as a Column Mapping.\r\r";
					bWriteError = true;
				}
			}
			// if quantity is mapped quantity unit id is required
			if (lstColMapValues.indexOf(',[5]') > -1){
				if (lstColMapValues.indexOf(',[6]') == -1){
					errmsg = errmsg + "- Quantity Unit ID Column Mapping is required if Initial Quantity is mapped.\rPlease add Quantity Unit ID as a Column Mapping.\r\r";
					bWriteError = true;
				}
			}
		}
		...
		if (document.form1.TemplateName.value.length == 0) {
			...
			//varMapName = document.form1.mapNames.value;
			//if (varMapName.indexOf(document.form1.TemplateName.value) > 0 && document.form1.mapAction.value != 'update'){
			if (varMapName.indexOf(document.form1.TemplateName.value) > 0){
				errmsg = errmsg + "- Template Name already exists. Please choose a new name.\r";
				bWriteError = true;
			}
		}
...
<input type="hidden" name="mapNames" value="<%=replace(mapNames,"""","\""")%>" />
<%
if mapAction = "update" then
...
<table border="0">
	...
	' ----------------------
	if stepValue = "2" then
	sql_mappings = "Select map_field_id as Value, display_name as DisplayText from INV_DATA_MAPPINGS IDM, INV_MAP_FIELDS IMF Where IMF.MAP_FIELD_ID=IDM.MAP_FIELD_ID_FK AND DATA_MAP_ID_FK = " & dataMapID & " And rownum < " & (CInt(NumDataCol)+1) & " Order by COLUMN_NUMBER"
	GetInvConnection()
...
<table border="0">
	<tr><td colspan="2" align="center">
		<span class="GuiFeedback">Please select a new name for your copy of <%=origName%>.</span><br /><br />
	</td></tr>
