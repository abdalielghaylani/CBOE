...
<body>
	...
	<tr>
		...
		<th align="center">
			<a Class="MenuLink" Href="#" onclick="OpenDialog('/Cheminv/GUI/Links.asp?fk_value=<%=fk_value%>&amp;fk_name=<%=fk_name%>&amp;Table_Name=<%=table_name%>&amp;URLtype=<%=URLType%>', 'Diag', 1); return false">New</a>
		</td>
	</tr>
<%
	...
	Else
		While (Not RS.EOF)
			...
			if isNull(Image_source) then Image_source = ""
			FK_value = RS("FK_value")
			FK_name = RS("FK_name")
			Table_Name = RS("Table_Name")
			URLType = RS("URL_Type")
			editData = "URLID=" & URLID & "&URLhref=" & server.URLEncode(URLhref) & "&Link_text=" & Link_Text & "&URLType=" & URLType & "&Image_source=" & server.URLEncode(Image_source) & "&FK_value=" & FK_value & "&FK_name=" & FK_name & "&Table_Name=" & Table_Name
%>
			<tr>
				<td align="center">
					<%=Table_Name%>
				...
				<td align="center">
					<a Class="MenuLink" Href="#" onclick="OpenDialog('/Cheminv/GUI/Links.asp?<%=editData%>', 'Diag', 1); return false">&nbsp;Edit</a>
					|
					<a Class="MenuLink" Href="#" onclick="OpenDialog('/Cheminv/GUI/Links.asp?action=delete&amp;<%=editData%>', 'Diag',1); return false">Delete&nbsp;</a>
				</td>
			</tr>
			<%rs.MoveNext
		Wend
	...
	Set Conn = nothing		
%>
<br><a HREF="#" onclick="opener.location.reload(); window.close(); return false;"><img SRC="../graphics/ok_dialog_btn.gif" border="0" WIDTH="61" HEIGHT="21"></a>
</center>
</body>
</html>
