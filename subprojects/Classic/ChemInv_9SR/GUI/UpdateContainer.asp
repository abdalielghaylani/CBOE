...
if Lcase(multiSelect) = "true" then 
	...
	if ContainerCount = 0 then 	
		...
		ContainerName =  multiSelect_dict.count & " containers will be deleted"
	end if
Else
	ContainerCount = 1
...
if ContainerCount > 0 then 
	...
	GetInvConnection()
	SQL = "SELECT min(qty_max) minQtyMax, max(qty_remaining) as maxQtyRemaining, min(" & Application("CHEMINV_USERNAME") & ".RESERVATIONS.GETTOTALQTYRESERVED(container_id)) as TotalQtyReserved FROM inv_containers WHERE container_id in (" & ContainerID & ")"
		...
		if (strMode.toLowerCase() == "edit") {
			...
			for (i=0; i<arrFields.length; i++) {
				...
				switch (arrFields[i]) {
					case "compound_id_fk":
						// if present
						if (document.form1.icompound_id_fk.value.length >0){
							if(IsValidCompoundID(document.form1.icompound_id_fk.value, false)==0){
								errmsg = errmsg + "- Invalid Compound ID.\r";
								bWriteError = true;
							}
						}
						break;
					case "location_id_fk":
						...
						if (document.form1.location_id_fk.value.length >0){
							...
							else if (!isPositiveNumber(document.form1.location_id_fk.value)){
	...
	<tr>
		...
		<td>		
			<SELECT NAME="containerFields" SIZE="8" MULTIPLE>
				...
				<OPTION VALUE="Quantity Available:qty_available">Quantity Available</OPTION>
	...
	if right(fieldName,2) = "fk" then
		Select case fieldName
			...
			Case "Compound:compound_id_fk"
				...
				theRow = theRow & "<input type=""text"" Size=""15"" name=""iCompoundID"" onpropertychange=""icompound_id_fk.value= this.value;""><input Type=""hidden"" name=""icompound_id_fk""><input Type=""hidden"" name=""iContainerName""><input Type=""hidden"" name=""RegID""><input Type=""hidden"" name=""BatchNumber""><input type=""hidden"" name=""NewCompoundID""></TD></TR>"
		...
		Select case fieldName
			Case "Comments:container_comments"
				...
				theRow = "<TD VALIGN=""top""><TEXTAREA ROWS=""8"" COLS=""60"" NAME=""icontainer_comments"" WRAP=""hard""></TEXTAREA></TD></TR>"
				Response.Write theRow
			Case else
...
<%sub CreateTextFieldRow(field)
	...
	Select Case lcase(fieldColumn)
		...
		case "purity"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("iunit_of_purity_id_fk", Session("UOPIDOptionValue"),"SELECT UNIT_ID AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 3 ORDER BY Unit_Name ASC"), False, False)			
		case "concentration"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("iunit_of_conc_id_fk", Session("UOCIDOptionValue"),"SELECT UNIT_ID AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 3 ORDER BY Unit_Name ASC"), False, False)
		case "density"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("iunit_of_density_id_fk", Session("UODIDOptionValue"),"SELECT UNIT_ID AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 6 ORDER BY Unit_Name ASC"), False, False)
		case "container_cost"
			theRow = ShowInputBox(fieldName, fieldColumn, 15, ShowSelectBox("iunit_of_cost_id_fk", Session("UOCostIDOptionValue"),"SELECT UNIT_ID AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 5 ORDER BY Unit_Name ASC"), False, False)
		case else
...
sub CreateSelectFieldRow(field)
	...
	Select Case fieldColumn
		...
		Case "physical_state_id_fk"
			sql = "SELECT physical_state_id AS Value, physical_state_name AS DisplayText FROM inv_physical_state ORDER BY lower(DisplayText) ASC"	
