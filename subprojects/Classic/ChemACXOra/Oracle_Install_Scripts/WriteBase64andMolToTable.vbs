'//////////////////////////////////////////////////////////////////////
'/ 
'/  Name:            WriteBase64andMolToTable.vbs
...
'ConStr = "FILE NAME=c:\inetpub\wwwroot2\chemacx\config\chemacx.udl;user id=Admin;Pasword="
ConStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=c:\chemoffice_data\chemacx\chemacx.mdb;user id=Admin;Password=;"
...
sLogFileName = "c:\WriteBase64andMolToTable_log.txt"
...
Set cdax = CreateObject("ChemDrawControl9.ChemDrawCtl")
...
conn.Open ConStr
Set RS = CreateObject("ADODB.Recordset")  
Set SMsDoc = CreateObject("MolServer8.Document")
...
Function GetMolFileFromB64(byRef b64)
	cdax.DataEncoded = true
	cdax.Objects.Data("chemical/x-cdx") = b64
	cdax.DataEncoded = false
	GetMolFileFromB64 = cdax.Data("chemical/x-mdl-molfile")
	cdax.Objects.clear
End function
...
Sub GetMolesFromMst()
	stop
	sql = "SELECT " & PkFieldName & ", " & b64FieldName & " ,mol FROM " & TableName & " WHERE " & PkFieldName & " >0 AND "  & b64FieldName & " is null order by mol_id asc"	
	'msgbox sql
	RS.Open sql, conn, 2, 3, 1
	If Err.Number Then
		Tracelogerror ERRORMSG, "WriteBase64andMolToTable", "Error 0x" & CStr(Hex(Err.Number)) & err.description & " SQL=" & sql
		Err.clear
	Else
		If Not (RS.EOF And RS.BOF) Then
			...
			if Not IsNull(sBase64) then 
				themol = GetMolFileFromB64(sBase64)
				themol = replace(themol, vbcrlf, "\n")
			Else 
				themol = null
			End if
			RS("mol").value = themol 
        		RS.Update
        		Tracelogerror INFOMSG, "WriteBase64andMolToTable", "Updated Structure with " & PkFieldName & "= " & pKValue 
			i = i + 1
 ...
 For Each File In ColFiles
	...
	fullFileName = FolderPath & fileName & FileExtension
        ...
        If oCDApp.Documents.Count = 1 Then
            ...
            oCDDoc.Close
			WriteBase64andMolToTable FolderPath & "temp.cdx", fileName 
			If Err.Number Then
...
Sub WriteBase64andMolToTable(PathtoMolecule, PkValue)
	sBase64 = oDecoder.Encode(PathtoMolecule)
	sql = "SELECT " & PkFieldName & ", " & b64FieldName & " , molFile FROM " & TableName & " WHERE " & PkFieldName & " =" & PkValue	
	Tracelogerror VERBOSEMSG, "WriteBase64andMolToTable", sql
        on error resume next		
	RS.Open sql, conn, 2, 3, 1
	If Err.Number Then
		Tracelogerror ERRORMSG, "WriteBase64andMolToTable", "Error 0x" & CStr(Hex(Err.Number)) & err.description & " SQL=" & sql
	...
	Else
		If Not (RS.EOF And RS.BOF) Then
			...
			RS("molFile") = GetMolFileFromB64(sBase64)
        		RS.Update
        		Tracelogerror INFOMSG, "WriteBase64andMolToTable", "Updated Structure with " & PkFieldName & "= " & pKValue 
			i = i + 1    
		Else
			Tracelogerror ERRORMSG, "WriteBase64andMolToTable", "No match found for " & PkFieldName & "= " & pKValue 
    		End If
    		RS.Close
	End If
End Sub
