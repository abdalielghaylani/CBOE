...
if return ="acxframeset" then
	...
	SQLQuery_1 = "SELECT Package.ProductID AS ProductID, Package.PackageID AS PackageID, Product.SupplierID AS SupplierID, Supplier.ShortName AS ShortName, Product.CatalogNum AS CatalogNum, Package.Catalog2Num AS Catalog2Num, Product.ProdName AS ProdName, Product.ProdDescrip AS ProdDescrip," & " Package." & Application("ProductSizeFieldName" & dbkey) & ", Package." & Application("Pricefield") &  " AS Price"
	SQLQUERY_2 = ", Package." & Application("LISTPRICEfield") &  " AS ListPrice"
	'SQLQuery_3 = " FROM Package, Product, Supplier WHERE Package.ProductID = Product.ProductID AND Product.SupplierID = Supplier.SupplierID AND Product.CSNum=" & strucid & " AND Supplier." & Application("tabWhereField") & " IN (" & Application("tab1WhereValue") & "," & Application("tab3WhereValue") & ")"
	SQLQuery_3 = " FROM Package, Product, Supplier WHERE Package.ProductID = Product.ProductID AND Product.SupplierID = Supplier.SupplierID AND Product.CSNum=? AND Supplier." & Application("tabWhereField") & " IN (" & BuildInClause(Application("tab1WhereValue") & "," & Application("tab3WhereValue")) & ")"
...
sqlquery_Parameters = "CsNum" & "|" & adInteger & "|" & adParamInput & "|" & "|" & strucid & ";" & _
									"InClause" & "|" & adVarChar & "|" & adParamInput & "|" & "|" & Application("tab1WhereValue") & "," & Application("tab3WhereValue") 'Name|Type|Direction|Size|Value

	'Response.Write sqlquery
	'Response.end
	'set skuRS = Server.CreateObject("ADODB.Recordset")
	set skuRS = GetRecordSet(sqlquery, sqlquery_parameters)
		'skuRs.ActiveConnection = AdoConn
		'skuRS.CursorType = 3
		'skuRS.Source = SQLQuery
		'skuRs.open
		'skuCount = skuRs.RecordCount
	'Response.Write skuCount
	'Response.end
	if return = "wddx" then
