...
<Worksheet ss:Name="CSXL_Settings">
  <Table ss:ExpandedColumnCount="2" ss:ExpandedRowCount="4" x:FullColumns="1"
   x:FullRows="1">
   <Row>
    <Cell><Data ss:Type="String">MajorVersion_ChemDraw</Data></Cell>
    <Cell><Data ss:Type="Number">9</Data></Cell>
   </Row>
   <Row>
    <Cell><Data ss:Type="String">MinorVersion_ChemDraw</Data></Cell>
    <Cell><Data ss:Type="Number">0</Data></Cell>
   </Row>
   <Row>
    <Cell><Data ss:Type="String">BugfixVersion_ChemDraw</Data></Cell>
    <Cell><Data ss:Type="Number">0</Data></Cell>
   </Row>
   <Row>
    <Cell><Data ss:Type="String">StyleSheetPath</Data></Cell>
   </Row>
  </Table>
		...
		<xsl:choose>
				...
				<xsl:for-each select="s:AttributeType">
					...
					<xsl:if test="(contains('MOL_ID,INDEXCOUNTER,PAGEROWNUM,ROWID',@name) = false) and (count(@rs:name[contains('ROWID',.)])=0)">
						...
						<Column ssAutoFitWidth="1" ss:Width="120"/>
					...
					<!--filter unwanted columns-->
						<xsl:if test="(string-length(@rs:name)>0)">
							<Column ssAutoFitWidth="1" ss:Width="150"/>
						</xsl:if>
					</xsl:for-each>
 ...
 <xsl:template match="s:ElementType" mode="groupings">
             <xsl:choose>
                    ...
                    <xsl:otherwise>
                           <Cell ss:StyleID="s120">
				 <xsl:if test="count(child::s:AttributeType[string-length(@rs:name)>0])=0">
                                 <xsl:attribute name="ss:MergeAcross"><xsl:value-of select="count(child::s:AttributeType[string-length(@rs:name)>0])"/></xsl:attribute>
				</xsl:if> 
				<xsl:if test="count(child::s:AttributeType[string-length(@rs:name)>0])>0">
	...
	<xsl:template match="rs:data/z:row">
		<Row AutoFitHeight="1">
			...
			<!-- now, if there's any child rows of this row, process them -->
			<xsl:call-template name="ProcessSubSubrows">
				<xsl:with-param name="currRow" select="."/>
			</xsl:call-template>
		</Row>
	</xsl:template>
	...
	<xsl:if test="(string-length(@rs:name)>0)">
				...
				<Cell ss:StyleID="s23">
					<Data ss:Type="String">
						<xsl:for-each select="$currRow/*[name()=$currSubtable]">
							<xsl:variable name="grandChildCount">
								<!-- assumes only 1 level down, ie, no great-grandchildren -->
							...
							<xsl:value-of select="./@*[name()=$currAttribute]"/>
								...
								<!--add blank lines for each grandchild record -->
								<xsl:for-each select="./*[position()!=$grandChildCount]">
										<xsl:text disable-output-escaping="yes"><![CDATA[&#10;]]></xsl:text>
		...
		<!-- try processing subtables with call-template -->
	<xsl:template name="ProcessSubSubrows">
		<xsl:param name="currRow"/>
		<xsl:for-each select="//s:Schema/s:ElementType/s:ElementType/s:ElementType">
			<xsl:variable name="currSubtable">
				<xsl:value-of select="@name"/>
			</xsl:variable>
	...
	<xsl:if test="(string-length(@rs:name)>0)">
				...
				<Cell ss:StyleID="s23">
					<Data ss:Type="String">
						<xsl:for-each select="$currRow/descendant::*[name()=$currSubtable]">
							<xsl:value-of select="./@*[name()=$currAttribute]"/>
								<xsl:if test="not (position()=last())">
										<xsl:text disable-output-escaping="yes"><![CDATA[&#10;]]></xsl:text>
								</xsl:if>
