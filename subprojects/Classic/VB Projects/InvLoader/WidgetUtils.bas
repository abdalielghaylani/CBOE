...
Public Sub WizStopNext(ByRef oWiz As ActiveWizard, ByVal OldPane As Integer, ByVal ErrMsg As String)
    ...
    DoEvents
    'CSBR 148452 SJ Displaying a valid message to the user in the case of Invalid username\password
    If InStr(LCase$(ErrMsg), "ora-01017") Then
        ErrMsg = "Invalid username or password. Please contact your system administrator."
    End If
    If InStr(LCase$(ErrMsg), "userid") Then
        ErrMsg = "UserID is a required parameter "
    End If
    If InStr(LCase$(ErrMsg), "password is a required parameter") Then
        ErrMsg = "Password is a required parameter "
    End If
...
' Convert dictionary whose keys are the readable text and values are meaningless keys aot a VSFlex picklist string
'Public Function PicklistFromDict(oDict As Dictionary, Optional ByVal bCombo As Boolean = False, Optional ByVal bClearable As Boolean = False) As String
'    Dim key As Variant
...
'    End If
'    If bClearable Then
'        PicklistFromDict = PicklistFromDict & ";|"
'    End If
'    bFirst = True
...
'            PicklistFromDict = PicklistFromDict & "|"
'        End If
'        PicklistFromDict = PicklistFromDict & "#" & oDict(key) & ";" & key
'    Next key
'End Function

' Convert dictionary whose keys are the readable text and values are meaningless keys aot a VSFlex picklist string
Public Function PicklistFromDictKeys(oDict As Dictionary, Optional ByVal bCombo As Boolean = False, Optional ByVal bClearable As Boolean = False) As String
    Dim key As Variant
    Dim bFirst As Boolean
    
    If bCombo Then
        PicklistFromDictKeys = "|"
    Else
        PicklistFromDictKeys = ""
    End If
    If bClearable Then
        PicklistFromDictKeys = PicklistFromDictKeys & ";|"
    End If
    bFirst = True
    For Each key In oDict
        If bFirst Then
        ...
        Else
            PicklistFromDictKeys = PicklistFromDictKeys & "|"
        End If
        PicklistFromDictKeys = PicklistFromDictKeys & key
    Next key
End Function

Function FlipDict(oDict As Dictionary) As Dictionary
    Dim key As Variant
    Set FlipDict = New Dictionary
    For Each key In oDict
        FlipDict.Add oDict(key), key
    Next key
End Function
...
Public Function DictFromRegPicklistXML(ByVal sXML As String) As Dictionary
    ...
    If oDoc.parseError.errorCode <> 0 Then
        ...
        Exit Function
    End If
    Set DictFromRegPicklistXML = New Dictionary
    If oDoc.documentElement.Text <> "" Then
        Set oDocNode = oDoc.documentElement
        If oDocNode Is Nothing Then
            Exit Function
        ...
        For Each oNode In oNodeList
            sEntry = oNode.Text
            sID = oNode.getAttribute("ID")
            If Not DictFromRegPicklistXML.Exists(sEntry) Then
                DictFromRegPicklistXML.Add sEntry, sID
            End If
        Next oNode
    End If
End Function
