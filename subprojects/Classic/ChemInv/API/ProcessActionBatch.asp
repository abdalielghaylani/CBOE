...
For Each actionNode In root.SelectNodes("CREATESUBSTANCE")
	...
	FormData = FormData & "&inv_compounds.substance_name=" &  Server.URLEncode(substanceName)
	...
	For Each reqAttrib In actionNode.Attributes
		...
		if pname <> "pID" and  pname <> "pUOMName" and pname <> "pContainerTypeName" then
			if reqAttrib.text <> ""  then
				...
				'DGB CSBR-130155  Ensure that IsRushOrder is numeric because that is how it is defined in the database
				if pname = "pIsRushOrder" then
						' Blank means false, any other string means true
						if reqAttrib.text <> "" then reqAttrib.text = 1 
				end if
				if not ((pname="pDeliveryLocationID" or pname="pProjectNo" or pname="pJobNo" or pname="pDueDate" or pname="pOrderReasonID") and Application("ORDER_WORKFLOW_OPTION")=2) then 
					if lcase(pname) = "popenasmodalframe" then 
						AddAttributeToElement xmlDoc, root, reqAttrib.NodeName, CnvString(reqAttrib.text)
					else
