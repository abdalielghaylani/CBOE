	...
	for each field in Request.Form
	   	if InStr(field, "field_") > 0 then
	   		...
	   		value = Request(field)
                ...
                if position > 5 then 
                    nametype = left(field,(position - 2))
                else
                    nametype = ""
                end if
                If nametype = "start_date" or nametype = "end_date"  then
                     paramtype = nametype
                else
	   		...
	   		if value <> "" then
	    		...
	    		elseif Rtrim(paramtype) = "location" then
	    				if ValuePairs<> "" then
	    					...
	    					ValuePairs =  name & " IN (SELECT Location_ID from inv_Locations CONNECT BY prior Location_ID = Parent_ID START WITH Location_ID = " & value & ")"
	    				End IF
	    						
	    		elseif Rtrim(paramtype) = "start_date" then
	    			if isDate(value) then
	    			    name=right(name,(len(name)))
	    			   if ValuePairs<> "" then
	    					ValuePairs = ValuePairs & " AND " & name & " >= " & GetOracleDateString(value)
	    				else
	    					ValuePairs = name & " >= " & GetOracleDateString(value)
	    				end if 	
	    		...
	    		elseif Rtrim(paramtype) = "end_date" then
	    			if isDate(value) then
	    			    name=right(name,(len(name)))
	    			     if ValuePairs<> "" then
    						ValuePairs = ValuePairs & " AND " & name & " <= " & GetOracleDateString2(value) 
    					else
    						ValuePairs =  name & " <= " & GetOracleDateString2(value) 
    					end if 
	    		...
	    		elseif Rtrim(paramtype) = "user_name" then
	    			if ValuePairs<> "" then
    					...
    					ValuePairs =  name & "='" & UCASE(value) & "'"
    				end if		
	    			
	    		elseif Rtrim(paramtype) = "text" then
    				dim NameValuePair
    				if InStr(value,"*") > 0 then 
    					NameValuePair = name & "  like '" & replace(value,"*","%") & "'"
    				else 
    					NameValuePair = name & "='" & value & "'"
    				end if 
    				if ValuePairs <> "" then
    				ValuePairs = ValuePairs & " AND " & NameValuePair
    				else
    				ValuePairs =  NameValuePair
    				end if
...
QueryText = RS("ReportSQL")
	if ValuePairs <> "" then
		if InStr(Ucase(QueryText),"WHERE")>0 then
			if(InStrRev(ucase(QueryText),"FROM",-1,1) < InStrRev(Ucase(QueryText),"WHERE",-1,1)) then 
				QueryText = QueryText & " And " & ValuePairs
			else
				QueryText = QueryText & " Where " & ValuePairs	
			end if 
		else
			QueryText = QueryText & " Where " & ValuePairs
		end if 
	...
	if Order_Id<>"" then 
		QueryText = QueryText & " AND Order_Id = " & Order_Id
	end if	
	...
	if QueryName <> "" AND QueryText <> "" then
		...
		ReportFileName = ReportQ.GenerateReport(ReportQueuePath, ReportDBPath, ReportName, "", "", ReportFormat, Application("REPORT_WAIT_TIMEOUT"))
	end if
	
	Response.ContentType = "text/html"
	If InStr(1,ReportFileName,"Report Error") = 0 Then	
		Response.Write ReportFileName 'Response.Write ReportsHTTPPath & ReportFileName
	Else 
