...
bDebugPrint = false
bWriteError = False
strError = "Error:UpdateReservation<BR>"
...
If Len(Request.QueryString) = 0 AND Len(Request.Form)= 0 then
	Response.Redirect "/cheminv/help/admin/api/UpdateReservation.htm"
	Response.end
End if
...
If IsEmpty(ReservationID) then
	strError = strError & "ReservationID is a required parameter<BR>"
	bWriteError = True
End if
...
Call GetInvCommand(Application("CHEMINV_USERNAME") & ".RESERVATIONS.UpdateReservation", adCmdStoredProc)
...
if bDebugPrint then
	...
	Next	
Else
	Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".RESERVATIONS.UpdateReservation")
End if
