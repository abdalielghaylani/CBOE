...
bDebugPrint = false
bWriteError = False
strError = "Error:UpdatePlateFormat<BR>"

PlateFormatID = Request("pPlateFormatID")
PlateFormatName = Request("pPlateFormatName")
PhysPlateID = Request("pPhysPlateID")

' Redirect to help page if no parameters are passed
If Len(Request.QueryString) = 0 AND Len(Request.Form)= 0 then
	Response.Redirect "/cheminv/help/admin/api/UpdatePlateFormat.htm"
	Response.end
End if
...
If IsEmpty(PlateFormatID) then
	strError = strError & "PlateFormatID is a required parameter<BR>"
	bWriteError = True
End if
...
Call GetInvCommand(Application("CHEMINV_USERNAME") & ".PLATESETTINGS.UpdatePlateFormat", adCmdStoredProc)

Cmd.Parameters.Append Cmd.CreateParameter("RETURN_VALUE",adNumeric, adParamReturnValue, 0, NULL)
Cmd.Parameters("RETURN_VALUE").Precision = 9
Cmd.Parameters.Append Cmd.CreateParameter("pPlateFormatID",131, 1, 0, PlateFormatID)
Cmd.Parameters("pPlateFormatID").Precision = 9
...
if bDebugPrint then
	...
	Next	
Else
	Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".PLATESETTINGS.UpdatePlateFormat")
End if

IF createDaughterMap = "1" THEN
	' Set up an ADO command
	Call GetInvCommand(Application("CHEMINV_USERNAME") & ".Reformat.CreateDaughteringMap", adCmdStoredProc)
	Cmd.Parameters.Append Cmd.CreateParameter("RETURN_VALUE",adNumeric, adParamReturnValue, 0, NULL)
	Cmd.Parameters("RETURN_VALUE").Precision = 9
	Cmd.Parameters.Append Cmd.CreateParameter("pPlateFormatID",131, 1, 0, PlateFormatID)
	Cmd.Parameters("pPlateFormatID").Precision = 5
	if bDebugPrint then
		For each p in Cmd.Parameters
			Response.Write p.name & " = " & p.value & "<BR>"
		Next	
	Else
		Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".Reformat.CreateDaughteringMap")
	End if
	xmldocID = Cmd.Parameters("RETURN_VALUE")
END IF
