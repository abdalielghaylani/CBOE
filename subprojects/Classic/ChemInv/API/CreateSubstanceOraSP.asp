...
' substances.   
' CSBR-134410 A location type parameter has been added to this procedure.  That parameter is always present and cannot be conditionally removed
...
if Application("ENABLE_OWNERSHIP")="TRUE" then 
    Cmd.Parameters.Append Cmd.CreateParameter("pLocationType", 200, adParamInput, 2000, LocationTypeID)
else
    Cmd.Parameters.Append Cmd.CreateParameter("pLocationType", 200, adParamInput, 2000, NULL)
end if
...
End if
tempCompoundId = oCompoundID
if CompoundId = 0 and not IsEmpty(CompoundId) then tempCompoundId = CompoundId
	'Check for duplicate structure
	dupStrucID = -1
	if structure <> ""  AND Request("isEmptyStruc") <> "1" then

		dupID = DupStr
		If dupID > 0 AND dupID <> tempCompoundId then
			dupStrucID = dupID
			dupID = CStr(dupID)
			Call AddToDuplicatesDict(dupID, "STRUCTURE")
			Call AddToDuplicatesDict(Cstr(tempCompoundId), "STRUCTURE")
	...
	If dupID > 0 AND dupID <> tempCompoundId then
		...
		Call AddToDuplicatesDict(Cstr(tempCompoundId), "inv_compounds.Substance_Name") 
	...
	If dupID > 0 AND dupID <> tempCompoundId then
		...
		Call AddToDuplicatesDict(Cstr(tempCompoundId), "inv_compounds.CAS")
	...
	If dupID > 0 AND dupID <> tempCompoundId then
		...
		Call AddToDuplicatesDict(Cstr(tempCompoundId), "inv_compounds.ACX_ID")
	...
	For each key in unique_alt_ids_dict
		...
		If dupID > 0 AND dupID <> tempCompoundId then
			...
			Call AddToDuplicatesDict(Cstr(tempCompoundId), key)
