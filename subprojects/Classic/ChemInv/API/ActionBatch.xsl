...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="OpenAsModalFrame" select="CHEMINVACTIONBATCH/@OpenAsModalFrame"/>
  <xsl:template match="/">
    <HTML>
	  <HEAD>
      ...
      <BODY>
		...
		<table>
			...
			<xsl:choose>
			   ...
			   <xsl:when test="CHEMINVACTIONBATCH/@actionType[.='commit']">
				<xsl:choose>
					<xsl:when test="CHEMINVACTIONBATCH/@OpenAsModalFrame[.='true']"> 
						<a class="MenuLink" href="#" onclick="closeContainerPopup()">Close</a> | <a class="MenuLink"><xsl:attribute name="href">/cheminv/cheminv/BrowseInventory_frset.asp?gotoNode=<xsl:value-of select="CHEMINVACTIONBATCH/CREATECONTAINER/@LocationID"/>&amp;SelectContainer=<xsl:value-of select="CHEMINVACTIONBATCH/CREATECONTAINER/RETURNCODE"/></xsl:attribute>View</a>
						<script>
								  function closeContainerPopup(){
								 		if(typeof parent.CloseModal == "undefined"){
										 		window.close(); window.opener.NewRegWindowHandle.refreshRecordDetails();
										} else {
