...
bDebugPrint = false
bWriteError = False
strError = "Error:DeleteRequest<BR>"

RequestID = Request("RequestID")

' Redirect to help page if no parameters are passed
If Len(Request.QueryString) = 0 AND Len(Request.Form)= 0 then
	Response.Redirect "/cheminv/help/admin/api/DeleteRequest.htm"
	Response.end
End if
...
Call GetInvCommand(Application("CHEMINV_USERNAME") & ".Requests.DeleteRequest", adCmdStoredProc)
...
if bDebugPrint then
	...
	Next	
Else
	Call ExecuteCmd(Application("CHEMINV_USERNAME") & ".Requests.DeleteRequest")
...
If AutomaticReservation Then
    sSQL = "Select V.Reservation_Id, V.Container_Id_Fk,V.Request_Id_Fk From INV_RESERVATIONS V " & _
           "Where V.Request_Id_Fk = " & RequestID 
    GetInvConnection()
