...
If IsEmpty(SolventID) then
	...
	bWriteError = True
End if
If Application("ENABLE_OWNERSHIP")="TRUE" AND IsEmpty(Ownership) then
	strError = strError & "Plate Ownership is a required parameter<BR>"
	bWriteError = True
End if
...
Cmd.Parameters.Append Cmd.CreateParameter("PBARCODELIST", 200, 1, len(BarcodeList), BarcodeList) 'Increased the size to fix CSBR 161850 SJ
...
Cmd.Parameters.Append Cmd.CreateParameter("POWNERSHIP", 131, 1, 0, Ownership)
if bDebugPrint then
