...
if ConflictingFields <> "" then 
	...
	bConflicts = true
End if
'DGB fix problem with Null structure crashing 9.0 plugin
if dbStructure <> "" then dbStructure = inLineMarker & dBStructure 
DisplaySubstance "", hdrText, false, false, false, false, false, dBStructure
	...
	<tr>
		<td colspan="4" align="right">
			...
			if CBool(Application("SHOW_MSDX_LOOKUP_LINK")) AND (Application("ACXServerName") <> "NULL") then
				if len(theCAS)>0 then
					bHasMSDX = -1
					if Application("MSDX_LOOK_AHEAD") then bHasMSDX = HasMSDX(theCAS, "", "")
					ShowMSDXLink theCAS, "", "", bHasMSDX
					Response.Write "&nbsp;|&nbsp;"
				End if	
			End if
			bMSDSShowLink = false
			if CBool(Application("SHOW_MSDS_LOOKUP_LINK")) AND (Application("ACXServerName") <> "NULL") then
				if len(theCAS)>0 then
					bHasMSDS = -1
					if Application("MSDS_LOOK_AHEAD") then bHasMSDS = HasMSDS(theCAS, "", "")
					ShowMSDSLink theCAS, "", "", bHasMSDS
					Response.Write "&nbsp;|&nbsp;"
				End if	
			End if
			%>
...
<tr>
	<td colspan="2" align="right">
		...
		<a HREF="#" onclick="top.close(); return false;"><img SRC="../graphics/cancel_dialog_btn.gif" border="0" alt="Cancel"></a><a HREF="#" onclick="DoSelectSubstance(<%=BaseID%>, document.cows_input_form.SubstanceName.value); return false;"><img SRC="../graphics/ok_dialog_btn.gif" border="0"></a>
	</td>
</tr>
<%end if%>
</table>
...
<table width="60%">
	...
	<%if EHSFoundData = "1" AND Application("DISPLAY_EHS_DATA") then%>
	<tr>
