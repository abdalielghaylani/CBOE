...
<html>
		...
		<script type="text/javascript" language="JavaScript">
			...
			function SetLocationSQL(LocationID){
				...
				if (formgroup == "plates_form_group" || formgroup == "plates_np_form_group"){
					...
					inputFieldName = "inv_plates.location_id_fk";
				}
				else
				{
				    tableName = "inv_containers.Location_ID_FK";
				    if( formgroup == "base_form_group" || formgroup == "containers_form_group_substructure" )
				    {
				        inputFieldName = "inv_containers_subsearch_alias.Location_ID_FK";
				    }
				    else
				    {
				        inputFieldName = "inv_containers.Location_ID_FK";
				    }				  
				}
				...
				//alert(sql)
				document.cows_input_form[inputFieldName].value = sql;
			}
			...
			//function to change the lables of batch fields 
			function ResetBatchFielslables(batchTypeid)
			{
			  var TempBatchStr= eval("document.cows_input_form.FieldStringBatch" + batchTypeid + ".value");
               if(TempBatchStr==''){
                      for(i=1;i<=3;i++){
	                     document.getElementById("BatchFld" + i).style.display='none';
	                     document.getElementById("inv_container_batches.batch_field_" + i).value='';
	                     document.getElementById("inv_vw_compounds_alias1.reg_number" ).value='';
	                 }  
                }
                else{
                    ...
                    for (i=0;i<arrBatchValues.length;i++){
                        ...
                        document.getElementById("inv_container_batches.batch_field_" + arrBatchValues[i].split(',')[1]).value='';
                       if(i==0)
                       {
                             var FieldName= arrBatchValues[i].split(",")[2];
                              if (FieldName=="REG_ID_FK")
                              {
        	                     document.getElementById("inv_container_batches.batch_field_1").style.display='none';
        	                     document.getElementById("inv_vw_compounds_alias1.reg_number").style.display='block';
                              }
                              else
                              { 
        	                     document.getElementById("inv_container_batches.batch_field_1").style.display='block';
        	                     document.getElementById("inv_vw_compounds_alias1.reg_number").style.display='none';
                              }
                              document.getElementById("inv_container_batches.batch_field_1").value='';
        	                  document.getElementById("inv_vw_compounds_alias1.reg_number" ).value='';
                       }
	                }
	...
	<script LANGUAGE="javascript">
		...
		function CompoundIDChange()
		{
		    // These hidden fields must be set whenever the compound ID is specified so that we do not search
		    // over dummy reg_id records in inv_compounds (that is, when reg_id_fk and batch_number_fk are not null)
		    if( document.cows_input_form["inv_containers.Compound_ID_FK"].value != "" )
		    {
		        document.cows_input_form["inv_compounds.reg_id_fk"].value = " (inv_compounds.reg_id_fk is null) ";
		        document.cows_input_form["inv_compounds.batch_number_fk"].value = " (inv_compounds.batch_number_fk is null) ";
		    }
		    else
		    {
		        document.cows_input_form["inv_compounds.reg_id_fk"].value = " (1 = 1) ";
		        document.cows_input_form["inv_compounds.batch_number_fk"].value = " (1 = 1) ";
		    }		    
		}
			
		function AfterOnLoad(){
			if (document.cows_input_form["inv_vw_compounds.CAS"]){
			document.cows_input_form["inv_vw_compounds.CAS"].focus();
			}
			
			if( document.cows_input_form["inv_containers.Compound_ID_FK"] )
			{
			    document.cows_input_form["inv_containers.Compound_ID_FK"].onchange = CompoundIDChange;
			}
		}
...
<%
	...
	'CBOE-1033 SJ Storing session values in the case of refine query
    PurityID = Session("SearchData" & "inv_containers.Unit_Of_Purity_ID_FK" & dbkey & formgroup)
    ConcID = Session("SearchData" & "inv_containers.Unit_Of_Conc_ID_FK" & dbkey & formgroup)
    DensityID = Session("SearchData" & "inv_containers.unit_of_Density_ID_FK" & dbkey & formgroup)
    ContainerTypeID = Session("SearchData" & "inv_containers.Container_Type_ID_FK" & dbkey & formgroup)
    ContainerStatusID = Session("SearchData" & "inv_containers.Container_Status_ID_FK" & dbkey & formgroup)
    UOMID = Session("SearchData" & "inv_containers.Unit_Of_Meas_ID_FK" & dbkey & formgroup)
	SupplierID = Session("SearchData" & "inv_containers.Supplier_ID_FK" & dbkey & formgroup) 		
    OrderID = Session("SearchData" & "inv_containers.Ordered_By_ID_FK" & dbkey & formgroup)
    OwnerID = Session("SearchData" & "inv_containers.Owner_ID_FK" & dbkey & formgroup)
    CurrentUserID = Session("SearchData" & "inv_containers.Current_User_ID_FK" & dbkey & formgroup) 	
%>
					...
					<%if Application("RegServerName") <> "NULL" then%>
		      		...
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_containers.Unit_Of_Purity_ID_FK", PurityID, "SELECT UNIT_ID AS Value, Unit_Abreviation AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 3 ORDER BY lower(Unit_Abreviation) ASC", 16, RepeatString(20, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_containers.Unit_Of_Conc_ID_FK", ConcID, "SELECT UNIT_ID AS Value, Unit_Abreviation AS DisplayText FROM inv_units WHERE Unit_type_id_FK in (3,6) ORDER BY lower(Unit_Abreviation) ASC", 16, RepeatString(20, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_containers.unit_of_Density_ID_FK", DensityID, "SELECT UNIT_ID AS Value, Unit_Abreviation AS DisplayText FROM inv_units WHERE Unit_type_id_FK = 6 ORDER BY lower(Unit_Abreviation) ASC", 16, RepeatString(20, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
				...
				<table border="0" width="250px">
					...
					<tr>
			        	<td>
							...
							<%= ShowSelectBox2("inv_containers.Container_Type_ID_FK", ContainerTypeID, "SELECT Container_Type_ID AS Value, Container_Type_Name AS DisplayText FROM inv_container_types ORDER BY lower(Container_Type_Name) ASC", 30, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
			        	<td>
							<span title="Pick an option from the list">Container Status:</span><br>
							<%= ShowSelectBox2("inv_containers.Container_Status_ID_FK", ContainerStatusID, "SELECT Container_Status_ID AS Value, Container_Status_Name AS DisplayText FROM inv_container_status ORDER BY lower(Container_Status_Name) ASC", 30, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
			        	<td>
							<span title="Pick an option from the list">Unit of Measure:</span><br>
							<%= ShowSelectBox2("inv_containers.Unit_Of_Meas_ID_FK", UOMID, "SELECT UNIT_ID AS Value, Unit_Name AS DisplayText FROM inv_units WHERE Unit_type_id_FK IN (1,2,4) ORDER BY lower(Unit_Name) ASC", 30, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
		      		...
		      		<tr>
		        		<td>
							...
							<SELECT name="inv_vw_compounds.Conflicting_Fields">
				...
				<table border="0" width="250px">
					...
					<tr>
			        	<td>
							...
							<%= ShowSelectBox2("inv_containers.Supplier_ID_FK", SupplierID, "SELECT Supplier_ID AS Value, Supplier_Short_Name AS DisplayText FROM inv_suppliers ORDER BY lower(Supplier_Short_Name) ASC", 27, RepeatString(43, "&nbsp;"), "")%>        
						</td>
		      		</tr>
					<tr>
			        	<td>
		      		...
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_containers.Ordered_By_ID_FK", OrderID, "SELECT User_ID AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText FROM CS_SECURITY.People ORDER BY lower(Last_Name) ASC", 27, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
		      		...
		      		<tr>
                                ...
                                <%ShowInputField dbkey, formgroup, "inv_containers.Date_Created", "DATE_PICKER:8","27"%>
</td>
                                </tr>
		      		<tr>
		        		<td>
		      		...
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_containers.Owner_ID_FK", OwnerID, "SELECT owner_id AS Value, description AS DisplayText FROM " & Application("CHEMINV_USERNAME") & ".inv_owners ORDER BY lower(description) ASC", 27, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
							<span title="User ID of current user responsible for the container">Current User:<br>
							<%= ShowSelectBox2("inv_containers.Current_User_ID_FK", CurrentUserID, "SELECT User_ID AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText FROM CS_SECURITY.People ORDER BY lower(Last_Name) ASC", 27, RepeatString(43, "&nbsp;"), "")%>
						</td>
		      		</tr>
					<tr>
					...
					For each key in custom_fields_dict
						...
						if inStr(Key, "DATE_") then
						    ...
						    if custom_lists_dict.Exists("CUSTOM_FIELDS." & key) then
							    Response.Write "<SELECT name='inv_containers." & lcase(key) & "'><OPTION Selected=""True"" value="""">" & GetCustomListOptions("CUSTOM_FIELDS", key, eval(key), null) & "</SELECT>"
							else    
		...
		<table border="0" cellpadding="0" cellspacing="0">
			...
			<td>
				<table border="0">
					<tr>
						<td>
							<%=getGetSearchOptions(dbkey, formgroup, "inv_vw_compounds.Structure", "AllOptions", "SelectList")%>
						</td>
						<td valign="bottom" align="right">
							<%ShowSearchButton()%>&nbsp;
						</td>
				...
				<table border="0">
					...
					<tr>
			        	<td>
							...
							<input type="hidden" name="inv_compounds.reg_id_fk1" value="=NULL" />       
						</td>
		      		</tr>
		      		<tr>
		        		<td>
					...
					<tr>
		        		<td>
							...
							<%ShowInputField dbkey, formgroup, "Alt_IDS.Identifier1", "0","30"%>
						</td>
					</tr>
					<%End if%>
		      		<tr>
		      		...
		      		<tr>
		        		<td>
							<span title="Full RegNumber">Full RegNumber:<br>
							<%ShowInputField dbkey, formgroup, "Reg_Numbers.Full_Reg_Number", "0","30"%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
				...
				<table border="0">
					...
					For each key in alt_ids_dict
						...
						if bCustom then
						    ...
						    Response.Write "<span title="""">"
						    Response.Write strLabel & "</span><br>"
						    ShowInputField dbkey, formgroup, "MolTable." &  Replace(key, "inv_vw_compounds.",""), "0","30"  
                        end if
						Response.Write "</td></TR>" & vblf 
					Next 
					%>
				...
				<table>
		      		...
		      		<tr>
			        	<td>
							...
							<%= ShowSelectBox3("inv_container_batches.batch_type", "", "SELECT ID AS Value, Batch_type AS DisplayText FROM " & Application("CHEMINV_USERNAME") & ".inv_batch_type where id in (select distinct batch_type_id_fk from "& Application("CHEMINV_USERNAME") & ".inv_container_batch_fields) ORDER BY Batch_order ASC", 30, "", "","ResetBatchFielslables(this.value);")%>
						</td>
		      		</tr>
					<tr>
		        		<td>
                    ...
                    <tr id="BatchFld1">
		        		<td>
							...
							<%ShowInputField dbkey, formgroup, "inv_container_batches.batch_field_" & 1 & "", "0","30"%>
							<%ShowInputField dbkey, formgroup, "inv_vw_compounds_alias1.reg_number" , "0","30"%>
						</td>
					</tr>
                    <tr id="BatchFld2">
		        		<td>
							<span id="BatchFieldDisplay2" style="display:block">BatchFieldDisplay2:</span>
							<%ShowInputField dbkey, formgroup, "inv_container_batches.batch_field_" & 2 & "", "0","30"%>
						</td>
					</tr>
                    <tr id="BatchFld3">
		        		<td>
							<span id="BatchFieldDisplay3" style="display:block">BatchFieldDisplay3:</span>
							<%ShowInputField dbkey, formgroup, "inv_container_batches.batch_field_" & 3 & "", "0","30"%>
						</td>
					</tr>
...
<script language="javascript">ResetBatchFielslables(1);</script>
<%
	Case "plate"
%>
		<table border="0" cellpadding="0" cellspacing="0">
					...
					<%if Application("RegServerName") <> "NULL" then%>
		      		...
		      		<tr>
		        		<td>
							...
							<%= ShowSelectBox2("inv_plates.conc_unit_fk", "", "SELECT UNIT_ID AS Value, Unit_Abreviation AS DisplayText FROM inv_units WHERE Unit_type_id_FK in (3,6) ORDER BY lower(Unit_Abreviation) ASC", 16, RepeatString(20, "&nbsp;"), "")%>
						</td>
		      		</tr>
		      		<tr>
		        		<td>
...
<!-- SMathur: Fixing CSBR-97468, there is an issue in input_form_footer_vbs.asp file. 
I am adding the following line to fix the issue and the following code would needs to be 
removed after fixing core bug CSBR-97594.-->
	<script language="javascript">DoAfterOnLoad ? AfterOnLoad():true;</script>
		...
		function SetSearchCriteria(){
			...
			if ((formgroup == "plates_form_group") || (formgroup == "plate_compounds_form_group")){
				...
				unchkfg = "containers_form_group_substructure";	
			}
