...
if Session("ScreenReportPageSize" & ArrayID) > 99 then
	Session("INVPageSize") = 99
else
	Session("INVPageSize") = Session("ScreenReportPageSize" & ArrayID)
end if
...
if Len(ColDefStr) > 0 then
	...
	if (Len(bSaveColDef) > 0) AND bUseDefault=""  then
		Response.Write "<script language=JavaScript>opener.focus();opener.DialogWindow=null;opener.location.reload();window.close();</script>"
	End if
End if
...
function FillColDef() {
   <% if lCase(entity) <> "request" and lCase(entity) <> "displayrack" and lCase(entity) <> "reservation" then %>
	    if (!isPositiveNumber(document.forms[0].PageSize.value) || document.forms[0].PageSize.value < 1 || document.forms[0].PageSize.value >99){
		    document.forms[0].PageSize.value = '';
		    document.forms[0].PageSize.focus();
		    document.forms[0].PageSize.select();
		    alert("The number of items per page must be a positive number between 1 and 99.");
		    return false;
	    }	  
	<%end if%>
	var list = window.document.forms[0].destList;
...
<%Else%>
	<table bgcolor="#C0C0C0">
		...
		<% if lCase(entity) <> "request" and lCase(entity) <> "displayrack" and lCase(entity) <> "reservation" then %>
		<tr>
			<td align="left" colspan="4"><%if lCase(entity) = "batch" then Response.write(entity & "es") else Response.write(entity & "s") end if%> per page:&nbsp;<input type="text" size="4" name="PageSize" value="<%=Session("INVPageSize")%>"></td>
		</tr>
		<%else%>
		<input type="hidden" name="PageSize" value=<%=Session("INVPageSize")%>>
		<% end if %>
		<tr>
