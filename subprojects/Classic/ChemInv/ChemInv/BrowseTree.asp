...
<script type="text/javascript" language="javascript">
	...
	function getRackDictionary() {
		var strURL = serverType + serverName + "/cheminv/cheminv/BrowseTreeDict.asp";
...
If (IsEmpty(Session("TVNodes" & TreeID)) OR ClearNodes = 1) Then
	...
	Else
		sNode = "NULL"
		Call PopulateNodeLayer(sNode,TreeLevel) ' Add the first three layers of nodes
	End if
...
Elseif (ExpNode = "Y") then
	...
	Else
    	TempNode=sNode
		Call PopulateNodeLayer(sNode,TreeLevel) ' Add the two layers below the selected node
...
'-- Purpose: Modified to get the length of server name; it was previously hard coded to 30; in case when server name has more characters, no racks where displayed
'-- Date: 27/09/2010
...
Sub PopulateNodeLayer(pNodeID, pLevel)
	...
	if InStr(1,Session("PrevExpandedNodesList" & TreeID),":" & pNodeID & ":") = 0 Then
		...
		If Session("INV_MANAGE_PLATE_MAPS" & dbkey) then
			LayerSQL = "SELECT Location_Name, Location_Barcode, collapse_child_nodes, " & Application("oraschemaname") & ".racks.open_position_count(location_id) as open_position_count, " &_ 
			 "(SELECT Location_Type_Name FROM inv_Location_Types WHERE Location_Type_ID = inv_Locations.Location_Type_ID_fk) AS LocationTypeName, " &_ 
			 "(SELECT gr.url_active from inv_location_types lt, inv_graphics gr where lt.graphic_id_fk=gr.graphic_id(+) and lt.location_type_id=inv_Locations.Location_Type_ID_fk) AS Img_URL_active, " &_ 
			 "(SELECT gr.url_inactive from inv_location_types lt, inv_graphics gr where lt.graphic_id_fk=gr.graphic_id(+) and lt.location_type_id=inv_Locations.Location_Type_ID_fk) AS Img_URL_inactive, " &_ 
			 "Location_Description, Location_id AS ID, case when (select count((select l1.location_id from inv_locations l1 where l1.location_id in (select st.location_id_fk from inv_grid_storage st where st.grid_storage_id in (select ge.grid_storage_id_fk from inv_grid_element ge where ge.location_id_fk = inv_locations.Parent_id)) and collapse_child_nodes=1)) from dual) = 0 then Parent_id else (select l1.location_id from inv_locations l1 where l1.location_id in (select st.location_id_fk from inv_grid_storage st where st.grid_storage_id in (select ge.grid_storage_id_fk from inv_grid_element ge where ge.location_id_fk = inv_locations.Parent_id)) and collapse_child_nodes=1) end as Parent_id, " &_ 
			 "(SELECT count(*) from inv_allowed_ptypes where location_id_fk = inv_Locations.Location_ID) AS AllowedPlates, (select grid_storage_id from inv_grid_storage where location_id_fk=location_id) as GridStorageID, " &_
             "case when (select count(*) from inv_locations l1 where l1.Parent_id=inv_Locations.Location_id ) = 0 then 0 else 1 end as HasChild,Level as NodeLevel " &_   
			 "FROM inv_Locations WHERE level " & pLevel & " AND Location_id not in (Select Location_id from inv_locations where parent_id in (Select Location_ID from inv_locations where collapse_child_nodes=1) and collapse_child_nodes is null) " &_
			 "CONNECT BY prior Location_id = Parent_id AND level " & pLevel & " START WITH Location_id IN (SELECT Location_id from inv_Locations where Parent_id " & pNodeID & ") " &_
			 "ORDER BY level, case when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(location_name) then UPPER(location_name) " &_
			 "when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(translate(UPPER(substr(location_name, instr(location_name, ' ', -1) + 1)), '0123456789ABCDEFGHIJLKMNOPQRSTUVWXYZ()~`!@#$%^&*-_=+{}[]\|;:<>,./?', '0123456789')) then UPPER(substr(location_name, 1, instr(location_name, ' ', -1))) else UPPER(location_name) end, " &_ 
			 "case when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(location_name) then 1000000 when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(translate(UPPER(substr(location_name, instr(location_name, ' ', -1) + 1)), '0123456789ABCDEFGHIJLKMNOPQRSTUVWXYZ()~`!@#$%^&*-_=+{}[]\|;:<>,./?', '0123456789')) then to_number(substr(location_name, instr(location_name, ' ', -1) + 1)) else 1000000 end"
		else
			LayerSQL = "SELECT Location_Name, Location_Barcode, collapse_child_nodes, " & Application("oraschemaname") & ".racks.open_position_count(location_id) as open_position_count, " &_ 
			"(SELECT Location_Type_Name FROM inv_Location_Types WHERE Location_Type_ID = inv_Locations.Location_Type_ID_fk) AS LocationTypeName, " &_ 
			"(SELECT gr.url_active from inv_location_types lt, inv_graphics gr where lt.graphic_id_fk=gr.graphic_id(+) and lt.location_type_id=inv_Locations.Location_Type_ID_fk) AS Img_URL_active, " &_ 
			...
			"(SELECT count(*) from inv_allowed_ptypes where location_id_fk = inv_Locations.Location_ID) AS AllowedPlates, (select grid_storage_id from inv_grid_storage where location_id_fk=location_id) as GridStorageID, " &_ 
             "case when (select count(*) from inv_locations l1 where l1.Parent_id=inv_Locations.Location_id ) = 0 then 0 else 1 end as HasChild,Level as NodeLevel " &_   
			"FROM inv_Locations WHERE location_type_id_fk not in (select location_type_id from inv_location_types where location_type_name = 'Plate Map') AND level " & pLevel & " AND Location_id not in (Select Location_id from inv_locations where parent_id in (Select Location_ID from inv_locations where collapse_child_nodes=1) and collapse_child_nodes is null) " &_ 
			"CONNECT BY prior Location_id = Parent_id AND level " & pLevel & " START WITH Location_id IN (SELECT Location_id from inv_Locations where Parent_id " & pNodeID & ") " &_ 
			"ORDER BY level, case when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(location_name) then UPPER(location_name) when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(translate(UPPER(substr(location_name, instr(location_name, ' ', -1) + 1)), '0123456789ABCDEFGHIJLKMNOPQRSTUVWXYZ()~`!@#$%^&*-_=+{}[]\|;:<>,./?', '0123456789')) then UPPER(substr(location_name, 1, instr(location_name, ' ', -1))) else UPPER(location_name) end, " &_ 
			"case when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(location_name) then 1000000 when length(substr(location_name, instr(location_name, ' ', -1) + 1)) = length(translate(UPPER(substr(location_name, instr(location_name, ' ', -1) + 1)), '0123456789ABCDEFGHIJLKMNOPQRSTUVWXYZ()~`!@#$%^&*-_=+{}[]\|;:<>,./?', '0123456789')) then to_number(substr(location_name, instr(location_name, ' ', -1) + 1)) else 1000000 end"
		end if
		if bDebugPrint then
...
Sub BuildTree(pSQL)
    Dim fldParentID
    Dim fldImageActive
    Dim fldImageInactive
    Dim fldCollapseChildNodes
    Dim fldLocationName
    Dim fldOpenPositionCount
    Dim fldLocationTypeName
    Dim fldLocationBarcode
    Dim fldLocationDescription
    Dim fldID
    Dim HasChild
    Dim NodeLevel
		'Response.Write pSQL
		...
		set fldParentID = RS.Fields("Parent_ID")
		set fldImageActive = RS.Fields("img_url_active")
		set fldImageInactive = RS.Fields("img_url_inactive")
		set fldCollapseChildNodes = RS.Fields("collapse_child_nodes")
		set fldLocationName = RS.Fields("Location_Name")
		set fldOpenPositionCount = RS.Fields("open_position_count")
		set fldLocationTypeName = RS.Fields("LocationTypeName")
        set fldLocationBarcode = RS.Fields("Location_barcode")
        set fldLocationDescription = RS.Fields("Location_Description")
		set HasChild=RS.Fields("HasChild")
		set NodeLevel=RS.Fields("NodeLevel")
		i = 0
		Do While Not RS.EOF
			i = i + 1
			id = fldID.Value
			ParentID = fldParentID.Value
			imgActive = fldImageActive.Value
			...
			Else
				...
				isRack = false
				if fldCollapseChildNodes.Value=1 then isRack = true
				
				'-- If Rack, set showInList value
				showInList = ""
                displayName = Left(fldLocationName.Value,200)                
   				if isRack then 
				    showInList = "racks"
				    '-- if this is a browse location tree, then show the number of open positions for the rack
				    if TreeID = 2 then
   				        openPositionText = " (" & fldOpenPositionCount.Value & " open)"
   				        locationText = Left(fldLocationName.Value,200-(len(openPositionText))) & openPositionText
   		    		    if multiSelect = "1" then
   		    		        checkText = ""
				...
				End if
				'-- adding a dummy node to show the + sign
				if cint(NodeLevel.value)=2 and cint(HasChild.value)=1 and not isRack then
				    Set Nodex = nothing 
				    'CSBR ID : 121726 SJ
				    'Comment : To resolve the dummy node issue while adding a child node.
                   Set Nodex = TreeView.Nodes.Add(id,4,id & "-1" ,"Click Parent to expand", imgInActive, imgInActive, , , fldLocationTypeName.Value & vblf & fldLocationBarcode.value & vblf & fldLocationDescription.value)
				end if 
				'removing dummy node of a parent, fixing for the case of page refresh 
