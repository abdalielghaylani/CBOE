...
If bHitsFound then
	...
	if NOT (RS.BOF AND RS.EOF) then 
		...
		if cint(mypage)> cint(maxcount) then mypage= cint(maxcount)
	...
	Do While NOT RS.EOF AND BatchCount < RS.pagesize
		...
		if Application("ShowRequestSample") and Session("INV_SAMPLE_REQUEST" & dbkey) then
		...
		Listx.URL = "#"" onclick=""OpenDialog('/cheminv/gui/RequestSample.asp?RequestType=&action=create&batchid=" & BatchID & "&QtyRequired=" & AmountAvailable & "&AmountAvailable=" & AmountAvailable & "&UOMAbv=" & UOMAbbrv & "&AllowSampleRequest=" & allowSampleRequest & "&AllowBatchRequest=" & AllowBatchRequest & "&usebatch=1" & "', 'DiagRequest', 2); return false"
