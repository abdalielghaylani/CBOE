	...
	<head>
		...
		<script language="JavaScript">
			...
			function updateDict(bClear) {
				...
				var strURL = serverType + serverName + "/cheminv/cheminv/MultiSelect.asp?selectChckBox=" + allChckBoxValues + "&removeList=" + removeList;	
			...
			function updateNumContainers(num) {
				...
				if (num == 1) {
					text = num + " Container Selected";
				}
				else {
					text = num + " Containers Selected";
				}
...
If bHitsFound then
	...
	If NOT IsArray(Session("ContainerReportFieldArray1")) then
		ColDefStr = GetUserProperty(Session("UserNameCheminv"),"ContainerChemInvFA1")
		If IsNull(ColDefStr) OR ColDefStr = "" then 
			' Default column definition
			ColDefstr= Application("ContainerReportColDef1")
			rc= WriteUserProperty(Session("UserNameCheminv"), "ContainerChemInvFA1", ColDefstr)
		End if
		fieldArray = GetFieldArray(colDefstr, Application("ContainerFieldMap"))
		Session("ContainerReportFieldArray1") = fieldArray
	Else
		'ReDim fieldArray(Ubound(Session("ContainerReportFieldArray1")),3)
		fieldArray = Session("ContainerReportFieldArray1")
	End if
	%>
		...
		<TABLE cellspacing=0 cellpadding=0 width=600>	
			...
			<TR>
				<td align=center COLSPAN="2">
					...
					<a class="MenuLink" HREF="containers_result_list.asp?view=3&clear=1&<%=QS%>">Cancel MultiSelect</a>
				</td>
			</tr>
			<tr>
				<TD ALIGN="LEFT" VALIGN="top" NOWRAP><SPAN CLASS="GuiFeedback" ID="containerNumSpan"><%if Session("bMultiSelect") then Response.Write"<SCRIPT LANGUAGE=""javascript"">updateNumContainers(" & multiSelect_dict.count & ");</SCRIPT>"%></SPAN></TD>
				...
				<%If Session("INV_EDIT_CONTAINER" & dbkey) then%>			
					|
					<a class="MenuLink" id="UpdateContainerLnk" HREF="Update inventory container" onclick="OpenDialog('/ChemInv/GUI/UpdateContainer.asp?multiSelect=true&action=edit', 'Diag', 1); return false">Update Containers</a>
				<%End if%>
			...
			<TR>
				<td align=center>
					<A CLASS="MenuLink" HREF="containers_result_list.asp?view=3&multiSelect=1&<%=QS%>" >Multi Select</A>
					|
					<A CLASS="MenuLink" HREF="containers_result_list.asp?view=0&<%=QS%>" >Large Icons</A>
					|
					<A CLASS="MenuLink" HREF="containers_result_list.asp?view=1&<%=QS%>" >Small Icons</A>
<!--					|
					<A CLASS="MenuLink" HREF="containers_result_list.asp?view=2&<%=QS%>" >List</A>-->
					|
					<A CLASS="MenuLink" HREF="containers_result_list.asp?view=3&<%=QS%>" >Details</A>
					|
					<a class="MenuLink" href="#" title="Click to choose report columns" onclick="OpenDialog('columnPicker2.asp?ArrayID=1&isSearch=1&formgroup=<%=formgroup%>&dbkey=<%=dbkey%>', 'Diag', 1); return false">Column Chooser</a>
					|
					<A CLASS="MenuLink" HREF="/cheminv/Gui/CreateReport_frset.asp?HitlistID=<%=Session("hitlistID" & "ChemInv" & formgroup)%>" target="RPT">Print Report</A>
				</td>
			</tr>
			<%End if%>
		</table>
...
If bHitsFound then
	...
	SQL = Replace(SQL, "FROM " & vblf & "inv_Containers", "FROM " & Application("CHEMINV_USERNAME") & ".CSDOHitlist, inv_Containers")
		SQL= SQL & "AND " & _ 
			...
			"AND " & _ 
			  Application("CHEMINV_USERNAME") & ".CSDOHitlist.HitlistID = ?"  
    ...
    if cint(Session("hitlistRecordCount" & dbkey & formgroup)) > 0  Then
	...
	prefPageSize = Session("UserNumListView" & dbkey)
	if (mypageSize <> prefPageSize) and (Session("bColChooserUpdate") <> "true") then
		myPageSize = prefPageSize
		Session("ScreenReportPageSize1") = mypagesize
		rc= WriteUserProperty(Session("UserNameCheminv"), "ScreenReportPageSize1" , mypagesize)
	end if
	Session("bColChooserUpdate") = "false"
	If mypagesize="" then
	...
	if NOT (RS.BOF AND RS.EOF) then 
		...
		if cint(Session("hitlistRecordCount" & dbkey & formgroup)) > 0  Then
           maxcount = SetMaxCount( Session("hitlistRecordCount" & dbkey & formgroup),RS.pagesize) 
		   RS.absolutepage = 1            
        else
	...
	Do While Not RS.EOF AND ContainerCount < RS.pagesize
		...
		basearray(0,aindex) = ContainerID
		Session("Base_RS" & dbkey & formgroup)(aindex) = ContainerID
		if Session("bMultiSelect") then
			chckedStr = ""
	...
	function SortReport(fieldName){
		...
		url = "containers_result_list.asp?formgroup=" + "<%=formgroup%>" + "&formmode=" + "<%=formmode%>" + "&dbname=cheminv" + "&BaseCurrentIndex=" + "<%=current_index%>" + "&BaseRecordCount=" + "<%=totalrecords%>" + "&dbsearchnames=cheminv" + "&SortByFieldName=" + fieldName;
...
'Fix for Bug CBOE-2475
function SetMaxCount(hitlistrecordcount,rspagesize)
    Dim floatval
    Dim decimalval
    Dim maxcountval    
    ' Get the float value with 2 decimal points after division
    floatval = FormatNumber(hitlistrecordcount / rspagesize,2)             
    'retrive only the decimal point value
     decimalval = right(CStr(floatval),2)       
     ' check if the decimal value is not 0 and less than 50          
      if (CInt(decimalval)<> 0 and CInt(decimalval) < 50) then
            'increment rounded off value by 1 i:e 1.45 will be incremented to 2
            maxcountval = Round(floatval) +1
            else
            maxcountval = Round(floatval)
      end if
     
            SetMaxCount = maxcountval
end function
