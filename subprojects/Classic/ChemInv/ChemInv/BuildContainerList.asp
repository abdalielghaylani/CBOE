...
<body bgcolor="#FFFFFF" onunload="top.window.defaultStatus=''">
<bgsound src="" id="snd1">
<form name="form1" method="POST" target="TabFrame">
<input type="hidden" name="removeList">
<div align="left" id="header" style="POSITION:Absolute;top:0;left:0;white;visibility:visible;z-index:200;background-color:White;">
...
'-- Purpose: Moving the code up since the maxcount is used to reset the current page based on the selection in drop down list
'-- Date: 23/09/2010
...
'-- Date: 23/09/2010
     If mypage="" or maxcount <= 1 then
'-- End of Change #131442#
      mypage=1
     end if
    SQL1 = "SELECT * FROM (SELECT rownum rnum, a.* FROM( "
	...
	Else
		...
		End if
		if Len(RegID) > 0 then
			SQL = SQL & " AND Inv_Compounds.Reg_ID_FK = ?"
...
'-- CSBR ID:126044
'-- Change Done by : Manoj Unnikrishnan
'-- Purpose: Modified to show the page size correctly in container list and reconcile containers; Also using CLng conversion in case of reconcile
'-- Date: 19/08/2010
    tmin=Clng(mypagesize)*(mypage-1)
    tmax=(Clng(mypagesize)*mypage)+1
'-- End of Change #126044#
    'DGB Uppercase the sorting field
    
    'CSBR ID:124366
	'Purpose: Sort the Date field
	'Date   : 01/06/2010		
     If (SortbyFieldName="DATE_Expires" Or SortbyFieldName="DATE_Created" Or SortbyFieldName="DATE_Produced" Or SortbyFieldName="DATE_Ordered" Or SortbyFieldName="DATE_Received" Or SortbyFieldName="Date_Certified" Or SortbyFieldName="Date_Approved" Or SortbyFieldName="Date_1" Or SortbyFieldName="Date_2" Or SortbyFieldName="Date_3" Or SortbyFieldName="Date_4" Or SortbyFieldName="Date_5") then
        SQL = SQL & " ORDER BY to_date(" & SortbyFieldName & ",'" & dateFormatString & "') " & SortDirectionTxt  
     Else
       if SortbyFieldName <> "Container_ID" then
        SQL = SQL & " ORDER BY Upper(" & SortbyFieldName & " ) " & SortDirectionTxt
        else 
           SQL= Replace(SQL,"/*+ ORDERED */" , " " )  
           SQL = SQL & " ORDER BY " & SortbyFieldName & " " & SortDirectionTxt
        End If 
     End If
     'End of change      
     SQL1 = SQL1 & SQL & " ) a where  ROWNUM < ?"
     Cmd.Parameters.Append Cmd.CreateParameter(, 131, 1, 0, tmax)

     SQL1 = SQL1 & " ) WHERE rnum > ?"
	'Response.Write(SQL)
	'Response.End
...
'-- CSBR ID:126044
'-- Change Done by : Manoj Unnikrishnan
'-- Purpose: Modified to show the missing containers correctly in case of reconcile containers
'-- Date: 19/08/2010
	if reconcile then
...
'-- End of Change #126044#
	...
	if NOT (RS.BOF AND RS.EOF AND RS1.BOF AND RS1.EOF) then 
	...
	if totalContainers = 1 then cPlural = ""
	...
	Set Conn = Nothing	
		...
		JSFunc = "function clickElement(elementid) {"
		JSFunc = JSFunc & "var e = document.getElementById(elementid);"
		JSFunc = JSFunc & "if (typeof e == 'object') {"
		JSFunc = JSFunc & "if(document.createEventObject) {"
		JSFunc = JSFunc & "e.fireEvent('onclick');"
		JSFunc = JSFunc & "return false;}}}"
		Response.Write JSFunc
		if ContainerCount = 1 OR CLng(SelectContainer)>0 then
			if CLng(SelectContainer)>0 then 
				ContainerID = SelectContainer
				if mypage > 1 AND request.querystring("whichpage") <>  mypage then
