...
<html>
	<head>
        ...
        <!--#INCLUDE VIRTUAL = "/cheminv/api/apiUtils.asp"--> <!-- WJC - Millenium -->
        <!--#INCLUDE VIRTUAL = "/cheminv/gui/compound_utils_vbs.asp"--> <!-- WJC - Millenium -->
	</head>
	<body bgcolor="#FFFFFF">
	<% 
		  ...
		  <tr>
			...
			<td valign="top">
				...
				<table border="0">
					...
					<tr>
		        		<td>
							<span title="Batch Number">Batch Number:<br>
			...
			<td valign="top">
				...
				<table border="0">
					...
					For each key in alt_ids_dict
						...
						if bCustom then
						    Dim Conn
						    strPicklist = ShowCustomPicklist(Key)
						    strPicklist = Replace(strPicklist, "inv_compounds.", "MolTable.")  ' WJC Why MolTable ?
						    response.Write strPicklist
						else
	...
	if not isEmpty(Application("DEFAULT_SEARCH_DATABASE")) and instr(Application("GlobalSearchDBs"),Application("DEFAULT_SEARCH_DATABASE")) > 0 then
		isChecked = "true;"
		if ucase(Application("DEFAULT_SEARCH_DATABASE")) = "INVREG" and hideRegSearch then isChecked = "false;"
		response.write("document.cows_input_form." & Application("DEFAULT_SEARCH_DATABASE") & ".checked = " & isChecked)
