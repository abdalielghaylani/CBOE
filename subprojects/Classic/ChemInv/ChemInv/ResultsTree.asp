...
Sub BuildResultsTree(ByVal pSearchType)
		...
		Do While Not RS.EOF
			if RS("IsRack") = "1" then
				...
				if prevRackID<>"" then 
					if  clng(RS("Parent_ID").value)=clng(prevRackID) then
						bPrevIsRack=true
					else
						bPrevIsRack=false
					end if
			...
			if (CLng(level) = 1) then
				...
				if (pSearchType = "ByCompoundID" OR  pSearchType = "ByRegID" OR  pSearchType = "ByACXID") AND bPrevIsRack=false then URL = URL & "&LocationName=" & Nodex.Text & "&LocationID=" & Nodex.Key
				Nodex.URL = URL 
				Nodex.DHTML = " id=""e" & Nodex.Key & """ onclick=OpenFolder(" & Nodex.Key &")"
			End if			
			RS.MoveNext 
