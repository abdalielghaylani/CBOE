...
<!--Hide JavaScript
	...
	function CalculateConcentration(frmField, plateNumber){
		...
		document.form1.diluteBy.value='byVolume';
		...
		//}
			...
			var tempURL = "MolarAmount=" + MolarAmountEncoded + "&MolarUnitFK=" + MolarUnitFK + "&SolventVolume1=" + SolventVolumeAdded + "&SolventVolumeUnitID1=" + SolventVolumeAddedUnitID + "&SolventVolume2=" + CurrSolventVolume + "&SolventVolumeUnitID2=" + CurrSolventVolumeUnitID + "&ConcentrationUnitID=" + ConcentrationUnitID
			var strURL = serverType + serverName + "/cheminv/api/CalcMolarConc.asp?" + URLEncode(tempURL);	
	...
	function CalculateVolume(frmField, plateNumber){
		var errmsg = 'Error:\r';
		document.form1.diluteBy.value='byConcentration';
		//calculate molar concentration
		MolarAmount = eval("document.all.MolarAmount" + plateNumber + ".value");
		MolarAmountEncoded = eval("document.all.MolarAmountEncoded" + plateNumber + ".value");
		MolarUnitFK = eval("document.all.MolarUnitFK" + plateNumber + ".value");
		...
		//}
			...
			var strURL = serverType + serverName + "/cheminv/api/CalcVolume.asp?MolarAmount=" + MolarAmountEncoded + "&MolarUnitFK=" + MolarUnitFK + "&Concentration=" + Concentration + "&ConcentrationUnitID=" + ConcentrationUnitID + "&VolumeUnitID=" + VolumeUnitID + "&CurrSolventVolume=" + CurrSolventVolume + "&CurrSolventVolumeUnitID=" + CurrSolventVolumeUnitID;	
			strURL = URLEncode(strURL);
			var httpResponse = JsHTTPGet(strURL) 
			/*
			alert(strURL);
			...
			*/
			if (httpResponse == -1) {
				...
				errmsg = errmsg + "  - no conversion factor to Molar exists in your system.\r";
			...
			for (i=0; i<numSourcePlates; i++) {
				if (i != plateNumber) {
					...
					if (eval("document.all.SolventID" + i + ".options")) {
						...
						eval("document.all.Concentration" + i + ".value = Concentration");
						if (document.form1.diluteBy.value=='byVolume'){
						CalculateConcentration(element,i);}
						else{
						CalculateVolume(eval("document.all.Concentration" + plateNumber),i);
						}
				}
			}
		}
...
<table border="0">
	<tr>
		<td colspan="2">
			<span class="GuiFeedback"><br><br><center>Cannot dilute more than 300 plates.</center></span><br><br>
		</td>
	</tr>
	<tr>
...
<INPUT TYPE="hidden" NAME="diluteBy">
<table border="0">
	<tr>
	...
	<TABLE BORDER="0">
		...
		<!--
		<tr>
			<td align="right" width="200" nowrap>New Concentration (M):</td>
			<td>
				<input TYPE="text" CLASS="GrayedText" SIZE="10" Maxlength="50" NAME="Concentration<%=i%>" VALUE="<%%>" READONLY>
		...
		<!--
		<TR>
			<td align="right" width="200" nowrap>Solvent Volume Added (&#956;L):</td>
			<td>
				<input TYPE="text" CLASS="GrayedText" SIZE="10" Maxlength="50" NAME="SolventVolumeAdded<%=i%>" VALUE="" READONLY>
				<INPUT TYPE="hidden" NAME="SolventVolumeAddedUnitID<%=i%>" VALUE="4">
			</td>
		</TR>
		-->
		<%'end if%>
