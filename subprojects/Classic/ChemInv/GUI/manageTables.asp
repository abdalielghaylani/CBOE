...
'-- tables that should show the pk column 
showPK = "inv_owners, inv_project_job_info, inv_country, inv_custom_field_groups,inv_lpr_definition"
%>
<html>
<head>
...
if TableName <> "" then
	...
	' WJC - Prepare dbOwner and dbTableName for later use with Oracle tables
	dbTableName = ucase(dbTableName)
	posPeriod = instr(dbTableName,".")
	if (posPeriod > 0) then
	    dbOwner = left(dbTableName, posPeriod - 1)
	    dbTableName = mid(dbTableName, posPeriod + 1)
	else
	    dbOwner = "CHEMINVDB2"
	end if
	...
	if InStr(UCase(dbTableName), "VIEW") = 1 then
	    dbTrueTableName = Mid(dbTableName, InStr(dbTableName, "_") + 1)
	else
	    dbTrueTableName = dbTableName
    end if
	SQL = "SELECT * FROM user_cons_columns WHERE owner = '" & dbOwner & "' AND table_name = '" & dbTrueTableName & "'"
	...
	Cmd.Parameters.Append Cmd.CreateParameter("PTABLENAME", 200, 1, 4000, dbTrueTableName)
	...
	'-- these tables will show the pk value as a column
	if instr(ucase(showPK),dbTableName) = 0 then fieldsNotShown = fieldsNotShown & "," & pkColumnName
	'Response.Write pkColumnName
	'Response.End
	
	' get column information
	SQL ="SELECT * FROM all_tab_columns WHERE owner = '" & dbOwner & "' AND table_name = '" & dbTableName & "' ORDER BY COLUMN_ID ASC "
	...
	while not rsSchema.EOF
		...
		if dataType<>"NUMBER" then 
		    datalength = rsSchema("DATA_LENGTH")
		else
		    datalength = rsSchema("DATA_PRECISION") & ","& rsSchema("DATA_SCALE")
		end if     
		nullable= rsSchema("NULLABLE")
		'Response.Write IDColumnName & ":" & colName & ":" & dataType & "<BR>"
		if instr(IDColumnName,colName) > 0 then
		QS = QS & "&" & colName & "=" & IDValue & ":" & dataType & "|" & datalength & "|" & nullable
		else
			QS = QS & "&" & colName & "=:" & dataType & "|" & datalength & "|" & nullable
 		end if
		rsSchema.movenext
	wend		
    DataTypeString= QS
...
Function ShowDataRow(ByRef RS, fieldsNotShown)
	...
	QS = QS & "&pkColumnName=" & trim(pkColumnName)
	count=0
	ArrDataTypeString= split(DataTypeString,"&")
	theRow = "<TR><TD>"
	for each Field in RS.Fields
		if instr(fieldsNotShown,Field.Name) = 0 then
			'write address link
    	...
    	tempstring = split(ArrDataTypeString(count+4),"=")
    	if instr(IDColumnName,colName) > 0 then
		
		QS = QS & IDValue & ":" & tempstring(1)
		else
			QS = QS &  tempstring(1)
 		end if
	count= count+1	
