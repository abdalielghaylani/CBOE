...
Dim Cmd
Dim RS
if lcase(Request.Form("submit1"))="filter" then 
	Session("audGrpByUserName") = false
	Session("audGrpByTableName") = false
	Session("audGrpByAction") = false
	Session("audSortBy")=""
end if 
...
Select case Ucase(grpByDate)
	Case "H"
		DateExpression = "to_char(timestamp, '" & GetDateForat("H") & "')"
		DateFieldName = " HOUR"
	Case "D"
		DateExpression = "to_char(timestamp, '" & GetDateForat("D") & "')"
		DateFieldName = " DAY"
	Case "M"
		DateExpression = "to_char(timestamp, '" & GetDateForat("M") & "')"
		DateFieldName = " MONTH"
	Case "Y"
		DateExpression = "to_char(timestamp, '" & GetDateForat("Y") & "')"
		DateFieldName = " YEAR"
End Select
...
if app = "inv" then
	...
	if grpByUserName then sql = sql & ", lower(user_name) Modified_by"
...
elseif app = "docmanager" then
	...
	sql =	"SELECT Count(*) Num"
	if grpByTableName then sql = sql & ", lower(table_Name) ""TABLE"""
	if grpByAction then sql = sql & ", DECODE(action, 'I', 'INSERT', 'U', 'UPDATE', 'D', 'DELETE') Action"		
	if grpByUserName then sql = sql & ", lower(user_name) Modified_by"
	sql = sql & ", " & DateExpression &  DateFieldName &_
...
Function GetDateForat(grpByValue)
    DateFormat= Application("DATE_FORMAT")
     Select Case DateFormat
        ...
        Case 10    
            Date_Format = "YYYY MON DD"
     end select
     
    Select Case grpByValue 
        Case "H"
           Date_Format= Date_Format & " HHAM" 
        Case "D"
           'Date_Format= Date_Format
        Case "M"
            Date_Format= replace(replace(Date_Format,"DD",""),"  "," ")
        Case "Y"
            Date_Format = "YYYY" 
    end select
    GetDateForat = Date_Format      
End Function
%>
