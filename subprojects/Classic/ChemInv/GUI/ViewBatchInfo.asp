...
<!--Hide JavaScript
   ...
   /*function postDataFunction(sTab) {
	...
	function showContainerDetails(ContainerBarcode){
		var detailsUrl = "";
		detailsUrl = "<%=Application("SERVER_TYPE")& ServerName%>/cheminv/gui/ViewBatchContainer.asp?getData=db&hideMenu=true&ContainerID="  + ContainerBarcode ;
		window.open(detailsUrl,"ViewContainerFromBatch","toolbar=no,location=no,status=no,height=400,resizable,scrollbars=yes");
		//window.close();
	}
//-->
</script>
...
<%
	...
	SQL_userLocations = "Select l.Location_ID from inv_containers c,inv_locations l where c.location_id_fk = l.location_id and (c.batch_id_fk=" & BatchID & " or c.batch_id2_fk=" & BatchID & " or c.batch_id3_fk=" & BatchID & ")" 
	...
	SQL = "SELECT (replace(" & schemaName & ".GUIUTILS.GETBATCHUOMAMOUNTSTRING(NULL," & BatchID & "),':',' ')) AS Batch_Amount_String, " & vblf & _
			" (Select Sum(Qty_Available*concentration) From inv_containers where batch_id_fk=" & BatchID & " or batch_id2_fk=" & BatchID & " or batch_id3_fk=" & BatchID & ") AS Batch_Amount_Value, " & vblf & _
			" CONTAINER_ID," & vblf & _
			" decode(BATCH_ID_FK," & BatchID & ", BATCH_ID_FK, decode(BATCH_ID2_FK," & BatchID & ", BATCH_ID2_FK,  BATCH_ID3_FK)) as BATCH_ID_FK," & vblf 
			if Application("RegServerName") <> "NULL" then
				SQL = SQL & " rb.RegNumber as RegNumber," & vblf & _
				" rb.BatchNumber as BatchNumber," & vblf 
				SQL = SQL & " DECODE(inv_compounds.Reg_ID_FK, NULL, (inv_compounds.substance_name) ,(rb.RegName)) as SubstanceName , " & vblf
			else
				SQL = SQL & " inv_compounds.substance_name as SubstanceName," & vblf
			end if
			'" MINIMUM_STOCK_THRESHOLD," & vblf & _
			SQL = SQL & " INV_CONTAINER_BATCHES.MINIMUM_STOCK_THRESHOLD," & vblf & _
			...
			if Application("RegServerName") <> "NULL" then
				SQL = SQL & "DECODE(inv_compounds.Reg_ID_FK, NULL, NULL, (Select RegBatchID from inv_vw_reg_batches Where inv_vw_reg_batches.regid = inv_compounds.reg_id_fk AND inv_vw_reg_batches.batchnumber = inv_compounds.Batch_Number_FK)) AS Reg_Batch_ID, "
				SQL = SQL & "DECODE(inv_compounds.Reg_ID_FK, NULL, inv_Compounds.BASE64_CDX, (Select BASE64_CDX From cheminvdb2.inv_vw_reg_structures Where inv_vw_reg_structures.regid=inv_compounds.reg_id_fk)) AS Base64_CDX, "
			else
				SQL = SQL & "inv_Compounds.BASE64_CDX AS BASE64_CDX, " & vblf
			End if
			SQL = SQL & " COMPOUND_ID" & vblf & _
			" FROM inv_containers, inv_container_status, inv_compounds, inv_units, inv_container_batches"
			if Application("RegServerName") <> "NULL" then
				SQL = SQL & ", inv_vw_reg_batches rb " & vblf 
			end if
			SQL = SQL &	" WHERE inv_containers.container_status_id_fk = container_status_id" & vblf & _
			    " AND inv_containers.UNIT_OF_MEAS_ID_FK = inv_units.UNIT_ID" & vblf & _
			    " AND (inv_containers.batch_id_fk = inv_container_batches.batch_id or inv_containers.batch_id2_fk = inv_container_batches.batch_id or inv_containers.batch_id3_fk = inv_container_batches.batch_id)" & vblf & _
				" AND inv_containers.compound_id_fk = inv_compounds.compound_id(+)" & vblf & _
				" AND inv_containers.container_status_id_fk not in (6,7)" & vblf
			if Application("RegServerName") <> "NULL" then
				SQL = SQL & " AND rb.regid = inv_compounds.reg_id_fk" & vblf & _
				"AND Batch_id =" & BatchID  & vblf & _
				" AND rb.batchnumber = inv_compounds.batch_number_fk" & vblf
			end if
			SQL = SQL & " AND (batch_id_fk = " & BatchID & " or batch_id2_fk = " & BatchID & " or batch_id3_fk = " & BatchID & ")"

	'Response.Write SQL
	'Response.End
	Shapestr = "SHAPE {" & SQL & "}" & _
               " APPEND ({" & _
               " SELECT container_id, decode(BATCH_ID_FK," & BatchID & ", BATCH_ID_FK, decode(BATCH_ID2_FK," & BatchID & ", BATCH_ID2_FK,  BATCH_ID3_FK)) as batch_id_fk, parent_container_id_fk, " & vblf & _
			   ...
			   " WHERE location_id_fk = l.location_id AND l.location_id not in (1,2,3,4) AND unit_of_meas_id_fk = uom.unit_id AND unit_of_conc_id_fk = uoc.unit_id AND container_status_id_fk not in (6,7) And l.location_id= gl.location_id(+) and gl.parent_id = lp.location_id(+) AND (batch_id_fk = " & BatchID & " or batch_id2_fk = " & BatchID & " or batch_id3_fk = " & BatchID & ")}" & _
	...
	If RS.BOF and RS.EOF then
		SQL = "SELECT BATCH_ID, " & vblf & _
            " replace(" & schemaName & ".GUIUTILS.GETBATCHUOMAMOUNTSTRING(NULL," & BatchID & "),':',' ')  AS Batch_Amount_String, " & vblf & _
			...
			" BATCH_FIELD_2, " & vblf & _
			" BATCH_FIELD_3 " & vblf & _
			" FROM inv_container_batches " & vblf & _
            " WHERE batch_id = " & BatchID & vblf
		...
		'-- get batch details
        SQL =   " SELECT container_id, decode(BATCH_ID_FK," & BatchID & ", BATCH_ID_FK, decode(BATCH_ID2_FK," & BatchID & ", BATCH_ID2_FK,  BATCH_ID3_FK)) as batch_id_fk, parent_container_id_fk, " & vblf & _
                " inv_Containers.Qty_Available" & vblf & _
			    " ,uom.Unit_Abreviation UOMString" & vblf & _
			    " ,inv_Containers.Concentration" & vblf & _
			    " ,uoc.Unit_Abreviation UOCString," & vblf & _
			    " " & schemaName & ".GUIUTILS.GETLOCATIONPATH(l.location_id) AS Location_Path, " & vblf & _
			    " " & schemaName & ".GUIUTILS.GETRACKLOCATIONPATH(l.location_id) AS Rack_Path, " & vblf & _
			    " lp.collapse_child_nodes as IsRack" & vblf & _
			    " , barcode" & vblf & _
			    " , l.location_name" & vblf & _
			    " , l.location_id, " & vblf
			    if Application("RegServerName") <> "NULL" then
                    SQL = SQL & " RegNumber as RegNumber," & vblf & _
				    " BatchNumber as BatchNumber," & vblf  
					SQL = SQL & " DECODE(inv_compounds.Reg_ID_FK, NULL, (inv_compounds.substance_name) ,(inv_vw_reg_batches.RegName)) as SubstanceName , " & vblf
				else
				    SQL = SQL & " inv_compounds.substance_name as SubstanceName," & vblf
			    end if
			    SQL = SQL & " case when (select count(( select e.location_id_fk from inv_grid_element e, inv_grid_storage s, inv_locations p where e.grid_storage_id_fk = s.grid_storage_id and p.location_id = s.location_id_fk and p.collapse_child_nodes = 1 and e.location_id_fk=l.location_id)) from dual) = 0 then l.location_id else        (select p.location_id from inv_grid_element e, inv_grid_storage s, inv_locations p where e.grid_storage_id_fk = s.grid_storage_id and p.location_id = s.location_id_fk and p.collapse_child_nodes = 1 and e.location_id_fk=l.location_id) end as parent_location_id " & vblf & _
			   " FROM inv_containers, inv_locations l, inv_compounds, inv_units uom, inv_units uoc, inv_vw_grid_location gl, inv_locations lp " & vblf
			    if Application("RegServerName") <> "NULL" then
				    SQL = SQL & " ,inv_vw_reg_batches " & vblf
			    end if
			    SQL = SQL & " WHERE location_id_fk = l.location_id " & vblf & _
			    " AND l.location_id not in (1,2,3,4) " & vblf & _
			    " AND inv_containers.compound_id_fk = inv_compounds.compound_id(+) " & vblf
	    		if Application("RegServerName") <> "NULL" then
    	    		SQL = SQL & " AND inv_compounds.reg_id_fk = inv_vw_reg_batches.regid(+) " & vblf
			    end if
			    SQL = SQL & " AND unit_of_meas_id_fk = uom.unit_id " & vblf & _
			    " AND unit_of_conc_id_fk = uoc.unit_id " & vblf & _
			    " AND container_status_id_fk not in (6,7) " & vblf & _
			    " AND l.location_id= gl.location_id(+) " & vblf & _
			    " AND gl.parent_id = lp.location_id(+) " & vblf & _
			    " AND (batch_id_fk = " & BatchID & " or batch_id2_fk = " & BatchID & " or batch_id3_fk = " & BatchID & ")"

        Set rsBatches = conn.Execute(SQL)
        'Response.Write SQL
		'Response.End
	%>
	<center>
			...
			<table border="0" cellpadding="5" cellspacing="2">
				...
				<tr>
					...
					<%Call GetInvConnection()
				     ...
				     SQL =" SELECT DISPLAY_NAME FROM " & Application("CHEMINV_USERNAME") & ".INV_CONTAINER_BATCH_FIELDS a," & Application("CHEMINV_USERNAME") & ".INV_CONTAINER_BATCHES b " & vblf & _
                      "where a.batch_type_id_fk = b.batch_type(+)" & vblf & _
					...
					do while not RS.eof 
                     if count mod 2 = 0 then response.Write "<tr>"
				    response.Write ShowField(RS("DISPLAY_NAME") & ":", "BatchField" & BatchField & count+1, 15, "BatchField" & BatchField & count+1)
				    if count mod 2 = 1 then response.Write "<td></td></tr>" 
					count=count+1
					RS.MoveNext
					Loop
					%>
				...
				For each Key in custom_batch_property_fields_dict
					...
					Response.Write("<tr><td align=""right"">" & custom_batch_property_fields_dict.item(key) & ":</td><td colspan=""3"" style=""background-color:#e1e1e1;padding:3px;"">" & FieldValue & "</td><td></td></tr>")
				Next
				batchesRecordset = "rsBatches"
				%>
	<%
			...
			<table border="0" cellpadding="5" cellspacing="2">
				...
				<%Call GetInvConnection()
				     SQL =" SELECT DISPLAY_NAME FROM " & Application("CHEMINV_USERNAME") & ".INV_CONTAINER_BATCH_FIELDS a," & Application("CHEMINV_USERNAME") & ".INV_CONTAINER_BATCHES b " & vblf & _
                      ...
                      " AND b.batch_id = " & BatchID 
                    Set Cmd = GetCommand(Conn, SQL, adCmdText)
					Set rsBatchField = Cmd.Execute 
					count=0 
					if not (rsBatchField.EOF and rsBatchField.BOF) then
					do while not rsBatchField.EOF 
					    if count mod 2 = 0 then response.Write "<tr>"
				        response.Write ShowField(rsBatchField("DISPLAY_NAME") & ":", "BatchField" & BatchField & count+1, 15, "BatchField" & BatchField & count+1)
				        if count mod 2 = 1 then response.Write "<td></td></tr>"     					
					    rsBatchField.MoveNext
					    count=count+1
					Loop
					%>
				   	<%End if %>
...
<%end if%>
						...
						<tr>
							...
							while not (rsBatches.BOF or rsBatches.EOF)
								...
								if ShowLinks then
								    ...
								    Session("isBatchSearch") = "True"
									response.write("<td align=""right"">")
									if instr(userLocations,ContainerLocationID) > 0 then
										Response.Write("<a class=""menulink"" title=""View Container Details"" href=""#"" onclick=""showContainerDetails('" & rsBatches("container_id") & "')"">")
								...
								if rsBatches("Concentration") <> "" then
								    response.write("<td align=""right"">" & FormatNumber(cDbl(rsBatches("Qty_Available"))*cDbl(rsBatches("Concentration")),2) & "&nbsp;" & rsBatches("UOMString") & "</td>")
                                ...
                                if batchesRecordset = "RS" then
								    response.write("<td>" & RS("SubstanceName") & "</td>")
								elseif batchesRecordset = "rsBatches" then
								    response.write("<td>" & rsBatches("SubstanceName") & "</td>")
								end if
								response.write("</tr>")
								rsBatches.MoveNext
							wend
	...
	<tr>
	    <td align="right"><a HREF="#" onclick="if (opener.location) {opener.location.reload();} window.close(); return false;"><img SRC="/cheminv/graphics/sq_btn/close_dialog_btn.gif" border="0"></a></td><td></td>
	</tr>
</table>
