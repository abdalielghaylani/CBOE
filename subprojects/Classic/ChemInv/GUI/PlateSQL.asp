...
SQL = "SELECT /*+ ORDERED */ p.PLATE_ID AS PLATE_ID, " & _
		...
		SQL = SQL & "p.IS_PLATE_MAP, "
		SQL = SQL & "Decode(CHEMINVDB2.Authority.PlateIsAuthorized(p.PLATE_ID),NULL,0,CHEMINVDB2.Authority.PlateIsAuthorized(p.PLATE_ID)) as isAuthorised, " &_
		"CHEMINVDB2.AUTHORITY.GETOWNERSHIP(p.PRINCIPAL_ID_FK) AS Ownership_Name, " &_
		"p.Principal_ID_FK "
		else
		 SQL = SQL & "p.IS_PLATE_MAP "
		end if
		SQL = SQL & "FROM " & schemaName & ".INV_PLATES p " & _
