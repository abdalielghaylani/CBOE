...
Else
	...
	if len(AddContainerID) > 0 and not (myDict.Exists(clng(AddContainerID))) then
		bLoad = false
	...
	elseif len(AddContainerID) > 0 and myDict.Exists(clng(AddContainerID)) then
		selectChckBox = selectChckBox & "," & AddContainerID
	End if
	RmvContainerID = Request("RmvContainerID")
	if len(RmvContainerID) > 0 then
		bLoad = false
		if myDict.Exists(clng(RmvContainerID)) then	myDict.Remove(clng(RmvContainerID))
	End if
End if
				...
				While (Not RS.EOF)
					...
					if( CInt(ContainerStatus) <> CInt(Application("StatusApproved")) ) then
					    ...
					    if( ScannedContainerID <> "" ) then
					        if( CLng(ContainerID) = CLng(ScannedContainerID) ) then
