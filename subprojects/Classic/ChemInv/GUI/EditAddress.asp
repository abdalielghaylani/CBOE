...
if not bNewTableEntry then
	...
	if TableName="inv_suppliers" then
	  SQL = "SELECT SUPPLIER_NAME FROM INV_SUPPLIERS WHERE supplier_id = ?"
	  Set Cmd = GetCommand(Conn, SQL, adCmdText)	
	  Cmd.Parameters.Append Cmd.CreateParameter("SupplierID", 5, 1, 0, TablePKID)
	  Set RS = Cmd.Execute
	  instructions = actionText & " the address for supplier " & RS("SUPPLIER_NAME")
	  Set RS = Nothing 
	elseif TableName="inv_locations" then
	  SQL = "SELECT LOCATION_NAME FROM INV_LOCATIONS WHERE location_id = ?"
	  Set Cmd = GetCommand(Conn, SQL, adCmdText)
	  Cmd.Parameters.Append Cmd.CreateParameter("LocationID", 131, 1, 0, TablePKID)
	  Set RS = Cmd.Execute
...
<!--Hide JavaScript
    ...
    // begin: SM fix CSBR-68471
    function FillState(){
        if(document.form1.CountryIDFK[document.form1.CountryIDFK.selectedIndex].value!=1000)
        {
	        var i;
	        var tempvar; // this variable is storing index of 'N/A'
	        for(i=0;i<document.form1.StateIDFK.length-1;i++){
		        if(document.form1.StateIDFK[i].value==""){
		        tempvar=i;
		        break;
		        }
	        }
	        document.form1.StateIDFK.selectedIndex=tempvar;
        }
    }
// end: SM fix CSBR-68471
...
<table border="0" cellspacing="0" cellpadding="0" width="500">
	...
	<tr height="25">	
		<%=ShowPickList2("State/Province:", "StateIDFK", StateIDFK,"SELECT State_ID AS Value, State_Abbreviation AS DisplayText FROM inv_States ORDER BY lower(DisplayText) ASC","200","N/A","","")%>
	</tr>
	<tr height="25">	
        ...
        <%=ShowPickList2("Country:", "CountryIDFK", CountryIDFK,"SELECT Country_ID AS Value, Country_Name AS DisplayText FROM inv_country ORDER BY lower(DisplayText) ASC","200","","","FillState();") %>
