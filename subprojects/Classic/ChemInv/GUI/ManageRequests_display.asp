...
'Cmd.Parameters.Append Cmd.CreateParameter("PUSERID",200, 1, 30, UserID)    'Fixing Bug CSBR-73905
Cmd.Parameters.Append Cmd.CreateParameter("PUSERID",200, 1, 30, NULL)       'Fixing Bug CSBR-73905
...
Cmd.Parameters.Append Cmd.CreateParameter("PSHIPTONAME",200, 1, 255, Null) 'Fixing Bug CSBR-73905
Cmd.Parameters.Append Cmd.CreateParameter("PCREATOR",200, 1, 255, UserID)'Fixing Bug CSBR-73905 and CSBR-78135
...
%>
	...
	<tr>
		...
		<!--CSBR ID:126702 : sjacob : Comments : Adding Request ID to be displayed in the page -->
		<!--Start of change-->
		<th>
			RequestID
		</th>
...
<%
	...
	Else
		While (Not RS.EOF)
			...
			'RequestUserID = RS("RUserID") 'Fixing Bug CSBR-73905
			RequestUserID = RS("Creator") 'Fixing Bug CSBR-73905
			...
			editData = "ContainerID=" & ContainerID & "&ContainerName=" & ContainerName & "&UOMAbv=" & UOMAbv &  "&RequestID=" & RequestID & "&allowContainerRequest=1&allowSampleRequest=0"
			Comments = RS("request_comments")	
%>
			<tr>
				<%if Ucase(RequestType) = "PENDING" then%>
				<td align="center"> 
					<a Class="MenuLink" Href="#" onclick="OpenDialog('/Cheminv/GUI/RequestSample.asp?action=edit&amp;<%=editdata%>', 'Diag', 1); return false">Edit</a>
				</td>
				<%else%>
				<td align="center"> 
					<a Class="MenuLink" Href="#" onclick="OpenDialog('/Cheminv/GUI/RequestSample.asp?action=undodelivery&amp;<%=editdata%>', 'Diag', 1); return false">Undo</a>
				</td>
				<%end if%>
				<!--CSBR ID:126702 : sjacob : Comments : Adding Request ID to be displayed in the page-->
				<!--Start of change-->
...
End if
%>
<!--CSBR ID : 126701 : sjacob : Comments : Hiding the OK button in the case of 'Closed Requests'--> 
<%If Ucase(RequestType)<>"CLOSED" Then %>
<table>
	<tr>
