...
Dim Rows
Dim Cols
Dim bDecrementParents

bDebug = false
bFileTemplateError = false
bFileParseError = false
bDecrementParents = true	' ToDo: hook this up to a check box on the CreatePlatesFromTextFile.asp form
...
Set oParentPlateIDList = server.CreateObject("scripting.dictionary")
InvServerName = Application("InvServerName")

'-- set the script timeout for this page to 30 hours
server.ScriptTimeout = (30 * 60 * 60)
...
Ownership=oUpload.Item("PrincipalID").value.string
end if
		...
		if ubound(arrLogData) > 0 then
			...
			if bCreateNewPlate then
				...
				if instr(ProblemBarcodes,currPlateBarcode) = 0 then
					...
					rsWell.MoveFirst
					sParentPlateIDList = ""
  					for each key in oParentPlateIDList
  						sParentPlateIDList = sParentPlateIDList & oParentPlateIDList(key) & ","
  					next
  					oPlateNode.setAttribute "PARENT_PLATE_ID_LIST", sParentPlateIDList
			...
			if bNewPlate  then
				...
				With oPlates
				    ...
				    With oRootNode
						...
						.setAttribute "DECREMENTPARENTS", CStr(bDecrementParents)
				...
				With oPlateNode
					...
					.setAttribute "PRINCIPAL_ID_FK", Ownership  
			...
			if ActionType = "target" then
				...
				if instr(ProblemBarcodes,TargetPlateBarcode) = 0 then
					...
					if currSourcePlate <> SourcePlateBarcode then
						SQL =   "SELECT iwc.compound_id_fk, iwc.reg_id_fk, iwc.batch_number_fk, iw.well_id, igp.sort_order, iw.solution_volume, iw.qty_remaining, iw.qty_unit_fk, iw.solvent_volume, iw.solvent_volume_unit_id_fk, iw.solvent_id_fk, iw.concentration, iw.conc_unit_fk, ip.plate_id " &_
					...
					if instr(ProblemBarcodes, currSourcePlate) = 0 then
						...
						SourcePlateID = rsSourceWell("plate_id")
  						if not oParentPlateIDList.Exists(SourcePlateID) then
  								oParentPlateIDList.Add SourcePlateID, SourcePlateID
  						end if
			...
			' Add the appropriate compound information to the XML for source plates
			...
			end if		' if ActionType = "source" then
						...
						if bUseWellCoordinates then
							...
							if ActionType = "target" then
								if TargetBarcode <> currTargetBarcode then
									...
									'targetRows = Rows
									'targetCols = Cols
								'-- SM- Fixing CARA issue (CSBR-98481)
								elseif currSourceBarcode <> SourceBarcode then 
								   arrSourceRowsCols = split(GetNumRowsCols(SourceBarcode),",")
									sourceRows = arrSourceRowsCols(0)
									sourceCols = arrSourceRowsCols(1)
								end if
...
Sub WriteCSFormatRow(ByRef oStream, SourceBarcode, SourceWellNum, TargetBarcode, TargetWellNum, QtyInitial, QtyUnit, Concentration, ConcUnit, SolventID, PField1, PField2, PField3, PField4, PField5, PDate1, PDate2, WField1, WField2, WField3, WField4, WField5, WDate1, WDate2, SolventVol, SolventVolUnit, SolutionVol, CompoundID, RegID, RegNumber, BatchNumber, SolventDilutionVol)
		...
		if (not isNull(RegID)) or (RegID <> "") then 
			...
			if (not isNull(RegNumber)) and (RegNumber <> "") then
			    'CSBR ID : 137445 SJ
			    'Comments : Replaced the table reg_numbers with the view VW_REGISTRYNUMBER and prefixed the database name - REGDB to the view name.
				SQL = "SELECT COUNT(*) as theCount, regid AS reg_id FROM REGDB.VW_REGISTRYNUMBER WHERE regnumber = ? GROUP BY regid"
				Set Cmd = GetCommand(Conn, SQL, adCmdText)
