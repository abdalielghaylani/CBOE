...
<% if URLID <> "" and Lcase(Request("action")) <> "delete" then %>
	<tr>
		...
		<td>
			<input Name="URLCurr" TYPE="text" SIZE="50" Maxlength="255" VALUE="<%=URLhref%>" class="readOnly" readonly>
		</td>
	</tr>
...
<% else %>
			<input Name="URLhrefInput" TYPE="text" SIZE="50" Maxlength="255" VALUE="" <%=disabled%>><br />
			<font size="1">Please use a valid web ( http://url/ or https://url/), ftp ( ftp://url/ ),<BR> email ( mailto: ) or server path protocal ( \\servername\filename\ ).</font>
<% end if %>
		</td>
	</tr>
	...
	<tr>
		<%=ShowPickList("<span class=""required"">Document Type:</span>", "URLType", URLType,"Select type_name as Value, type_name as DisplayText from inv_doc_types order by DisplayText")%>
	</tr>
	<% else %>
	<tr>
