...
if Application("ENABLE_OWNERSHIP")="TRUE" AND isPublic=1 then 
	QueryString =  "ContainerIDs=" & ContainerIDList
	QueryString = QueryString & "&ValuePairs=principal_id_fk%3d" & PrincipalID &  Credentials
	httpResponse1 = CShttpRequest2("POST", ServerName, "/cheminv/api/UpdateContainer2.asp", "ChemInv", QueryString)
end if 
'Response.End
	...
	<tr>
		<td align="center">
			...
			if (httpResponse = LocationID) then
				...
				' SMathur- CSBR-142735- Changing call form isRack location to CheckRackLocation as with a comma in LocationID was causing issues.
				if instr(LocationID,",") > 0 or CheckRackLocation(LocationID) then 
			...
			Else
				...
				Response.end
			End if
			' SMathur- CSBR-142735- Adding a new function for handling a comma in Location ID.
			Function CheckRackLocation(LocationIdString)			    
		        if instr(LocationIdString,",") > 0 then
			        arrLocID = split(LocationIdString,",")
			        For Each location In arrLocID
			            IsRacLoc=isRackLocation(location)
			            If IsRacLoc=0 Then
			                Exit For
			            End If			            
			        Next			        
			    else
		            IsRacLoc=isRackLocation(LocationIdString)		            
		        End If
		        CheckRackLocation=IsRacLoc
			End Function			
			%>
		</td>
	</tr>
