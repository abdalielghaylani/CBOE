...
FormData = replace(Request.Form,"&","<br>") & Credentials
...
For each field in Request.Form
	if InStr("iContainerIDs,multiscan,multiSelect,fieldList", field) = 0 then
		...
		Select case lcase(field)
			case "icompound_id_fk"
				if Request("iRegID") <> "" and Request("iBatchNumber") <> "" then
					ValuePairs = ValuePairs & "reg_id_fk%3D" & Request("iRegID") & "::" & "batch_number_fk%3D" & Request("iBatchNumber") & "::compound_id_fk%3DNULL" & "::"
				else
					ValuePairs = ValuePairs & mid(field,2) & "%3D" & Request(field) & "::" & "reg_id_fk%3DNULL::batch_number_fk%3DNULL" & "::"
				end if
			case "ilocation_id_fk","iqty_max","iqty_initial","iqty_remaining","iqty_minstock","iqty_maxstock","icontainer_type_id_fk","ipurity","iconcentration","iweight","itare_weight","ifinal_wght","inet_wght","iqty_available","iphysical_state_id_fk","isupplier_id_fk","icontainer_status_id_fk","icontainer_cost","idef_location_id_fk","idensity","ipo_line_number","isolvent_id_fk"
				ValuePairs = ValuePairs & mid(field,2) & "%3D" & Request(field) & "::"
			case "iowner_id_fk","ireceived_by_id_fk","iordered_by_id_fk","icurrent_user_id_fk","icontainer_name","igrade","icontainer_comments","ilot_num","isupplier_catnum","ipo_number","ireq_number","ifield_1","ifield_2","ifield_3","ifield_4","ifield_5","ifield_6","ifield_7","ifield_8","ifield_9","ifield_10"
			...
			case "iunit_of_wght_id_fk","iunit_of_conc_id_fk","iunit_of_purity_id_fk","iunit_of_density_id_fk","iunit_of_cost_id_fk","iunit_of_meas_id_fk"
				fieldVal = left(Request(field),(Instr(Request(field),"=")-1))
				ValuePairs = ValuePairs & mid(field,2) & "%3D" & fieldVal & "::"
		end select
	end if
next
