...
Ownership=Server.HTMLEncode(oUpload.Item("PrincipalID").value.string)
end if
' Grab the file name
...
if fs.FileExists(sPath) then
	sError = "File name already exists. Please rename the file and try again."
...
cnExcel.Open "Driver={Microsoft Excel Driver (*.xls, *.xlsx, *.xlsm, *.xlsb)};DBQ=" & sPath
On Error GoTo 0
if cnExcel.State = 0 then
fs.DeleteFile(sPath)
	sError = "The uploaded Excel file is not a valid Microsoft Excel file and hence deleted it. You may try uploading with a valid Excel file."
	Response.redirect "CreatePlatesFromExcel.asp?errmsg=" & sError
end if
set fs=nothing
...
If rsData.RecordCount > 0 Then
	...
	if statusCol="" or isnull(statusCol) or isempty(statusCol) then 
 ...
 if not bError then
	...
	'Response.End
		...
		for each oNode in nlTransforms
			...
			if barcodes<>"" and not isnull(barcodes) and numCopies<>"" then 
			    ...
			    if numBarcodes <> cInt(numCopies) then
				    ...
				    QueryString = "Barcodes=" & barcodes & "&BarcodeType=plate" & Credentials
                ...
                if numCopies="" or isnull(numCopies) then 
                    sError = sError & "Row " & rowNum & " could not be processed because the number of copies is null.<BR><BR>"
				    bError = true
				end if    
				if barcodes="" or isNULL(barcodes)then 
                    sError = sError & "Row " & rowNum & " could not be processed because the barcodes is null.<BR><BR>"
				    bError = true
				end if    
			end if 		
			...
			if RS.BOF or RS.EOF then
				...
				bStatusError = true
				statusID=5 	'5 is enum_id for enum_value "unknown". 
			else
			...
			if bError or bLibError or bStatusError then 
			    errCount = errCount + 1		
			    bLibError=false
			    bError=false
			    bStatusError=false
...
<%
			If isNumeric(arrNewPlateIDs(0)) then
				if Cdbl(arrNewPlateIDs(0)) >= 0 and bError = false then
					theAction = "Exit"					
				Else
...
<%else
    Response.Write "<center><SPAN class=""GuiFeedback"">" & sError & "</SPAN></center>"
	Response.Write "<P><center><a HREF=""3"" onclick=""history.go(-2); return false;""><img SRC=""../graphics/ok_dialog_btn.gif"" border=""0""></a></center>"						
end if %>
</Body>			
