...
if Request("RequestTypeID") = "2" and (action="edit" or action="create") and not RequestSampleByAmount then
		...
		<td height="50" valign="middle" nowrap>
			...
			If IsNumeric(httpresponse) then 
				If Clng(httpResponse) > 0 then
					...
					Response.Write "<center><SPAN class=""GuiFeedback"">Request has been processed.</SPAN><center><br/>"
					Response.write "Please note your RequestID: " & httpResponse & "</center>"
					containerId = Request("containerID")
					if containerId = "" then containerId = 0
					Response.Write "<P><center><a HREF=""Ok"" onclick=""if(typeof(parent.CloseModal) == 'function'){parent.CloseModal(false);} if ((opener.NewRegWindowHandle) && typeof (opener.NewRegWindowHandle.refreshRecordDetails) != 'undefined') {opener.NewRegWindowHandle.refreshRecordDetails(); } else if (opener.cows_input_form){opener.location.reload();}else{SelectLocationNode(0, " & Request("cLocationID") & ", 0, '" & Session("TreeViewOpenNodes1") & "'," & containerId & "); }opener.focus(); window.close(); return false;""><img SRC=""/cheminv/graphics/sq_btn/ok_dialog_btn.gif"" border=""0"" title=""Close dialog window""></a></center>"
					...
					Response.Write "<P><center><a HREF=""Ok"" onclick=""if(typeof(parent.CloseModal) == 'function'){parent.CloseModal(false);} opener.focus();window.close(); return false;""><img SRC=""../graphics/ok_dialog_btn.gif"" border=""0"" title=""Close dialog window""></a></center>"		
				End if
			Else
