...
<%else%>
	<tr>
		...
		<% if action = "create" then Response.Write( "New" ) %> Organization Details:</span><br><br>
		</td>
	</tr>
	<tr>
		<td align="left">
	...
	<tr>
		...
		<%=ShowPickList3("", "selectedUsers", UserID, "SELECT User_ID AS Value, Last_Name||DECODE(First_Name, NULL, '', ', '||First_Name) AS DisplayText, '' as DefaultValue FROM CS_SECURITY.People Where User_ID in (SELECT USER_ID_FK FROM INV_ORG_USERS Where Org_Unit_ID_FK=" & OrgUnitID & ") ORDER BY lower(Last_Name) ASC", 55, "", "", "", 12, true, "No Members")%>
		</tr>
		</table>
		</td>
	</tr>
