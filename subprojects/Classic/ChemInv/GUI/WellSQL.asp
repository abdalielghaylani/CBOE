...
FlatWellSQL = "SELECT w.WELL_ID AS WELL_ID, " & _
		...
		"p.plate_barcode AS PLATE_BARCODE, " & _
		"p.location_id_fk AS LOCATION_ID_FK, " & _
		"DECODE(cpd.Reg_ID_FK, NULL,wc.compound_id_fk,NULL) AS COMPOUND_ID_FK, " & _
		"cpd.CAS AS CAS, " & vblf & _
		"cpd.ACX_ID AS ACX_ID, " & vblf & vblf
		if Application("RegServerName") <> "NULL" then
			FlatWellSQL = FlatWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (Select RegBatchID FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Reg_Batch_ID, "
			FlatWellSQL = FlatWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (Select RegNumber FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Reg_Number, "
			FlatWellSQL = FlatWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT BASE64_CDX AS REG_BASE64_CDX FROM cheminvdb2.inv_vw_reg_structures ivrs WHERE ivrs.regid = cpd.reg_id_fk)) AS Base64_CDX, "
			FlatWellSQL = FlatWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, cpd.Substance_Name, (Select RegName FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Substance_Name, "
		else
			FlatWellSQL = FlatWellSQL & "cpd.BASE64_CDX AS BASE64_CDX, " & vblf
			FlatWellSQL = FlatWellSQL & "cpd.Substance_Name AS Substance_Name, " & vblf 

	    End if
	    FlatWellSQL = FlatWellSQL & "cpd.Reg_ID_FK,  " & vblf & _
		"cpd.Batch_Number_FK,  " & vblf & _
		"(SELECT count(*) FROM inv_well_compounds WHERE well_id_fk = w.well_id) as CompoundCount, " & _
		"(SELECT count(*) FROM inv_well_parent WHERE child_well_id_fk = w.well_id) as ParentCount " & _
		"FROM inv_wells w " & _
...
WellCompoundSQL = "SELECT " & _
		...
		"cpd.ACX_ID AS ACX_ID, " & vblf & vblf
		if Application("RegServerName") <> "NULL" then
			WellCompoundSQL = WellCompoundSQL & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (Select RegBatchID FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Reg_Batch_ID, "
			WellCompoundSQL = WellCompoundSQL & "DECODE(cpd.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT BASE64_CDX AS REG_BASE64_CDX FROM cheminvdb2.inv_vw_reg_structures ivrs WHERE ivrs.regid = cpd.reg_id_fk)) AS Base64_CDX, "
			WellCompoundSQL = WellCompoundSQL & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (Select RegNumber FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Reg_Number, "
			WellCompoundSQL = WellCompoundSQL & "DECODE(cpd.Reg_ID_FK, NULL, cpd.Substance_Name, (Select RegName FROM cheminvdb2.inv_vw_reg_batches ivrg WHERE ivrg.regid = cpd.reg_id_fk AND ivrg.batchnumber= cpd.Batch_Number_FK)) AS Substance_Name, "
		else
			WellCompoundSQL = WellCompoundSQL & "cpd.BASE64_CDX AS BASE64_CDX, " & vblf 
			WellCompoundSQL = WellCompoundSQL & "cpd.Substance_Name AS Substance_Name, " & vblf 
	    End if

	    WellCompoundSQL = WellCompoundSQL & "cpd.Reg_ID_FK,  " & vblf & _
		"cpd.Batch_Number_FK  " & vblf & _
		"FROM " & _
		" INV_WELL_COMPOUNDS WC " & _
		", inv_compounds cpd " & _
...
WellCompoundSQL2 = "SELECT " & _
		...
		"cpd.ACX_ID AS ACX_ID, " & vblf & vblf
		if Application("RegServerName") <> "NULL" then
			WellCompoundSQL2 = WellCompoundSQL2 & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (Select RegBatchID From inv_vw_reg_batches ivrb WHERE ivrb.regid = cpd.reg_id_fk AND ivrb.batchnumber= cpd.Batch_Number_FK)) AS Reg_Batch_ID, "
	    End if
	    WellCompoundSQL2 = WellCompoundSQL2 & "cpd.Reg_ID_FK,  " & vblf & _
		"cpd.Batch_Number_FK  " & vblf & _
		"FROM " & _
		" INV_WELL_COMPOUNDS WC " & _
		", inv_compounds cpd " & _
	    ...
	    '"cpd.Substance_Name AS Substance_Name, " & vblf & _
		'if Application("RegServerName") <> "NULL" then
		'	ParentWellSQL = ParentWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, NULL, (SELECT reg_numbers.reg_number||'-'||batches.batch_Number FROM reg_numbers, batches WHERE reg_numbers.reg_id = batches.reg_internal_id AND reg_numbers.reg_id= cpd.reg_id_fk AND batches.batch_number= w.Batch_Number_FK)) AS Reg_Batch_ID, "
		'	ParentWellSQL = ParentWellSQL & "DECODE(cpd.Reg_ID_FK, NULL, cpd.BASE64_CDX, (SELECT Structures.BASE64_CDX AS REG_BASE64_CDX FROM structures, batches, reg_numbers WHERE reg_numbers.reg_id = batches.reg_internal_id AND structures.CPD_Internal_ID = batches.CPD_Internal_ID AND reg_numbers.reg_id=cpd.reg_id_fk AND batches.batch_number=cpd.Batch_Number_FK)) AS Base64_CDX, "
		'else
		'	ParentWellSQL = ParentWellSQL & "cpd.BASE64_CDX AS BASE64_CDX, " & vblf
	    'End if
	    'ParentWellSQL = ParentWellSQL & "cpd.Reg_ID_FK,  " & vblf & _
		'"w.Batch_Number_FK,  " & vblf & _
		'", inv_vw_compounds cpd " & _
