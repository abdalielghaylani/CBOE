...
ELSE
      ...
      SELECT distinct request_id,
        ...
        -- Addition ends for CSBR-121935
				ivrb1.REGNOTEBOOK,
									...
									ivrb.REGID,
									ivrb.ROOT_NUMBER
			FROM inv_requests r, inv_locations l, &&securitySchemaName..people p1,&&securitySchemaName..people p2, inv_container_types ct, inv_request_status rs, inv_container_batches icb ,inv_vw_reg_batches ivrb, inv_vw_reg_batches ivrb1,inv_containers c, inv_compounds cmpd
      WHERE	r.user_id_fk = p1.user_id(+)
      AND   r.batch_id_fk = icb.batch_id(+)
      ...
      AND (ivrb1.regid(+)=batch.GetBatchRegBatchvalues(request_id,'REG_ID_FK') and ivrb1.BATCHNUMBER(+)=batch.GetBatchRegBatchvalues(request_id,'BATCH_NUMBER_FK'))
      AND ivrb.regid(+)=batch.GetBatchRegBatchvalues(request_id,'REG_ID_FK') 
      AND 	r.request_status_id_fk = rs.request_status_id
      AND	Upper(r.delivered_by_id_fk) = p2.user_id(+)
      AND	r.delivery_location_id_fk = l.location_id
      AND   r.container_type_id_fk = ct.container_type_id(+)
...
END IF;
  ...
  my_sql := my_sql || 'cb.batch_status_id_fk as BatchStatus,
                  ...
                  r.DATE_1,
                  r.DATE_2
                  FROM inv_requests r, inv_locations l, &&securitySchemaName..people p1, &&securitySchemaName..people p2, inv_container_batches cb, inv_request_status rs,inv_vw_reg_batches ivrb1,inv_vw_reg_batches ivrb';
