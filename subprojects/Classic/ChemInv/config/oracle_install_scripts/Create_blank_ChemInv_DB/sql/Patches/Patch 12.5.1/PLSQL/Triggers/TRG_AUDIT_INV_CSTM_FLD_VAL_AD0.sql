...
declare
  ...
  old_customfieldname inv_custom_fields.custom_field_name%TYPE;

begin
  select seq_audit.nextval into raid from dual;
  select rid into compound_rid from inv_compounds where compound_id = :old.COMPOUND_ID_FK;
  select custom_field_name into old_customfieldname from inv_custom_fields where custom_field_id = :old.CUSTOM_FIELD_ID_FK;
  ...
  audit_trail.check_val(raid, 'CUSTOM_FIELD_NAME', null, old_customfieldname);
