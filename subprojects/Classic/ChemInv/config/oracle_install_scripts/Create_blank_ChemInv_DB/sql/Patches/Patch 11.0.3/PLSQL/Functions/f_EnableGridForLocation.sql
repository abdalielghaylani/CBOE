create or replace function "&&SchemaName"."ENABLEGRIDFORLOCATION"
...
begin
	...
	from inv_locations where location_id = p_location_id;
        -- get Ownership
        select Principal_ID_Fk into lPrincipal_ID_FK
	from inv_locations where location_id = p_location_id;
  ...
  if Length(l_NameDelimeter) = 0 then
	...
	for grid_positions_cur in grid_positions loop
		...
		(parent_id, location_name, location_type_id_fk,Principal_ID_FK)
		values
		(p_location_id, parent_location_name || l_NameDelimeter || grid_positions_cur.name, 0,lPrincipal_ID_FK)
