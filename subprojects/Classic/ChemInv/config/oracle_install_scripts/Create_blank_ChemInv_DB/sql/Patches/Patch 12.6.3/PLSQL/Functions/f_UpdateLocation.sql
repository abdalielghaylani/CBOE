...
(
 ...
 pPropagateownership IN NUMBER:=NULL,
...
CURSOR dupName_cur(LocationName_in in Inv_Locations.Location_Name%Type) IS
  SELECT Location_ID FROM Inv_Locations WHERE UPPER(Location_Name) = UPPER(LocationName_in)
  AND Parent_ID IN
...
--CSBR:136437
if  pParentId is Null OR pIspublic=1 then
...
CLOSE dupName_cur;
  ...
  if pPrincipal_ID_FK is null AND pPropagateownership<>-1 then
...
SET
  ...
  Location_Name = convert(pName,'WE8ISO8859P1','utf8'),
  Parent_ID = l_parentID,
  Location_Description = pDesc,
  Location_Type_ID_FK = lLocationTypeID,
  Owner_ID_FK = pLocationOwner,
  Principal_ID_Fk=pPrincipal_ID_FK,
  ispublic=pIspublic
WHERE
...
-- propagate ownership to children
IF pPropagateownership=1 Then
  Update Inv_containers SET  Principal_ID_FK=pPrincipal_ID_FK where LOCATION_ID_FK=pLocationId;
  Update inv_plates    SET  Principal_ID_FK=pPrincipal_ID_FK where LOCATION_ID_FK=pLocationId;
  PROPAGATEGROUPOWNER2LOCATIONS(pLocationId, pPrincipal_ID_FK);
  PROPAGATEGROUPOWNER2CONTAINERS(pLocationId);
  PROPAGATEGROUPOWNER2PLATES(pLocationId);
END IF;
RETURN pLocationID;
exception
