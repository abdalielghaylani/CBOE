...
PACKAGE "COMPOUNDS"
   ...
   FUNCTION CREATESYNONYM
	...
	-- Registers a molucule passed in as a clob
    	...
    	pHBondDonors in inv_compounds.hbond_donors%TYPE,
			...
			pConflictingFields in inv_compounds.conflicting_fields%type,
                        pLocationType in inv_compounds.LOCATION_TYPE_ID_FK%type) Return inv_compounds.compound_id%type;
    ...
    pHBondDonors in inv_compounds.hbond_donors%TYPE,
		...
		pStructure in inv_compounds.base64_cdx%type,
                pLocationType in inv_compounds.LOCATION_TYPE_ID_FK%type,
   ...
   FUNCTION DELETESUBSTANCE (pCompoundID in Inv_compounds.compound_ID%Type)
	...
	Function UpdateInvCompound(
    	...
    	pHBondDonors in inv_compounds.hbond_donors%TYPE,
			...
			pCompoundID in inv_compounds.compound_id%type,
			pConflictingFields in inv_compounds.conflicting_fields%type,
                        pLocationType in inv_compounds.LOCATION_TYPE_ID_FK%type) Return inv_compounds.compound_id%type AS
