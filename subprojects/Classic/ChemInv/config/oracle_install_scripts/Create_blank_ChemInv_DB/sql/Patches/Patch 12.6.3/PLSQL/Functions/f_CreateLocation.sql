...
CURSOR dupName_cur(LocationName_in in Inv_Locations.Location_Name%Type) IS
  SELECT Location_ID FROM Inv_Locations WHERE UPPER(inv_Locations.Location_Name) = LocationName_in AND inv_Locations.Parent_ID = pParentID;
...
CLOSE dupName_cur;
begin
l_pname:= convert(pName,'WE8ISO8859P1','utf8');
exception
when others then
l_pname:=pName;
end;
