CREATE OR REPLACE FUNCTION "ORDERCONTAINER"
 (
  ...
  pLocationType IN inv_containers.Location_Type_ID_FK%TYPE:=NULL
  )
...
vNumCopies int;
v_compound_id inv_compounds.compound_id%TYPE;
...
-- Get the actual compound_id
if pRegID is not null and pBatchNumber is not null then
	v_compound_id := CREATEREGCOMPOUND( pRegID, pBatchNumber );
else
	v_compound_id := pCompoundID;
end if;
...
*/
  ...
  VALUES (
    ...
    IF pLocationType = 0 and v_compound_id is not null Then
                select LOCATION_TYPE_ID_FK into vLocationType from inv_Compounds where Compound_ID=(select Compound_ID_FK from Inv_Containers where Container_ID=NewContainerID);
                update Inv_Containers set LOCATION_TYPE_ID_FK=vLocationType where Container_ID=NewContainerID;
                else
                update Inv_Containers set LOCATION_TYPE_ID_FK=pLocationType where Container_ID=NewContainerID;
                End IF;
