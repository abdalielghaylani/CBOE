...
DECLARE
  raid number(10);
  l_RegIDFK inv_compounds.reg_id_fk%TYPE;
  l_BatchNumberFK inv_compounds.batch_number_fk%TYPE;
...
BEGIN
  ...
  --audit_trail.check_val(raid, 'TIMESTAMP', :new.TIMESTAMP, :old.TIMESTAMP);
	...
	-- Take this opportunity to update the corresponding compound record if it's a Reg compound
	-- TSM 06/03/08: CAS and substance name global searches return duplicate hits against both Inv and Reg.
	-- Pulling this update sequence until a feasible way to add the necessary where clauses to each
	-- different database/table is found.
	--if :new.COMPOUND_ID_FK is not null then
	if false then
		select reg_id_fk , batch_number_fk into l_RegIDFK, l_BatchNumberFK
		from inv_compounds
		where inv_compounds.compound_id = :new.COMPOUND_ID_FK;

		if l_RegIDFK is not null and l_BatchNumberFK is not null then
			l_TempCompoundID := UpdateRegCompound(l_RegIDFK,l_BatchNumberFK);
		end if;
	end if;
