...
*/
    ...
    vDistinctValues integer;
    vFirstUnitValue integer;
    vTempCount integer;
	BEGIN
		vPlateIDList_t := STRINGUTILS.split(pPlateIDList, ',');
		FOR i in vPlateIDList_t.First..vPlateIDList_t.Last
		LOOP
		    ...
		    IF vCount = 1 THEN
				...
				SELECT count(distinct round(qty_remaining,3)) INTO vDistinctValues FROM inv_wells WHERE qty_remaining is not null AND qty_unit_fk is not null AND plate_id_fk = vPlateIDList_t(i);
				IF vDistinctValues >1 then
					vAvgQtyRemaining := null;
					vAvgQtyRemainingUnit := null;
				END IF;
			ELSE
				SELECT count(*) INTO vTempCount from inv_wells WHERE qty_unit_fk is not null and plate_id_fk = vPlateIDList_t(i) and rownum=1;	
				IF vTempCount >0 THEN
					SELECT qty_unit_fk INTO vFirstUnitValue from inv_wells WHERE qty_unit_fk is not null and plate_id_fk = vPlateIDList_t(i)  and rownum=1;			
				ELSE
					vFirstUnitValue := Null;
				END IF;
				IF vFirstUnitValue is not null THEN 
					SELECT count (distinct (round(chemcalcs.convert(qty_remaining,qty_unit_fk,vFirstUnitValue),3))) INTO vDistinctValues FROM inv_wells WHERE qty_remaining is not null AND qty_unit_fk is not null and plate_id_fk = vPlateIDList_t(i); 
				ELSE
					vDistinctValues := null;
				END IF;
				IF vDistinctValues> 1 or vDistinctValues is null THEN 
					vAvgQtyRemaining := null;
					vAvgQtyRemainingUnit := null;
				ELSE
					vIsFirst := true;
	    ...
	    IF vCount = 1 THEN
				...
				SELECT count(distinct round(solvent_volume,3)) INTO vDistinctValues FROM inv_wells WHERE solvent_volume is not null AND solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i);
				IF vDistinctValues >1 THEN
					vAvgSolventVolume := null;
					vAvgSolventVolumeUnit := null;
				END IF;
			ELSE
				SELECT count(*) INTO vTempCount from inv_wells WHERE solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i)  and rownum=1;	
				IF vTempCount >0 THEN
					SELECT solvent_volume_unit_id_fk INTO vFirstUnitValue from inv_wells WHERE solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i)  and rownum=1;			
				ELSE
					vFirstUnitValue := Null;
				END IF;
				IF vFirstUnitValue is not null THEN 
					SELECT count (distinct (round(chemcalcs.convert(solvent_volume,solvent_volume_unit_id_fk,vFirstUnitValue),3))) INTO vDistinctValues FROM inv_wells WHERE solvent_volume is not null AND solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i); 
				ELSE
					vDistinctValues := null;
				END IF;
				IF vDistinctValues> 1 or vDistinctValues is null THEN 
	    ...
	    IF vCount = 1 THEN
				...
				SELECT count(distinct round(solution_volume,3)) INTO vDistinctValues FROM inv_wells WHERE solution_volume is not null AND solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i);
				IF vDistinctValues >1 THEN
					vAvgSolutionVolume := null;
					vAvgSolventVolumeUnit := null;
				END IF;
			ELSE		
				SELECT count(*) INTO vTempCount from inv_wells WHERE solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i) and rownum=1;	
			...
			ELSE
				...
				END IF;
				IF vFirstUnitValue is not null THEN 
					SELECT count (distinct (round(chemcalcs.convert(solution_volume,solvent_volume_unit_id_fk,vFirstUnitValue),3))) INTO vDistinctValues FROM inv_wells WHERE solution_volume is not null AND solvent_volume_unit_id_fk is not null and plate_id_fk = vPlateIDList_t(i); 
				ELSE
					vDistinctValues := null;
				END IF;
				IF vDistinctValues> 1 or vDistinctValues is null THEN 
	    ...
	    IF vCount = 1 THEN
				...
				SELECT count(distinct round(concentration,3)) INTO vDistinctValues FROM inv_wells where concentration is not null AND conc_unit_fk IS NOT NULL and plate_id_fk = vPlateIDList_t(i);
				IF vDistinctValues >1 THEN
					vAvgConcentration := null;
					vAvgConcentrationUnit := null;
				END IF;
		ELSE
			SELECT count(*) INTO vTempCount FROM inv_wells WHERE conc_unit_fk is not null and plate_id_fk = vPlateIDList_t(i) and rownum=1;	
			IF vTempCount >0 THEN
				SELECT decode(conc_unit_fk,null,null,conc_unit_fk) INTO vFirstUnitValue FROM inv_wells WHERE conc_unit_fk is not null and plate_id_fk = vPlateIDList_t(i)  and rownum=1;			
			ELSE
				vFirstUnitValue := Null;
			END IF;
			IF vFirstUnitValue is not null THEN 
				SELECT count (distinct (round(chemcalcs.convert(concentration,conc_unit_fk,vFirstUnitValue),3))) INTO vDistinctValues FROM inv_wells WHERE concentration is not null AND conc_unit_fk is not null and plate_id_fk = vPlateIDList_t(i); 
			ELSE
				vDistinctValues := null;
			END IF;
			IF vDistinctValues> 1 or vDistinctValues is null THEN 
