CREATE OR REPLACE PROCEDURE PROPAGATEGROUPOWNER2CONTAINERS(
...
BEGIN
    FOR REC IN CUR(pLocationID) LOOP
        UPDATE CHEMINVDB2.INV_CONTAINERS C
        SET PRINCIPAL_ID_FK=rec.principal_id_FK
        WHERE C.LOCATION_ID_FK=rec.location_id;
    END LOOP;
END PROPAGATEGROUPOWNER2CONTAINERS;
/
