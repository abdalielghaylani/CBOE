CREATE OR REPLACE PACKAGE BODY "GUIUTILS" AS
	...
	RETURN VARCHAR2 IS
		...
		vIsFirst           BOOLEAN := TRUE;
		l_batchID          inv_container_batches.batch_id%TYPE := NULL;
		vTempStr1 varchar2(2000) := '';
		l_tempstr_t Stringutils.t_char;
		vConverted  BOOLEAN;
		vTemapArr Stringutils.t_char;
		vConvertedQty      inv_containers.qty_available%TYPE;
		vTempQtyAvailable  inv_containers.qty_available%TYPE;
		vTempUnitID         inv_units.unit_id%TYPE;
		vTempUnitAbbrev    inv_units.unit_abreviation%TYPE;
		vContainerUnitID         inv_units.unit_id%TYPE;
		vContainerUnitAbbrev          inv_units.unit_abreviation%TYPE;
	BEGIN
		IF pContainerID IS NOT NULL and pBatchID is null THEN
			--' get batch id
			SELECT batch_id_fk
			INTO l_batchID
			FROM inv_containers
		...
		IF pContainerID IS NOT NULL THEN
		  SELECT 	ic.unit_of_meas_id_fk, iu.unit_abreviation INTO vContainerUnitID, vContainerUnitAbbrev FROM inv_containers ic, inv_units iu
		  WHERE ic.container_id=pContainerID  AND iu.unit_id = ic.unit_of_meas_id_fk;
		End if;
		vTempQtyAvailable :=0;
		vTempStr1 :=',';
		IF l_batchID IS NOT NULL THEN
			FOR batch_rec IN (SELECT SUM(GUIUTILS.GETQTYAVAILABLE(ic.container_id)) qty_available,
			ic.unit_of_meas_id_fk as UOMID,
								iu.unit_abreviation
						   FROM inv_containers ic, inv_units iu
						   WHERE (ic.batch_id_fk = l_batchID
               or ic.batch_id2_fk = l_batchID
               or ic.batch_id3_fk = l_batchID)
						   AND GUIUTILS.IS_CONTAINER_AVAILABLE(ic.container_id) = 1
						   AND iu.unit_id = ic.unit_of_meas_id_fk
                                                   AND AUTHORITY.CONTAINERISAUTHORIZED(ic.container_id,USER)=1
						   GROUP BY ic.unit_of_meas_id_fk, iu.unit_abreviation
						   ORDER BY iu.unit_abreviation DESC)
			LOOP
				IF vContainerUnitID IS NULL THEN
					IF vIsFirst THEN
					  vIsFirst        := FALSE;
					  vTempStr1 := batch_rec.qty_available || ' ' || batch_rec.unit_abreviation  || ' ' || batch_rec.UOMID || ',';
					ELSE
					  vConverted := false;
					  l_tempstr_t := STRINGUTILS.split(vTempStr1,',');
					  FOR i IN l_tempstr_t.FIRST..l_tempstr_t.LAST
					  LOOP
						IF vConverted= false THEN
						  vTemapArr:= STRINGUTILS.split(l_tempstr_t(i), ' ');
						  --try to convert
						  vConvertedQty := chemcalcs.convert(batch_rec.qty_available, batch_rec.UOMID,vTemapArr(3));
						  IF vConvertedQty >= 0 THEN
							vTempStr1:= replace(vTempStr1, vTemapArr(1) || ' ' || vTemapArr(2), (vConvertedQty + vTemapArr(1)) || ' ' || vTemapArr(2));
							vConverted := true;
							EXIT;
						  END IF;
						END IF;
					  ...
					  -- could not convert
					  IF vConverted= false THEN
						vTempStr1:= vTempStr1  || batch_rec.qty_available || ' ' || batch_rec.unit_abreviation  || ' ' || batch_rec.UOMID || ',';
					  END IF;
					END IF;
				ELSE
					vConvertedQty := chemcalcs.convert(batch_rec.qty_available, batch_rec.UOMID,vContainerUnitID);
					IF vConvertedQty >= 0 THEN
						vTempQtyAvailable := vTempQtyAvailable + vConvertedQty;
						vTempStr1 := vTempQtyAvailable || ':' || vContainerUnitAbbrev  || ',';
					END IF;
				END IF;
			END LOOP;
 			IF vContainerUnitID IS NULL THEN
				...
				LOOP
				  vTemapArr:= STRINGUTILS.split(l_tempstr_t(i), ' ');
				  vTempStr1:= vTempStr1 || vTemapArr(1) || ':' || vTemapArr(2) || ',';
				END LOOP;
			END IF;  
			-- close batch_rec;
			vBatchAmountString := TRIM(',' FROM vTempStr1);
		END IF;
		RETURN vBatchAmountString;
		...
		IF p_assetType = 'plate' THEN
			...
			OPEN O_RS FOR
				...
				SELECT plate_id AS pk,
					  ...
					  (select '_'|| plate_id from inv_plates where plate_id=p_rootID) ||  sys_connect_by_path(plate_id, '_') AS id2
...
PROCEDURE GETFKReference(   pColumnName IN  user_cons_columns.Column_name%TYPE, pTableName IN  user_cons_columns.Table_name%TYPE, pOwner IN  user_cons_columns.Owner%TYPE,  O_RS OUT CURSOR_TYPE  )
  ...
  select ucc.constraint_name from user_cons_columns ucc,user_constraints uc where ucc.Column_name=pColumnName and ucc.Table_name=pTableName and ucc.Owner=pOwner  and uc.owner = ucc.owner and  uc.constraint_name = ucc.constraint_name and  uc.constraint_type = 'R' ));
...
FUNCTION GETCONTAINERSAMOUNTSTRING(pRegId inv_containers.REG_ID_FK%type, pBatchNumber inv_containers.BATCH_NUMBER_FK%type default null) RETURN VARCHAR2 IS
	  vBatchAmountString VARCHAR2(500) := '';
	  vNoConvert         VARCHAR2(500) := '';
	  vCurrUnitID        inv_units.unit_id%TYPE;
	  vCurrUnitAbbrev    inv_units.unit_abreviation%TYPE;
	  ...
	  vConvertedQty      inv_containers.qty_available%TYPE;
	  vIsFirst           BOOLEAN := TRUE;
	  batch_rec			 SYS_REFCURSOR;
 	  vTempStr1 varchar2(2000) := '';
  	  l_tempstr_t Stringutils.t_char;
	  vConverted  BOOLEAN;
	  vTemapArr Stringutils.t_char;
	BEGIN
	  --container status id for "available" is id 1
	  IF(pBatchNumber is NULL) THEN
		open batch_rec for 'SELECT SUM(qty_available * decode(concentration, null, 1, 0)) qty_available,
									unit_of_meas_id_fk AS UOMID,
									unit_abreviation
						FROM inv_containers, inv_units
						WHERE unit_id = unit_of_meas_id_fk
						AND reg_id_fk = :r
						AND container_status_id_fk = :s
						GROUP BY unit_of_meas_id_fk, unit_abreviation
						ORDER BY qty_available DESC' using pRegId, 1;
	  ELSE
		open batch_rec for 'SELECT SUM(qty_available * decode(concentration, null, 1, 0)) qty_available,
									unit_of_meas_id_fk AS UOMID,
									unit_abreviation
						...
						WHERE unit_id = unit_of_meas_id_fk
						AND reg_id_fk = :r
						AND batch_number_fk = :b
						AND container_status_id_fk = :s
						GROUP BY unit_of_meas_id_fk, unit_abreviation
						ORDER BY qty_available DESC' using pRegId, pBatchNumber, 1;
	  END IF;
	  LOOP
		FETCH batch_rec INTO vTempQtyAvailable, vTempUnitID, vTempUnitAbbrev;
		EXIT WHEN batch_rec%NOTFOUND;
		IF vIsFirst THEN
		  vCurrUnitID := vTempUnitID;
		  vCurrUnitAbbrev := vTempUnitAbbrev;
		  vTotal   := vTempQtyAvailable;
		  vIsFirst := FALSE;
          vTempStr1 := vTotal || ' ' || vCurrUnitAbbrev  || ' ' || vCurrUnitID || ',';
		ELSE
			vConverted := false;
			l_tempstr_t := STRINGUTILS.split(vTempStr1,',');
			FOR i IN l_tempstr_t.FIRST..l_tempstr_t.LAST
			LOOP
				IF vConverted= false THEN 
					vTemapArr:= STRINGUTILS.split(l_tempstr_t(i), ' ');
					--try to convert
					vConvertedQty := chemcalcs.convert(vTempQtyAvailable, vTempUnitID,vTemapArr(3));
					IF vConvertedQty >= 0 THEN
			...
			-- could not convert
			IF vConverted= false THEN
			vTempStr1:= vTempStr1  || vTempQtyAvailable || ' ' || vTempUnitAbbrev  || ' ' || vTempUnitID || ',';
			END IF;
  	    END IF;
	  END LOOP;
	  l_tempstr_t := STRINGUTILS.split(vTempStr1,',');
