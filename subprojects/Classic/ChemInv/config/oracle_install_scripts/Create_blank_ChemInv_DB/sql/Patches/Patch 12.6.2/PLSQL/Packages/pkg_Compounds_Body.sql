...
IS
	FUNCTION CREATESYNONYM (pCompoundID IN inv_Synonyms.Compound_ID_FK%Type, pSubstanceName IN inv_Synonyms.Substance_Name%Type)
	   ...
	   tempSynonymIDCount inv_Synonyms.Synonym_ID%Type:=0;
	   duplicate_synonym exception;
	BEGIN
		select count(*) into tempSynonymIDCount from inv_Synonyms where upper(Substance_Name) = upper(pSubstanceName);
		if tempSynonymIDCount > 0 then
		  RAISE duplicate_synonym;
		end if;
		...
		RETURN newSynonymID;
	exception
		WHEN duplicate_synonym then
			--RETURN 'A synonym with the same name already exists';
		    RETURN -133;
	END CREATESYNONYM;

	FUNCTION UPDATESYNONYM (pSynonymID IN inv_Synonyms.Synonym_ID%Type, pSubstanceName IN inv_Synonyms.Substance_Name%Type)
	   RETURN inv_Synonyms.Synonym_ID%Type AS
	   tempSynonymCount inv_Synonyms.Synonym_ID%Type:=0;
	   duplicate_synonym exception;
	BEGIN
	   select count(*) into tempSynonymCount from inv_Synonyms where upper(Substance_Name) = upper(pSubstanceName);
	   if tempSynonymCount > 0 then
		RAISE duplicate_synonym;
	   end if;
	   UPDATE inv_Synonyms
   ...
   FUNCTION DELETESUBSTANCE (pCompoundID in Inv_compounds.compound_ID%Type)
	...
	--CSBR-136739_AMendez: check if there are references from synonyms to the compound to delete.
	SELECT count(*) INTO pSynonyms
		FROM inv_Synonyms
		WHERE COMPOUND_ID_FK = pCompoundID;

	--CSBR-136739_AMendez: If there are synonyms referencing compound, delete them.
	IF pSynonyms > 0 THEN
		DELETE FROM inv_Synonyms
			WHERE COMPOUND_ID_FK = pCompoundID;
	END IF;
	...
	BEGIN
   	...
   	select cscartridge.molweight(pStructure) into vMolWeight from dual;
		IF vMolWeight > 0 and pStructure is NOT null THEN
		...
		--	if vExistingCompoundID > 0 and not vRegisterIfConflicts then  -- Use existing compound
			if vExistingCompoundID > 0  then  -- Use existing compound
		   		if vExistingCompoundID = pCompoundID then --This is and edit request
		   			oCompoundID := UpdateInvCompound(pSubstanceName, pStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, vConflictingFields,vLocationType);
		   		else
	   		...
	   		if vRegisterIfConflicts then -- Create a duplicate compound
		  		if pCompoundID is not Null then  -- This is an edit request which turns the edited compound into a duplicate
		  		  oCompoundID := UpdateInvCompound(pSubstanceName, pStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, pCompoundID, vConflictingFields,vLocationType);
		  			...
		  			oCompoundID := InsertInvCompound(pSubstanceName, pStructure, pCAS, pACXID, pDensity, pcLogP, pRotatableBonds, pTotPolSurfArea, pHBondAcceptors, pHBondDonors, pAltID_1, pAltID_2, pAltID_3, pAltID_4, pAltID_5, vConflictingFields,vLocationType);
					end if;
    ...
    FUNCTION UPDATECOMPOUNDREGISTRYBATCH(
	   pSourceRegID in inv_compounds.Reg_ID_FK%Type,
	   pSourceBatchNumber in inv_compounds.Batch_Number_FK%Type,
	   pTargetRegID in inv_compounds.Reg_ID_FK%Type,
	   pTargetBatchNumber in inv_compounds.Batch_Number_FK%Type)
   Return integer
   IS
   LInvRegCount integer;
   ReturnVal integer;
   BEGIN

   SELECT Count(*)
   INTO LInvRegCount
   FROM INV_COMPOUNDS WHERE REG_ID_FK=pSourceRegID AND BATCH_NUMBER_FK=pSourceBatchNumber AND ROWNUM = 1;

   IF LInvRegCount = 1 THEN
	update INV_COMPOUNDS
	SET REG_ID_FK=pTargetRegID , BATCH_NUMBER_FK=pTargetBatchNumber
	WHERE REG_ID_FK=pSourceRegID AND BATCH_NUMBER_FK=pSourceBatchNumber;
	commit;
	ReturnVal := 1;
   ELSE
	   ReturnVal := 0;
  ...
  EXCEPTION
	  when others then
  		RETURN -1;
  END UPDATECOMPOUNDREGISTRYBATCH;
