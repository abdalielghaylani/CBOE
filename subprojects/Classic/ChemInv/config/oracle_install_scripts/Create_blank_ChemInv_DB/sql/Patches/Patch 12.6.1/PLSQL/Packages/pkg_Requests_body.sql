...
AS
  ...
  FUNCTION CreateRequest(
    ...
    p_special_instructions inv_requests.special_instructions%TYPE,
    p_proof_approval_filename inv_requests.proof_approval_filename%TYPE,
    p_proof_approval_filetype inv_requests.PROOF_APPROVAL_FILETYPE%TYPE,
    p_proof_approval_filesize inv_requests.PROOF_APPROVAL_FILESIZE%TYPE    
  ...
  BEGIN
    ...
    INSERT INTO inv_requests (
           ...
           proof_approval_filename,
           proof_approval_filetype,
           proof_approval_filesize)
    VALUES (
      p_containerId,
      p_batchId,
      p_userId,
      ...
      p_proof_approval_filename,
      p_proof_approval_filetype,
      p_proof_approval_filesize
...
FUNCTION UpdateRequest(
    ...
    p_date2 inv_requests.date_2%TYPE,
    p_requiredUnitId inv_requests.required_unit_id_fk%TYPE,
    p_batchType inv_requests.Batch_Type_id_fk%TYPE,
	p_special_instructions inv_requests.special_instructions%TYPE,
    p_proof_approval_filename inv_requests.PROOF_APPROVAL_FILENAME%TYPE,
    p_proof_approval_filetype inv_requests.PROOF_APPROVAL_FILETYPE%TYPE,
    p_proof_approval_filesize inv_requests.PROOF_APPROVAL_FILESIZE%TYPE
...
BEGIN
    ...
    Batch_Type_id_fk=p_batchType,
	special_instructions = p_special_instructions
    WHERE
        request_id = p_requestId;
		
		IF p_proof_approval_filesize > 0 THEN 
		UPDATE inv_requests SET
      proof_approval = EMPTY_BLOB(),
			proof_approval_filename = p_proof_approval_filename,
			proof_approval_filetype = p_proof_approval_filetype,
			proof_approval_filesize = p_proof_approval_filesize        
		WHERE
		  request_id = p_requestId;    
		END IF;
    ...
    IF p_proof_approval_filesize = 0 THEN 
		UPDATE inv_requests SET
      proof_approval = EMPTY_BLOB(),
			proof_approval_filename = NULL,
			proof_approval_filetype = NULL,
			proof_approval_filesize = NULL        
		WHERE
		  request_id = p_requestId;    
		END IF;
...
END UpdateRequest;
  ...
  AS
    ...
    l_Sql varchar(30000);         --CSBR 164627 SJ Increasing the size of the variable to accomodate the update query. 
	  rowCount integer;
	  O_RS CURSOR_TYPE;
	BEGIN
    QtyUnFilfilled := 1;
  ...
  RETURN inv_requests.request_ID%Type AS
	...
	PROCEDURE GETBATCHCONTAINERS(
		...
		vRequestUnitAbbvr varchar2(10);
	BEGIN
		select unit_abreviation INTO vRequestUnitAbbvr from  inv_units u, inv_requests r where u.unit_id=r.required_unit_id_fk and r.request_id=pRequestID;
		OPEN O_RS FOR
          Select distinct
            cb.batch_id
            , c.barcode
            , c.container_id
            , c.location_id_fk
            , chemcalcs.convert(c.qty_remaining, c.UNIT_OF_MEAS_ID_FK,r.required_unit_id_fk) as qty_remaining
            , vRequestUnitAbbvr AS uomAbbrv
            , concentration
        ...
        -- Addition ends for CSBR-123488
          From inv_container_batches cb
               ...
               , inv_locations lp
               , inv_requests r
          Where (cb.batch_id = c.batch_id_fk or cb.batch_id = c.batch_id2_fk or cb.batch_id = c.batch_id3_fk)
       ...
       AND chemcalcs.convert(c.qty_remaining, c.UNIT_OF_MEAS_ID_FK,r.required_unit_id_fk)>0
          AND (r.batch_id_fk = c.batch_id_fk or r.batch_id_fk = c.batch_id2_fk or r.batch_id_fk = c.batch_id3_fk)
    ...
    pDateFormat IN VARCHAR2,
      ...
      case when (Select count(c1.Qty_Available) From &&schemaName..inv_containers c1 Where c1.batch_id_fk = r.batch_id_fk AND c1.container_status_id_fk not in (6,7)) = 0 then 0 else (Select Sum(c2.Qty_Available*c2.concentration) From &&schemaName..inv_containers c2 Where c2.batch_id_fk = r.batch_id_fk AND c2.container_status_id_fk not in (6,7)) end as AmountRemaining,
      ...
      r.proof_approval_filename,
      r.proof_approval_filetype,
      r.proof_approval_filesize,
      r.request_status_id_fk
      ...
      AND (r.container_id_fk = c.container_id or r.batch_id_fk = c.batch_id_fk or r.batch_id_fk = c.batch_id2_fk or r.batch_id_fk = c.batch_id3_fk)
...
IF p_regServer = 'NULL' THEN
      ...
      SELECT	request_id,
        ...
        replace(&&schemaName..GUIUTILS.GETBATCHUOMAMOUNTSTRING(NULL,r.batch_id_fk),':','') as Batch_Amount,
...
ELSE
      ...
      SELECT distinct request_id,
        ...
        (Select doc_id from inv_docs where inv_docs.table_name='INV_REQUESTS' and field_name='REQUEST_ID' and field_value=r.request_id and doc_type_id_fk=2) as RequestWorksheetDocID,
        (select unit_abreviation FROM inv_units u where r.required_unit_id_fk = u.unit_id(+) And rownum = 1) as RequiredUOMabbrv,
        REQUESTS.GETNUMSHIPPEDCONTAINERS(r.request_id) AS NumShippedContainers,
        GETLOCATIONPATH(l.location_id) AS LocationPath,
        -- Added for CSBR-121935
        r.FIELD_1,
        r.FIELD_2,
	...
	ivrb1.REGID,
	ivrb1.ROOT_NUMBER
      FROM inv_requests r, inv_locations l, &&securitySchemaName..people p1, &&securitySchemaName..people p2, inv_container_types ct, inv_request_status rs, inv_container_batches icb , inv_vw_reg_batches ivrb1,inv_containers c, inv_vw_compounds cmpd
      WHERE     r.user_id_fk = p1.user_id(+)
      	    AND r.batch_id_fk = icb.batch_id(+)
      	    AND (icb.batch_id = c.batch_id_fk or icb.batch_id = c.batch_id2_fk or icb.batch_id=c.batch_id3_fk)
      	    AND c.compound_id_fk = cmpd.compound_id(+)
      	    AND cmpd.reg_batch_id = ivrb1.REGBATCHID(+)
      	    AND r.request_status_id_fk = rs.request_status_id
      	    AND Upper(r.delivered_by_id_fk) = p2.user_id(+)
      	    AND r.delivery_location_id_fk = l.location_id
      	    AND r.container_type_id_fk = ct.container_type_id(+)
      	   ...
      	   AND ((r.user_id_fk LIKE NVL(p_UserID, '%') or r.creator LIKE NVL(p_UserID, '%') or p_UserID in (select USER_ID_FK from INV_ORG_USERS where org_unit_id_fk=r.org_unit_id_fk)))
      	AND   request_status_id_fk not in (9)
      	Order by request_id asc,request_status_id_fk asc, timestamp desc;
...
END IF;
  ...
  END GETORDERSFORREQUEST;
	...
	BEGIN
     ...
     my_sql := 'SELECT distinct request_id,
  ...
  my_sql := my_sql || 'cb.batch_status_id_fk as BatchStatus,
                  ...
                  requests.GetOrdersForRequest(r.request_id) as OrderList,
                  r.creator,
                  case when (Select count(c1.Qty_Available) From &&schemaName..inv_containers c1 Where c1.batch_id_fk = r.batch_id_fk AND c1.container_status_id_fk not in (6,7)) = 0 then 0 else (Select Sum(c2.Qty_Available* decode(c2.concentration,NULL,1,c2.concentration)) From &&schemaName..inv_containers c2 Where c2.batch_id_fk = r.batch_id_fk AND c2.container_status_id_fk not in (6,7) and c2.unit_of_meas_id_fk=r.required_unit_id_fk and GUIUTILS.is_container_available(c2.container_id) = 1) end as AmountRemaining,
                  ...
                  r.DATE_2,
                  r.proof_approval_filename
                  FROM inv_requests r, inv_locations l, &&securitySchemaName..people p1, &&securitySchemaName..people p2, inv_request_status rs, inv_container_batches cb , inv_vw_reg_batches ivrb1,inv_containers c, inv_vw_compounds cmpd';
               my_sql := my_sql || ' WHERE Upper(r.user_id_fk) = Upper(p1.user_id)  
               				AND (cb.batch_id = c.batch_id_fk or cb.batch_id = c.batch_id2_fk or cb.batch_id=c.batch_id3_fk) 
                     	    AND c.compound_id_fk = cmpd.compound_id(+)
      	    				AND cmpd.reg_batch_id = ivrb1.REGBATCHID(+) ';
 ...
 vReturnString varchar2(16384) := '';
 vUOMCnt integer;
 vReqQty number;
 vContainerQty number;
 duplicate_uom exception;
  vRequestUOMAbv inv_units.unit_abreviation%TYPE;
 BEGIN
      select count(distinct unit_of_meas_id_fk) into vUOMCnt from inv_requests, inv_containers
        where (inv_requests.batch_id_fk = inv_containers.batch_id_fk or inv_requests.batch_id_fk = inv_containers.batch_id2_fk or inv_requests.batch_id_fk = inv_containers.batch_id3_fk)
      ...
      SELECT UNIT_ABREVIATION INTO vRequestUOMAbv FROM inv_units WHERE unit_id= vRequestUOMID;
      FOR i IN vBatchContainerIDs_t.First..vBatchContainerIDs_t.Last
      LOOP
          SELECT container_id, barcode,  chemcalcs.convert(inv_containers.qty_remaining, inv_containers.unit_of_meas_id_fk,vRequestUOMID), vRequestUOMID
          INTO vCurrBatchContainerID, vCurrBatchBarcode, vTempQty, vQtyID
          ...
          vContainerQty:=vCurrTempQty;
         if vCurrTempQty>0 then
          ...
          LOOP
              ...
              if vTempQty>0 then
                  vCurrTempQty := vTempQty;
                  if vReqQty>0 then
                  vTempQty := PlateChem.QuantitySubtraction(vTempQty, vQtyID, vReqQty, vRequestUOMID);
                  else
                  vTempQty := PlateChem.QuantitySubtraction(vTempQty, vQtyID, vQtyList_t(vCurrQtyIndex), vRequestUOMID);
                  end if;
                  --RETURN vTempQty;
               --   vReqQty:=vQtyList_t(vCurrQtyIndex);
                  IF vTempQty >= 0 THEN
                     vCurrTempQty := vTempQty;
                     ...
                     vReqQty:=0;
                  ELSE
                   vReqQty:=vQtyList_t(vCurrQtyIndex)-vCurrTempQty;
                  vQtyList_t(vCurrQtyIndex):=vReqQty;
                   vCurrTempQty:=0;
                    vNumSamples := vNumSamples + 1;
                     EXIT;
                  END IF;
          ...
          END LOOP;
          End if;
          vReturnString := vReturnString || vCurrBatchContainerID || ':' || vCurrBatchBarcode || ':' || vNumSamples || ':' || vCurrTempQty || ':' || vRequestUOMAbv || ':' || vContainerQty|| ',';
      END LOOP;
...
WHERE request_id = pRequestID;
  ...
  -- Updating Amount delivered
  SELECT SUM(qty_remaining) INTO vAmountDelivered FROM inv_containers WHERE container_id IN (SELECT container_id_fk FROM  inv_request_samples  WHERE request_id_fk=pRequestID) ; 
  UPDATE inv_requests SET qty_delivered= vAmountDelivered WHERE request_id=pRequestID;	  
END;
...
PROCEDURE GETBATCHREQUESTBYREQUESTEDUNIT(
	...
	END GETBATCHREQUESTBYREQUESTEDUNIT;
	 ...
	 PROCEDURE UpdateBlob (p_requestId inv_requests.request_id%TYPE,
						amount IN INTEGER,
    ...
    BEGIN
      ...
      SELECT PROOF_APPROVAL INTO doc_blob
      FROM INV_REQUESTS
      WHERE request_id = p_requestId
      FOR UPDATE;
