CREATE OR REPLACE PACKAGE BODY RACKS IS
  ...
  l_count INT;
				...
				SELECT c.barcode AS Barcode,
					  ...
					  CASE
						  WHEN ((SELECT uom.unit_abreviation
						   ...
						   c.qty_remaining * c.concentration || 'mg'
						  ELSE
						   c.qty_remaining ||
						   (SELECT uom.unit_abreviation
						    FROM inv_units uom
	...
	PROCEDURE SEARCHRACKS(p_BatchField1 inv_container_batches.batch_field_1%TYPE,
					  ...
					  p_BatchType   inv_container_batches.batch_type%TYPE := NULL,
					  O_RS OUT CURSOR_TYPE) AS
		...
		IF p_BatchField1 IS NOT NULL OR p_BatchField2 IS NOT NULL OR
      	...
      	my_sql := my_sql || ' where ';
			...
			my_sql:= my_sql || l_Predicate ||
              ...
              where  (icb.batch_id = ic.batch_id_fk or icb.batch_id = ic.batch_id2_fk or icb.batch_id= ic.batch_id3_fk)
              and icb.batch_type='|| p_BatchType ||'
							and ('; 
            l_Predicate := '';
			if p_BatchField1 is not null then 
               my_sql := my_sql || ' upper(icb.batch_field_1) like upper(''%' || BATCH.GETBATCHFIELDVAL(p_BatchField1, p_BatchType, 1) || '%'')';
               l_Predicate := 'and';
              end if;     
             if p_BatchField2 is not null then 
                my_sql := my_sql || l_Predicate || ' upper(icb.batch_field_2) like upper(''%' || BATCH.GetBatchFieldVal(p_BatchField2, p_BatchType, 2) || '%'')';
				l_Predicate := 'and';
              end if;
              if p_BatchField3 is not null then 
                my_sql := my_sql || l_Predicate || ' upper(icb.batch_field_3) like upper(''%' || BATCH.GetBatchFieldVal(p_BatchField3, p_BatchType, 3) || '%'')';
              end if;              
        my_sql := my_sql || '))
    )';
			bAndPredicate := TRUE;
  ...
  FUNCTION NUMBEROFGRIDPOSITIONS(p_LocationID inv_locations.location_id%TYPE, p_startingPosition inv_locations.location_id%TYPE)
	...
	IF p_containerId is NOT NULL THEN 
				...
				END IF;
    SELECT location_id INTO l_firstOpen FROM
    (
        ...
        -- ToDo: instead of SORT_ORDER, there should be something like FILL_ORDER
        -- since the rack can be set up to fill in an order that may differ from the 
        -- sort order (CSBR-59043)
        ORDER BY SORT_ORDER 
    )
	  WHERE rownum = 1;
	...
	*/
    ...
    ELSE
      ...
      FROM inv_vw_grid_location gl
  		...
  		IF l_count > 0 THEN
        SELECT location_id INTO l_nextOpen FROM
        (
    		  SELECT gl.location_id
    		  FROM inv_vw_grid_location gl
    		  WHERE gl.parent_id = l_parentId
