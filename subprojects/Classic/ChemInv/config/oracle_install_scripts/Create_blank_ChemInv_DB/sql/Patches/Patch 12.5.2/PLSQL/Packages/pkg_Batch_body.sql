...
PACKAGE BODY BATCH
	...
	PROCEDURE GETBATCHFIELDS
   	...
   	BEGIN
   		OPEN p_RS FOR
        SELECT * FROM inv_container_batch_fields ORDER BY BATCH_TYPE_ID_FK, SORT_ORDER;
   	END GETBATCHFIELDS;
 ...
 FUNCTION GetRegBatchid ( pBatchid IN INV_CONTAINER_BATCHES.BATCH_ID %TYPE) RETURN VARCHAR2
    ...
    Function CheckBatchModifications
	  ...
	  Function GETBATCHFIELDVAL
          ...
          AS
          vFieldName INV_CONTAINER_BATCH_FIELDS.FIELD_NAME%TYPE;
          vReturnVal varchar2 (1000);
          l_Sql  varchar2(1000);
          Begin
			SELECT FIELD_NAME INTO vFieldName FROM INV_CONTAINER_BATCH_FIELDS WHERE BATCH_TYPE_ID_FK =  pBatchType  and sort_order = pSortOrder;
			if upper(vFieldName) = 'REG_ID_FK' then
				l_Sql := 'SELECT distinct regid FROM inv_vw_reg_batches WHERE regnumber =''' || pFieldVal || '''';
				EXECUTE IMMEDIATE  l_Sql INTO vReturnVal;
			else
				vReturnVal := pFieldVal;
			end if;
			return vReturnVal;
