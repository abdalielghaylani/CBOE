...
is
  ...
  l_isNullbatchValue boolean;
begin

  l_cID := pContainerID;
FOR rec IN (SELECT ID FROM INV_BATCH_TYPE) LOOP
  l_isNullbatchValue := false;
 ...
 -- l_batchFieldName1 := Rtrim(Ltrim(constants.cContainerBatchField1));
  ...
  if Length(l_batchFieldName1) > 0 then
    ...
    if l_batchFieldValue1 is NULL then 
      l_isNullbatchValue := true; 
    end if; 
  ...
  if Length(l_batchFieldName2) > 0 then
    ...
    if l_batchFieldValue2 is NULL then 
      l_isNullbatchValue := true; 
    end if; 
    l_UpdateBatchIdFk := true;
  ...
  if Length(l_batchFieldName3) > 0 then
    ...
    if l_batchFieldValue3 is NULL then 
      l_isNullbatchValue := true; 
    end if;    
    l_UpdateBatchIdFk := true;
...
if (l_isNullbatchValue = false) then
if Length(l_batchCriteriaSql) > 0 then
        l_joinClose := 'AND';
      else
        l_joinClose := 'WHERE';
