CREATE OR REPLACE PACKAGE BODY "REFORMAT" AS
	...
	BEGIN
		...
		l_plateXML := xmlutils.RemoveXMLElement(l_parser,
																						DBMS_XMLQuery.getXML('Select * from inv_plates where plate_id in (' ||
																																 p_plateIDs || ')'));
		l_wellXML  := xmlutils.RemoveXMLElement(l_parser,
																						DBMS_XMLQuery.getXML('Select w.*, row_index, col_index from inv_wells w, inv_grid_position gp where w.plate_id_fk in (' ||
																																 p_plateIDs ||
	...
	FUNCTION GetNumTargetPlates(p_reformatMapID inv_xmldocs.xmldoc_id%TYPE)
		...
		l_mapDoc          xmldom.DOMDocument;
		l_targetPlatesNL  xmldom.DOMNodeList;
	...
	BEGIN
		...
		l_targetPlatesNL  := xmldom.getElementsByTagName(l_mapDoc,
																										 'TARGET_PLATE');
		l_numTargetPlates := xmldom.getLength(l_targetPlatesNL);
	
		xmldom.freeDocument(l_mapDoc);
		xmlparser.freeParser(l_parser);
	...
	FUNCTION GetPlateFormats(p_reformatMapID inv_xmldocs.xmldoc_id%TYPE,
		...
		l_engine             xslprocessor.Processor := xslprocessor.newProcessor;
		l_parser             xmlparser.Parser;
		l_mapClob            inv_xmldocs.xmldoc%TYPE;
		l_mapDoc             xmldom.DOMDocument;
		l_sourcePlatesNL     xmldom.DOMNodeList;
		l_counter            NUMBER;
	...
	BEGIN
		...
		END IF;
		l_sourcePlatesNL := xmldom.getElementsByTagName(l_mapDoc, l_elementName);
		FOR l_counter IN 0 .. xmldom.getLength(l_sourcePlatesNL) - 1
		LOOP
			l_node := xmldom.item(l_sourcePlatesNL, l_counter);
			IF l_counter = 0 THEN
				l_sourcePlateFormats := xslprocessor.valueOf(l_node,
																										 './@PLATE_FORMAT_ID_FK');
			ELSE
				l_formatID           := xslprocessor.valueOf(l_node,
																										 './@PLATE_FORMAT_ID_FK');
				l_sourcePlateFormats := l_sourcePlateFormats || ',' || l_formatID;
			END IF;
		END LOOP;
	
		xslprocessor.freeProcessor(l_engine);
		xmlparser.freeParser(l_parser);
		xmldom.freeDocument(l_mapDoc);
	...
	BEGIN
		...
		END LOOP;
		RETURN TRIM(',' FROM l_xmlDocIDList);
	...
	BEGIN
		...
		RETURN TRIM(',' FROM l_plateBarcodeList);
  ...
  */
	...
	BEGIN
		...
		IF length(l_amountProblemPlateIDs) > 0 THEN
			l_amountProblemPlateIDs := TRIM(',' FROM l_amountProblemPlateIDs);
			RAISE lNoMaterial;
		END IF;
	...
	BEGIN
		...
		IF length(l_typeProblemPlateIDs) > 0 THEN
			l_typeProblemPlateIDs := TRIM(',' FROM l_typeProblemPlateIDs);
	...
	FUNCTION CreateTargetPlateXML(p_engine              xslprocessor.Processor,
		...
		l_mapClob CLOB;
		l_mapDoc  xmldom.DOMDocument;
		l_mapNode xmldom.DOMNode;
	
		l_queryClob  CLOB;
		l_mergeDoc   xmldom.DOMDocument;
		l_xsltClob   CLOB;
		l_targetDoc  xmldom.DOMDocument;
		l_targetNode xmldom.DOMNode;
	...
	BEGIN
		...
		l_query     := 'SELECT plate_id_fk, well_id as parent_well_id_fk, row_index, col_index, compound_id_fk, reg_id_fk, batch_number_fk
		...
		l_targetDoc  := XMLUTILS.CLOB2DOC(p_parser,
																			XMLUTILS.transformXML(l_xsltClob,
																														...
																														p_parser,
																														p_engine));
		l_targetNode := xmldom.MakeNode(l_targetDoc);
		...
		LOOP
			l_node    := xslprocessor.selectSingleNode(l_targetNode,
																								 '/PLATES/PLATE[' || i || ']');
			l_element := xmldom.makeElement(l_node);
			IF NOT p_isDaughter THEN
				IF instr(p_barcodeList_t(i), ':auto') > 0 THEN
					l_barcodeDescId := rtrim(p_barcodeList_t(i), ':auto');
					l_attribute     := xmldom.createAttribute(l_targetDoc,
																										'BARCODE_DESC_ID');
					xmldom.setValue(l_attribute, l_barcodeDescId);
				ELSE
					l_barcode   := p_barcodeList_t(i);
					l_attribute := xmldom.createAttribute(l_targetDoc,
																								'PLATE_BARCODE');
					xmldom.setValue(l_attribute, l_barcode);
				END IF;
				l_attribute := xmldom.setAttributeNode(l_element, l_attribute);
			END IF;
			xmldom.setValue(xmldom.getAttributeNode(l_element, 'LOCATION_ID_FK'),
											p_locationID);
			xmldom.setValue(xmldom.getAttributeNode(l_element,
																							'PLATE_TYPE_ID_FK'),
											p_plateTypeID);
		END LOOP;
		...
		LOOP
			...
			LOOP
				...
				xmldom.setAttribute(l_element,
														'WELL_FORMAT_ID_FK',
														wellFormat_rec.Well_Format_Id_Fk);
			
				l_attribute := xmldom.createAttribute(l_targetDoc, 'CONCENTRATION');
				xmldom.setValue(l_attribute, wellFormat_rec.Concentration);
				l_attribute := xmldom.setAttributeNode(l_element, l_attribute);
				l_attribute := xmldom.createAttribute(l_targetDoc, 'CONC_UNIT_FK');
				xmldom.setValue(l_attribute, wellFormat_rec.Conc_Unit_Fk);
				l_attribute := xmldom.setAttributeNode(l_element, l_attribute);
			END LOOP;
		END LOOP;
		...
		xmldom.freeDocument(l_mapDoc);
		xmldom.freeDocument(l_mergeDoc);
	
	END CreateTargetPlateXML;

	FUNCTION CreateDaughterPlates(p_plateXMLDoc   xmldom.DOMDocument,
																p_plateXMLNode  xmldom.DOMNode,
																p_barcodeList_t STRINGUTILS.t_char)
		RETURN VARCHAR2 IS
		l_node          xmldom.DOMNode;
		l_element       xmldom.DOMElement;
	...
	BEGIN
		...
		LOOP
			IF instr(p_barcodeList_t(i), ':auto') > 0 THEN
				l_barcodeDescID := rtrim(p_barcodeList_t(i), ':auto');
				l_attribute     := xmldom.createAttribute(p_plateXMLDoc,
																									'BARCODE_DESC_ID');
				xmldom.setValue(l_attribute, l_barcodeDescID);
			ELSE
				l_barcode   := p_barcodeList_t(i);
				...
				xmldom.setValue(l_attribute, l_barcode);
			END IF;
			l_node      := xslprocessor.selectSingleNode(p_plateXMLNode,
																									 '/PLATES/PLATE[1]');
			l_element   := xmldom.makeElement(l_node);
			l_attribute := xmldom.setAttributeNode(l_element, l_attribute);
			DBMS_LOB.CREATETEMPORARY(l_plateXMLClob, FALSE, DBMS_LOB.CALL);
			xmldom.writeToClob(p_plateXMLNode, l_plateXMLClob);
		...
		RETURN TRIM(',' FROM l_newPlateIDs);
	...
	BEGIN
		...
		ELSE
			...
			xmldom.writeToClob(l_targetNode, l_targetClob);
		...
		platechem.SetAggregatedPlateData(TRIM(',' FROM l_newPlateIDs));
		platechem.SetAggregatedPlateData(p_sourcePlateIDList);
	
		--Clean Up
		xmldom.freeDocument(l_targetDoc);
	...
	FUNCTION CreateDaughteringMap(p_plateFormatID inv_plate_format.plate_format_id%TYPE)
		...
		l_xsltDoc     xmldom.DOMDocument;
		l_xslt        xslprocessor.Stylesheet;
	...
	BEGIN
		...
		xmlparser.parseClob(l_parser, l_queryClob);
		l_queryDoc := xmlparser.getDocument(l_parser);
		SELECT xslt
			INTO l_xsltClob
			FROM inv_xslts
		 WHERE xslt_name = 'Create Daugtering Map';
		DBMS_LOB.CREATETEMPORARY(l_targetClob, FALSE, DBMS_LOB.CALL);
		xmlparser.parseClob(l_parser, l_xsltClob);
		l_xsltDoc := xmlparser.getDocument(l_parser);
		l_xslt    := xslprocessor.newStyleSheet(l_xsltDoc, NULL);
		xslprocessor.processXSL(l_engine, l_xslt, l_queryDoc, l_targetClob);
	
		--get query XML without the <?xml?> tag
		l_targetNode := XMLUTILS.CLOB2NODE(l_parser, l_targetClob);
		l_mapNode    := xmldom.getLastChild(l_targetNode);
		DBMS_LOB.CREATETEMPORARY(l_mapClob, FALSE, DBMS_LOB.CALL);
		xmldom.writeToClob(l_mapNode, l_mapClob);
