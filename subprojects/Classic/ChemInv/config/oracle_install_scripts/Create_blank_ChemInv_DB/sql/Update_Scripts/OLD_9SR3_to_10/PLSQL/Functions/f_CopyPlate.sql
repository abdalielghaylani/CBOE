		...
		if pCopyType = 0 then
			...
			LOOP
				...
				--' add the well compounds
  				FOR compound_rec IN (SELECT compound_id_fk, reg_id_fk, batch_number_fk FROM inv_wells, inv_well_compounds WHERE well_id = well_id_fk(+) AND plate_id_fk = pPlateID AND grid_position_id_fk = wellformats_rec.grid_position_id)
				LOOP
