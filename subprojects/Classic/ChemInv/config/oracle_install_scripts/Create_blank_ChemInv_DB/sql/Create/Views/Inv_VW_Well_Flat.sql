CREATE OR REPLACE VIEW INV_VW_WELL_FLAT(
  ...
  (SELECT reg_id_fk FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS reg_id_fk,
  (SELECT batch_number_fk FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS batch_number_fk,
  (SELECT mol_id FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS mol_id,
  (SELECT cas FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS cas,
  (SELECT acx_id FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS acx_id,
  (SELECT substance_name FROM inv_compounds WHERE compound_id = INV_WELL_COMPOUNDS.compound_id_fk) AS substance_name,
