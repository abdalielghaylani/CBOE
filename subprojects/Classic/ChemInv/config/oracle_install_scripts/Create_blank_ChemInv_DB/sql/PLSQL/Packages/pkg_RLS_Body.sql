...
AS
  ...
  BEGIN
    ...
    l_predicate := l_predicate || '(Select Distinct role_id	FROM &&securitySchemaName..security_roles s, &&securitySchemaName..privilege_tables p WHERE  s.privilege_table_int_id = p.privilege_table_id AND p.privilege_table_name LIKE NVL(''CHEMINV_PRIVILEGES'', ''%'') AND role_name IN (SELECT granted_role AS ROLE_NAME from dba_role_privs WHERE Upper(grantee) = Upper(''' || l_username || '''))))';      
