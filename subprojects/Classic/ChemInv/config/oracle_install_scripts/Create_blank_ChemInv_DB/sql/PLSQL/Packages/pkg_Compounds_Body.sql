...
IS
    ...
    Function UpdateInvCompound(
        ...
        pCompoundID in inv_compounds.compound_id%type,
        pConflictingFields in inv_compounds.conflicting_fields%type) Return inv_compounds.compound_id%type
    AS
    ...
    BEGIN
        ...
        IF vStructure is NOT null THEN
            IF DBMS_LOB.GetLength(vStructure) > 0 THEN
                select cscartridge.molweight(pStructure) into vMolWeight from dual;
                IF vMolWeight > 0 THEN
                    vSQL := 'select compound_id, ''structure'' AS conflictType from INV_COMPOUNDS where '||
                            'cscartridge.MoleculeContains(base64_cdx, ''SELECT query FROM cscartridge.tempqueries '||
                            'WHERE id = 0'','''',''FULL=YES,IDENTITY=YES'')=1 AND rownum < 2 ';
               END IF;
            END IF;
        END IF;
    ...
    Function GetCompoundIDFromMolecule(
        pQuery in inv_compounds.base64_cdx%type)
