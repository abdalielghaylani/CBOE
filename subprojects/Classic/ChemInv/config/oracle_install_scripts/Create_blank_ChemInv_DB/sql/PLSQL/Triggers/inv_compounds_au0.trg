...
begin
  ...
  --audit_trail.check_val(raid, 'BASE64_CDX', :new.BASE64_CDX, :old.BASE64_CDX);
      IF ( :new.BASE64_CDX <> :old.BASE64_CDX OR
           (:new.BASE64_CDX IS NULL AND :old.BASE64_CDX IS NOT NULL) OR
           (:new.BASE64_CDX IS NOT NULL AND :old.BASE64_CDX IS NULL) )
      THEN
										IF length(:new.BASE64_CDX) > 4000 THEN 
													l_newval := 'The structure data exceeds the storage capacity for this audit field.'; 
										ELSE
														l_newval := :new.BASE64_CDX;
										END IF;
										IF length(:old.BASE64_CDX) > 4000 THEN 
													l_oldval := 'The structure data exceeds the storage capacity for this audit field.  It is being stored in the table AUDIT_COMPOUND.'; 
										ELSE
														l_oldval := :old.BASE64_CDX;
										END IF;
	        audit_trail.column_update(raid, upper('BASE64_CDX'), l_oldval, l_newval);
      END IF;
