...
Function DoDupsSearch( dbkey, formgroup )
	...
	compound_moltable = "DRUGDEG_PARENTS"

	'get the base64str for this records
	Base64Str = Request.Form( "DRUGDEG_PARENTS.Structure" )
	if Base64Str <> "" then
...
Function GetStatusList( ByRef connDB )
	...
	sSQL = "select * from DRUGDEG_STATUSES order by STATUS_KEY"
...
Function GetSaltNameListFromSaltCodeList( ByRef sCodesList )
	...
	if "" <> sCodesList then
		...
		for each sCode in asCodes
			sCode = Trim( sCode )
			sSQL = "Select * from DRUGDEG_SALTS where SALT_CODE = '" & sCode & "'"
...
Function ConditionHasExperiments( ByVal keyCondition, ByRef connDB )
	...
	sSQL = "select * from DRUGDEG_EXPTS where DEG_COND_FK = " & keyCondition
...
Function FunctionalGroupHasDegradants( ByVal keyFGroup, ByRef connDB )
	...
	sSQL = "select * from DrugDeg_DegsFGroup, DrugDeg_Degs where DrugDeg_DegsFGroup.Deg_Cmpd_ID = DrugDeg_Degs.Deg_Cmpd_Key and DrugDeg_DegsFGroup.Deg_FGroup_ID = " & keyFGroup
...
Function GetCurrentFunctionalGroups(dbkey, formgroup, byRef Conn, return_field1, return_field2, orderby, Deg_Cmpd_ID)
	...
	sql ="SELECT DRUGDEG_FGROUPS.* from DRUGDEG_FGROUPS,DRUGDEG_DEGSFGROUP  " &_
		" WHERE DRUGDEG_FGROUPS.Deg_FGroup_Key=DRUGDEG_DEGSFGROUP.Deg_FGroup_ID " &_
		" AND DRUGDEG_DEGSFGROUP.Deg_Cmpd_ID=?" &_
		" ORDER BY " & orderby
...
' List Editors
'#####
Function ListAllEditors(dbkey, formgroup, byRef Conn, return_field1, return_field2, order_by, iType)
	on error resume next
	...
	cmd.CommandType = adCmdText
	if iType = 1 then
		sql="select distinct submitted_by from drugdeg_parents where submitted_by != ' ' Order by " & order_by
	else
		sql="select distinct submitted_by from drugdeg_expts where  submitted_by != ' ' Order by " & order_by
	end if
	cmd.CommandText = sql
	RS.Open cmd
	...
	if rs.eof or rs.bof then
		...
		theEditorList = GetLookupList(RS,return_field1, return_field2)		
	end if
	ListAllEditors = theEditorList
	CloseRS(RS)
End Function
...
Function ListAllFGroupsOrdered(dbkey, formgroup, byRef Conn, return_field1, return_field2, order_by)
	...
	sql="Select * from DRUGDEG_FGroups Order by " & order_by
	cmd.CommandText = sql
	RS.Open cmd
...
Function DoAddDegFGroup(dbkey, formgroup, byRef conn, deg_fgroups_list, iDeg_Cmpd_ID)
	...
	sSQL = "Delete from DrugDeg_DegsFgroup where Deg_Cmpd_ID = " & iDeg_Cmpd_ID
	conn.Execute sSQL
	...
	if deg_fgroups_list <> "" then
		...
		for i = 0 to UBound(temp_array)
			...
			'insert into DrugDeg_DegsFgroup
			set cmd = server.CreateObject("adodb.command")
			cmd.ActiveConnection =  conn
			cmd.CommandType = adCmdText
			sql = "INSERT INTO DrugDeg_DegsFgroup(Deg_Cmpd_ID, Deg_FGroup_ID)VALUES(?,?)"
			cmd.CommandText = sql
...
Function DoUpdateMWDifference(dbkey, formgroup, byRef conn, iDeg_CMPD_ID)
	...
	sSQL= "select drugdeg_base64.* from drugdeg_base64, DRUGDEG_DEGS, DRUGDEG_EXPTS, DRUGDEG_PARENTS "&_
	"where DRUGDEG_DEGS.DEG_CMPD_KEY=" & iDeg_CMPD_ID &" and "&_
	"		DRUGDEG_DEGS.DEG_EXPT_FK = DRUGDEG_EXPTS.EXPT_KEY and DRUGDEG_EXPTS.PARENT_CMPD_FK = DRUGDEG_PARENTS.PARENT_CMPD_KEY "&_
	"		and DRUGDEG_PARENTS.MOL_ID = drugdeg_base64.mol_id"
	...
	sParentMW = getShowCFWChemResult( dbkey, formgroup, "MolWeight", "DRUGDEG_BASE64.MolWeight", rsStructure( "MOL_ID" ), "raw", "1", "17" )
	sParentMW = replace(sParentMW,"&nbsp;","")
	'sParentMW = MyFormatNumber( sMW, 4 )
	
	' Get the molecular formula.
	sParentFormula = getShowCFWChemResult( dbkey, formgroup, "Formula", "DRUGDEG_BASE64.Formula", rsStructure( "MOL_ID" ), "raw", "1", "17" )
	rsStructure.Close
				
	sSQL= "select drugdeg_base64.* from drugdeg_base64, DRUGDEG_DEGS "&_
	"where DRUGDEG_DEGS.DEG_CMPD_KEY=" & iDeg_CMPD_ID &" and "&_
	" DRUGDEG_DEGS.Mol_ID = drugdeg_base64.mol_id"
	rsStructure.Open sSQL, DataConn
				
	'JHS 1/15/2008 - First clear these session variables...this will be useful for display also
	'without doing this display and calculation are incorrect
	Session("MW" & dbkey & formgroup & "DRUGDEG_BASE64" & "MolWeight" & rsStructure("MOL_ID")) = ""
	Session("Formula" & dbkey & formgroup & "DRUGDEG_BASE64" & "Formula" & rsStructure("MOL_ID")) = ""
				
	sDegradantMW = getShowCFWChemResult( dbkey, formgroup, "MolWeight", "DRUGDEG_BASE64.MolWeight", rsStructure( "MOL_ID" ), "raw", "1", "17" )
	sDegradantMW = replace(CStr(sDegradantMW),"&nbsp;","")
	if sDegradantMW = "" then
		sDegradantMW = 0
	end if
	' Get the molecular formula.
	sDegradantFormula = getShowCFWChemResult( dbkey, formgroup, "Formula", "DRUGDEG_BASE64.Formula", rsStructure( "MOL_ID" ), "raw", "1", "17" )
	if hasRgroup(sParentFormula) = false and hasRGroup(sDegradantFormula) = false then
		...
		if iMWChange <> "" then
			iMWChange = Round(iMWChange)
			sSQL = "Update DRUGDEG_DEGS set MW_AMT_CHG = " & iMWChange  & " where DEG_CMPD_KEY = " & iDeg_CMPD_ID 
...
Sub DeleteParent( ByVal keyParent, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_PARENTS where PARENT_CMPD_KEY = " & keyParent
	...
	if not rsParent.BOF and not rsParent.EOF then
		...
		if "" <> molidParent then
			...
			sSQL = "Delete from DRUGDEG_BASE64 where MOL_ID = " & molidParent
			connDB.Execute sSQL
	...
	' Delete the parent.
	sSQL = "Delete from DRUGDEG_PARENTS where PARENT_CMPD_KEY = " & keyParent
	connDB.Execute sSQL
...
Sub DeleteExperimentsOfParent( ByVal keyParent, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_EXPTS where PARENT_CMPD_FK = " & keyParent
...
Sub DeleteExperiment( ByVal keyExpt, ByRef connDB )
	...
	' Delete the experiment.
	sSQL = "Delete from DRUGDEG_EXPTS where EXPT_KEY = " & keyExpt
	connDB.Execute sSQL
...
Sub DeleteDegradantsOfExperiment( ByVal keyExpt, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_DEGS where DEG_EXPT_FK = " & keyExpt
...
Sub DeleteDegradant( ByVal keyDegCmpd, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_DEGS where DEG_CMPD_KEY = " & keyDegCmpd
	...
	if not rsDegradant.BOF and not rsDegradant.EOF then
		...
		if "" <> molidDegradant then
			...
			sSQL = "Delete from DRUGDEG_BASE64 where MOL_ID = " & molidDegradant
			connDB.Execute sSQL
	...
	'first delete anything in the drugdeg_degsfgroup table
	sSQL = "delete from drugdeg_degsfgroup where DEG_CMPD_ID = " & keyDegCmpd
	connDB.Execute sSQL
	
	'now delete mechanisms
	sSQL = "delete from drugdeg_mechs where DEG_CMPD_FK = " & keyDegCmpd
	connDB.Execute sSQL
	
	'now delete the degradant
	sSQL = "Delete from DRUGDEG_DEGS where DEG_CMPD_KEY = " & keyDegCmpd
	connDB.Execute sSQL
...
Sub DeleteMechanismsOfDegradant( ByVal keyDegCmpd, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_MECHS where DEG_CMPD_FK = " & keyDegCmpd
...
Sub DeleteMechanism( ByVal keyMech, ByRef connDB )
	...
	sSQL = "Select * from DRUGDEG_MECHS where MECH_KEY = " & keyMech
	...
	if not rsMechanism.BOF and not rsMechanism.EOF then
		...
		if "" <> molidMechanism then
			...
			sSQL = "Delete from DRUGDEG_BASE64 where MOL_ID = " & molidMechanism
			connDB.Execute sSQL
	...
	sSQL = "Delete from DRUGDEG_MECHS where MECH_KEY = " & keyMech
	connDB.Execute sSQL
...
Sub DeleteCondition( ByVal keyCond, ByRef connDB )
	...
	sSQL = "Delete from DRUGDEG_CONDS where DEG_COND_KEY = " & keyCond
	connDB.Execute sSQL
...
Sub DeleteFunctionalGroup( ByVal keyFGroup, ByRef connDB )
	...
	sSQL = "Delete from DRUGDEG_FGROUPS where Deg_FGroup_Key = " & keyFGroup
	connDB.Execute sSQL
...
Sub DeleteDocumentLinksOfParent( ByVal keyParent, ByRef connDB )
	...
	sSQL = "SELECT * FROM DrugDeg_DocMgr_Links WHERE Parent_Cmpd_Key = " & keyParent
...
Sub DeleteDocumentLink( ByVal keyDocLink, ByRef connDB )
	...
	sSQL = "Delete from DRUGDEG_DOCMGR_LINKS where LINK_KEY = " & keyDocLink
	connDB.Execute sSQL
...
'	sPath = Application( "TempFileDirectory" & dbkey ) & "DRUGDEG_BASE64BASE64_CDX_" & rsBase64.Fields( "MOL_ID" ) & ".cdx"
...
Sub FillRecordsetWithDocumentLinksToParent( ByRef connDrugDeg, ByRef rsDocLinks, ByVal Parent_Cmpd_Key )
	' Fill the record set for the document links.
	sQuery = "SELECT * FROM DrugDeg_DocMgr_Links WHERE Parent_Cmpd_Key = " & Parent_Cmpd_Key
	rsDocLinks.Open sQuery, connDrugDeg
...
%>
<select  name="DRUGDEG_DOCMGR_LINKS.DocIDlist" onChange="updateSelectedValue(this.form,this)" size ="1">
...
%></select><input type="hidden" name="DRUGDEG_DOCMGR_LINKS.DocID" value="" >
<%
...
'#####
' Utilities for the DrugDeg-DocManager-DLIMS integration.
'#####

Sub MakeNewConnectionToDocManagerDB( ByRef connDocMgr )
	' Make a connection to the DrugDeg database.
...
Sub MakeNewConnectionToDrugDegDB( ByRef connDrugDeg )
	' Make a connection to the DrugDeg database.
	Set connDrugDeg = Server.CreateObject( "ADODB.Connection" )

	' For now, ...
	'sConn = "DSN=drugdeg;UID=doc_submitter;PWD=doc_submitter"	

	sConn = "DSN=drugdeg;UID=" & Session( "UserName" & dbkey )
	sConn = sConn & ";PWD=" & Session( "UserID" & dbkey )

	connDrugDeg.Open( sConn )
End Sub
...
function getStatusText (byVal sStatusKey, ByRef connDrugDeg)
	Set rsStatus = Server.CreateObject( "ADODB.Recordset" )
	sSQL = "Select STATUS_TEXT from DRUGDEG_STATUSES where STATUS_KEY =" & sStatusKey
	
	rsStatus.Open sSQL, connDrugDeg
...
function IsRecordEditable (byVal sTable, byVal sKeyColumn, byVal recId, ByRef connDrugDeg)
	...
	'can they edit at all?
		...
		rsRecordStatus.Open sSQL, connDrugDeg
	
		if not rsRecordStatus.eof then
			sStatus = rsRecordStatus("STATUS")
			if sStatus <> Application("STATUS_APPROVED") then ' if not in the approved state
...
function SendStatusMail (byval sNewStatus , byVal sFormGroup_LowerCase )
	...
	if sKey <> "" then
		...
		if sFormGroup_LowerCase ="base_form_group" or sFormGroup_LowerCase ="addparent_form_group" then
			sSQL = "Select SUBMITTED_BY, GENERIC_NAME, TRADE_NAME, COMMON_NAME,  PARENT_CMPD_KEY from DRUGDEG.DRUGDEG_PARENTS dd where dd.PARENT_CMPD_KEY = " & Session( "PrimaryKey" & dbkey )
		else
			sSQL = "Select de.SUBMITTED_BY, dp.GENERIC_NAME, dp.TRADE_NAME, dp.COMMON_NAME,  dc.DEG_COND_TEXT, de.PARENT_CMPD_FK, de.EXPT_KEY from DRUGDEG.DRUGDEG_EXPTS de, DRUGDEG.DRUGDEG_CONDS dc, DRUGDEG.DRUGDEG_PARENTS dp  where de.EXPT_KEY = " & Session( "PrimaryKey" & dbkey ) & " and  de.DEG_COND_FK = dc.DEG_COND_KEY and dp.PARENT_CMPD_KEY = de.PARENT_CMPD_FK"
		end if
		...
		if not rsEmail.EOF then
			...
			GENERIC_NAME = rsEmail("GENERIC_NAME")
			TRADE_NAME = rsEmail("TRADE_NAME")
			COMMON_NAME = rsEmail("COMMON_NAME")
			
			if  sFormGroup_LowerCase ="base_form_group" or  sFormGroup_LowerCase ="addparent_form_group" then
				PARENT_CMPD_KEY = rsEmail("PARENT_CMPD_KEY")
			else
				PARENT_CMPD_KEY = rsEmail("PARENT_CMPD_FK")
				EXPT_KEY = rsEmail("EXPT_KEY")
				DEG_COND_TEXT = rsEmail("DEG_COND_TEXT")
			end if
			
			if GENERIC_NAME <> "" then
				strCompound = GENERIC_NAME & " (generic name)"
			elseif COMMON_NAME <> "" then
				strCompound = COMMON_NAME & " (common name)"
			elseif TRADE_NAME <> "" then
				strCompound = TRADE_NAME & " (trade name)"
			else
				strCompound = ""
			end if
		end if
		...
		Select Case UCase( sNewStatus )
			...
			'needs review
				if sFormGroup_LowerCase ="base_form_group" or sFormGroup_LowerCase ="addparent_form_group" then
					...
					if strCompound <> "" then
						sContent = "Compound: " & strCompound & vbCrLf
					end if
					sContent = sContent & "A new compound has been submitted or "
					sContent = sContent & "it has had its status changed to ""Needs Review""."
					sContent = sContent & vbCrLf & "Please go to: " & Application("SERVER_TYPE") & Request.ServerVariables("SERVER_NAME") & " to view the compound."
				else
					sSubject = "Submitted experiment needs review"
					if strCompound <> "" then
						sContent =  "Experiment for parent compound: " & strCompound & vbCrLf
					end if
					
					if DEG_COND_TEXT <> "" then
						sContent = sContent & "Experimental condition: " & DEG_COND_TEXT & vbCrLf
					end if
					sContent = sContent & "A new experiment has been submitted or has had its status changed to ""Needs Review""."
					sContent = sContent & vbCrLf & "Please go to: " & Application("SERVER_TYPE") & Request.ServerVariables("SERVER_NAME") & " to view the experiment."
				end if
				
				
				'new additions are sent to the admin email. so if they change it to new, they will get an email, 
				' not the submitter therefor they should always change the status to rejected if they want 
				' to send an email if they dont want to send an email, they change to this state.
				' eventually records in this state will be hidden from searches done by the general public
				sTo = Application("DRUGDEG_EMAIL")
			...
			'approved
				if sFormGroup_LowerCase ="base_form_group" then
					...
					if DEG_COND_TEXT <> "" then
						sContent = sContent & "Expirimental condition: " & DEG_COND_TEXT & vbCrLf
					end if
					sContent = "The experiment you have submitted has been approved." & Application("SERVER_TYPE") & Request.ServerVariables("SERVER_NAME") & " to view the experiment."
				end if
		...
		sFrom = Application("DRUGDEG_FROM_EMAIL")
