...
rsExperiment.Open "Select * from DrugDeg_EXPTS where PARENT_CMPD_FK = " & rsParent.Fields( "PARENT_CMPD_KEY" ), connDrugDeg
...
<%
				...
				while not rsExperiment.EOF
					...
					parentPassThruURL = "/" & Application("appkey") & "/DrugDeg/parent_details_form.asp?formgroup=base_form_group&dbname="& dbkey &"&formmode=edit&unique_id="&unique_id&"&commit_type=&keyexpt=" & rsExperiment.Fields( "EXPT_KEY" )
				%>
				...
				<nobr>
					...
					<!-- Now the degradants. -->
						...
						sSQL = "SELECT * FROM DrugDeg_Degs WHERE Deg_Expt_FK = " & rsExperiment.Fields( "EXPT_KEY" ) & " ORDER BY DrugDeg_Degs.Mol_ID"
					...
					<%
								...
								rsBase64_deg.Open "Select * from DrugDeg_BASE64 where MOL_ID = " & rsDegradants.Fields( "MOL_ID" ), connDrugDeg
								ShowResult dbkey, formgroup, rsBase64_deg, "DrugDeg_BASE64.BASE64_CDX", "Base64CDX_NO_EDIT", widthDegStructDrawing, heightDegStructDrawing
								sMW = ""
								sFormula = ""
								' Get the molecular weight.
								sMW = getShowCFWChemResult( dbkey, formgroup, "MolWeight", "DrugDeg_BASE64.MolWeight", rsBase64_deg( "MOL_ID" ), "raw", "1", "17" )
								...
								' Get the molecular formula.
								sFormulaOriginal = getShowCFWChemResult( dbkey, formgroup, "Formula", "DrugDeg_BASE64.BASE64_CDX", rsBase64_deg( "MOL_ID" ), "raw", "1", "17" )
								' Get a nice HTML string for the formula, so the "3" in "CH3" is set lower than the letters.
					...
					%>
								<table>
									...
									<%end if%>
									<%
									current_deg_fgroup_list=GetCurrentFunctionalGroups(dbkey, formgroup,  connDrugDeg,  "Deg_FGroup_Key", "Deg_FGroup_text", "DrugDeg_FGroups.Deg_FGroup_text", rsDegradants.Fields("DEG_CMPD_KEY"))
