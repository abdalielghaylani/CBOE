...
Function DoUserValidate(dbkey, formgroup, appType, baseTable, UserName)
	...
	if UCase(appType) <> "" then 'a reg system
		...
		if isValidUser > 0 then 'it is a valid user
			...
			else 'no granted roles
				fullyValidated = 0
				Session("LoginErrorMessage" & dbkey) = "no valid roles assigned to user"
			end if
		else 'it is not a valid user
			fullyValidated = 0
		end if
	Else ' non_reg system
		isValidUser = -1
...
'Connect to database with the given username and password.
'DataConn comes from GetConnection(), which use the given username and password in 
'connection string.
'If connection is established, the user is validated. Otherwise denied.
Function ValidateUser(dbkey, ByRef DataConn)
	...
	if DataConn.State = 1 then
		...
		userValid = -1
		Session("LoginErrorMessage" & dbkey) = "Invalid username or password"	
	end if
		ValidateUser = userValid
End Function

'Get all roles assigned to this user, concatenate together as a comma delimited string
Function GetGrantedRoles(byVal dbkey, ByRef DataConn, ByVal privName)
	...
	ClearOLEDBError(DataConn.Errors)
	if DataConn.Errors.Count > 0 then
		Session("LoginErrorMessage" & dbkey)= "user does not have any access to any tables within this application"
		exit function
	end if
...
Sub ClearOLEDBError(byref ErrorObj)
	For each EE in ErrorObj
		if EE.Number = 265946 then
			ErrorObj.Clear
		End if
	Next
End Sub
