...
while true
    ...
    While (doLoop)
        ...
        ElseIf Mid(query, p, 1) = STAR Or Mid(query, p, 1) = PERCENT Then
            ...
            p = p + 1
			'logaction ("inStarPercent")
        ElseIf Mid(query, p, 1) = SQUOTE Then
            m_qrySubterm = m_qrySubterm & SQUOTE
            
            p = p + 1
            'JHS Now that AND and OR searches go through here a new bug was noted
            'This line was making it end up with 3 single quotes
    ...
    Else ' a regular field
        ...
        Else
            ...
            If IsNumericField(fieldType) = False Then
                m_out = m_out & SQUOTE
                'jhs 8/17/2006 - fixes bug 69887 in conjunction with WebServer Crashing BUG on OR searches
                'adds double truncation 
                'ex. fieldname like benzene OR toluene
                'becomes fieldname like *benzene* OR fieldname *toluene*
                'but allows the user to specify benzene* OR toluene*
                'converting that to be fieldname like benzene* or fieldname like toluene*
                'next three lines added -this if was almost certainly never being used without WS core fix
                if left(m_qrySubterm,1) <> STAR and right(m_qrySubterm,1) <> STAR then
					m_out = m_out & STAR
                end if
            End If
			'JHS 10/9/06 - spaces were being preserved for text fields 	
            'm_out = m_out & m_qrySubterm
            m_out = m_out & trim(m_qrySubterm)
        End If
...
'   JHS 10/9/06
'	The following line was treating any text field with a - as a range.....
'   If firstTime And (InStr(Mid(query, p), "-") Or InStr(Mid(query, p), "to")) Then
    'If (firstTime And (InStr(Mid(query, p), "-") Or InStr(Mid(query, p), "to"))) AND ((fieldtype <> kfieldText) OR (fieldtype <> kfieldCAS) OR (fieldtype <> kfieldTextOther)) Then
If (firstTime And (InStr(Mid(query, p), "-") Or InStr(Mid(query, p), "to"))) And ((fieldType <> kfieldText) And fieldType <> kfieldTextOther) Then
        ...
        If IsDate(dt1) And IsDate(dt2) Then
            ...
            If dt1 = dt2 Then
				if Not UCase(SQLSyntax) = "ORACLE" then
					...
					'SYAN modified on 12/2/2005 to fix CSBR-60211
					if Application("DATE_FORMAT") = "8" then
...
'8/6/2007 - there was a problem with a lead space
s = trim(s)
