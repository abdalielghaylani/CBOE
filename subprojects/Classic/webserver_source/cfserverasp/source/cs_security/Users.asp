...
if UCase(Application("AUTHENTICATION_MODE")) = "LDAP"  or UCase(Application("AUTHENTICATION_MODE")) = "COELDAP" then
	LDAP = true
...
<script language="JavaScript">
	...
	function ValidateUser(){
		...
		if ((document.user_manager.iPassword)&& (document.user_manager.isAlreadyInOracle.value != 1)){
			...
			pw2 = document.user_manager.iPassword2.value
			if ((pw != orig_user_password)||(pw != pw2)||(pw.length == 0)){
				if (pw.length == 0) {
...
<%end if%>
	...
	<tr>
		...
		<td>
			<%
			Set Aconn = GetAdminCS_SecurityConnection()
			Response.write ShowSelectBox2(AConn, "SupervisorID", SupervisorID, "SELECT  p.Person_ID AS Value, max(p.Last_Name||DECODE(p.First_Name, NULL, '', ', '||p.First_Name)) AS DisplayText from dba_role_privs, cs_security.people p WHERE  upper(dba_role_privs.grantee) = upper(p.user_id) GROUP BY p.person_id ORDER BY 2 ASC", 30, "None Assigned", "")
