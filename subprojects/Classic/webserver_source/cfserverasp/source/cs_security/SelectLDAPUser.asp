...
if LDAPUserName <> "" then
	...
	elseif UCase(Application("AUTHENTICATION_MODE")) = "COELDAP" then
		GetUserInfo_coeldap LDAPUserName
	end if
...
Sub GetUserInfo_coeldap(un)
	...
	on error resume next
	'Set ldResults = oLDAPAuthenticator.GetUserInfo(Application("LDAPConfigXmlPath"), un, svcAccount, pwd)
	results = SSOobj.GetUserInfoAsString(un)
	...
	Set ldResults = xmlDoc.childNodes(0)
	Set SSOobj = Nothing
...
<%else%>
	...
	<%else%>
			...
			elseif UCase(Application("AUTHENTICATION_MODE")) = "COELDAP" then
				set nodes = ldresults.childNodes
			end if
