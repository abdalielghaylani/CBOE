...
function buildMultiSelectBox(Name,thestring, thesize){

	
	document.write ('<select multiple name="' + Name + '">')
	...
	document.write ("</select>")
	//document.write ("<input type='hidden' value='place_holder' name='" + Name + "_hidden'" + ">")
...
RoleID =<%=Application("mainwindow")%>.document.forms["user_manager"].elements["roles_hidden"]
RoleID.value = <%=Application("mainwindow")%>.document.forms["user_manager"].elements["roles"].options[<%=Application("mainwindow")%>.document.forms["user_manager"].elements["roles"].selectedIndex].value

}

function  updatecurrent_roles(){

RoleID =<%=Application("mainwindow")%>.document.forms["user_manager"].elements["current_roles_hidden"]
RoleID.value = <%=Application("mainwindow")%>.document.forms["user_manager"].elements["current_roles"].options[<%=Application("mainwindow")%>.document.forms["user_manager"].elements["current_roles"].selectedIndex].value

}

function  updateusers(){

Users =<%=Application("mainwindow")%>.document.forms["user_manager"].elements["users_hidden"]
Users.value = <%=Application("mainwindow")%>.document.forms["user_manager"].elements["users"].options[<%=Application("mainwindow")%>.document.forms["user_manager"].elements["users"].selectedIndex].value
			...
			if (theName.indexOf(" ") != -1){
				...
				alert("Spaces within user name are not allowed")
					...
					if (theName.length > 35){
						...
						document.user_manager.action = "/<%=Application("AppKey")%>/cs_security/manage_security/manage_users_new.asp?dbname=<%=dbkey%>&formgroup=manage_users_form_group&process_request=" + action
...
function addCurrentList(){
	addItemtext = <%=Application("mainwindow")%>.document.forms[0].elements["roles"].options[<%=Application("mainwindow")%>.document.forms["user_manager"].elements["roles"].selectedIndex].text
...
function removeCurrentList(){
	roleCount = <%=Application("mainwindow")%>.document.forms[0].elements["current_roles"].options.length;
	
	removeItemtext = <%=Application("mainwindow")%>.document.forms[0].elements["current_roles"].options[<%=Application("mainwindow")%>.document.forms["user_manager"].elements["current_roles"].selectedIndex].text
	
	if (roleCount == 1){
		alert("at least one role is required");
	}
	else if ((removeItemtext!= "undefined")||(removeItemtext!= null)){
...
function updateHidden(){
	...
	theavailroles = <%=Application("mainwindow")%>.document.forms[0].elements["roles_hidden"]
	thecurrentroles = <%=Application("mainwindow")%>.document.forms[0].elements["current_roles_hidden"]
	//294
...
function fillAvailList(){
	var temp = w_avail_list.split(",")
	
	<%=Application("mainwindow")%>.document.forms[0].elements["roles"].length = 0
	for (i=0;i<temp.length;i++){
		<%=Application("mainwindow")%>.document.forms[0].elements["roles"].options[i] = new Option(temp[i])
	}
}
...
function fillCurrentList(){
	var temp = w_current_list.split(",")
	
	<%=Application("mainwindow")%>.document.forms[0].elements["current_roles"].length = 0
	for (i=0;i<temp.length;i++){
		<%=Application("mainwindow")%>.document.forms[0].elements["current_roles"].options[i] = new Option(temp[i])
	}
}
...
function doUpdateUser(orig_list, orig_password, user_name){
	...
	current_roles = <%=Application("mainwindow")%>.document.forms[0].elements["current_roles_hidden"].value
	if (current_roles != orig_list){
	bChangeRoles = true
	}

	new_password = <%=Application("mainwindow")%>.document.forms[0].elements["password"].value
	...
	if ((bChangeRoles ==true)||(bChangePassword==true)){
		document.user_manager.action = "/<%=Application("AppKey")%>/cs_security/manage_security/manage_users_result_form.asp?change_roles=" + bChangeRoles + "&change_password=" + bChangePassword + "&dbname=<%=dbkey%>&formgroup=manage_users_form_group&process_request=update_user&uniqueid=" + user_name
		document.user_manager.submit()
	}
	else {
...
function doUpdateUserPwd(orig_password, user_name){
	...
	if (bChangePassword==true){
		document.user_manager.action = "/<%=Application("AppKey")%>/cs_security/manage_security/manage_pwd_result_form.asp?change_password=" + bChangePassword + "&dbname=<%=dbkey%>&formgroup=manage_users_form_group&process_request=update_pwd&uniqueid=" + user_name
		document.user_manager.submit()
	}
	else {
		if(bPasswordError == true){
...
function doDeleteUser(user_name){
	...
	if(confirm("You are about to remove " + user_name + ".  Continue?")){
		document.user_manager.action = "/<%=Application("AppKey")%>/cs_security/manage_security/manage_users_result_form.asp?user_name=" + user_name + "&dbname=<%=dbkey%>&formgroup=manage_users_form_group&process_request=delete_user&uniqueid=" + user_name
		document.user_manager.submit()
	}
	else{
;
...
function AddItemToList(formname, new_item){
	
	var newItemIndex = <%=Application("mainwindow")%>.document.forms[formname].elements["roles"].length
	<%=Application("mainwindow")%>.document.forms[formname].elements["roles"].options[newItemIndex] = new Option(new_item)
	
}
function getOtherRoleWindow(myUrl,dbkey,thecaller){
	var mywindow = ""
	if (mywindow.name == null){
		mywindow = window.open(myUrl,"add_role", "toolbar=0,status=0,menubar=0,scrollbars=0,resizable=0,width=400,height=100")
		mywindow.focus()
	}else{
		mywindow.focus()
	}
	
	mywindow.document.write ('<%' + '@ LANGUAGE="VBSCRIPT"%>')
	mywindow.document.write('<html>')
	mywindow.document.write('<head>')
	mywindow.document.write('</head>')
	mywindow.document.write('<body>')
	mywindow.document.write('<form name="cows_input_form" method="post" action="/<%=Application("AppKey")%>/cs_security/manage_security/manage_security_action.asp?dbname='+ dbkey + '&theCaller' + thecaller + '&formgroup=manage_roles_form_group&dataaction=add_existing_role&roleExisting=true">')
	mywindow.document.write('<input type="text" name="role_name" value = "" width = "100">')
	mywindow.document.write('<input type="hidden" name="theCaller" value = "' + thecaller + '">')

	mywindow.document.write('<input type="button" name="Add Existing Role" value = "Add Existing Role" onClick="document.cows_input_form.submit()">')
