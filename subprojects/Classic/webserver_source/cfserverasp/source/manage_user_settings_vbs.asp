...
Function getCurrentSettingsRS(ByVal dbkey, ByVal formgroup, ByVal User_id, ByVal SettingName, ByVal Operator)
	...
	Select Case(DBMSTYPE)
		Case "ORACLE"
			if UCase(Operator) = "LIKE" then
				...
				'Need to replace the username which is part of the setting in order to find the public forms
				modSettingName = replace(settingName, Session("USER_SETTINGS_ID" & dbkey),"%",1,1)
			end if
			sql = "SELECT SETTING_VALUE FROM " & schema & ".USER_SETTINGS WHERE Upper(SETTING_NAME) " & operator & " ? AND Upper(USER_ID) = ?"
			sql = sql & " UNION ALL SELECT SETTING_VALUE FROM " & schema & ".USER_SETTINGS WHERE Upper(SETTING_NAME) " & operator & " ? AND Upper(USER_ID) = 'PUBLIC'"

		Case "ACCESS"
			
			if UCase(Operator) = "LIKE" then
				'DGB The OLEDB provider does not like * it expects %
				'SettingName = SettingName & "*"
				SettingName = SettingName & "%"
				'Need to replace the username which is part of the setting in order to find the public forms
				modSettingName = replace(settingName, Session("USER_SETTINGS_ID" & dbkey),"%",1,1)
			...
			sql = sql & " UNION SELECT SETTING_VALUE FROM USER_SETTINGS WHERE SETTING_NAME  " & operator & " ? AND USER_ID = 'PUBLIC'"
	...
	Cmd.Parameters.Append Cmd.CreateParameter("pSettingName2", 200, 1, Len(SettingName) + 1, Trim(Ucase(ModSettingName))) 
