...
function cd_getSpecificObjectTag(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, ISISDraw) {
    ...
    if (cd_currentUsing == 1) {	// ActiveX Control
        ...
        if (objHeight.toString().indexOf("px") < 0)
            objHeight += "px";
        if (objWidth.toString().indexOf("px") < 0)
            objWidth += "px";
...
function cd_isCDPluginInstalled() {
    if (cd_testBrowserType("Microsoft Internet Explorer")) {
        ...
        document.write(str);
        document.cookie = "isCDP=true; path=/"
        return true;
    }
...
function cd_isCDActiveXInstalled() {
    ...
    if (js_canUseTry) {
        ...
        str = str + " try\n";
        str = str + " {\n";
        str = str + "  // Try 17.0Constant\n";
        str = str + "  var obj17c = new ActiveXObject(\"ChemDrawControlConst11.ChemDrawCtl.17.0\");\n";
        str = str + "  CD_CONTROL_CLSID = CD_CONTROL170CONSTCLSID; obj17c='';\n";
        str = str + " } catch(e17c)\n";
        str = str + " {\n";
        str = str + "try\n";
        str = str + "{\n";
        ...
        if (document.all("test_170").Selection != null)
            ...
            if (document.all("test_170c").Selection != null)
                CD_CONTROL_CLSID = CD_CONTROL170CONSTCLSID;
            else {
...
function cd_setIsCDPPluginCookie() {
