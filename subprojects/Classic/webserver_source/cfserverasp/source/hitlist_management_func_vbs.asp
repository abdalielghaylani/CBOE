...
Function getSessionInfoFromCSDOHitlist(dbkey, formgroup, user_id)
	...
	Select Case currentRDBMS
	    Case "ORACLE"
	        DateCreatedSQL = "to_char(DATE_CREATED, '" & Application("Date_Format_String") & ":HH:MI:SS PM') DATE_CREATED"
	    Case else
	        DateCreatedSQL = "DATE_CREATED"
	End Select
	
	sql = "SELECT NAME, DESCRIPTION, ID, USER_ID, FORMGROUP, NUMBER_HITS, IS_PUBLIC, " & DateCreatedSQL &_
...
Function getSavedHitlistsSQL(dbkey, formgroup, user_id, showPublic)
	...
	sql = "SELECT NAME, DESCRIPTION, ID, USER_ID, FORMGROUP, NUMBER_HITS, IS_PUBLIC, DATE_CREATED" &_
		...
		if showPublic then
		    'DGB Changed OR to OR (A AND B) to address CSBR-91859
			sql = sql & " OR (" & hitListIDTableName & ".FORMGROUP='"& Ucase(formgroup) & "'" &_
			" AND " & hitListIDTableName & ".IS_PUBLIC='1')"
