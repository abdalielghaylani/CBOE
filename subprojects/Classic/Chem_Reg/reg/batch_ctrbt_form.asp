...
<%  if Not checkHideField("SOURCE") then
	...
	<%end if%>
<%end if%>
<title>Chemical Registration - Add Batch Input Form</title>
...
<%	on error resume next
	...
	if UCase(Application("Batch_Leval")) = "SALT" then
		'remove_required = "TEMPORARY_STRUCTURES.SALT_MW;2:Salt MW,TEMPORARY_STRUCTURES.SALT_NAME;0:Salt Name"
		doRemoveRequired dbkey, formgroup, remove_required
	end if
...
<input type = "hidden" name = "batch_add_from_results" value ="<%=request("batch_add_from_results")%>" ID="Hidden1">
...
<table border="0">
	<tr>
		<td width="40%" align="left" nowrap>
			<p align="left"><strong><font face="Arial" size="3" color="#182889">New	Batch Submission Form</font></strong></p>
		</td>
...
<table  <%=registered_compound_table%> width = "780">
