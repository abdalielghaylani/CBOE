...
c_type = request("compound_type")
session("c_type") = c_type

c_type_text = request("compound_type_text")
session("c_type_text") = c_type_text
%>
...
<script language="javascript">
	...
	function reRendorForm(compound_type, compound_type_text) {
		//alert(compound_type);
		var oldUrl = location.href;
		var newUrl;
		var start = oldUrl.indexOf('&compound_type=');
		if (start < 0) {
			newUrl = oldUrl + '&compound_type=' + compound_type + '&compound_type_text=' + compound_type_text;
		}
		else {
			newUrl = oldUrl.substr(0, start);
			newUrl = newUrl + '&compound_type=' + compound_type + '&compound_type_text=' + compound_type_text;
...
<title>Chemical Registration - Add Compounds Input Form</title>

<%	
'stop
'alternative required field for CARA
'Response.Write c_type
'Application.Lock
'if CInt(c_type) = 1 then 
	'Application(formgroup & dbkey)(13) = GetFormGroupVal(dbkey, formgroup, kRequiredFields)
	'kRequiredFields = 13
'elseif CInt(c_type) = 2 then
	'kRequiredFields = 15
'end if
'Application.UnLock
...
if Application("DEFAULT_PREFIX") <> "" then
	default_prefix_id = getValueFromTablewConn(DataConn, "Sequence", "Prefix", Application("DEFAULT_PREFIX"), "Sequence_ID")
else
	default_prefix_id =""
end if
...
<table border="0">
	<tr>
		...
		<td width="60%" align="right" nowrap>
			...
			</b></font> <font face="Arial" size="4" color="#182889"><b>Registration
			Enterprise</b></font></p>
		</td>
	</tr>
</table>
							    ...
							    <table border="0" >
									...
									<%if CBool(application("compound_types_used")) = True then%>
										...
										<tr>
											...
											<%'ShowLookUpList dbkey, formgroup,BaseRS, "Temporary_Structures.Compound_Type",Compound_Type_List, "","",0,true,"value:validate:checkCompoundType(this.name)","0" %>
											...
											<%'SYAN modified on 10/25/2007 to fix CSBR-85023
											if Application("REFRESH_FORM_ON_COMPOUND_TYPE") = "1" then
											    ShowLookUpList dbkey, formgroup,BaseRS, "Temporary_Structures.Compound_Type",Compound_Type_List, session("c_type"), "", 0, true, "value:validate:reRendorForm(this.options[this.selectedIndex].value, this.options[this.selectedIndex].text)","0" 
											  else
											    ShowLookUpList dbkey, formgroup,BaseRS, "Temporary_Structures.Compound_Type",Compound_Type_List, "", "", 0, true, "value:validate:checkCompoundType(this.name)","0" 
											  end if  
											  'End of SYAN modification
											%></font>
											<!--select name="Temporary_Structures.Compound_Typelist" onChange="updateSelectedValue(this.form,this);reRendorForm(this.selectedIndex)" size ="1">
												<option>-- Select one --</option>
												<option value=4>acquired</option>
												<option value=3>in-house</option>
												<option value=1>no_comment</option>
												<option value=2>no_structure</option></select>
												<input type="hidden" name="Temporary_Structures.Compound_Type" value="" ></font-->
...
<%'start batch info%> 
						...
						<td> 
							<table width="675" >
								<tr>
									<td>
										<table width="675" >
											<tr><%'start user lookup labels SEARCH and ADD Mode
 ...
 <%'end batch info%> 
<%
if request("special") = "edit_query" then
	compound_type = getValueFromTablewConn(DataConn, "compound_type", "compound_type", Session("SearchData" & "Temporary_Structures.Compound_Type" &  dbkey & formgroup), "description")
