...
<script language="javascript">
	function launchDocMgrWindow(){
		window.open( '/docmanager/default.asp?dataaction=db&formgroup=base_form_group&dbname=docmanager&extAppName=Chem_Reg&LinkType=<%=LinkType%>&linkfieldname=<%=FK_name%>&showselect=true&extlinkid=<%=FK_value%>', 'docmgrwindow', 'toolbar=no,location=no,scrollbars=yes,width=800,height=600')
	}
</script>

<%
If  Session("SEARCH_DOCS" & "reg") then
	...
	pData = pData & "&csusername=" & Session("username" & "reg")  & "&csuserid=" & Session("UserID" & "reg")
