...
<%
'stop
'DGB 2008/12/09  Added trace output
function removepwd(str)
	Dim startPos
	Dim endPos
	Dim pwd
	Dim token
	...
	token ="user_pwd=" 
	startPos = instr(1, str, token, 1)
	trace "start" & startPos, 20
	if startPos > 0 then
		startPos = startPos + 9
		endPos = instr(startPos, str, "&", 1)
		trace "end" & EndPos, 20
		pwd = token & mid(str, startPos, endPos-startPos)
		str = replace(str, pwd, token & "*****",1) 
	end if
	removepwd = str	   	
end function
trace "HTTP API CALL Posted Form Data:" &  removepwd(request.form), 20
trace "HTTP API CALL QueryString Data:" &  removepwd(request.QueryString), 20
								...
								if bDups = True AND bMultiDups = False then
										...
										Select Case UCase(reg_parameter)
											...
											Case "UNIQUE_DEL_TEMP2" 'response  includes the regnumber and description of action
												...
												'RemoveFromTemp dbkey, formgroup, temp_id
											Case "USER_INPUT"
												'DGB partial fix of CSBR-105046:  Do not write the record to temp as it never gets deleted.
												'temp_id = DoRegTemp(dbkey, formgroup, commit_type)
												if inStr(temp_id, "LOAD_TERMINATED")> 0 then
													RegNumber = getValueFromTable(dbkey, formgroup, "reg_numbers", "reg_id",dup_id,"Reg_number")
...
'logAction("outpostInterface" & Timer())
		...
		trace "HTTP API RESULT: " & post_result, 20 
		Response.Write post_result 
