...
<% Select Case UCase(action)
	Case "UPDATE_PWD"
		...
		<%=Application("mainwindow")%>.location.replace('/<%=Application("AppKey")%>/<%=dbkey%>/mainpage.asp?dbname=<%=dbkey%>&Timer=<%=Timer%>')
		</script>
		<%
		else
...
<%end select%>
<script language="javascript">
	<%=Application("nav_bar_window")%>.location.replace("/<%=Application("appkey")%>/navbar.asp?nav_override=true&dbname=<%=dbkey%>&formmode=manage_passwords")
</script>
</body>
</html>
