...
export class RegRecordDetail implements IFormContainer, OnInit, OnDestroy, OnChanges {
  ...
  private update(forceUpdate: boolean = true) {
    ...
    let state = this.ngRedux.getState();
    let histListId = this.temporary ? state.registry.tempRecords.data.hitlistId : state.registry.records.data.hitlistId;
    this.backButtonEnabled = histListId > 0;
    if (forceUpdate) {
  ...
  back() {
    let state = this.ngRedux.getState();
    let histListId = this.temporary ? state.registry.tempRecords.data.hitlistId : state.registry.records.data.hitlistId;
    this.router.navigate([`records/restore/${histListId}`]);
  }
  register() {
    let recordDoc = this.getUpdatedRecord();
    if (!recordDoc) {
