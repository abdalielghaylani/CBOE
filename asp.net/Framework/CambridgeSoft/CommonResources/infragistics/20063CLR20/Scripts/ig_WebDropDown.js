...
function igdrp_combo(elem,props,ce)
{
	...
	this.setEnabled=function(v){this._state(v?0:3);this.inputBox.disabled=!v;this.update("Enabled",v);}
	...
	{
		if(ig_csom.IsIE&&document.readyState!='complete')return;
		...
		ig_csom.absPosition(e,pan,64+this.dropDownAlignment,tPan,this.inputBox);
	...
	{
		var v=this.editor?this.editor.getDate():null,o=this.old;
	...
	this.doEvt=function(evt,type,src)
	{
		...
		else if(this.isEnabled()&&this.Loaded&&!this.isReadOnly())switch(type)
		{
			...
			case "keypress":this.onImgKey(evt);return;
		}
	}
}
...
function igdc_new(id,prop,df,ce,calID)
{
	var me=igdrp_all[id],elem=igdrp_getElementById(id);
	if(me&&me.doEvt&&elem)
	{
		...
		me.postField=me.elemCal=me.container=me.Element=me.inputBox=me.Calendar=me.webGrid=null;
		ig_dispose(me);
	}
	...
	me._addLsnr(me.DropButton.Image,["keypress"]);
	...
	me.setValue=function(d,fire){this.old=null;this.editor.setDate(d);this.updateValue(!fire,true);}
	me.setText=function(v,fire){this.old=null;this.editor.setText(v);this.updateValue(!fire,true);}
...
function igdc_initDateChooser(id,p,df,ce,calID){return igdc_new(id,p,df,ce,calID);}
...
function igmask_date(me,e,di,v,lf,nullTxt,nullable)
{
	...
	this.doKey=function(e,a)
	{
		...
		if(k<32&&k!=8)return;
		if(e.ctrlKey||e.altKey){if(k==86&&a==2)this.paste(this.elem.value);return;}
		if(a==1)this.k1=k;
	...
	this._d=function(d)
	{
		...
		else if(!this.allowNull)d=new Date();
		return this.date=d;
	}
	...
	this.isOk=function()
	{
		var p=this.parent;var d=p.old;if(this._ok||this.foc||d||d===this.date)return;
		d=unescape(p.postField.value);
