...
{
    /// <summary>
    /// GetApprovedRegistriesCountTest class contains unit test methods for GetApprovedRegistriesCount
    /// </summary>
    [TestClass]
    public class GetApprovedRegistriesCountTest
    {
        #region Variables
        ...
        #region GetApprovedRegistriesCount Test Methods
        ...
        public void GetCustomItem_LoadingFormattedText() 
        {
            try
            {
                GetApprovedRegistriesCount theGetApprovedRegistriesCount = new GetApprovedRegistriesCount();
                ...
                foreach (Group myGroup in homeData.Groups)
                {
                    if (myGroup.PageSectionTarget.ToUpper().Equals("DASHBOARD"))
                    {
                        foreach (CustomItems customItems in myGroup.CustomItems)
                        {
                            ...
                            theGetApprovedRegistriesCount.SetConfiguration(customItems.Configuration);
                        }
                        break;
                    }
                }

                string strResult = theGetApprovedRegistriesCount.GetCustomItem();

                Assert.IsTrue(!string.IsNullOrEmpty(strResult), "GetApprovedRegistriesCount.GetCustomItem() did not return the expected value.");
            }
            catch (Exception ex)
            {
