...
function igtab_getTabById(id)
{
	...
	for(var i in igtab_all)if((o=igtab_all[i])!=null)
		if(o.ID==id||o.ID_==id||o.uniqueId==id)return o;
	return null;
}
...
function igtab_new(elem,id,prop,evts,e0)
{
	...
	this.val=function(o,i,t){o=(o==null||o.length<=i)?"":o[i];return t?ig_csom.replace(ig_csom.replace(o,"&quot;","\""),"&pipe;","|"):o;}
	...
	var css,o,j=0,i=-1,a=elem.getAttribute("acss");
	if(a&&a.length>5)
	{
		a=a.split('+');
	...
	this.beforeCBSubmit=function(){return this.fireEvt(this,this.Events.beforeCBSubmit);}
	...
	if((j&2)!=0)
	{
		...
		for(i=0;i<4;i++)o[i]=a?(a[i]+' '+a[4]):(id+'_'+i+'R');
		if((j&4)==0)o[3]=o[0];
		if(this.noHov)o[1]=o[0];
	}
	...
	else
	{
		...
		for(i=0;i<4;i++)
		{
			...
			o=this.css[i];
			if(j>0)o=o?(o+" "+id+"_b"+(((j&(1<<i))!=0)?i:0)):"";
	...
	while(++i<prop.length-1)
	{
		...
		var tab=new igtab_newT(id,prop[i+1].split('|'),i,css,this,elem);
		if(tab._cb)
		{
...
function igtab_newT(id,prop,i,css0,own,elem)
{
	this.owner=own;
	this.element=elem;
	...
	this.focus=function(){if(this.elemIndex!=null)try{this.elemIndex.focus();}catch(i){};}
	...
	this.doState=function(st)
	{
		...
		if(this.elemLeft!=null&&this.elemLeft.className!=this.owner._cssL[st])this.elemLeft.className=this.owner._cssL[st];
		if(this.elemRight!=null&&this.elemRight.className!=this.owner._cssR[st])this.elemRight.className=this.owner._cssR[st];
	}
...
function igtab_getTabFromElement(e)
{
	...
	while(true)
	{
		...
		try{if(e.getAttribute!=null)ids=e.getAttribute("tabID");}catch(t){}
