...
{
    /// <summary>
    /// GetDuplicateComponentsCountTest class contains unit test methods for GetDuplicateComponentsCount
    /// </summary>
    [TestClass]
    public class GetDuplicateComponentsCountTest
    {
        #region Variables
        ...
        #region GetDuplicateComponentsCount Test Methods
        ...
        public void GetCustomItem_LoadingFormattedText() 
        {
            try
            {
                GetDuplicateComponentsCount theGetDuplicateComponentsCount = new GetDuplicateComponentsCount();
                ...
                foreach (Group myGroup in homeData.Groups)
                {
                    if (myGroup.PageSectionTarget.ToUpper().Equals("DASHBOARD"))
                    {
                        foreach (CustomItems customItems in myGroup.CustomItems)
                        {
                            ...
                            theGetDuplicateComponentsCount.SetConfiguration(customItems.Configuration);
                        }
                        break;
                    }
                }

                string strResult = theGetDuplicateComponentsCount.GetCustomItem();

                Assert.IsTrue(!string.IsNullOrEmpty(strResult), "GetDuplicateComponentsCount.GetCustomItem() did not return the expected value.");
            }
            catch (Exception ex)
            {
